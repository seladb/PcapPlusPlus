...
static struct option UdpAssemblyOptions[] = {{"interface", required_argument, 0, 'i'},
...
class GlobalConfig
{
  ...
  public:
	...
	// a flag indicating whether to write UDP data to actual files or to console
	bool writeToConsole;
...
typedef std::map<std::string, UdpReassemblyData> UdpReassemblyMgr;
typedef std::map<std::string, UdpReassemblyData>::iterator UdpReassemblyMgrIter;
...
void printUsage()
{
	std::cout << std::endl
			  ...
			  << "    -e bpf_filter : Apply a BPF filter to capture file or live interface, meaning UDP reassembly "
...
static void OnUdpMessageReadyCallback(pcpp::UdpPacketData *udpData, void *userCookie)
{
	...
	UdpReassemblyMgr *mgr = (UdpReassemblyMgr *)userCookie;

	// check if this tuple already appears in the manager. If not add it
	UdpReassemblyMgrIter iter = mgr->find(udpData->getTupleName());
	if (iter == mgr->end())
	{
		mgr->insert(std::make_pair(udpData->getTupleName(), UdpReassemblyData()));
	...
	if (iter->second.fileStream == NULL)
	{
		...
		int result =
			GlobalConfig::getInstance().getRecentFilesWithActivity()->put(udpData->getTupleName(), &nameToCloseFile);
		...
		if (result == 1)
		{
			UdpReassemblyMgrIter iter2 = mgr->find(nameToCloseFile);
			if (iter2 != mgr->end())
			{
	...
	iter->second.bytes += (int)udpData->getDataLength();
 ...
 */
static void onPacketArrives(pcpp::RawPacket *packet, pcpp::PcapLiveDevice *dev, void *udpReassemblyCookie)
{
	// get a pointer to the UDP reassembly instance and feed the packet arrived to it
	pcpp::UDPReassembly *udpReassembly = (pcpp::UDPReassembly *)udpReassemblyCookie;
	udpReassembly->reassemblePacket(packet);
}

/**
 * The method responsible for UDP reassembly on pcap/pcapng files
 */
void doUdpReassemblyOnPcapFile(std::string fileName, pcpp::UDPReassembly &udpReassembly, std::string bpfFilter = "")
{
	...
	// run in a loop that reads one packet from the file in each iteration and feeds it to the UDP reassembly instance
	pcpp::RawPacket rawPacket;
	while (reader->getNextPacket(rawPacket))
	{
		udpReassembly.reassemblePacket(&rawPacket);
	}
 ...
 */
void doUdpReassemblyOnLiveTraffic(pcpp::PcapLiveDevice *dev, pcpp::UDPReassembly &udpReassembly,
								  std::string bpfFilter = "")
{
	...
	dev->startCapture(onPacketArrives, &udpReassembly);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "i:r:o:e:f:mcvhl", UdpAssemblyOptions, &optionIndex)) != -1)
	{
		switch (opt)
		{
		case 0:
	...
	UdpReassemblyMgr mgr;

	//  create the UDP reassembly instance
	pcpp::UDPReassembly udpReassembly(OnUdpMessageReadyCallback, &mgr);

	// analyze in pcap file mode
	if (!inputPcapFileName.empty())
	{
		doUdpReassemblyOnPcapFile(inputPcapFileName, udpReassembly, bpfFilter);
	}
	else // analyze in live traffic mode
	{
		...
		// start capturing packets and do UDP reassembly
		doUdpReassemblyOnLiveTraffic(dev, udpReassembly, bpfFilter);
	}
}
