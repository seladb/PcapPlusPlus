 ...
 * ========================================
 * This utility demonstrates how to transfer files between 2 machines using only
 * ICMP messages. This is the catcher part of the utility For more information
...
/**
 * A struct used for starting a file transfer, mainly sending or getting the
 * file name
 */
...
/**
 * A callback used in the receiveFile() method and responsible to wait for the
 * pitcher to send an ICMP request containing the file name to be received
 */
static bool waitForFileTransferStart(pcpp::RawPacket* rawPacket,
    ...
    // verify the source IP is the pitcher's IP and the dest IP is the catcher's
    // IP
    ...
    // check the ICMP timestamp field which contains the type of message delivered
    // between pitcher and catcher in this case the catcher is waiting for a
    // file-transfer start message from the pitcher containing the file name which
    // is of type ICMP_FT_START
    ...
    icmpFTStart->fileName =
        std::string((char*)icmpLayer->getEchoRequestData()->data);
    ...
    // send the pitcher an ICMP response containing an ack message (of type
    // ICMP_FT_ACK) so it knows the catcher has received the file name and it's
    // ready to start getting the file data
    if (!sendIcmpResponse(dev, dev->getMacAddress(), ethLayer->getSourceMac(),
                          icmpFTStart->catcherIPAddr, icmpFTStart->pitcherIPAddr,
                          icmpId, ICMP_FT_ACK, nullptr, 0))
        EXIT_WITH_ERROR("Cannot send ACK message to pitcher");

    // set the current ICMP ID. It's important for the catcher to keep track of
    // the ICMP ID to make sure it doesn't miss any message
...
/**
 * A callback used in the receiveFile() method and responsible to receive file
 * data chunks arriving from the pitcher and write them to the local file
 */
static bool getFileContent(pcpp::RawPacket* rawPacket,
    ...
    // extract message type from the ICMP request. Message type is written in ICMP
    // request timestamp field
    uint64_t resMsg = icmpLayer->getEchoRequestData()->header->timestamp;

    // if message type is ICMP_FT_END it means pitcher finished sending all file
    // chunks
    ...
    // if message type is not ICMP_FT_END not ICMP_FT_DATA - do nothing, it's
    // probably an ICMP request not relevant for this file transfer
    else if (resMsg != ICMP_FT_DATA)
        return false;

    // compare the ICMP ID of the request to the ICMP ID we expect to see. If it's
    // smaller than expected it means catcher already saw this message so it can
    // be ignored
    if (pcpp::netToHost16(icmpLayer->getEchoRequestData()->header->id) <
        icmpData->expectedIcmpId)
        return false;

    // if ICMP ID is bigger than expected it probably means catcher missed one or
    // more packets. Since a reliability mechanism isn't currently implemented in
    // this program, the only thing left to do is to exit the program with an
    // error
    if (pcpp::netToHost16(icmpLayer->getEchoRequestData()->header->id) >
        icmpData->expectedIcmpId) {
        ...
        EXIT_WITH_ERROR_AND_RUN_COMMAND(
            "Didn't get expected ICMP message #"
    ...
    icmpData->file->write((char*)icmpLayer->getEchoRequestData()->data,
                          icmpLayer->getEchoRequestData()->dataLength);
...
void receiveFile(pcpp::IPv4Address pitcherIP, pcpp::IPv4Address catcherIP) {
    ...
    pcpp::PcapLiveDevice* dev =
        pcpp::PcapLiveDeviceList::getInstance().getPcapLiveDeviceByIp(catcherIP);
    if (dev == nullptr)
    ...
    int res =
        dev->startCaptureBlockingMode(waitForFileTransferStart, &icmpFTStart, -1);
    if (!res)
    ...
    if (file.is_open()) {
        ...
        // get all file data from the pitcher. This method blocks until all file is
        // received
        res =
            dev->startCaptureBlockingMode(getFileContent, &icmpFileContentData, -1);
        if (!res) {
            file.close();
            EXIT_WITH_ERROR_AND_RUN_COMMAND(
...
/**
 * A callback used in the sendFile() method and responsible to wait for the
 * pitcher to send a keep-alive ICMP request. When such message arrives this
 * callback takes care of sending an ICMP response to the pitcher which is data
 * contains the file name to send
 */
static bool startFileTransfer(pcpp::RawPacket* rawPacket,
                              pcpp::PcapLiveDevice* dev, void* icmpVoidData) {
    // first, parse the packet
    pcpp::Packet parsedPacket(rawPacket);
    ...
    // check the ICMP timestamp field which contains the type of message delivered
    // between pitcher and catcher in this case the catcher is waiting for a
    // keep-alive message from the pitcher which is of type
    ...
    if (!sendIcmpResponse(dev, dev->getMacAddress(), ethLayer->getSourceMac(),
                          ...
                          (uint8_t*)icmpFTStart->fileName.c_str(),
...
/**
 * A callback used in the sendFile() method and responsible to wait for ICMP
 * requests coming from the pitcher and send file data chunks as a reply in the
 * ICMP response data
 */
static bool sendContent(pcpp::RawPacket* rawPacket, pcpp::PcapLiveDevice* dev,
                        void* icmpVoidData) {
    // first, parse the packet
    pcpp::Packet parsedPacket(rawPacket);
    ...
    IcmpFileContentDataSend* icmpFileContentData =
        (IcmpFileContentDataSend*)icmpVoidData;
    ...
    // if it's not an abort message, catcher is only waiting for data messages
    // which are of type ICMP_FT_WAITING_DATA
    ...
    if (!icmpFileContentData->readingFromFile) {
        // send an ICMP response with ICMP_FT_END value in the timestamp field,
        // indicating the pitcher all file was sent to it
        if (!sendIcmpResponse(dev, dev->getMacAddress(), ethLayer->getSourceMac(),
                              icmpFileContentData->catcherIPAddr,
                              icmpFileContentData->pitcherIPAddr, icmpId,
                              ICMP_FT_END, nullptr, 0))
    ...
    if (icmpFileContentData->file->read(icmpFileContentData->memblock,
        ...
        // if managed to read a full block, send it via the ICMP response to the
        // pitcher. The data chunk will be sent in the response data
        if (!sendIcmpResponse(dev, dev->getMacAddress(), ethLayer->getSourceMac(),
                              icmpFileContentData->catcherIPAddr,
                              icmpFileContentData->pitcherIPAddr, icmpId,
                              ICMP_FT_DATA,
                              (uint8_t*)icmpFileContentData->memblock,
                              icmpFileContentData->blockSize))
    ...
    // if read only partial block, it means it's the last block
        ...
        // send the remaining bytes of data to the pitcher via the ICMP response.
        // The data chunk will be sent in the response data
        if (!sendIcmpResponse(dev, dev->getMacAddress(), ethLayer->getSourceMac(),
                              icmpFileContentData->catcherIPAddr,
                              icmpFileContentData->pitcherIPAddr, icmpId,
                              ICMP_FT_DATA,
    ...
    // if couldn't read anything, it means the previous block was the last block
    // of the file
    else {
...
void sendFile(const std::string& filePath, pcpp::IPv4Address pitcherIP,
              pcpp::IPv4Address catcherIP, size_t blockSize) {
    // identify the interface to listen and send packets to
    pcpp::PcapLiveDevice* dev =
        pcpp::PcapLiveDeviceList::getInstance().getPcapLiveDeviceByIp(catcherIP);
    if (dev == nullptr)
    ...
    if (file.is_open()) {
        ...
        // be delivered to the pitcher
        std::string fileName = getFileNameFromPath(filePath);

        std::cout << "Waiting for pitcher to send a keep-alive signal..."
                  << std::endl;

        IcmpFileTransferStart icmpFTStart = {pitcherIP, catcherIP, fileName, 0};

        // first, establish a connection with the pitcher and send it the file name.
        // This method waits for the pitcher to send an ICMP request which indicates
        // it's alive. The response to the request will contain the file name in the
        // ICMP response data
        int res =
            dev->startCaptureBlockingMode(startFileTransfer, &icmpFTStart, -1);
        ...
        IcmpFileContentDataSend icmpFileContentData = {
            pitcherIP, catcherIP, &file, true, 0, blockSize, nullptr};

        // create the memory block that will contain the file data chunks that will
        // be transferred to the pitcher
        icmpFileContentData.memblock = new char[blockSize];

        // wait for ICMP requests coming from the pitcher and send file data chunks
        // as a reply in the ICMP response data this method returns when all file
        // was transferred to the pitcher
...
int main(int argc, char* argv[]) {
    ...
    // disable stdout buffering so all std::cout command will be printed
    // immediately
    setbuf(stdout, nullptr);

    // read and parse command line arguments. This method also takes care of
    // arguments correctness. If they're not correct, it'll exit the program
    readCommandLineArguments(argc, argv, "catcher", "pitcher", sender, receiver,
                             catcherIP, pitcherIP, fileNameToSend, packetsPerSec,
                             blockSize);
