 ...
 * ICMP messages. This is the pitcher part of the utility For more information
 ...
 * catcher to send an ICMP response containing the file name to be received
 */
                                     ...
                                     void* icmpVoidData)
{
    ...
    IcmpFileTransferStartRecv* icmpFTStart =
        (IcmpFileTransferStartRecv*)icmpVoidData;
    ...
    // verify the source IP is the catcher's IP and the dest IP is the pitcher's
    // IP
    ...
    // extract the message type in the ICMP reply timestamp field and check if
    // it's  ICMP_FT_START
    ...
    icmpFTStart->fileName =
        std::string((char*)icmpLayer->getEchoReplyData()->data);

    // signal the receiveFile() file name was extracted and it can stop capturing
    // packets
                           ...
                           pcpp::PcapLiveDevice* dev, void* icmpVoidData)
{
    ...
    IcmpFileContentData* icmpFileContentData =
        (IcmpFileContentData*)icmpVoidData;
    ...
    // if message type is ICMP_FT_END it means all file was sent by the catcher.
    ...
    // verify we're not missing any message by checking the ICMP ID of the reply
    // and compare it to the expected ICMP ID. If one or more message were missed,
        ...
        icmpFileContentData->expectedIcmpId)
    {
        ...
        std::cout << std::endl
                  ...
                  << icmpFileContentData->expectedIcmpId << ", got #"
                  << pcpp::netToHost16(icmpLayer->getEchoReplyData()->header->id)
                  << std::endl;
        return;
    }
                 ...
                 int packetPerSec)
{
    ...
    pcpp::MacAddress catcherMacAddr =
        pcpp::NetworkUtils::getInstance().getMacAddress(
            catcherIP, dev, arpResTO, pitcherMacAddr, pitcherIP, 10);
    ...
    // since it's the pitcher's job to send ICMP requests and the catcher's job to
    // get them and send ICMP replies, sending a file from the catcher to the
    // pitcher is a bit more complicated so for start the pitcher needs the file
    // name. It sends an ICMP request with ICMP_FT_WAITING_FT_START message in the
    // timestamp field and awaits for catcher response that should include the
    // file name

    // start capturing ICMP packets. The waitForFileTransferStart callback should
    // look for the catcher reply and set icmpFTStart.gotFileTransferStartMsg to
    // true
    ...
    if (file.is_open())
    {
        ...
        IcmpFileContentData icmpFileContentData = {
            pitcherIP, catcherIP, &file, icmpId, 0, 0, false, false};

        // the next thing to do is start getting the file data. For doing that the
        // pitcher sends the catcher ICMP requests with message type
        // ICMP_FT_WAITING_DATA in the timestamp field. The catcher should send an
        // ICMP response for each such request with data chunk of the file

        // calculate how many microseconds (usec) the pitcher needs to sleep between
        // sending the ICMP_FT_WAITING_DATA message (calculated from user defined
        // packetPerSec parameter). The calculation is done in usec as in most cases
        // the pitcher needs to sleep less than 1 second between chunks. However if
        // packetPerSec equals to 1 it means sleeping for 1 second and in this case
        // we can't use usleep (as it's not working for 1 sec or more) and we use
        // sleep instead
        uint32_t sleepBetweenPackets = 0;
        if (packetPerSec > 1)
            sleepBetweenPackets = (uint32_t)(1000000UL / packetPerSec);

        // start capturing ICMP packets. The getFileContent callback should look for
        // the catcher replies containing data chunks of the file and write them to
        ...
        if (icmpFileContentData.fileTransferError)
        {
            ...
            file.close();
            EXIT_WITH_ERROR_AND_RUN_COMMAND(
                "Sent abort message to catcher. Exiting...",
                std::remove(icmpFTStart.fileName.c_str()));
        }
 ...
 * responses coming from the catcher indicating it's alive and ready for file
 * transfer to start
 */
                                        ...
                                        void* icmpVoidData)
{
    ...
    IcmpFileTransferStartSend* icmpData =
        (IcmpFileTransferStartSend*)icmpVoidData;
    ...
    // verify the ICMP ID of the reply matched the ICMP ID the pitcher sent in the
    // request
    if (icmpLayer->getEchoReplyData()->header->id !=
        pcpp::hostToNet16(icmpData->icmpMsgId))
        return false;
    ...
    // if arrived to here it means we got a response from the catcher and it's
    // ready for file transfer to start
    return true;
}
    ...
    if (file.is_open())
    {
        ...
        while (1)
        {
            ...
            if (!sendIcmpRequest(dev, pitcherMacAddr, catcherMacAddr, pitcherIP,
                                 catcherIP, icmpId, ICMP_FT_START, memblock,
                                 fileName.length() + 1))
                EXIT_WITH_ERROR("Cannot send file transfer start message");

            // now wait for the catcher to answer. The timeout is
            // SEND_TIMEOUT_BEFORE_FT_START. After that another ICMP request will be
            // sent
            int res = dev->startCaptureBlockingMode(waitForFileTransferStartAck,
                                                    &ftStartData,
                                                    SEND_TIMEOUT_BEFORE_FT_START);
            if (!res)
                EXIT_WITH_ERROR("Cannot start capturing packets");

            // res == 1 means we got the catcher response so we can break the endless
            // loop
            if (res == 1)
                break;
        ...
        uint32_t sleepBetweenPackets = 0;
        // calculate how many microseconds (usec) the pitcher needs to sleep between
        // sending each file data chunk (calculated from user defined packetPerSec
        // parameter). The calculation is done in usec as in most cases the pitcher
        // needs to sleep less than 1 second between chunks. However if packetPerSec
        // equals to 1 it means sleeping for 1 second and in this case we can't use
        // usleep (as it's not working for 1 sec or more) and we use sleep instead
        if (packetPerSec > 1)
            sleepBetweenPackets = (uint32_t)(1000000UL / packetPerSec);
        ...
        // done sending the file to the catcher, send an ICMP request with message
        // type ICMP_FT_END (in the timestamp field) to the catcher to indicate all
        // file was sent
        if (!sendIcmpRequest(dev, pitcherMacAddr, catcherMacAddr, pitcherIP,
                             catcherIP, icmpId, ICMP_FT_END, nullptr, 0))
...
int main(int argc, char* argv[])
{
    ...
    readCommandLineArguments(argc, argv, "pitcher", "catcher", sender, receiver,
                             pitcherIP, catcherIP, fileNameToSend, packetsPerSec,
                             blockSize);
