...
void printUsage(const std::string& thisSide, const std::string& otherSide)
{
    ...
    std::string messagesPerSecLong =
        (thisSide == "pitcher")
            ? "    -p messages_per_sec  : Set number of messages to be sent per "
              "seconds. Default is max possible speed\n"
            : "";
    ...
    std::cout
        ...
        << "Usage:" << std::endl
        << "------" << std::endl
        << pcpp::AppName::get() << " [-h] [-v] [-l] -i " << thisSideInterface
        << " -d " << otherSideIP << " -s file_path -r " << messagesPerSecShort
        << "[-b block_size]" << std::endl
        << std::endl
        << "Options:" << std::endl
        << std::endl
        << "    -i " << thisSideInterface
        << " : Use the specified interface. Can be interface name (e.g eth0) or "
           "interface IPv4 address"
        << std::endl
        << "    -d " << otherSideIP << "        : " << otherSide
        << " IPv4 address" << std::endl
        << "    -s file_path         : Send file mode: send file_path to "
        << otherSide << std::endl
        << "    -r                   : Receive file mode: receive file from "
        << otherSide << std::endl
        << messagesPerSecLong
        << "    -b block_size        : Set the size of data chunk sent in each "
           "ICMP message (in bytes). Default is "
...
void readCommandLineArguments(int argc, char* argv[],
                              const std::string& thisSide,
                              const std::string& otherSide, bool& sender,
                              ...
                              size_t& blockSize)
{
    ...
    if (interfaceNameOrIP.empty())
        EXIT_WITH_ERROR_PRINT_USAGE("Please provide " << thisSide
    ...
    if (sender && receiver)
        EXIT_WITH_ERROR_PRINT_USAGE("Cannot set both send file mode (-s) and "
                                    "receive file mode (-r) switches");

    if (!sender && !receiver)
        EXIT_WITH_ERROR_PRINT_USAGE("Must set either send file mode (-s) or "
                                    "receive file mode (-r) switches");

    // cannot set block size if in receiving file mode
    if (!sender && blockSizeSet)
        EXIT_WITH_ERROR_PRINT_USAGE(
            "Setting block size (-b switch) is relevant for sending files only");

    // validate block size
    if (blockSize < 1 || blockSize > 1464)
        EXIT_WITH_ERROR_PRINT_USAGE(
            "Block size must be a positive integer lower or equal to 1464 bytes "
            "(which is the maximum size for a standard packet)");

    // validate packets per sec
    if (packetsPerSecSet && packetsPerSec < 1)
        EXIT_WITH_ERROR_PRINT_USAGE(
            "message_per_sec must be a positive value greater or equal to 1");
}

bool sendIcmpMessage(pcpp::PcapLiveDevice* dev, pcpp::MacAddress srcMacAddr,
                     ...
                     bool sendRequest)
{
    ...
    if (sendRequest && icmpLayer.setEchoRequestData(icmpMsgId, 0, msgType, data,
                                                    dataLen) == nullptr)
        EXIT_WITH_ERROR("Cannot set ICMP echo request data");
    else if (!sendRequest && icmpLayer.setEchoReplyData(icmpMsgId, 0, msgType,
                                                        data, dataLen) == nullptr)
...
bool sendIcmpRequest(pcpp::PcapLiveDevice* dev, pcpp::MacAddress srcMacAddr,
