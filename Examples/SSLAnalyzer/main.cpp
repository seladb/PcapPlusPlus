 ...
 * ========================
 * This application analyzes SSL/TLS traffic and presents detailed and diverse
 ...
 * data per flow
 * - SSL/TLS data: number of client-hello and server-hello messages, number of
 * flows ended with successful handshake, number of flows ended with SSL alert
 * - hostname map (which hostnames were used and how much. Taken from the
 * server-name-indication extension in the client-hello message)
...
#define PRINT_STAT_LINE(description, counter, measurement)                      \
    std::cout << std::left << std::setw(46) << (std::string(description) + ":") \
...
void printUsage()
{
    std::cout << std::endl
              ...
              << "    -f           : The input pcap/pcapng file to analyze. "
                 "Required argument for this mode"
              << std::endl
              ...
              << "-------------------------" << std::endl
              << pcpp::AppName::get()
              << " [-hvld] [-o output_file] [-r calc_period] -i interface"
              << std::endl
              << std::endl
              << "Options:" << std::endl
              << std::endl
              ...
              << std::endl
              << "    -o output_file : Save all captured SSL packets to a pcap "
                 "file. Notice this may cause performance degradation"
              << std::endl
              << "    -r calc_period : The period in seconds to calculate rates. "
                 "If not provided default is 2 seconds"
              << std::endl
...
/**
 * An auxiliary method for sorting the string count map. Used in
...
void printServerNames(ClientHelloStats& clientHelloStatsCollector)
{
    ...
    std::vector<std::pair<std::string, int>> map2vec(
        clientHelloStatsCollector.serverNameCount.begin(),
                   ...
                   const std::string& headline)
{
    ...
    // since it's not possible to sort a std::map you must copy it to a
    // std::vector and sort it then
    std::vector<std::pair<uint16_t, int>> map2vec(versionMap.begin(),
                                                  versionMap.end());
...
void printCipherSuites(ServerHelloStats& serverHelloStats)
{
    ...
    // std::vector and sort it then
    std::vector<std::pair<std::string, int>> map2vec(
        serverHelloStats.cipherSuiteCount.begin(),
...
void printPorts(SSLGeneralStats& stats)
{
    ...
    // since it's not possible to sort a std::map you must copy it to a
    // std::vector and sort it then
    std::vector<std::pair<uint16_t, int>> map2vec(stats.sslPortCount.begin(),
                                                  stats.sslPortCount.end());
    std::sort(map2vec.begin(), map2vec.end(), &uint16CountComparer);
...
void printStatsSummary(SSLStatsCollector& collector)
{
    ...
    PRINT_STAT_LINE("Sample time", collector.getGeneralStats().sampleTime,
                    "Seconds");
    PRINT_STAT_LINE("Number of SSL packets",
                    collector.getGeneralStats().numOfSSLPackets, "Packets");
    PRINT_STAT_LINE("Rate of SSL packets",
                    collector.getGeneralStats().sslPacketRate.totalRate,
                    "Packets/sec");
    PRINT_STAT_LINE("Number of SSL flows",
                    collector.getGeneralStats().numOfSSLFlows, "Flows");
    PRINT_STAT_LINE("Rate of SSL flows",
                    collector.getGeneralStats().sslFlowRate.totalRate,
                    "Flows/sec");
    PRINT_STAT_LINE("Total SSL data",
                    collector.getGeneralStats().amountOfSSLTraffic, "Bytes");
    PRINT_STAT_LINE("Rate of SSL data",
                    collector.getGeneralStats().sslTrafficRate.totalRate,
                    "Bytes/sec");
    ...
    PRINT_STAT_LINE("Average data per flow",
                    ...
                    "Bytes");
    PRINT_STAT_LINE("Client-hello message",
                    collector.getClientHelloStats().numOfMessages, "Messages");
    PRINT_STAT_LINE("Server-hello message",
                    collector.getServerHelloStats().numOfMessages, "Messages");
    PRINT_STAT_LINE("Number of SSL flows with successful handshake",
                    collector.getGeneralStats().numOfHandshakeCompleteFlows,
                    "Flows");
    PRINT_STAT_LINE("Number of SSL flows ended with alert",
                    collector.getGeneralStats().numOfFlowsWithAlerts, "Flows");

    printStatsHeadline("SSL/TLS ports count");
    printPorts(collector.getGeneralStats());

    printStatsHeadline("SSL/TLS versions count");
    printVersions(collector.getGeneralStats().sslVersionCount,
                  std::string("SSL/TLS version"));
...
void printCurrentRates(SSLStatsCollector& collector)
{
    ...
    PRINT_STAT_LINE("Rate of SSL packets",
                    ...
                    "Packets/sec");
    PRINT_STAT_LINE("Rate of SSL flows",
                    collector.getGeneralStats().sslFlowRate.currentRate,
                    "Flows/sec");
    PRINT_STAT_LINE("Rate of SSL data",
                    collector.getGeneralStats().sslTrafficRate.currentRate,
                    "Bytes/sec");
    PRINT_STAT_LINE("Rate of SSL requests",
                    collector.getClientHelloStats().messageRate.currentRate,
                    "Requests/sec");
    PRINT_STAT_LINE("Rate of SSL responses",
                    collector.getServerHelloStats().messageRate.currentRate,
                    "Responses/sec");
}
...
void analyzeSSLFromPcapFile(const std::string& pcapFileName)
{
    ...
    // read the input file packet by packet and give it to the SSLStatsCollector
                               ...
                               const std::string& savePacketsToFileName)
{
    ...
    // The check is made for well known ports because currently SSLLayer does not
    // support customizing of ports considered as SSL/TLS.
...
int main(int argc, char* argv[])
{
    ...
    else // analyze in live traffic mode
    {
        ...
        analyzeSSLFromLiveTraffic(dev, printRatesPeriodically, printRatePeriod,
                                  savePacketsToFileName);
    }
}
