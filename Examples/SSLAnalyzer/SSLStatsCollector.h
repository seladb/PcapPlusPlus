...
struct SSLGeneralStats {
    ...
    int numOfHandshakeCompleteFlows;   // number of flows which handshake was
                                       // complete
    int numOfFlowsWithAlerts;          // number of flows that were terminated because of
                                       // SSL/TLS alert
...
struct ClientHelloStats {
    ...
    std::map<std::string, int>
        serverNameCount; // a map for counting the server names seen in traffic
...
struct ServerHelloStats {
    ...
    std::map<std::string, int>
        cipherSuiteCount; // count of the different chosen cipher-suites
    ...
    void calcRates() {
        ...
        if (diffSecTotal != 0) {
            ...
            m_GeneralStats.sslPacketRate.totalRate =
                m_GeneralStats.numOfSSLPackets / diffSecTotal;
            m_GeneralStats.sslFlowRate.totalRate =
                m_GeneralStats.numOfSSLFlows / diffSecTotal;
            m_ClientHelloStats.messageRate.totalRate =
                m_ClientHelloStats.numOfMessages / diffSecTotal;
            m_ServerHelloStats.messageRate.totalRate =
                m_ServerHelloStats.numOfMessages / diffSecTotal;
        }
