...
void printUsage()
{
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " input_file -o output_file [-d frag_ids] [-f bpf_filter] [-a] [-h] "
           "[-v]"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        ...
        << std::endl
        << "    -d frag_ids     : De-fragment only fragments that match this "
           "comma-separated list of IP IDs (for IPv4) or"
        << std::endl
        << "                      fragment IDs (for IPv6) in decimal format"
        << std::endl
        << "    -f bpf_filter   : De-fragment only fragments that match "
           "bpf_filter. Filter should be provided in Berkeley Packet Filter (BPF)"
        << std::endl
        << "                      syntax (http://biot.com/capstats/bpf.html) "
           "i.e: 'ip net 1.1.1.1'"
        << std::endl
        << "    -a              : Copy all packets (those who were de-fragmented "
...
/**
 * This method reads packets from the input file, decided which fragments pass
 * the filters set by the user, de-fragment the fragments who pass them, and
 * writes the result packets to the output file
 */
                    ...
                    bool copyAllPacketsToOutputFile, DefragStats& stats)
{
    ...
    while (reader->getNextPacket(rawPacket))
    {
        ...
        if (defragPacket)
        {
            ...
            // - packet isn't a fragment or isn't an IP packet and the user asked to
            // write all packets to output
...
void printStats(const DefragStats& stats, bool filterByIpID, bool filterByBpf)
{
    ...
    if (filterByIpID)
    {
        ...
        stream << "IPv6 packets match fragment ID list:     "
               << stats.ipv6PacketsMatchFragIDs << std::endl;
    }
    if (filterByBpf)
        stream << "IP packets match BPF filter:             "
               << stats.ipPacketsMatchBpfFilter << std::endl;
    stream << "Total fragments matched:                 "
           << (stats.ipv4FragmentsMatched + stats.ipv6FragmentsMatched)
           << std::endl;
    stream << "IPv4 fragments matched:                  "
           << stats.ipv4FragmentsMatched << std::endl;
    stream << "IPv6 fragments matched:                  "
           << stats.ipv6FragmentsMatched << std::endl;
    stream << "Total packets reassembled:               "
           << (stats.ipv4PacketsDefragmented + stats.ipv6PacketsDefragmented)
           << std::endl;
    stream << "IPv4 packets reassembled:                "
           << stats.ipv4PacketsDefragmented << std::endl;
    stream << "IPv6 packets reassembled:                "
           << stats.ipv6PacketsDefragmented << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    DefragStats stats;
    processPackets(reader, writer, filterByBpfFilter, bpfFilter, filterByFragID,
                   fragIDMap, copyAllPacketsToOutputFile, stats);
