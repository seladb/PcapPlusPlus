...
void printUsage() {
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " input_file -s frag_size -o output_file [-d ip_ids] [-f bpf_filter] "
           "[-a] [-h] [-v]"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        ...
        << "    -o output_file  : Output file. Output file type (pcap/pcapng) "
           "will match the input file type"
        << std::endl
        << "    -d ip_ids       : Fragment only packets that match this "
           "comma-separated list of IP IDs in decimal format"
        << std::endl
        << "    -f bpf_filter   : Fragment only packets that match bpf_filter. "
           "Filter should be provided in Berkeley Packet Filter (BPF)"
        << std::endl
        << "                      syntax (http://biot.com/capstats/bpf.html) "
           "i.e: 'ip net 1.1.1.1'"
        << std::endl
        << "    -a              : Copy all packets (those who were fragmented "
           "and those who weren't) to output file"
        << std::endl
 ...
 */
void setIPv4FragmentParams(pcpp::IPv4Layer* fragIpLayer, size_t fragOffset,
                           bool lastFrag) {
...
/**
 * Add IPv6 fragmentation extension to an IPv6 fragment packet and set
 * fragmentation parameters
 */
void setIPv6FragmentParams(pcpp::IPv6Layer* fragIpLayer, size_t fragOffset,
                           bool lastFrag, uint32_t fragId) {
...
/**
 * Generate a 4-byte positive random number. Used for generating IPv6 fragment
 * ID
 */
...
/**
 * A method that takes a raw packet and a requested fragment size and splits the
 * packet into fragments. Fragments are written to a  RawPacketVector instance
 * supplied by the user. The input packet isn't modified in any way. If the
 * packet isn't of type IPv4 or IPv6, nothing happens and the result vector
 * remains empty. If the packet payload size is smaller or equal than the
 * request fragment size the packet isn't fragmented, but the packet is copied
 * and pushed into the result vector
 */
void splitIPPacketToFragmentsBySize(pcpp::RawPacket* rawPacket,
                                    size_t fragmentSize,
    ...
    // if packet payload size is less than the requested fragment size, don't
    // fragment and return
    ...
    // go over the payload and create fragments until reaching the end of the
    // payload
    size_t curOffset = 0;
    while (curOffset < ipLayer->getLayerPayloadSize()) {
        ...
        // check if this is the last fragment by comparing the size of the rest of
        // the payload to the requested fragment size
        ...
        // first, duplicate the input packet and create a new parsed packet out of
        // it
        pcpp::RawPacket* newFragRawPacket =
            new pcpp::RawPacket(*packet.getRawPacket());
        ...
        // create a new PayloadLayer with the fragmented data and add it to the new
        // fragment packet
        pcpp::PayloadLayer newPayload(ipLayer->getLayerPayload() + curOffset,
                                      curFragSize, false);
        newFrag.addLayer(&newPayload);

        // set fragment parameters in IPv4/6 layer
        if (ipProto == pcpp::IPv4)
            setIPv4FragmentParams((pcpp::IPv4Layer*)fragIpLayer, curOffset,
                                  lastFrag);
        else // ipProto == IPv6
            setIPv6FragmentParams((pcpp::IPv6Layer*)fragIpLayer, curOffset, lastFrag,
                                  randomNum);
...
/**
 * This method reads packets from the input file, decided which packets pass the
 * filters set by the user, fragment packets who pass them, and write the result
 * packets to the output file
 */
void processPackets(pcpp::IFileReaderDevice* reader,
                    pcpp::IFileWriterDevice* writer, int fragSize,
    ...
    while (reader->getNextPacket(rawPacket)) {
        ...
        if (parsedPacket.isPacketOfType(pcpp::IPv4)) {
            ipProto = pcpp::IPv4;
            stats.ipv4Packets++;
        } else if (parsedPacket.isPacketOfType(
                       pcpp::IPv6)) // check if packet is of type IPv6
        {
            ipProto = pcpp::IPv6;
            stats.ipv6Packets++;
        } else // if not - set the packet as not marked for fragmentation
        ...
        if (filterByIpID) {
            ...
            if (ipLayer != nullptr) {
                ...
                if (ipIDs.find(pcpp::netToHost16(ipLayer->getIPv4Header()->ipId)) !=
                    ipIDs.end()) {
        ...
        if (fragPacket) {
            ...
            // if result list contains only 1 packet it means packet wasn't fragmented
        ...
        // even if packet didn't pass the filters but user requested to write all
        // packet to output file, write it
...
void printStats(const FragStats& stats, bool filterByIpID, bool filterByBpf) {
    ...
    stream << "Total packets read:                      "
           << stats.totalPacketsRead << std::endl;
    stream << "IPv4 packets read:                       " << stats.ipv4Packets
           << std::endl;
    stream << "IPv6 packets read:                       " << stats.ipv6Packets
           << std::endl;
    if (filterByIpID)
        stream << "IPv4 packets match IP ID list:           "
               << stats.ipv4PacketsMatchIpIDs << std::endl;
    if (filterByBpf)
        stream << "IP packets match BPF filter:             "
               << stats.ipPacketsMatchBpfFilter << std::endl;
    stream << "IP packets smaller than fragment size:   "
           << stats.ipPacketsUnderSize << std::endl;
    stream << "IPv4 packets fragmented:                 "
           << stats.ipv4PacketsFragmented << std::endl;
    stream << "IPv6 packets fragmented:                 "
           << stats.ipv6PacketsFragmented << std::endl;
    stream << "Total packets written to output file:    "
           << stats.totalPacketsWritten << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "o:s:d:f:ahv", FragUtilOptions,
                              &optionIndex)) != -1) {
        switch (opt) {
        case 0: {
            break;
    ...
    FragStats stats;
    processPackets(reader, writer, fragSize, filterByBpfFilter, bpfFilter,
                   filterByIpID, ipIDMap, copyAllPacketsToOutputFile, stats);
