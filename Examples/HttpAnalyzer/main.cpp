 ...
 * ========================
 * This application analyzes HTTP traffic and presents detailed and diverse
 * information about it. It can operate in live traffic mode where this
 * information is collected on live packets or in file mode where packets are
 * being read from a pcap/pcapng file. The information collected by this
 * application includes:
 * - general data: number of packets, packet rate, amount of traffic, bandwidth
 * - flow data: number of flow, flow rate, average packets per flow, average
 * data per flow
 * - HTTP data: number and rate of HTTP requests, number and rate of HTTP
 * responses, transaction count and rate, average transactions per flow, HTTP
 * header size (total and average), HTTP body size, number of HTTP pipelining
 * flows
 * - hostname map
 * - HTTP method map
...
#define PRINT_STAT_LINE(description, counter, measurement)                      \
    std::cout << std::left << std::setw(40) << (std::string(description) + ":") \
              << std::right << std::setw(15) << std::fixed << std::showpoint    \
              << std::setprecision(3) << counter << " [" << measurement << "]"  \
              << std::endl;
...
void printUsage() {
    std::cout
        ...
        << "Options:" << std::endl
        << std::endl
        << "    -f             : The input pcap/pcapng file to analyze. Required "
        ...
        << "    -i interface   : Use the specified interface. Can be interface "
           "name (e.g eth0) or interface IPv4 address"
        << std::endl
        << "    -p dst_port    : Use the specified port (optional parameter, the "
           "default is 80)"
        << std::endl
        << "    -o output_file : Save all captured HTTP packets to a pcap file. "
           "Notice this may cause performance degradation"
        << std::endl
        << "    -r calc_period : The period in seconds to calculate rates. If "
           "not provided default is 2 seconds"
        << std::endl
 ...
 */
void httpPacketArrive(pcpp::RawPacket* packet, pcpp::PcapLiveDevice* dev,
                      void* cookie) {
    // parse the packet
...
void printMethods(const HttpRequestStats& reqStatscollector) {
    ...
    std::vector<std::pair<pcpp::HttpRequestLayer::HttpMethod, int>> map2vec(
        reqStatscollector.methodCount.begin(),
        reqStatscollector.methodCount.end());
    std::sort(
        map2vec.begin(), map2vec.end(),
        [](const std::pair<pcpp::HttpRequestLayer::HttpMethod, int>& left,
           const std::pair<pcpp::HttpRequestLayer::HttpMethod, int>& right) {
    ...
    for (auto iter : map2vec) {
        ...
        case pcpp::HttpRequestLayer::HttpGET:
            values << "GET"
                   << "|"
                   << reqStatscollector.methodCount.at(
...
/**
 * An auxiliary method for sorting the hostname count map. Used only in
 * printHostnames()
 */
bool hostnameComparer(const std::pair<std::string, int>& leftHost,
                      const std::pair<std::string, int>& rightHost) {
...
/**
 * Print the hostname count map to a table sorted by popularity (most popular
 * hostnames will be first)
 */
void printHostnames(HttpRequestStats& reqStatscollector) {
    ...
    std::vector<std::pair<std::string, int>> map2vec(
        reqStatscollector.hostnameCount.begin(),
...
void printStatusCodes(const HttpResponseStats& resStatscollector) {
    ...
    std::vector<std::pair<std::string, int>> map2vec(
        resStatscollector.statusCodeCount.begin(),
...
void printContentTypes(const HttpResponseStats& resStatscollector) {
    ...
    std::vector<std::pair<std::string, int>> map2vec(
        resStatscollector.contentTypeCount.begin(),
...
/**
 * Print a summary of all statistics collected by the HttpStatsCollector. Should
...
void printStatsSummary(HttpStatsCollector& collector) {
    ...
    PRINT_STAT_LINE("Sample time", collector.getGeneralStats().sampleTime,
                    "Seconds");
    PRINT_STAT_LINE("Number of HTTP packets",
                    collector.getGeneralStats().numOfHttpPackets, "Packets");
    PRINT_STAT_LINE("Rate of HTTP packets",
                    collector.getGeneralStats().httpPacketRate.totalRate,
                    "Packets/sec");
    PRINT_STAT_LINE("Number of HTTP flows",
                    collector.getGeneralStats().numOfHttpFlows, "Flows");
    PRINT_STAT_LINE("Rate of HTTP flows",
                    collector.getGeneralStats().httpFlowRate.totalRate,
                    "Flows/sec");
    PRINT_STAT_LINE("Number of HTTP pipelining flows",
                    collector.getGeneralStats().numOfHttpPipeliningFlows,
                    "Flows");
    PRINT_STAT_LINE("Number of HTTP transactions",
                    collector.getGeneralStats().numOfHttpTransactions,
                    "Transactions");
    PRINT_STAT_LINE("Rate of HTTP transactions",
                    collector.getGeneralStats().httpTransactionsRate.totalRate,
                    "Transactions/sec");
    PRINT_STAT_LINE("Total HTTP data",
                    collector.getGeneralStats().amountOfHttpTraffic, "Bytes");
    PRINT_STAT_LINE("Rate of HTTP data",
                    collector.getGeneralStats().httpTrafficRate.totalRate,
                    "Bytes/sec");
    PRINT_STAT_LINE("Average packets per flow",
                    collector.getGeneralStats().averageNumOfPacketsPerFlow,
                    "Packets");
    PRINT_STAT_LINE(
        "Average transactions per flow",
        collector.getGeneralStats().averageNumOfHttpTransactionsPerFlow,
        "Transactions");
    PRINT_STAT_LINE("Average data per flow",
                    collector.getGeneralStats().averageAmountOfDataPerFlow,
                    "Bytes");

    printStatsHeadline("HTTP request stats");
    PRINT_STAT_LINE("Number of HTTP requests",
                    collector.getRequestStats().numOfMessages, "Requests");
    PRINT_STAT_LINE("Rate of HTTP requests",
                    collector.getRequestStats().messageRate.totalRate,
                    "Requests/sec");
    PRINT_STAT_LINE("Total data in headers",
                    collector.getRequestStats().totalMessageHeaderSize, "Bytes");
    PRINT_STAT_LINE("Average header size",
                    collector.getRequestStats().averageMessageHeaderSize,
                    "Bytes");

    printStatsHeadline("HTTP response stats");
    PRINT_STAT_LINE("Number of HTTP responses",
                    collector.getResponseStats().numOfMessages, "Responses");
    PRINT_STAT_LINE("Rate of HTTP responses",
                    collector.getResponseStats().messageRate.totalRate,
                    "Responses/sec");
    PRINT_STAT_LINE("Total data in headers",
                    collector.getResponseStats().totalMessageHeaderSize, "Bytes");
    PRINT_STAT_LINE("Average header size",
                    collector.getResponseStats().averageMessageHeaderSize,
                    "Bytes");
    PRINT_STAT_LINE("Num of responses with content-length",
                    collector.getResponseStats().numOfMessagesWithContentLength,
                    "Responses");
    PRINT_STAT_LINE("Total body size (may be compressed)",
                    collector.getResponseStats().totalContentLengthSize, "Bytes");
    PRINT_STAT_LINE("Average body size",
...
void printCurrentRates(HttpStatsCollector& collector) {
    ...
    PRINT_STAT_LINE("Rate of HTTP packets",
                    collector.getGeneralStats().httpPacketRate.currentRate,
                    "Packets/sec");
    PRINT_STAT_LINE("Rate of HTTP flows",
                    collector.getGeneralStats().httpFlowRate.currentRate,
                    "Flows/sec");
    PRINT_STAT_LINE("Rate of HTTP transactions",
                    collector.getGeneralStats().httpTransactionsRate.currentRate,
                    "Transactions/sec");
    PRINT_STAT_LINE("Rate of HTTP data",
                    collector.getGeneralStats().httpTrafficRate.currentRate,
                    "Bytes/sec");
    PRINT_STAT_LINE("Rate of HTTP requests",
                    collector.getRequestStats().messageRate.currentRate,
                    "Requests/sec");
    PRINT_STAT_LINE("Rate of HTTP responses",
                    collector.getResponseStats().messageRate.currentRate,
                    "Responses/sec");
}
 ...
 */
void analyzeHttpFromPcapFile(const std::string& pcapFileName,
                             uint16_t dstPort) {
    ...
    // read the input file packet by packet and give it to the HttpStatsCollector
 ...
 */
void analyzeHttpFromLiveTraffic(pcpp::PcapLiveDevice* dev,
                                bool printRatesPeriodically,
                                int printRatePeriod,
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "i:p:f:o:r:hvld", HttpAnalyzerOptions,
                              &optionIndex)) != -1) {
        switch (opt) {
        case 0:
            break;
    ...
    } else // analyze in live traffic mode
    {
        ...
        analyzeHttpFromLiveTraffic(dev, printRatesPeriodically, printRatePeriod,
                                   savePacketsToFileName, nPort);
    }
}
