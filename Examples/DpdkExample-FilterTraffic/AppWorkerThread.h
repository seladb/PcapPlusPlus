...
/**
 * The worker thread class which does all the work: receive packets from
 * relevant DPDK port(s), matched them with the packet matching engine and send
 * them to TX port and/or save them to a file. In addition it collects packets
 * statistics. Each core is assigned with one such worker thread, and all of
                 ...
                 iter != m_WorkerConfig.InDataCfg.end(); iter++)
            {
                     ...
                     iter2 != iter->second.end(); iter2++)
                {
                    ...
                    for (int i = 0; i < packetsReceived; i++)
                    {
                        ...
                        // hash the packet by 5-tuple and look in the flow table to see
                        // whether this packet belongs to an existing or new flow
                        uint32_t hash = pcpp::hash5Tuple(&parsedPacket);
                        std::map<uint32_t, bool>::const_iterator iter3 =
