 ...
 * =======================================
 * An application that listens to one or more DPDK ports (a.k.a DPDK devices),
 * captures all traffic and matches packets by user-defined matching criteria.
 * Matching criteria is given on startup and can contain one or more of the
 * following: source IP, destination IP, source TCP/UDP port, destination
 * TCP/UDP port and TCP or UDP protocol. Matching is done per flow, meaning the
 * first packet received on a flow is matched against the matching criteria and
 * if it's matched then all packets of the same flow will be matched too.
 * Packets that are matched can be send to a DPDK port and/or be save to a pcap
 ...
 * of matched packets.
 *
 * The application uses the concept of worker threads. Number of cores can be
 * set by the user or set to default (default is all machine cores minus one
 * management core). Each core is assigned with one worker thread. The
 * application divides the DPDK ports and RX queues equally between worker
 * threads. For example: if there are 2 DPDK ports to listen to, each one with 6
 * RX queues and there are 3 worker threads, then worker #1 will get RX queues
 * 1-4 of port 1, worker #2 will get RX queues 5-6 of port 1 and RX queues 1-2
 * of port 2, and worker #3 will get RX queues 3-6 of port 2. Each worker thread
 * does exactly the same work: receiving packets, collecting packet statistics,
 * matching flows and sending/saving matched packets
 *
 * __Important__: this application (like all applications using DPDK) should be
 * run as 'sudo'
 */
...
void printUsage() {
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " [-hvl] [-s PORT] [-f FILENAME] [-i IPV4_ADDR] [-I IPV4_ADDR] [-p "
           "PORT] [-P PORT] [-r PROTOCOL]"
        << std::endl
        << "                  [-c CORE_MASK] [-m POOL_SIZE] [-r NUM_QUEUES] [-t "
           "NUM_QUEUES] -d PORT_1,PORT_3,...,PORT_N"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        << std::endl
        ...
        << std::endl
        << "    -l|--list                                  : Print the list of "
           "DPDK ports and exists"
        << std::endl
        << "    -d|--dpdk-ports PORT_1,PORT_3,...,PORT_N   : A comma-separated "
           "list of DPDK port numbers to receive"
        << std::endl
        << "                                                 packets from. To "
           "see all available DPDK ports use the -l switch"
        << std::endl
        << "    -s|--send-matched-packets PORT             : DPDK port to send "
           "matched packets to"
        << std::endl
        << "    -f|--save-matched-packets FILEPATH         : Save matched "
           "packets to pcap files under FILEPATH. Packets"
        << std::endl
        << "                                                 matched by core X "
           "will be saved under 'FILEPATH/CoreX.pcap'"
        << std::endl
        << "    -i|--match-source-ip      IPV4_ADDR        : Match source IPv4 "
           "address"
        << std::endl
        << "    -I|--match-dest-ip        IPV4_ADDR        : Match destination "
           "IPv4 address"
        << std::endl
        << "    -p|--match-source-port    PORT             : Match source "
           "TCP/UDP port"
        << std::endl
        << "    -P|--match-dest-port      PORT             : Match destination "
           "TCP/UDP port"
        << std::endl
        << "    -o|--match-protocol       PROTOCOL         : Match protocol. "
           "Valid values are 'TCP' or 'UDP'"
        << std::endl
        << "    -c|--core-mask            CORE_MASK        : Core mask of cores "
           "to use."
        << std::endl
        << "                                                 For example: use 7 "
        ...
        << std::endl
        << "                                                 Default is using "
           "all cores except management core"
        << std::endl
        << "    -m|--mbuf-pool-size       POOL_SIZE        : DPDK mBuf pool size "
           "to initialize DPDK with."
        << std::endl
        << "                                                 Default value is "
           "4095"
        << std::endl
        << "    -r|--rx-queues            NUM_QUEUES       : Number of RX queues "
           "to open. Cannot exceed the max allowed by the NIC or "
        << MAX_QUEUES << std::endl
        << "                                                 The default is 1"
        << std::endl
        << "    -t|--tx-queues            NUM_QUEUES       : Number of TX queues "
           "to open. Cannot exceed the max allowed by the NIC or "
        << MAX_QUEUES << std::endl
        << "                                                 The default is 1"
        << std::endl
...
/**
 * Prepare the configuration for each core. Configuration includes: which
 * DpdkDevices and which RX queues to receive packets from, where to send the
 * matched packets, etc.
 */
void prepareCoreConfiguration(std::vector<pcpp::DpdkDevice*>& dpdkDevicesToUse,
    ...
    // calculate how many RX queues each core will read packets from. We divide
    // the total number of RX queues with total number of core
    int numOfRxQueuesPerCore = totalNumOfRxQueues / coresToUse.size();
    int rxQueuesRemainder = totalNumOfRxQueues % coresToUse.size();

    // prepare the configuration for every core: divide the devices and RX queue
    // for each device with the various cores
    int i = 0;
    ...
    for (std::vector<pcpp::SystemCore>::iterator iter = coresToUse.begin();
        ...
        packetFileName << packetFilePath << "Core" << workerConfigArr[i].CoreId
                       << ".pcap";
...
void onApplicationInterrupted(void* cookie) {
    ...
    // print final stats for every worker thread plus sum of all threads and free
...
/**
 * main method of the application. Responsible for parsing user args, preparing
 * worker thread configuration, creating the worker threads and activate them.
...
int main(int argc, char* argv[]) {
    ...
    // removing DPDK master core from core mask because DPDK worker threads cannot
    // run on master core
    coreMaskToUse =
        coreMaskToUse &
        ~(pcpp::DpdkDeviceList::getInstance().getDpdkMasterCore().Mask);
    ...
    for (std::vector<pcpp::DpdkDevice*>::iterator iter =
        ...
        std::cout << "Opened device #" << (*iter)->getDeviceId() << " with "
                  << rxQueues << " RX queues and " << txQueues << " TX queues."
    ...
    prepareCoreConfiguration(dpdkDevicesToUse, coresToUse, writePacketsToDisk,
                             packetFilePath, sendPacketsTo, workerConfigArr,
                             coresToUse.size(), rxQueues);
