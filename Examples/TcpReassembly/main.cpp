 ...
 * =========================
 * This is an application that captures data transmitted as part of TCP
 * connections, organizes the data and stores it in a way that is convenient for
 * protocol analysis and debugging. This application reconstructs the TCP data
 * streams and stores each connection in a separate file(s). TcpReassembly
 * understands TCP sequence numbers and will correctly reconstruct data streams
 * regardless of retransmissions, out-of-order delivery or data loss.
 * TcpReassembly works more or less the same like tcpflow
 * (https://linux.die.net/man/1/tcpflow) but probably with less options. The
 * main purpose of it is to demonstrate the TCP reassembly capabilities in
 * PcapPlusPlus. Main features and capabilities:
 ...
 *   - Limit the max number of open files in each point in time (to avoid
 * running out of file descriptors for large files / heavy traffic)
 *   - Write a metadata file (txt file) for each connection with various stats
 * on the connection: number of packets (in each side + total), number of TCP
 * messages (in each side + total), number of bytes (in each side + total)
...
// unless the user chooses otherwise - default number of concurrent used file
// descriptors is 500
...
class GlobalConfig {
   ...
   */
    GlobalConfig()
        : m_RecentConnsWithActivity(nullptr), writeMetadata(false),
          writeToConsole(false), separateSides(false),
          maxOpenFiles(DEFAULT_MAX_NUMBER_OF_CONCURRENT_OPEN_FILES) {}

    // A least-recently-used (LRU) list of all connections seen so far. Each
    // connection is represented by its flow key. This LRU list is used to decide
    // which connection was seen least recently in case we reached max number of
    // open file descriptors and we need to decide which files to close
    pcpp::LRUList<uint32_t>* m_RecentConnsWithActivity;

  public:
    // a flag indicating whether to write a metadata file for each connection
    // (containing several stats)
    bool writeMetadata;
    ...
    // a flag indicating whether to write both side of a connection to the same
    // file (which is the default) or write each side to a separate file
    bool separateSides;
    ...
    /**
   * A method getting connection parameters as input and returns a filename and
   * file path as output. The filename is constructed by the IPs (src and dst)
   ...
   */
    std::string getFileName(pcpp::ConnectionData connData, int side,
        ...
        // - add the dir path to the return value
        if (!outputDir.empty())
        ...
        if (side <= 0 || !useSeparateSides)
            stream << sourceIP << '.' << connData.srcPort << '-' << destIP << '.'
                   << connData.dstPort;
        else // side == 1 means data is sent from server->client
            stream << destIP << '.' << connData.dstPort << '-' << sourceIP << '.'
                   << connData.srcPort;
    ...
    /**
   * Open a file stream. Inputs are the filename to open and a flag indicating
   ...
   */
    pcpp::LRUList<uint32_t>* getRecentConnsWithActivity() {
        ...
        // DEFAULT_MAX_NUMBER_OF_CONCURRENT_OPEN_FILES but the user can choose
        // another number
   ...
   * d'tor
   */
    ~GlobalConfig() { delete m_RecentConnsWithActivity; }
};
...
struct TcpReassemblyData {
    ...
    // flags indicating whether the file in each side was already opened before.
    // If the answer is yes, next time it'll be opened in append mode (and not in
    // overwrite mode)
    ...
    // stats data: num of data packets on each side, bytes seen on each side and
    // messages seen on each side
...
void printUsage() {
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " [-hvlcms] [-r input_file] [-i interface] [-o output_dir] [-e "
           "bpf_filter] [-f max_files]"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        << std::endl
        << "    -r input_file : Input pcap/pcapng file to analyze. Required "
           "argument for reading from file"
        << std::endl
        << "    -i interface  : Use the specified interface. Can be interface "
           "name (e.g eth0) or interface IPv4 address. Required argument for "
           "capturing from live interface"
        << std::endl
        << "    -o output_dir : Specify output directory (default is '.')"
        << std::endl
        << "    -e bpf_filter : Apply a BPF filter to capture file or live "
           "interface, meaning TCP reassembly will only work on filtered packets"
        << std::endl
        << "    -f max_files  : Maximum number of file descriptors to use"
        << std::endl
        << "    -c            : Write all output to console (nothing will be "
           "written to files)"
        << std::endl
        << "    -m            : Write a metadata file for each connection"
        << std::endl
        << "    -s            : Write each side of each connection to a separate "
           "file (default is writing both sides of each connection to the same "
           "file)"
        << std::endl
...
/**
 * The callback being called by the TCP reassembly module whenever new data
 * arrives on a certain connection
 */
static void tcpReassemblyMsgReadyCallback(const int8_t sideIndex,
    ...
    // if the user wants to write each side in a different file - set side as the
    // sideIndex, otherwise write everything to the same file ("side 0")
    ...
    if (flow->second.fileStreams[side] == nullptr) {
        ...
        // file(s) in order to open a new one. The connection with the least
        // recently used file is the return value
        uint32_t flowKeyToCloseFiles;
        int result = GlobalConfig::getInstance().getRecentConnsWithActivity()->put(
            ...
            if (flow2 != connMgr->end()) {
                ...
                for (int index = 0; index < 2; index++) {
                    if (flow2->second.fileStreams[index] != nullptr) {
                        ...
                        // set the reopen flag to true to indicate that next time this file
                        // will be opened it will be opened in append mode (and not
                        // overwrite mode)
        ...
        std::string fileName = GlobalConfig::getInstance().getFileName(
                                   tcpData.getConnectionData(), sideIndex,
                                   GlobalConfig::getInstance().separateSides) +
                               ".txt";

        // open the file in overwrite mode (if this is the first time the file is
        // opened) or in append mode (if it was already opened before)
        flow->second.fileStreams[side] = GlobalConfig::getInstance().openFileStream(
            fileName, flow->second.reopenFileStreams[side]);
    }
    ...
    flow->second.fileStreams[side]->write((char*)tcpData.getData(),
                                          tcpData.getDataLength());
}

/**
 * The callback being called by the TCP reassembly module whenever a new
 * connection is found. This method adds the connection to the connection
 * manager
 */
static void
...
/**
 * The callback being called by the TCP reassembly module whenever a connection
 * is ending. This method removes the connection from the connection manager and
 * writes the metadata file if requested by the user
 */
static void tcpReassemblyConnectionEndCallback(
    ...
    if (GlobalConfig::getInstance().writeMetadata) {
        ...
        metadataFile << "Number of data packets in side 0:  "
                     << connection->second.numOfDataPackets[0] << std::endl;
        metadataFile << "Number of data packets in side 1:  "
                     << connection->second.numOfDataPackets[1] << std::endl;
        metadataFile << "Total number of data packets:      "
                     << (connection->second.numOfDataPackets[0] +
                         connection->second.numOfDataPackets[1])
                     << std::endl;
        metadataFile << std::endl;
        metadataFile << "Number of bytes in side 0:         "
                     << connection->second.bytesFromSide[0] << std::endl;
        metadataFile << "Number of bytes in side 1:         "
                     << connection->second.bytesFromSide[1] << std::endl;
        metadataFile << "Total number of bytes:             "
                     << (connection->second.bytesFromSide[0] +
                         connection->second.bytesFromSide[1])
                     << std::endl;
        metadataFile << std::endl;
        metadataFile << "Number of messages in side 0:      "
                     << connection->second.numOfMessagesFromSide[0] << std::endl;
        metadataFile << "Number of messages in side 1:      "
                     << connection->second.numOfMessagesFromSide[1] << std::endl;
        metadataFile.close();
    }
...
void doTcpReassemblyOnPcapFile(const std::string& fileName,
    ...
    // after all packets have been read - close the connections which are still
    // opened
...
void doTcpReassemblyOnLiveTraffic(pcpp::PcapLiveDevice* dev,
    ...
    // start capturing packets. Each packet arrived will be handled by
    ...
    std::cout << "Done! processed "
              << tcpReassembly.getConnectionInformation().size() << " connections"
              << std::endl;
}
