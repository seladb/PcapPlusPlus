 ...
 * ================================
 * This application resolves a target MAC address by its IPv4 address by sending
 * an ARP request and translating the ARP response. Its basic input is the
 * target IP address and the interface name/IP to send the ARP request from
 */
...
void printUsage() {
    std::cout << std::endl
              ...
              << "------" << std::endl
              << pcpp::AppName::get()
              << " [-hvl] [-c count] [-w timeout] [-s mac_addr] [-S ip_addr] -i "
                 "interface -T ip_addr"
              << std::endl
              << std::endl
              << "Options:" << std::endl
...
int main(int argc, char* argv[]) {
    ...
    if (!ifaceNameOrIpProvided)
        EXIT_WITH_ERROR(
            "You must provide at least interface name or interface IP (-i switch)");
    ...
    // if source MAC is still invalid, it means it couldn't be extracted from
    // interface
    ...
    if (!sourceIP.isValid() || sourceIP == pcpp::IPv4Address::Zero)
        EXIT_WITH_ERROR("Source IPv4 address wasn't supplied and couldn't be "
    ...
    while (i <= maxTries && !shouldStop) {
        ...
        } else // Succeeded fetching MAC address
        {
            ...
            std::cout << "Reply from " << targetIP << " "
                      << "[" << result << "]  " << std::fixed << arpResponseTimeMS
                      << "ms  "
                      << "index=" << i << std::endl;
        }
