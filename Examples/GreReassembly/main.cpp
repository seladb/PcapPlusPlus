...
static struct option GreAssemblyOptions[] = {{"interface", required_argument, 0, 'i'},
...
class GlobalConfig
{
  ...
  public:
	...
	// a flag indicating whether to write GRE data to actual files or to console
	bool writeToConsole;
...
typedef std::map<std::string, GreReassemblyData> GreReassemblyMgr;
typedef std::map<std::string, GreReassemblyData>::iterator GreReassemblyMgrIter;
...
void printUsage()
{
	std::cout << std::endl
			  ...
			  << "    -e bpf_filter : Apply a BPF filter to capture file or live interface, meaning GRE reassembly "
...
static void OnGreMessageReadyCallback(pcpp::GrePacketData *greData, void *userCookie)
{
	/* 	1. manager 存 GreReassemblyData   									yes
		2. manager 的指定 GreReassemblyData 里边fileStream 是否为NULL
			2.1 将当前（指传入的参数）的名称加入opened列表
			2.2 如果打开的文件已达上限，关闭目前的
			2.3 设置文件名
			2.4 打开文件， 模式由之前2.2设置的reopenFileStreams决定
		3. 更改GreReassemblyData里的统计值
		4. 将数据写入打开的文件里
	 */
	...
	GreReassemblyMgr *mgr = (GreReassemblyMgr *)userCookie;

	// check if this tuple already appears in the manager. If not add it
	GreReassemblyMgrIter iter = mgr->find(greData->getTupleName());
	if (iter == mgr->end())
	{
		mgr->insert(std::make_pair(greData->getTupleName(), GreReassemblyData()));
		iter = mgr->find(greData->getTupleName());
	}
	...
	if (iter->second.fileStream == NULL)
	{
		...
		int result =
			GlobalConfig::getInstance().getRecentFilesWithActivity()->put(greData->getTupleName(), &nameToCloseFile);
		...
		if (result == 1)
		{
			GreReassemblyMgrIter iter2 = mgr->find(nameToCloseFile);
			if (iter2 != mgr->end())
			{
		...
		std::string name = greData->getTupleName() + ".txt";
	...
	iter->second.bytes += (int)greData->getDataLength();
	...
	iter->second.fileStream->write((char *)greData->getData(), greData->getDataLength());
}
 ...
 */
static void onPacketArrives(pcpp::RawPacket *packet, pcpp::PcapLiveDevice *dev, void *greReassemblyCookie)
{
	// get a pointer to the GRE reassembly instance and feed the packet arrived to it
	pcpp::GREReassembly *greReassembly = (pcpp::GREReassembly *)greReassemblyCookie;
	greReassembly->reassemblePacket(packet);
}

/**
 * The method responsible for GRE reassembly on pcap/pcapng files
 */
void doGreReassemblyOnPcapFile(std::string fileName, pcpp::GREReassembly &greReassembly, std::string bpfFilter = "")
{
	...
	// run in a loop that reads one packet from the file in each iteration and feeds it to the GRE reassembly instance
	pcpp::RawPacket rawPacket;
	while (reader->getNextPacket(rawPacket))
	{
		greReassembly.reassemblePacket(&rawPacket);
	}
...
/**
 * The method responsible for GRE reassembly on live traffic
 */
void doGreReassemblyOnLiveTraffic(pcpp::PcapLiveDevice *dev, pcpp::GREReassembly &greReassembly,
								  std::string bpfFilter = "")
{
	...
	dev->startCapture(onPacketArrives, &greReassembly);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "i:r:o:e:f:mcvhl", GreAssemblyOptions, &optionIndex)) != -1)
	{
		switch (opt)
		{
		case 0:
	...
	GreReassemblyMgr mgr;

	//  create the GRE reassembly instance
	pcpp::GREReassembly greReassembly(OnGreMessageReadyCallback, &mgr);

	// analyze in pcap file mode
	if (!inputPcapFileName.empty())
	{
		doGreReassemblyOnPcapFile(inputPcapFileName, greReassembly, bpfFilter);
	}
	else // analyze in live traffic mode
	{
		...
		// start capturing packets and do GRE reassembly
		doGreReassemblyOnLiveTraffic(dev, greReassembly, bpfFilter);
	}
}
