 ...
 * ==========================================
 * An application that listens to one or more PF_RING interface, captures all
 * traffic and matches packets by user-defined matching criteria. Matching
 * criteria is given on startup and can contain one or more of the following:
 * source IP, destination IP, source TCP/UDP port, destination TCP/UDP port and
 * TCP or UDP protocol. Matching is done per flow, meaning the first packet
 * received on a flow is matched against the matching criteria and if it's
 * matched then all packets of the same flow will be matched too. Packets that
 * are matched can be send to another PF_RING interface and/or be save to a pcap
 * file. In addition the application collect statistics on received and matched
 * packets: number of packets per protocol, number of matched flows and number
 * of matched packets.
 *
 * The application uses PfRingDevice's multi-threaded capturing. Number of
 * capture threads can be set by the user (to the maximum of machine's core
 * number minus 1) or set to default (default is all machine cores minus one
 * management core the application runs on). Each core is assigned with one
 * capture thread. PfRingDevice tries to assign one RX channel for each
 * capturing thread (to improve performance), but if NIC doesn't enough RX
 * channels to provide one for each thread, it will assign several thread with
 * the same RX channel For example: if NIC supports 4 RX channels but the user
 * asks for 6 capturing threads than 4 cores will share 2 RX channels and the 2
 * remaining cores will use RX channels of their own. Each capturing thread does
 * exactly the same work: receiving packets, collecting packet statistics,
 * matching flows and sending/saving matched packets
 *
 * Another thing shown here is getting interface capabilities such as total RX
 * channels available, MAC address, PF_RING interface index, MTU, etc.
 *
 * __Important__:
 * 1. Before compiling this application make sure you set "Compile PcapPlusPlus
 * with PF_RING" to "y" in configure-linux.sh. Otherwise the application won't
 * compile
 * 2. Before running the application make sure you load the PF_RING kernel
 * module: sudo insmod <PF_RING_LOCATION>/kernel/pf_ring.ko Otherwise the
 * application will exit with an error log that instructs you to load the kernel
 * module
 * 3. This application (like all applications using PF_RING) should be run as
 * 'sudo'
 */
...
struct CaptureThreadArgs {
    ...
    CaptureThreadArgs()
        : packetStatArr(NULL), matchingEngine(NULL), flowTables(NULL),
          sendPacketsTo(NULL), pcapWriters(NULL) {}
};
...
void printUsage() {
    std::cout << std::endl
              ...
              << "------" << std::endl
              << pcpp::AppName::get()
              << " [-hvl] [-s INTERFACE_NAME] [-f FILENAME] [-i IPV4_ADDR] [-I "
                 "IPV4_ADDR] [-p PORT] [-P PORT] [-r PROTOCOL]"
              << std::endl
              ...
              << std::endl
              << "    -l|--list                                  : Print the "
                 "list of PF_RING devices and exists"
              << std::endl
              << "    -n|--interface-name       INTERFACE_NAME   : A PF_RING "
                 "interface name to receive packets from."
              << std::endl
              << "                                                 To see all "
                 "available interfaces use the -l switch"
              << std::endl
              << "    -s|--send-matched-packets INTERFACE_NAME   : PF_RING "
                 "interface name to send matched packets to"
              << std::endl
              << "    -f|--save-matched-packets FILEPATH         : Save matched "
                 "packets to pcap files under FILEPATH."
              << std::endl
              << "                                                 Packets "
                 "matched by thread X will be saved under"
              << std::endl
              << "                                                 "
                 "'FILEPATH/ThreadX.pcap'"
              << std::endl
              << "    -i|--match-source-ip      IPV4_ADDR        : Match source "
                 "IPv4 address"
              << std::endl
              << "    -I|--match-dest-ip        IPV4_ADDR        : Match "
                 "destination IPv4 address"
              << std::endl
              << "    -p|--match-source-port    PORT             : Match source "
                 "TCP/UDP port"
              << std::endl
              << "    -P|--match-dest-port      PORT             : Match "
                 "destination TCP/UDP port"
              << std::endl
              << "    -r|--match-protocol       PROTOCOL         : Match "
                 "protocol. Valid values are 'TCP' or 'UDP'"
              << std::endl
              << "    -t|--num-of-threads       NUM_OF_THREADS   : Number of "
                 "capture threads to open. Should be in"
              << std::endl
              << "                                                 the range of "
                 "1 to NUM_OF_CORES_ON_MACHINE-1."
              << std::endl
              << "                                                 Default is "
                 "using all machine cores except the core"
              << std::endl
              << "                                                 the "
...
void listPfRingDevices() {
    // suppress errors as there may be devices (like lo) that their MAC address
    // can't be read, etc.
    pcpp::Logger::getInstance().suppressLogs();

    const std::vector<pcpp::PfRingDevice*>& devList =
        pcpp::PfRingDeviceList::getInstance().getPfRingDevicesList();
    for (std::vector<pcpp::PfRingDevice*>::const_iterator iter = devList.begin();
        ...
        std::cout << "    -> Name: " << std::left << std::setw(8)
                  << (*iter)->getDeviceName() << " Index: " << std::setw(5)
                  << interfaceIndex.str() << " MAC address: " << std::setw(19)
                  << ((*iter)->getMacAddress() == pcpp::MacAddress::Zero
                          ? "N/A"
                          : (*iter)->getMacAddress().toString())
                  << " Available RX channels: " << std::setw(3)
                  << (int)(*iter)->getTotalNumOfRxChannels()
                  << " MTU: " << (*iter)->getMtu() << std::endl;
    }
...
/**
 * The method that is called each time a packet is received on any of the
 * threads. It collects all relevant stats for the packet and matches it with
 * the matching engine. If packet is matched it sends it to the TX interface (if
 * needed) or writes it to the thread's pcap file (if needed)
 */
void packetArrived(pcpp::RawPacket* packets, uint32_t numOfPackets,
                   uint8_t threadId, pcpp::PfRingDevice* device,
                   void* userCookie) {
    ...
    for (uint32_t i = 0; i < numOfPackets; i++) {
        ...
        // hash the packet by 5-tuple and look in the flow table to see whether this
        // packet belongs to an existing or new flow
        uint32_t hash = pcpp::hash5Tuple(&packet);
        std::map<uint32_t, bool>::const_iterator iter =
            args->flowTables[threadId].find(hash);
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "n:s:t:f:i:I:p:P:r:hvl",
                              PfFilterTrafficOptions, &optionIndex)) != -1) {
        switch (opt) {
        case 0: {
            break;
        }
        case 'n': {
            ...
            dev = pcpp::PfRingDeviceList::getInstance().getPfRingDeviceByName(
                ifaceName);
            if (dev == NULL)
        ...
        case 's': {
            ...
            sendPacketsToIface =
                pcpp::PfRingDeviceList::getInstance().getPfRingDeviceByName(
                    sendPacketsToIfaceName);
            if (sendPacketsToIface == NULL)
                EXIT_WITH_ERROR("Could not find PF_RING device '"
                                << sendPacketsToIfaceName << "'");
        ...
        case 't': {
            ...
            if (numOfCaptureThreads < 1 || numOfCaptureThreads > totalNumOfCores - 1)
                EXIT_WITH_ERROR(
                    "Number of capture threads must be in the range of 1 to "
                    << totalNumOfCores - 1);
            break;
        }
        case 'f': {
            ...
            if (packetFilePath.length() > 1 &&
                (0 != packetFilePath.compare(packetFilePath.length() - 1, 1, "/")))
                packetFilePath += "/";
    ...
    // open the PF_RING device in multi-thread mode. Distribution of packets
    // between threads will be done per-flow (as opposed to round-robin)
    if (!dev->openMultiRxChannels(numOfCaptureThreads,
                                  pcpp::PfRingDevice::PerFlow))
        EXIT_WITH_ERROR("Couldn't open " << numOfCaptureThreads
                                         << " RX channels on interface '"
                                         << dev->getDeviceName() << "'");

    if (sendPacketsToIface != NULL && !sendPacketsToIface->open())
        EXIT_WITH_ERROR("Couldn't open PF_RING device '"
                        << sendPacketsToIface->getDeviceName()
    ...
    while (threadCount < numOfCaptureThreads) {
        if (pcpp::SystemCores::IdToSystemCore[threadId].Id !=
            dev->getCurrentCoreId().Id) {
            coreMask |= pcpp::SystemCores::IdToSystemCore[threadId].Mask;
            packetStatsArr[threadId].ThreadId =
                pcpp::SystemCores::IdToSystemCore[threadId].Id;
            threadCount++;
        }
    ...
    PacketMatchingEngine matchingEngine(srcIPToMatch, dstIPToMatch,
                                        srcPortToMatch, dstPortToMatch,
                                        protocolToMatch);
    ...
    if (writePacketsToDisk) {
        ...
        for (int coreId = 0; coreId < totalNumOfCores; coreId++) {
            ...
            pcapWriters[coreId] =
                new pcpp::PcapFileWriterDevice(packetFileName.str());
    ...
    std::cout << "Start capturing on " << numOfCaptureThreads
              << " threads core mask = 0x" << std::hex << coreMask << std::dec
              << std::endl;
    ...
    if (!dev->startCaptureMultiThread(packetArrived, &args, coreMask)) {
        EXIT_WITH_ERROR("Couldn't start capturing on core mask 0x"
                        << std::hex << coreMask << " on interface '"
                        << dev->getDeviceName() << "'");
    }
    ...
    // print final stats for every capture thread plus sum of all threads and free
