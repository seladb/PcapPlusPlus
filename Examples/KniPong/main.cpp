...
inline void printUsage()
{
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " [-hv] [-n KNI_DEVICE_NAME] [-p PORT] -s SRC_IPV4 -d DST_IPV4"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        << "    -s --src SRC_IPV4           : IPv4 address to assign to the "
           "created KNI device"
        << std::endl
        << "    -d --dst DST_IPV4           : Virtual IPv4 address to "
           "communicate with. Must be in /24 subnet with SRC_IPV4"
        << std::endl
        << "    -n --name KNI_DEVICE_NAME   : Name for KNI device. Default: \""
        << DEFAULT_KNI_NAME << "\"" << std::endl
        << "    -p --port PORT              : Port for communication. Default: "
        << DEFAULT_PORT << std::endl
        << "    -v --version                : Displays the current version and "
           "exits"
        << std::endl
        << "    -h --help                   : Displays this help message and "
           "exits"
        << std::endl
        << std::endl;
}
...
inline void parseArgs(int argc, char* argv[], KniPongArgs& args)
{
    ...
    if (!outIp.matchNetwork(pcpp::IPv4Network(kniIp, "255.255.255.0")))
    {
        EXIT_WITH_ERROR(
            "Provided Virtual IP '"
            << outIp
            << "' is not in same required subnet '255.255.255.0' as KNI IP '"
            << kniIp << "'");
    }
}
...
inline void processArp(pcpp::Packet& packet, pcpp::ArpLayer* arpLayer)
{
    ...
    arpHdr.hardwareSize =
        sizeof(((pcpp::arphdr*)0)->senderMacAddr); // sizeof(MAC)
    arpHdr.protocolSize =
        sizeof(((pcpp::arphdr*)0)->senderIpAddr); // sizeof(IPv4)
    arpHdr.opcode = pcpp::hostToNet16(pcpp::ARP_REPLY);
    std::memcpy(arpHdr.targetMacAddr, origArpHdr->senderMacAddr,
                sizeof(((pcpp::arphdr*)0)->senderMacAddr));
    std::memcpy(&arpHdr.targetIpAddr, &origArpHdr->senderIpAddr,
                sizeof(((pcpp::arphdr*)0)->senderIpAddr));
    std::memcpy(&arpHdr.senderIpAddr, &origArpHdr->targetIpAddr,
                sizeof(((pcpp::arphdr*)0)->senderIpAddr));
...
extern "C" void signal_handler(int) { doContinue = false; }
