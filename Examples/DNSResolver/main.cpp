...
void printUsage() {
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " [-hvl] [-t timeout] [-d dns_server] [-g gateway] [-i interface] -s "
           "hostname"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        ...
        << "    -i interface : Use the specified interface. Can be interface "
           "name (e.g eth0) or interface IPv4 address. If not set"
        << std::endl
        << "                   one of the interfaces that has a default gateway "
           "will be used"
        << std::endl
        << "    -d dns_server: IPv4 address of DNS server to send the DNS "
           "request to. If not set the DNS request will be sent to the gateway"
        << std::endl
        << "    -g gateway   : IPv4 address of the gateway to send the DNS "
           "request to. If not set the default gateway will be chosen"
        << std::endl
        << "    -t timeout   : How long to wait for a reply (in seconds). "
...
int main(int argc, char* argv[]) {
    ...
    if (!resultIP.isValid())
        ...
        std::cout << std::endl
                  << "IP address of [" << hostname << "] is: " << resultIP
                  << "  DNS-TTL=" << dnsTTL << "  time=" << (int)responseTime
                  << "ms" << std::endl;
}
