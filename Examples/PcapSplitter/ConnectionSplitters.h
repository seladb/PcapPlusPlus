 ...
 * If the user wants to limit the number of files, the splitter will divide the
...
class TwoTupleSplitter : public ValueBasedSplitter
{
   ...
   * number is lower or equal to 0 it's considered not to have a file count
   * limit
   */
...
/**
 * Splits a pcap file by connection (IP src + IP dst + port src + port dst +
 * protocol) Works for IPv4, IPv6, TCP and UDP. All packets that aren't
 * IPv4/IPv6 or TCP/UDP will be placed in one file. If the user wants to limit
 * the number of files, the splitter will divide the connections equally between
...
class FiveTupleSplitter : public ValueBasedSplitter
{
   ...
   */
    int getFileNumber(pcpp::Packet& packet, std::vector<int>& filesToClose)
    {
        ...
        else // flow is found in the flow table
        {
            if (packet.isPacketOfType(pcpp::TCP))
            {
                ...
                //(with the same 5-tuple as the previous one), so assign a new file
                //number to it.
    ...
    /**
   * Re-implement Splitter's getFileName() method, this time with the
   * IPs/Ports/protocol value
   */
