...
class Splitter
{
   ...
   */
    virtual int getFileNumber(pcpp::Packet& packet,
                              std::vector<int>& filesToClose) = 0;

    /**
   * Most splitters have a parameter (for example: packet count for each file or
   * max file size, etc.). This method should return true if the parameter value
   * is legal or false otherwise. If parameter value is illegal it should return
   * a proper error string that will be displayed for the user
   */
    virtual bool isSplitterParamLegal(std::string& errorString) = 0;

    /**
   * A method that enables the splitter to decide what will be the output file
   * names based on the file number (determined also by the splitter), the
   * output path and input file name (determined by the user) and the first
   * packet that will be written to this file. The default implementation is the
   * following: '
...
/**
 * A virtual abstract splitter which represent splitters that may or may not
 * have a limit on the number of output files after the split Since any OS has a
 * limit on concurrently open files, this class implements a mechanism that
 * makes sure no more than a certain number of files are opened concurrently
 * without limiting the number of output files desired by the user. The idea is
 * to use a LRU list that holds the number of currently open files. Each time a
 * packet is written to a certain file, the file number is advance to the head
 * of the LRU list (or added if it's not there), and if the list is full then
 * the least recently used file is leaving it and gets closed. The next time a
...
class SplitterWithMaxFiles : public Splitter
{
  ...
  protected:
    ...
    /**
   * A helper method that needs to be called by child classes each time a packet
   * is written to a certain file. This method puts the file in the LRU list,
   * and if the list is full it pulls out the least recently used file and
   ...
   */
    ...
    /**
   * A helper method that is called by child classes and returns the next file
   * number. If there's no output file limit it just return prev_file_number+1.
   * But if there is a file limit it return file number in cyclic manner,
   * meaning if reached the max file number, the next file number will be 0. In
   * addition the method puts the next file in the LRU list and if the list is
   ...
   */
    ...
    /**
   * A protected c'tor for this class which gets the output file limit size. If
   * maxFile is UNLIMITED_FILES_MAGIC_NUMBER, it's considered there's no output
   * files limit
   */
  ...
  public:
    ...
    /**
   * This method checks the maximum number of file parameter. If it equals
...
/**
 * An abstract virtual splitter which represent splitters that needs to keep a
 * mapping between a certain packet value to a certain file number the packet
 * needs to be written to. For example: in client-ip splitter all flows with a
 * certain client-ip should be written to the same file. So this class will
 * enable it to keep a mapping between client-ips and file numbers. This class
...
class ValueBasedSplitter : public SplitterWithMaxFiles
{
  protected:
    ...
    // a map between the relevant packet value (e.g client-ip) and the file to
    // write the packet to
    std::map<uint32_t, int> m_ValueToFileTable;

    /**
   * A protected c'tor for this class that only propagate the maxFiles to its
   * ancestor
   */
    explicit ValueBasedSplitter(int maxFiles)
        : SplitterWithMaxFiles(maxFiles, 1) {}
   ...
   */
    int getFileNumberForValue(uint32_t value, std::vector<int>& filesToClose)
    {
        ...
        // if it's not there, use SplitterWithMaxFiles's helper method to get a new
        // file number, put it in the map and return this file number
