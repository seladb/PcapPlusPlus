 ...
 * ==============================
 *
 * This application demonstrates how to extract and use TLS fingerprinting data
 * using PcapPlusPlus. Please read the README.md file for more information.
 *
 * You can also run `TLSFingerprinting -h` for modes of operation and
 * parameters.
 */
...
bool isNotAlphanumeric(char c) { return std::isalnum(c) == 0; }

/**
 * An auxiliary method for sorting the TLS fingerprint count map. Used in
 * printCommonTLSFingerprints()
 */
bool stringCountComparer(const std::pair<std::string, uint64_t>& first,
...
void printUsage() {
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " [-hvlcms] [-r input_file] [-i interface] [-o output_file_name] [-s "
           "separator] [-t tls_fp_type] [-f bpf_filter]"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        << std::endl
        << "    -r input_file       : Input pcap/pcapng file to analyze. "
           "Required argument for reading from file"
        << std::endl
        << "    -i interface        : Use the specified interface. Can be "
           "interface name (e.g eth0) or IP address."
        << std::endl
        << "                          Required argument for capturing from live "
           "interface"
        << std::endl
        << "    -o output_file_name : Output file name. This is a csv file "
           "(where 'tab' is the default separator)"
        << std::endl
        << "                          which contains information about all of "
           "the TLS fingerprints found in the"
        << std::endl
        << "                          capture file or live interface. It "
           "includes the TLS fingerprint itself"
        << std::endl
        << "                          (raw string and MD5), IP addresses, TCP "
           "ports and SSL message type (ClientHello"
        << std::endl
        << "                          or ServerHello). If this argument is not "
           "specified the output file name is the"
        << std::endl
        << "                          name of capture file or the live interface "
           "and it is written to the current"
        << std::endl
        << "                          directory ('.')" << std::endl
        << "    -s separator        : The separator to use in the csv output "
           "file. Valid values are a single character"
        << std::endl
        << "                          which is not alphanumeric and not one of "
           "the following: '.', ',', ':', '-'."
        << std::endl
        << "                          If this argument is not specified the "
           "default separator is 'tab' ('\\t')"
        << std::endl
        << "    -t tls_fp_type      : Specify whether to calculate TLS "
           "fingerprints for ClientHello packets only ('ch'),"
        << std::endl
        << "                          ServerHello packets only ('sh') or both "
           "('ch_sh'). The only valid values are"
        << std::endl
        << "                          'ch', 'sh', 'ch_sh'. If this argument is "
           "not specified the default value is"
        << std::endl
        << "                          ClientHello ('ch')" << std::endl
        << "    -f bpf_filter       : Apply a BPF filter to the capture file or "
           "live interface, meaning TLS fingerprint"
        << std::endl
        << "                          will only be generated for the filtered "
           "packets"
        << std::endl
 ...
 * This method takes the parsed packets and the TLS fingerprint as inputs,
 * extracts the rest of the data such as IP addresses and TCP ports, and writes
 * a single row to the output file
 */
void writeToOutputFile(std::ofstream* outputFile,
                       const pcpp::Packet& parsedPacket,
                       const std::string& tlsFPString,
                       const std::string& tlsFP_MD5,
                       const std::string& tlsFPType,
    ...
    *outputFile << tlsFP_MD5 << separator << tlsFPString << separator << tlsFPType
                << separator << ipSrcDest.first.toString() << separator
                << tcpPorts.first << separator << ipSrcDest.second.toString()
...
void writeHeaderToOutputFile(std::ofstream& outputFile,
                             const std::string& separator) {
    outputFile << "TLS Fingerprint (MD5)" << separator << "TLS Fingerprint"
               << separator << "TLS Fingerprint type" << separator << "IP Source"
               << separator << "TCP Source Port" << separator << "IP Dest"
               << separator << "TCP Dest Port" << std::endl;
}

struct TLSFingerprintingStats {
    TLSFingerprintingStats()
        : numOfPacketsTotal(0), numOfCHPackets(0), numOfSHPackets(0) {}
...
/**
 * Print cipher-suite map in a table sorted by number of occurrences (most
 * common cipher-suite will be first)
 */
void printCommonTLSFingerprints(
    const std::map<std::string, uint64_t>& tlsFingerprintMap,
    int printCountItems) {
    // create the table
    ...
    // since it's not possible to sort a std::map you must copy it to a
    // std::vector and sort it then
    std::vector<std::pair<std::string, int>> map2vec(tlsFingerprintMap.begin(),
                                                     tlsFingerprintMap.end());
    std::sort(map2vec.begin(), map2vec.end(), &stringCountComparer);

    // go over all items (fingerprints + count) in the sorted vector and print
    // them
...
void printStats(const TLSFingerprintingStats& stats, bool chFP, bool shFP) {
    ...
    if (chFP) {
        stream << "TLS ClientHello packets:              " << stats.numOfCHPackets
               << std::endl;
        stream << "Unique ClientHello TLS fingerprints:  "
               << stats.chFingerprints.size() << std::endl;
    }
    if (shFP) {
        stream << "TLS ServerHello packets:              " << stats.numOfSHPackets
               << std::endl;
        stream << "Unique ServerHello TLS fingerprints:  "
               << stats.shFingerprints.size() << std::endl;
    }
    ...
    // if user requested to extract ClientHello TLS fingerprints and there is data
    // to show
    ...
    // if user requested to extract ServerHello TLS fingerprints and there is data
    // to show
...
/**
 * Handle an intercepted packet: identify if it's a ClientHello or ServerHello
 * packets, extract the TLS fingerprint and write it to the output file
 */
            ...
            if (data->chFP) {
                ...
                if (clientHelloMessage != nullptr) {
                    ...
                    pcpp::SSLClientHelloMessage::ClientHelloTLSFingerprint
                        tlsFingerprint = clientHelloMessage->generateTLSFingerprint();
                    std::pair<std::string, std::string> tlsFingerprintStringAndMD5 =
                        tlsFingerprint.toStringAndMD5();
                    data->stats->chFingerprints[tlsFingerprintStringAndMD5.second]++;
                    // write data to output file
                    writeToOutputFile(data->outputFile, parsedPacket,
                                      tlsFingerprintStringAndMD5.first,
                                      tlsFingerprintStringAndMD5.second, "ClientHello",
                                      data->separator);
                    return;
                }
            }
            ...
            if (data->shFP) {
                ...
                if (servertHelloMessage != nullptr) {
                    ...
                    pcpp::SSLServerHelloMessage::ServerHelloTLSFingerprint
                        tlsFingerprint = servertHelloMessage->generateTLSFingerprint();
                    std::pair<std::string, std::string> tlsFingerprintStringAndMD5 =
                        tlsFingerprint.toStringAndMD5();
                    ...
                    writeToOutputFile(data->outputFile, parsedPacket,
                                      tlsFingerprintStringAndMD5.first,
                                      tlsFingerprintStringAndMD5.second, "ServerHello",
                                      data->separator);
                }
            }
        }
 ...
 */
void doTlsFingerprintingOnPcapFile(const std::string& inputPcapFileName,
                                   std::string& outputFileName,
                                   const std::string& separator, bool chFP,
                                   bool shFP, const std::string& bpfFilter) {
    // open input file (pcap or pcapng file)
    pcpp::IFileReaderDevice* reader =
        pcpp::IFileReaderDevice::getReader(inputPcapFileName.c_str());
    ...
    if (outputFileName.empty()) {
        ...
        std::string fileNameWithoutExtension = inputPcapFileName.substr(
            fileNameOffset, extensionOffset - fileNameOffset);
...
/**
 * packet capture callback - called whenever a packet arrives on the live
 * interface (in live device capturing mode)
 */
 ...
 */
void doTlsFingerprintingOnLiveTraffic(const std::string& interfaceNameOrIP,
                                      std::string& outputFileName,
                                      const std::string& separator, bool chFP,
                                      bool shFP, const std::string& bpfFilter) {
    ...
    if (outputFileName.empty()) {
        ...
        outputFileName.erase(remove_if(outputFileName.begin(), outputFileName.end(),
                                       isNotAlphanumeric),
                             outputFileName.end());
    ...
    // start capturing packets. Each packet arrived will be handled by the
    // onPacketArrives method
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "i:r:o:t:f:s:vhl",
                              TLSFingerprintingOptions, &optionIndex)) != -1) {
        switch (opt) {
        case 0:
            break;
        case 'i':
    ...
    // if no interface or input pcap file provided or both are provided- exit with
    // error
    if (inputPcapFileName.empty() == interfaceNameOrIP.empty()) {
        EXIT_WITH_ERROR("Please provide an interface or an input pcap file");
    }

    // if the user chosen a separator which is not the default, check if this
    // separator is allowed. allowed separators are a single character which is
    // not alphanumeric and not one of the following: '.', ',', ':', '-'
    static const std::string disallowedSeparatorsArr[] = {".", ",", ":", "-"};
    std::vector<std::string> disallowedSeparatorsVec(
        disallowedSeparatorsArr,
        disallowedSeparatorsArr +
            sizeof(disallowedSeparatorsArr) / sizeof(disallowedSeparatorsArr[0]));
    if (separator.empty() || separator.size() > 1 || std::isalnum(separator[0]) ||
        std::find(disallowedSeparatorsVec.begin(), disallowedSeparatorsVec.end(),
                  separator) != disallowedSeparatorsVec.end()) {
        EXIT_WITH_ERROR("Allowed separators are single characters which are not "
                        "alphanumeric and not ',', '.', ':', '-'");
    }

    // validate TLS fingerprint type the user has requested
    if (tlsFingerprintType != TLS_FP_CH_ONLY &&
        tlsFingerprintType != TLS_FP_SH_ONLY &&
        tlsFingerprintType != TLS_FP_CH_AND_SH) {
        EXIT_WITH_ERROR(
            "Possible options for TLS fingerprint types are 'ch' (Client Hello), "
            "'sh' (Server Hello) or 'ch_sh' (Client Hello & Server Hello)\n");
    }
    ...
    if (!inputPcapFileName.empty()) {
        doTlsFingerprintingOnPcapFile(inputPcapFileName, outputFileName, separator,
                                      chFP, shFP, bpfFilter);
    } else {
        doTlsFingerprintingOnLiveTraffic(interfaceNameOrIP, outputFileName,
