...
void printUsage()
{
	std::cout << std::endl
		...
		<< "    -i interface        : Use the specified interface. Can be interface name (e.g eth0) or IP address." << std::endl
		<< "                          Required argument for capturing from live interface" << std::endl
		<< "    -o output_file_name : Output file name. This is a csv file (where 'tab' is the default separator)" << std::endl
		<< "                          which contains information about all of the TLS fingerprints found in the" << std::endl
		<< "                          capture file or live interface. It includes the TLS fingerprint itself" << std::endl
		...
		<< "                          name of capture file or the live interface and it is written to the current" << std::endl
		<< "                          directory ('.')" << std::endl
		<< "    -s separator        : The separator to use in the csv output file. Valid values are a single character" << std::endl
		<< "                          which is not alphanumeric and not one of the following: '.', ',', ':', '-'." << std::endl
		<< "                          If this argument is not specified the default separator is 'tab' ('\\t')" << std::endl
...
void writeHeaderToOutputFile(std::ofstream& outputFile, std::string separator)
{
	outputFile <<
		...
		"TLS Fingerprint" << separator <<
		"TLS Fingerprint type" << separator <<
		"IP Source" << separator <<
		"TCP Source Port" << separator <<
		"IP Dest" << separator <<
		...
		if (sslHandshakeLayer != NULL)
		{
			...
			if (data->chFP)
			{
				...
				if (clientHelloMessage != NULL)
				{
					...
					writeToOutputFile(data->outputFile, parsedPacket, tlsFingerprintStringAndMD5.first, tlsFingerprintStringAndMD5.second, "ClientHello", data->separator);
					return;
				}
			}
			...
			if (data->shFP)
			{
				...
				if (servertHelloMessage != NULL)
				{
					...
					// write data to output file
					writeToOutputFile(data->outputFile, parsedPacket, tlsFingerprintStringAndMD5.first, tlsFingerprintStringAndMD5.second, "ServerHello", data->separator);
				}
			}
		}
	}
 ...
 */
void doTlsFingerprintingOnPcapFile(const std::string& inputPcapFileName, std::string& outputFileName, const std::string& separator, bool chFP, bool shFP, const std::string& bpfFilter)
{
	...
	if (outputFileName.empty())
	{
		...
		std::string fileNameWithoutExtension = inputPcapFileName.substr(fileNameOffset, extensionOffset - fileNameOffset);
 ...
 */
void doTlsFingerprintingOnLiveTraffic(const std::string& interfaceNameOrIP, std::string& outputFileName, const std::string& separator, bool chFP, bool shFP, const std::string& bpfFilter)
{
...
int main(int argc, char* argv[])
{
	...
	std::vector<std::string> disallowedSeparatorsVec(disallowedSeparatorsArr, disallowedSeparatorsArr + sizeof(disallowedSeparatorsArr) / sizeof(disallowedSeparatorsArr[0]) );
	if (separator.empty() || separator.size() > 1 || std::isalnum(separator[0]) || std::find(disallowedSeparatorsVec.begin(), disallowedSeparatorsVec.end(), separator) != disallowedSeparatorsVec.end())
	{
		EXIT_WITH_ERROR("Allowed separators are single characters which are not alphanumeric and not ',', '.', ':', '-'");
	}

	// validate TLS fingerprint type the user has requested
	if (tlsFingerprintType != TLS_FP_CH_ONLY && tlsFingerprintType != TLS_FP_SH_ONLY && tlsFingerprintType != TLS_FP_CH_AND_SH)
	{
