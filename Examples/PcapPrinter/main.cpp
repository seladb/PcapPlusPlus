 ...
 * =======================
 * This application takes a pcap or pcapng file, parses its packets using
 * Packet++ and output each layer in each packet as a readable string (quite
 * similar to the way Wireshark shows packets). In addition it prints a short
 * summary of the file (with details such as file name, size, etc.) The result
 * is printed to stdout (by default) or to a file (if specified). It can also
 * print only the first X packets of a file
 *
...
void printUsage() {
    std::cout
        ...
        << "------" << std::endl
        << pcpp::AppName::get()
        << " pcap_file [-h] [-v] [-o output_file] [-c packet_count] [-i filter] "
           "[-s]"
        << std::endl
        << std::endl
        << "Options:" << std::endl
        ...
        << "    -o output_file : Save output to text file (default output is "
           "stdout)"
        << std::endl
 ...
 */
int printPcapPackets(pcpp::IFileReaderDevice* reader, std::ostream* out,
                     int packetCount) {
 ...
 */
int printPcapNgPackets(pcpp::PcapNgFileReaderDevice* reader, std::ostream* out,
                       int packetCount) {
    // read packets from the file until end-of-file or until reached user
    // requested packet count
    int packetCountSoFar = 0;
    ...
    while (reader->getNextPacket(rawPacket, packetComment) &&
           packetCountSoFar != packetCount) {
        ...
        // print packet to string
        (*out) << "Link layer type: "
               << linkLayerToString(rawPacket.getLinkLayerType()) << std::endl;
        (*out) << parsedPacket.toString() << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "o:c:i:svh", PcapPrinterOptions,
                              &optionIndex)) != -1) {
        switch (opt) {
        case 0:
            break;
