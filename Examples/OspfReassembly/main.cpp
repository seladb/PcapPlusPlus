...
// unless the user chooses otherwise - default number of concurrent used file descospftors is 500
#define DEFAULT_MAX_NUMBER_OF_CONCURRENT_OPEN_FILES 500

static struct option OspfAssemblyOptions[] = {{"interface", required_argument, 0, 'i'},
...
class GlobalConfig
{
  private:
	...
	// file descospftors and we need to decide which files to close
  ...
  public:
	...
	// a flag indicating whether to write OSPF data to actual files or to console
	bool writeToConsole;
...
typedef std::map<std::string, OspfReassemblyData> OspfReassemblyMgr;
typedef std::map<std::string, OspfReassemblyData>::iterator OspfReassemblyMgrIter;
...
void printUsage()
{
	std::cout << std::endl
			  ...
			  << "    -e bpf_filter : Apply a BPF filter to capture file or live interface, meaning OSPF reassembly "
				 "will only work on filtered packets"
			  << std::endl
			  << "    -f max_files  : Maximum number of file descospftors to use" << std::endl
...
static void OnOspfMessageReadyCallback(pcpp::OspfPacketData *ospfData, void *userCookie)
{
	/* 	1. manager 存 OspfReassemblyData   									yes
		2. manager 的指定 OspfReassemblyData 里边fileStream 是否为NULL
			2.1 将当前（指传入的参数）的名称加入opened列表
			2.2 如果打开的文件已达上限，关闭目前的
			2.3 设置文件名
			2.4 打开文件， 模式由之前2.2设置的reopenFileStreams决定
		3. 更改OspfReassemblyData里的统计值
		4. 将数据写入打开的文件里
	 */
	...
	OspfReassemblyMgr *mgr = (OspfReassemblyMgr *)userCookie;

	// check if this tuple already appears in the manager. If not add it
	OspfReassemblyMgrIter iter = mgr->find(ospfData->getTupleName());
	if (iter == mgr->end())
	{
		mgr->insert(std::make_pair(ospfData->getTupleName(), OspfReassemblyData()));
		iter = mgr->find(ospfData->getTupleName());
	}
	...
	if (iter->second.fileStream == NULL)
	{
		...
		int result =
			GlobalConfig::getInstance().getRecentFilesWithActivity()->put(ospfData->getTupleName(), &nameToCloseFile);
		...
		if (result == 1)
		{
			OspfReassemblyMgrIter iter2 = mgr->find(nameToCloseFile);
			if (iter2 != mgr->end())
			{
		...
		std::string name = ospfData->getTupleName() + ".txt";
	...
	ospfData->getLayer()->ToStructuredOutput(*iter->second.fileStream);
}
 ...
 */
static void onPacketArrives(pcpp::RawPacket *packet, pcpp::PcapLiveDevice *dev, void *ospfReassemblyCookie)
{
	// get a pointer to the OSPF reassembly instance and feed the packet arrived to it
	pcpp::OSPFReassembly *ospfReassembly = (pcpp::OSPFReassembly *)ospfReassemblyCookie;
	ospfReassembly->reassemblePacket(packet);
}

/**
 * The method responsible for OSPF reassembly on pcap/pcapng files
 */
void doOspfReassemblyOnPcapFile(std::string fileName, pcpp::OSPFReassembly &ospfReassembly, std::string bpfFilter = "")
{
	...
	// run in a loop that reads one packet from the file in each iteration and feeds it to the OSPF reassembly instance
	pcpp::RawPacket rawPacket;
	while (reader->getNextPacket(rawPacket))
	{
		ospfReassembly.reassemblePacket(&rawPacket);
	}
...
/**
 * The method responsible for OSPF reassembly on live traffic
 */
void doOspfReassemblyOnLiveTraffic(pcpp::PcapLiveDevice *dev, pcpp::OSPFReassembly &ospfReassembly,
								  std::string bpfFilter = "")
{
	...
	dev->startCapture(onPacketArrives, &ospfReassembly);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "i:r:o:e:f:mcvhl", OspfAssemblyOptions, &optionIndex)) != -1)
	{
		switch (opt)
		{
		case 0:
	...
	OspfReassemblyMgr mgr;

	//  create the OSPF reassembly instance
	pcpp::OSPFReassembly ospfReassembly(OnOspfMessageReadyCallback, &mgr);

	// analyze in pcap file mode
	if (!inputPcapFileName.empty())
	{
		doOspfReassemblyOnPcapFile(inputPcapFileName, ospfReassembly, bpfFilter);
	}
	else // analyze in live traffic mode
	{
		...
		// start capturing packets and do OSPF reassembly
		doOspfReassemblyOnLiveTraffic(dev, ospfReassembly, bpfFilter);
	}
}
