...
namespace pcpp_test
{
	TEST(GeneralUtilsTests, byteArrayToHexString)
	{
		...
		EXPECT_EQ(pcpp::byteArrayToHexString(byteArr.data(), byteArr.size()), "aa2b10");
	};

	TEST(GeneralUtilsTests, hexStringToByteArray)
	{
		std::array<uint8_t, 3> resultByteArr = { 0 };
		EXPECT_EQ(pcpp::hexStringToByteArray("aa2b10", resultByteArr.data(), resultByteArr.size()), 3);
