	...
	def test_split_by_file_size(self, tmpdir):
		...
		for filename in os.listdir(tmpdir):
			if not os.path.splitext(filename)[0].endswith('29'):
				assert 98500 <= os.path.getsize(os.path.join(tmpdir, filename)) <= 101500
			...
			if os.path.splitext(filename)[0].endswith('miscellaneous'):
				...
				for packet in packets:
					...
					if isinstance(ip_addr, ipaddress.IPv4Address):
						...
						assert ipaddress.ip_address(packet[IP].src) == ip_addr or ipaddress.ip_address(packet[IP].dst) == ip_addr
					else:
						assert packet.haslayer(IPv6)
						assert ipaddress.ip_address(packet[IPv6].src) == ip_addr or ipaddress.ip_address(packet[IPv6].dst) == ip_addr
	...
	def test_split_by_ip_src_dst(self, tmpdir):
		...
		for filename in os.listdir(tmpdir):
			...
			for packet in packets:
				...
				assert ip_src_dst == frozenset([packet[ip_type].src, packet[ip_type].dst])
	...
	def test_split_by_connection(self, tmpdir):
		...
		for filename in os.listdir(tmpdir):
			...
			if net_layer is not None and trans_layer is not None:
				conn = frozenset([trans_layer, \
					packets[0][net_layer].src, packets[0][net_layer].dst, \
					packets[0][trans_layer].sport, packets[0][trans_layer].dport])
			else:
				conn = frozenset([])
			...
			for packet in packets:
				...
				packet_conn = frozenset([trans_layer, \
					packet[net_layer].src, packet[net_layer].dst, \
					packet[trans_layer].sport, packet[trans_layer].dport])
