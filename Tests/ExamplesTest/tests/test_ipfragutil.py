...
class TestIPFragUtil(object):
	...
	def test_sanity(self, tmpdir):
		...
		completed_process = run_example(example_name='IPFragUtil', args=args)
		assert filecmp.cmp(path.join(tmpdir, 'output.pcap'), path.join('expected_output', 'http_req_frag.pcap'), shallow=False)
		with open(path.join('expected_output', 'ipfragutil_sanity.txt')) as f:
			assert f.read() == completed_process.stdout

	def test_missing_input_file(self):
		args = {}
		completed_process = run_example(example_name='IPFragUtil', args=args, expected_return_code=1)
	...
	def test_ip_ids(self, tmpdir):
		...
		completed_process = run_example(example_name='IPFragUtil', args=args)
		assert filecmp.cmp(path.join(tmpdir, 'output.pcap'), path.join('expected_output', 'ipfragutil_ip_ids.pcap'), shallow=False)
	...
	def test_bpf_filter(self, tmpdir):
		...
		completed_process = run_example(example_name='IPFragUtil', args=args)
		assert filecmp.cmp(path.join(tmpdir, 'output.pcap'), path.join('expected_output', 'ipfragutil_bpf_filter.pcap'), shallow=False)
