...
class TestIPDefragUtil(object):
	...
	def test_multiple_frag_packets(self, tmpdir):
		...
		completed_process = run_example(example_name='IPDefragUtil', args=args)
		assert filecmp.cmp(path.join(tmpdir, 'output.pcap'), path.join('expected_output', 'no-ip-frag.pcap'), shallow=False)
		with open(path.join('expected_output', 'ipdefragutil_multiple_frag_packets.txt')) as f:
			assert f.read() == completed_process.stdout
	...
	def test_ipv6(self, tmpdir):
		...
		completed_process = run_example(example_name='IPDefragUtil', args=args)
		assert filecmp.cmp(path.join(tmpdir, 'output.pcap'), path.join('expected_output', 'no-ipv6-frag.pcap'), shallow=False)
	...
	def test_defrag_all(self, tmpdir):
		...
		completed_process = run_example(example_name='IPDefragUtil', args=args)
		assert filecmp.cmp(path.join(tmpdir, 'output.pcap'), path.join('expected_output', 'no-ip-frag-all.pcap'), shallow=False)
	...
	def test_defrag_ip_ids(self, tmpdir):
		...
		completed_process = run_example(example_name='IPDefragUtil', args=args)
		assert filecmp.cmp(path.join(tmpdir, 'output.pcap'), path.join('expected_output', 'no-frag-ip-ids.pcap'), shallow=False)
