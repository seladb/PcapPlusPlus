...
PTF_TEST_CASE(TestTcpReassemblySanity)
{
	...
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 19, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 2, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 2, num);
	...
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_sec, 1491516383, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_usec, 915793, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_sec, 0, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_usec, 0, num);
...
PTF_TEST_CASE(TestTcpReassemblyRetran)
{
	...
	TcpReassemblyMultipleConnStats::Stats &stats = tcpReassemblyResults.stats;
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 21, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 2, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 2, num);
...
PTF_TEST_CASE(TestTcpReassemblyMissingData)
{
	...
	TcpReassemblyMultipleConnStats::Stats &stats = tcpReassemblyResults.stats;
	PTF_ASSERT_EQUAL(stats.begin()->second.totalMissingBytes, expectedLoss, num);
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 17, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 2, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 2, num);
...
PTF_TEST_CASE(TestTcpReassemblyOutOfOrder)
{
	...
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 18, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 2, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 2, num);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsStarted);
	PTF_ASSERT_FALSE(stats.begin()->second.connectionsEnded);
...
PTF_TEST_CASE(TestTcpReassemblyWithFIN_RST)
{
	...
	TcpReassemblyMultipleConnStats::Stats &stats = tcpReassemblyResults.stats;
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 5, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 1, num);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsStarted);
	...
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 2, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 1, num);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsStarted);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsEnded);
	...
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 6, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 1, num);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsStarted);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsEnded);
...
PTF_TEST_CASE(TestTcpReassemblyMultipleConns)
{
	...
	PTF_ASSERT_EQUAL(stats.size(), 3, num);
	PTF_ASSERT_EQUAL(results.flowKeysList.size(), 3, num);

	TcpReassemblyMultipleConnStats::Stats::iterator iter = stats.begin();

	PTF_ASSERT_EQUAL(iter->second.numOfDataPackets, 2, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[0], 1, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[1], 1, num);
	...
	PTF_ASSERT_EQUAL(managedConnections.size(), 3, num);
	...
	PTF_ASSERT_GREATER_THAN(tcpReassembly.isConnectionOpen(iterConn1->second), 0, num);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn2->second), 0, num);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn3->second), 0, num);

	pcpp::ConnectionData dummyConn;
	dummyConn.flowKey = 0x12345678;
	PTF_ASSERT_LOWER_THAN(tcpReassembly.isConnectionOpen(dummyConn), 0, num);
...
PTF_TEST_CASE(TestTcpReassemblyIPv6)
{
	...
	TcpReassemblyMultipleConnStats::Stats &stats = tcpReassemblyResults.stats;
	PTF_ASSERT_EQUAL(stats.size(), 1, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfDataPackets, 10, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[0], 3, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.numOfMessagesFromSide[1], 3, num);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsStarted);
	PTF_ASSERT_FALSE(stats.begin()->second.connectionsEnded);
	PTF_ASSERT_TRUE(stats.begin()->second.connectionsEndedManually);
	PTF_ASSERT_TRUE(stats.begin()->second.connData.srcIP.isValid());
	...
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.srcIP, expectedSrcIP, object);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.dstIP, expectedDstIP, object);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_sec, 1147551796, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_usec, 702602, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_sec, 0, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_usec, 0, num);
...
PTF_TEST_CASE(TestTcpReassemblyIPv6MultConns)
{
	...
	TcpReassemblyMultipleConnStats::Stats &stats = tcpReassemblyResults.stats;
	PTF_ASSERT_EQUAL(stats.size(), 4, num);
	...
	PTF_ASSERT_EQUAL(iter->second.numOfDataPackets, 14, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[0], 3, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[1], 3, num);
	PTF_ASSERT_TRUE(iter->second.connectionsStarted);
	PTF_ASSERT_FALSE(iter->second.connectionsEnded);
	PTF_ASSERT_TRUE(iter->second.connectionsEndedManually);
	PTF_ASSERT_TRUE(iter->second.connData.srcIP.isValid());
	PTF_ASSERT_TRUE(iter->second.connData.dstIP.isValid());
	PTF_ASSERT_EQUAL(iter->second.connData.srcIP, expectedSrcIP, object);
	PTF_ASSERT_EQUAL(iter->second.connData.dstIP, expectedDstIP1, object);
	PTF_ASSERT_EQUAL(iter->second.connData.srcPort, 35995, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_sec, 1147551795, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_usec, 526632, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_sec, 0, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_usec, 0, num);
	...
	PTF_ASSERT_EQUAL(iter->second.numOfDataPackets, 10, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[0], 1, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[1], 1, num);
	PTF_ASSERT_TRUE(iter->second.connectionsStarted);
	PTF_ASSERT_FALSE(iter->second.connectionsEnded);
	...
	PTF_ASSERT_EQUAL(iter->second.connData.srcIP, expectedSrcIP, object);
	PTF_ASSERT_EQUAL(iter->second.connData.dstIP, expectedDstIP1, object);
	PTF_ASSERT_EQUAL(iter->second.connData.srcPort, 35999, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_sec, 1147551795, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_usec, 526632, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_sec, 0, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_usec, 0, num);
	...
	PTF_ASSERT_TRUE(iter->second.connData.dstIP.isValid());
	PTF_ASSERT_EQUAL(iter->second.connData.srcIP, expectedSrcIP, object);
	PTF_ASSERT_EQUAL(iter->second.connData.dstIP, expectedDstIP2, object);
	PTF_ASSERT_EQUAL(iter->second.connData.srcPort, 40426, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_sec, 1147551795, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_usec, 526632, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_sec, 0, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_usec, 0, num);
	...
	PTF_ASSERT_EQUAL(iter->second.numOfDataPackets, 13, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[0], 4, num);
	PTF_ASSERT_EQUAL(iter->second.numOfMessagesFromSide[1], 4, num);
	PTF_ASSERT_TRUE(iter->second.connectionsStarted);
	PTF_ASSERT_FALSE(iter->second.connectionsEnded);
	PTF_ASSERT_TRUE(iter->second.connectionsEndedManually);
	PTF_ASSERT_TRUE(iter->second.connData.srcIP.isValid());
	...
	PTF_ASSERT_EQUAL(iter->second.connData.srcIP, expectedSrcIP, object);
	PTF_ASSERT_EQUAL(iter->second.connData.dstIP, expectedDstIP1, object);
	PTF_ASSERT_EQUAL(iter->second.connData.srcPort, 35997, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_sec, 1147551795, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.startTime.tv_usec, 526632, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_sec, 0, num);
	PTF_ASSERT_EQUAL(stats.begin()->second.connData.endTime.tv_usec, 0, num);
...
PTF_TEST_CASE(TestTcpReassemblyCleanup)
{
	...
	PTF_ASSERT_NOT_EQUAL(iterConn2, managedConnections.end(), object_no_str);
	PTF_ASSERT_NOT_EQUAL(iterConn3, managedConnections.end(), object_no_str);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn1->second), 0, num);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn2->second), 0, num);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn3->second), 0, num);
	...
	PTF_ASSERT_EQUAL(tcpReassembly.getConnectionInformation().size(), 2, num);

	tcpReassembly.purgeClosedConnections(); // manually initiated cleanup of 1 item
	PTF_ASSERT_EQUAL(tcpReassembly.getConnectionInformation().size(), 1, num);

	tcpReassembly.purgeClosedConnections(0xFFFFFFFF); // manually initiated cleanup of all items
	PTF_ASSERT_EQUAL(tcpReassembly.getConnectionInformation().size(), 0, num);
	...
	PTF_ASSERT_NOT_EQUAL(iterConn2, managedConnections.end(), object_no_str);
	PTF_ASSERT_NOT_EQUAL(iterConn3, managedConnections.end(), object_no_str);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn1->second), -1, num);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn2->second), -1, num);
	PTF_ASSERT_EQUAL(tcpReassembly.isConnectionOpen(iterConn3->second), -1, num);
...
PTF_TEST_CASE(TestTcpReassemblyMaxOOOFrags)
{
	...
	PTF_ASSERT_EQUAL(managedConnections1.size(), 1, num);
	PTF_ASSERT_EQUAL(managedConnections2.size(), 1, num);
	PTF_ASSERT_EQUAL(results1.flowKeysList.size(), 1, num);
	PTF_ASSERT_EQUAL(results2.flowKeysList.size(), 1, num);
	...
	PTF_ASSERT_EQUAL(tcpReassembly1.isConnectionOpen(iterConn1->second), 1, num);
	PTF_ASSERT_EQUAL(tcpReassembly2.isConnectionOpen(iterConn2->second), 1, num);
	PTF_ASSERT_EQUAL(results1.stats.begin()->second.numOfDataPackets, 1, num); // The second data packet is incomplete so we stopped at one
	PTF_ASSERT_EQUAL(results2.stats.begin()->second.numOfDataPackets, 7, num); // We hit the fragment limit so skipped the missing fragment and continued to the end
	...
	PTF_ASSERT_EQUAL(results1.stats.begin()->second.numOfDataPackets, 7, num);
	PTF_ASSERT_EQUAL(results2.stats.begin()->second.numOfDataPackets, 7, num);
} // TestTcpReassemblyCleanup
