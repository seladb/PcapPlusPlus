...
PTF_TEST_CASE(TestTcpReassemblyHighPrecision)
{
    ...
    PTF_ASSERT_EQUAL(
        std::chrono::duration_cast<std::chrono::nanoseconds>(stats[flowKeys[2]].connData.startTimePrecise.time_since_epoch())
            .count(),
        1361916156716947055);
    ...
    PTF_ASSERT_EQUAL(
        std::chrono::duration_cast<std::chrono::nanoseconds>(stats[flowKeys[2]].connData.endTimePrecise.time_since_epoch())
            .count(),
        1361916156800214055);

    std::string expectedReassemblyData = readFileIntoString(std::string("PcapExamples/three_http_streams_conn_1_output.txt"));
    PTF_ASSERT_EQUAL(expectedReassemblyData, stats.begin()->second.reassembledData);
