...
PTF_TEST_CASE(QuicV1ParsingTest)
{
	// Initial packet with empty token
	{
		...
		PTF_ASSERT_EQUAL(quicInitialLayer->getDestinationConnectionIdAsString(), "c5fc34");
		PTF_ASSERT_EQUAL(quicInitialLayer->getSourceConnectionIdAsString(), "2e05f87969a4940b");
		PTF_ASSERT_EQUAL(quicInitialLayer->getLength(), 1231);
		PTF_ASSERT_EQUAL(quicInitialLayer->getTokenAsString(), "");
	...
	// Initial packet with non-empty token
	{
		...
		PTF_ASSERT_TRUE(quicInitialLayer->getFixedBit());
		PTF_ASSERT_EQUAL(quicInitialLayer->getVersion(), 1);
		PTF_ASSERT_EQUAL(quicInitialLayer->getDestinationConnectionIdAsString(), "3c548c2db5aa5e4f64702c3961");
		PTF_ASSERT_EQUAL(quicInitialLayer->getSourceConnectionIdAsString(), "4c8342");
		PTF_ASSERT_EQUAL(quicInitialLayer->getLength(), 553);
		PTF_ASSERT_EQUAL(
		    quicInitialLayer->getTokenAsString(),
	...
	// Handshake
	{
		...
		PTF_ASSERT_EQUAL(quicHandshakeLayer->getDestinationConnectionIdAsString(), "f147b631d2ec8eb3");
		PTF_ASSERT_EQUAL(quicHandshakeLayer->getSourceConnectionIdAsString(), "3bccd4");
	...
	// 0-RTT packet
	{
		...
		PTF_ASSERT_EQUAL(quicZeroRttLayer->getDestinationConnectionIdAsString(), "3c548c2db5aa5e4f64702c3961");
		PTF_ASSERT_EQUAL(quicZeroRttLayer->getSourceConnectionIdAsString(), "4c8342");
	...
	// Retry
	{
		...
		PTF_ASSERT_EQUAL(quicRetryLayer->getDestinationConnectionIdAsString(), "");
		PTF_ASSERT_EQUAL(quicRetryLayer->getSourceConnectionIdAsString(), "f067a5502a4262b5");
		PTF_ASSERT_EQUAL(quicRetryLayer->getRetryTokenAsString(), "746f6b656e");
		PTF_ASSERT_EQUAL(quicRetryLayer->getRetryIntegrityTagAsString(), "04a265ba2eff4d829058fb3f0f2496ba");
	...
	// Version negotiation
	{
		...
		PTF_ASSERT_EQUAL(quicVersionNegotiationLayer->getDestinationConnectionIdAsString(), "9aac5a49ba87a849");
		PTF_ASSERT_EQUAL(quicVersionNegotiationLayer->getSourceConnectionIdAsString(), "f92f4336fa951ba1");
...
PTF_TEST_CASE(QuicV1MalformedPacketsTest)
{
	...
	// Long-header packet with DCID length but no data
	{
		...
		auto initialLayer = dynamic_cast<pcpp::QuicV1InitialLayer*>(layer.get());
		PTF_ASSERT_NOT_NULL(initialLayer);
		PTF_ASSERT_EQUAL(initialLayer->getDestinationConnectionIdAsString(), "");
		PTF_ASSERT_EQUAL(initialLayer->getSourceConnectionIdAsString(), "");
		PTF_ASSERT_EQUAL(initialLayer->getTokenAsString(), "");
	...
	// SCID length present but no data
	{
		...
		auto initialLayer = dynamic_cast<pcpp::QuicV1InitialLayer*>(layer.get());
		PTF_ASSERT_NOT_NULL(initialLayer);
		PTF_ASSERT_EQUAL(initialLayer->getDestinationConnectionIdAsString(), "aa");
		PTF_ASSERT_EQUAL(initialLayer->getSourceConnectionIdAsString(), "");
	}
	...
	// SCID claims more than is captured
	{
		...
		PTF_ASSERT_EQUAL(initialLayer2->getSourceConnectionIdAsString(), "");
	}
	...
	// Version Negotiation takes priority over longPacketType when version == 0
	{
		...
		PTF_ASSERT_EQUAL(vnLayer->getDestinationConnectionIdAsString(), "1122");
		PTF_ASSERT_EQUAL(vnLayer->getSourceConnectionIdAsString(), "3344");
	...
	// getSupportedVersions() must catch it and return an empty list rather than propagate
	{
		...
		auto vnLayer = dynamic_cast<pcpp::QuicV1VersionNegotiationLayer*>(layer.get());
		PTF_ASSERT_NOT_NULL(vnLayer);
		PTF_ASSERT_EQUAL(vnLayer->getDestinationConnectionIdAsString(), "");
		PTF_ASSERT_EQUAL(vnLayer->getSourceConnectionIdAsString(), "");
	...
	// Retry packet truncated before the 16-byte integrity tag
	{
		...
		PTF_ASSERT_EQUAL(retryLayer->getSourceConnectionIdAsString(), "5566");
		PTF_ASSERT_EQUAL(retryLayer->getRetryTokenAsString(), "");
		PTF_ASSERT_EQUAL(retryLayer->getRetryIntegrityTagAsString(), "");
	...
	// Retry packet with exactly 16 bytes left after the SCID
	{
		...
		auto retryLayer = dynamic_cast<pcpp::QuicV1RetryLayer*>(layer.get());
		PTF_ASSERT_NOT_NULL(retryLayer);
		PTF_ASSERT_EQUAL(retryLayer->getSourceConnectionIdAsString(), "7788");
		PTF_ASSERT_EQUAL(retryLayer->getRetryTokenAsString(), "");
		PTF_ASSERT_EQUAL(retryLayer->getRetryIntegrityTagAsString(), "0102030405060708090a0b0c0d0e0f10");
	...
	// it and fall back to m_DataLen rather than propagate the exception
	{
		...
		auto retryLayer = dynamic_cast<pcpp::QuicV1RetryLayer*>(layer.get());
		PTF_ASSERT_NOT_NULL(retryLayer);
		PTF_ASSERT_EQUAL(retryLayer->getDestinationConnectionIdAsString(), "");
		PTF_ASSERT_EQUAL(retryLayer->getSourceConnectionIdAsString(), "");
		PTF_ASSERT_EQUAL(retryLayer->getRetryTokenAsString(), "");
		PTF_ASSERT_EQUAL(retryLayer->getRetryIntegrityTagAsString(), "");
	}
