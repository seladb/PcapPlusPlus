...
PTF_TEST_CASE(IPv6UdpPacketParseAndCreate)
{
	...
	PTF_ASSERT_EQUAL(ipv6Layer->getIPv6Header()->nextHeader, 17, num);
	PTF_ASSERT_EQUAL(ipv6Layer->getIPv6Header()->ipVersion, 6, num);
	...
	PTF_ASSERT_EQUAL(pUdpLayer->getDstPort(), 1900, num);
	PTF_ASSERT_EQUAL(pUdpLayer->getSrcPort(), 63628, num);
	PTF_ASSERT_EQUAL(pUdpLayer->getUdpHeader()->length, htobe16(154), num);
	PTF_ASSERT_EQUAL(pUdpLayer->getUdpHeader()->headerChecksum, htobe16(0x5fea), num);
	...
	PTF_ASSERT_EQUAL(ip6UdpPacketNew.getRawPacket()->getRawDataLen(), bufferLength1, num);
...
PTF_TEST_CASE(IPv6FragmentationTest)
{
	...
	PTF_ASSERT_EQUAL(fragHeader->getFragmentOffset(), 0, num);
	PTF_ASSERT_EQUAL(be32toh(fragHeader->getFragHeader()->id), 0xf88eb466, num);
	PTF_ASSERT_EQUAL(fragHeader->getFragHeader()->nextHeader, pcpp::PACKETPP_IPPROTO_UDP, num);
	...
	PTF_ASSERT_FALSE(fragHeader->isLastFragment());
	PTF_ASSERT_EQUAL(fragHeader->getFragmentOffset(), 1448, num);
	PTF_ASSERT_EQUAL(be32toh(fragHeader->getFragHeader()->id), 0xf88eb466, num);
	PTF_ASSERT_EQUAL(fragHeader->getFragHeader()->nextHeader, pcpp::PACKETPP_IPPROTO_UDP, num);
	...
	PTF_ASSERT_FALSE(fragHeader->isFirstFragment());
	PTF_ASSERT_FALSE(fragHeader->isLastFragment());
	PTF_ASSERT_EQUAL(fragHeader->getFragmentOffset(), 2896, num);
	PTF_ASSERT_EQUAL(be32toh(fragHeader->getFragHeader()->id), 0xf88eb466, num);
	PTF_ASSERT_EQUAL(fragHeader->getFragHeader()->nextHeader, pcpp::PACKETPP_IPPROTO_UDP, num);

	ipv6Layer = frag4.getLayerOfType<pcpp::IPv6Layer>();
	PTF_ASSERT_EQUAL(ipv6Layer->getHeaderLen(), 48, num);
	fragHeader = ipv6Layer->getExtensionOfType<pcpp::IPv6FragmentationHeader>();
	PTF_ASSERT_EQUAL(fragHeader->getExtensionType(), pcpp::IPv6Extension::IPv6Fragmentation, enum);
	PTF_ASSERT_NOT_NULL(fragHeader);
	PTF_ASSERT_FALSE(fragHeader->isFirstFragment());
	PTF_ASSERT_TRUE(fragHeader->isLastFragment());
	PTF_ASSERT_EQUAL(fragHeader->getFragmentOffset(), 4344, num);
	PTF_ASSERT_EQUAL(be32toh(fragHeader->getFragHeader()->id), 0xf88eb466, num);
	PTF_ASSERT_EQUAL(fragHeader->getFragHeader()->nextHeader, pcpp::PACKETPP_IPPROTO_UDP, num);
	...
	PTF_ASSERT_EQUAL(newIPv6Layer.getHeaderLen(), 48, num);
	newIPv6Layer.removeAllExtensions();
	PTF_ASSERT_EQUAL(newIPv6Layer.getHeaderLen(), 40, num);
	...
	PTF_ASSERT_EQUAL(frag4.getRawPacket()->getRawDataLen(), newFrag.getRawPacket()->getRawDataLen(), num);
...
PTF_TEST_CASE(IPv6ExtensionsTest)
{
	...
	PTF_ASSERT_EQUAL(ipv6Layer->getExtensionCount(), 1, num);
	...
	PTF_ASSERT_EQUAL(destExt->getOptionCount(), 2, num);
	pcpp::IPv6TLVOptionHeader::IPv6Option option = destExt->getFirstOption();
	PTF_ASSERT_FALSE(option.isNull());
	PTF_ASSERT_EQUAL(option.getType(), 11, num);
	PTF_ASSERT_EQUAL(option.getTotalSize(), 3, num);
	PTF_ASSERT_EQUAL(option.getDataSize(), 1, num);
	PTF_ASSERT_EQUAL(option.getValueAs<uint8_t>(), 9, num);
	option = destExt->getNextOption(option);
	PTF_ASSERT_FALSE(option.isNull());
	PTF_ASSERT_EQUAL(option.getType(), 1, num);
	PTF_ASSERT_EQUAL(option.getTotalSize(), 3, num);
	PTF_ASSERT_EQUAL(option.getDataSize(), 1, num);
	PTF_ASSERT_EQUAL(option.getValueAs<uint8_t>(), 0, num);
	option = destExt->getNextOption(option);
	...
	PTF_ASSERT_EQUAL(hopByHopExt->getOptionCount(), 2, num);
	...
	PTF_ASSERT_EQUAL(option.getType(), 5, num);
	PTF_ASSERT_EQUAL(option.getTotalSize(), 4, num);
	PTF_ASSERT_EQUAL(option.getDataSize(), 2, num);
	PTF_ASSERT_EQUAL(option.getValueAs<uint16_t>(), 0, num);
	option = hopByHopExt->getNextOption(option);
	PTF_ASSERT_FALSE(option.isNull());
	PTF_ASSERT_EQUAL(option.getType(), 1, num);
	PTF_ASSERT_EQUAL(option.getTotalSize(), 2, num);
	PTF_ASSERT_EQUAL(option.getDataSize(), 0, num);
	PTF_ASSERT_EQUAL(option.getValueAs<uint8_t>(), 0, num);
	option = hopByHopExt->getNextOption(option);
	PTF_ASSERT_TRUE(option.isNull());
	...
	PTF_ASSERT_EQUAL(routingExt->getRoutingHeader()->routingType, 0, num);
	PTF_ASSERT_EQUAL(routingExt->getRoutingHeader()->segmentsLeft, 2, num);
	PTF_ASSERT_EQUAL(routingExt->getRoutingAdditionalDataLength(), 36, num);
	...
	PTF_ASSERT_EQUAL(routingExt->getExtensionType(), pcpp::IPv6Extension::IPv6Routing, enum);
	PTF_ASSERT_EQUAL(routingExt->getRoutingHeader()->routingType, 0, num);
	PTF_ASSERT_EQUAL(routingExt->getRoutingHeader()->segmentsLeft, 1, num);
	PTF_ASSERT_EQUAL(routingExt->getRoutingAdditionalDataLength(), 20, num);
	PTF_ASSERT_EQUAL(routingExt->getRoutingAdditionalDataAsIPv6Address(4), pcpp::IPv6Address("2200::210:2:0:0:4"), object);
	...
	PTF_ASSERT_EQUAL(authHdrExt->getAuthHeader()->securityParametersIndex, htobe32(0x100), num);
	PTF_ASSERT_EQUAL(authHdrExt->getAuthHeader()->sequenceNumber, htobe32(32), num);
	PTF_ASSERT_EQUAL(authHdrExt->getIntegrityCheckValueLength(), 12, num);
	...
	PTF_ASSERT_EQUAL(ipv6Layer->getExtensionCount(), 4, num);
	PTF_ASSERT_NOT_NULL(ipv6Layer->getExtensionOfType<pcpp::IPv6AuthenticationHeader>());
	PTF_ASSERT_EQUAL(ipv6Layer->getExtensionOfType<pcpp::IPv6AuthenticationHeader>()->getAuthHeader()->securityParametersIndex, be32toh(0x100), num);
	PTF_ASSERT_NOT_NULL(ipv6Layer->getExtensionOfType<pcpp::IPv6DestinationHeader>());
	PTF_ASSERT_EQUAL(ipv6Layer->getExtensionOfType<pcpp::IPv6DestinationHeader>()->getFirstOption().getType(), 11, num);
	PTF_ASSERT_NOT_NULL(ipv6Layer->getExtensionOfType<pcpp::IPv6HopByHopHeader>());
	PTF_ASSERT_EQUAL(ipv6Layer->getExtensionOfType<pcpp::IPv6HopByHopHeader>()->getFirstOption().getType(), 5, num);
	PTF_ASSERT_NOT_NULL(ipv6Layer->getExtensionOfType<pcpp::IPv6RoutingHeader>());
	PTF_ASSERT_EQUAL(ipv6Layer->getExtensionOfType<pcpp::IPv6RoutingHeader>()->getRoutingHeader()->routingType, 0, num);
	...
	PTF_ASSERT_EQUAL(ipv6Dest.getRawPacket()->getRawDataLen(), newPacket.getRawPacket()->getRawDataLen(), num);
	...
	PTF_ASSERT_EQUAL(newIPv6Layer2.getHeaderLen(), 48, num);
	newIPv6Layer2.removeAllExtensions();
	PTF_ASSERT_EQUAL(newIPv6Layer2.getHeaderLen(), 40, num);
	...
	PTF_ASSERT_EQUAL(ipv6HopByHop.getRawPacket()->getRawDataLen(), newPacket2.getRawPacket()->getRawDataLen(), num);
	...
	PTF_ASSERT_EQUAL(newIPv6Layer3.getHeaderLen(), 64, num);
	newIPv6Layer3.removeAllExtensions();
	PTF_ASSERT_EQUAL(newIPv6Layer3.getHeaderLen(), 40, num);
	...
	PTF_ASSERT_EQUAL(ipv6Routing2.getRawPacket()->getRawDataLen(), newPacket3.getRawPacket()->getRawDataLen(), num);
	...
	PTF_ASSERT_EQUAL(newIPv6Layer4.getHeaderLen(), 64, num);
	newIPv6Layer4.removeAllExtensions();
	PTF_ASSERT_EQUAL(newIPv6Layer4.getHeaderLen(), 40, num);
	...
	PTF_ASSERT_EQUAL(ipv6AuthHdr.getRawPacket()->getRawDataLen(), newPacket4.getRawPacket()->getRawDataLen(), num);
	...
	PTF_ASSERT_EQUAL(ipv6MultipleOptions.getRawPacket()->getRawDataLen(), newPacket5.getRawPacket()->getRawDataLen(), num);
