...
PTF_TEST_CASE(X509ParsingTest)
{
	...
	auto x509CertFroFile = pcpp::X509Certificate::fromDERFile("PacketExamples/X509.der");

	std::vector<pcpp::X509Certificate*> x509Certificates = {
		x509CertFromString.get(),
		x509CertFromDerData.get(),
		x509CertFroFile.get()
	};

	for (auto const& x509Certificate : x509Certificates)
	{
		PTF_ASSERT_EQUAL(x509Certificate->getVersion(), pcpp::X509Version::V3, enumclass);
		PTF_ASSERT_EQUAL(x509Certificate->getSerialNumber(), "008f7cfe109efa987f116812e198008497");
