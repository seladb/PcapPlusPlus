...
PTF_TEST_CASE(SSLClientHelloParsingTest)
{
	...
	PTF_ASSERT_EQUAL(handshakeLayer->getHandshakeMessagesCount(), 1, num);
	...
	PTF_ASSERT_EQUAL(clientHelloMessage->getSessionIDLength(), 0, num);
	PTF_ASSERT_NULL(clientHelloMessage->getSessionID());
	PTF_ASSERT_EQUAL(clientHelloMessage->getCipherSuiteCount(), 11, num);
	...
	for (int i = 0; i < clientHelloMessage->getCipherSuiteCount(); i++)
	{
		...
		PTF_ASSERT_EQUAL(curCipherSuite->getID(), cipherSuiteIDs[i], num);
	...
	PTF_ASSERT_EQUAL(clientHelloMessage->getCompressionMethodsValue(), 0, num);
	PTF_ASSERT_EQUAL(handshakeLayer->getHeaderLen(), 188, num);

	int extCount = clientHelloMessage->getExtensionCount();
	PTF_ASSERT_EQUAL(extCount, 9, num);
	PTF_ASSERT_EQUAL(clientHelloMessage->getExtensionsLenth(), 116, num);
	...
	PTF_ASSERT_EQUAL(ecPointFormatList.size(), 1, num);
	PTF_ASSERT_EQUAL(ecPointFormatList.at(0), 0, num);
	...
	PTF_ASSERT_EQUAL(supportedGroups.size(), 3, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(0), 23, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(1), 24, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(2), 25, num);
...
PTF_TEST_CASE(SSLExtensionWithZeroSizeTest)
{
	...
	PTF_ASSERT_EQUAL(clientHelloMessage->getExtensionCount(), 7, num);
	...
	PTF_ASSERT_EQUAL(zeroSizeExt->getLength(), 0, num);
	PTF_ASSERT_EQUAL(zeroSizeExt->getTotalLength(), 4, num);
...
PTF_TEST_CASE(SSLAppDataParsingTest)
{
	...
	PTF_ASSERT_EQUAL(appDataLayer->getEncryptedDataLen(), 880, num);
	...
	PTF_ASSERT_EQUAL(appDataLayer->getRecordVersion().asEnum(), pcpp::SSLVersion::TLS1_2, enum);
	PTF_ASSERT_EQUAL(appDataLayer->getRecordType(), pcpp::SSL_APPLICATION_DATA, enum);
	PTF_ASSERT_EQUAL(appDataLayer->getEncryptedDataLen(), 41, num);
	PTF_ASSERT_EQUAL(appDataLayer->getEncryptedData()[0], 0, hex);
...
PTF_TEST_CASE(SSLAlertParsingTest)
{
	...
	PTF_ASSERT_EQUAL(clearAlertLayer->getRecordLayer()->length, be16toh(2), num);
	...
	PTF_ASSERT_EQUAL(encAlertLayer->getRecordLayer()->length, be16toh(26), num);
	PTF_ASSERT_EQUAL(encAlertLayer->getHeaderLen(), 31, num);
} // SSLAlertParsingTest
...
PTF_TEST_CASE(SSLMultipleRecordParsingTest)
{
	...
	PTF_ASSERT_EQUAL(serverHelloMessage->getSessionIDLength(), 32, num);
	...
	PTF_ASSERT_EQUAL(serverHelloMessage->getExtensionsLenth(), 20, num);
	PTF_ASSERT_EQUAL(serverHelloMessage->getExtensionCount(), 3, num);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		PTF_ASSERT_EQUAL(curExt->getData()[0], extensionDataFirstByte[i], num);
	}
	...
	PTF_ASSERT_EQUAL(ccsLayer->getHeaderLen(), 6, num);

	handshakeLayer = multipleRecordsPacket.getNextLayerOfType<pcpp::SSLHandshakeLayer>(handshakeLayer);
	PTF_ASSERT_NOT_NULL(handshakeLayer);
	PTF_ASSERT_EQUAL(handshakeLayer->getHandshakeMessagesCount(), 3, num);
	...
	PTF_ASSERT_EQUAL(helloRequest->getMessageLength(), 4, num);
	...
	PTF_ASSERT_EQUAL(helloRequest2->getMessageLength(), 4, num);
	...
	PTF_ASSERT_EQUAL(handshakeLayer->getHandshakeMessageAt(2)->getMessageLength(), 32, num);
...
PTF_TEST_CASE(SSLMultipleRecordParsing2Test)
{
	...
	PTF_ASSERT_EQUAL(clientKeyExMsg->getMessageLength(), 70, num);
	PTF_ASSERT_EQUAL(clientKeyExMsg->getClientKeyExchangeParamsLength(), 66, num);
...
PTF_TEST_CASE(SSLMultipleRecordParsing3Test)
{
	...
	PTF_ASSERT_EQUAL(handshakeLayer->getHandshakeMessagesCount(), 5, num);
	...
	PTF_ASSERT_EQUAL(certMsg->getMessageLength(), 4966, num);
	PTF_ASSERT_EQUAL(certMsg->getNumOfCertificates(), 3, num);
	...
	PTF_ASSERT_EQUAL(cert->getDataLength(), 1509, num);
	...
	PTF_ASSERT_NOT_NULL(cert);
	PTF_ASSERT_TRUE(cert->allDataExists());
	PTF_ASSERT_EQUAL(cert->getDataLength(), 1728, num);
	...
	PTF_ASSERT_NOT_NULL(cert);
	PTF_ASSERT_TRUE(cert->allDataExists());
	PTF_ASSERT_EQUAL(cert->getDataLength(), 1713, num);
	certBuffer = std::string(cert->getData(), cert->getData()+cert->getDataLength());
	pos = certBuffer.find("Internal Development CA");
	PTF_ASSERT_TRUE(pos != std::string::npos);
	...
	PTF_ASSERT_EQUAL(certReqMsg->getCertificateTypes().size(), 2, num);
	...
	PTF_ASSERT_EQUAL(certReqMsg->getCertificateAuthorityLength(), 110, num);
...
PTF_TEST_CASE(SSLMultipleRecordParsing4Test)
{
	...
	PTF_ASSERT_EQUAL(serverKeyExMsg->getMessageLength(), 333, num);
	PTF_ASSERT_EQUAL(serverKeyExMsg->getServerKeyExchangeParamsLength(), 329, num);
	...
	PTF_ASSERT_EQUAL(serverHelloDoneMsg->getMessageLength(), 4, num);
...
PTF_TEST_CASE(SSLPartialCertificateParseTest)
{
	...
	PTF_ASSERT_EQUAL(certMsg->getNumOfCertificates(), 1, num);
	pcpp::SSLx509Certificate* cert = certMsg->getCertificate(0);
	PTF_ASSERT_FALSE(cert->allDataExists());
	PTF_ASSERT_EQUAL(cert->getDataLength(), 1266, num);
	...
	PTF_ASSERT_FALSE(cert->allDataExists());
	PTF_ASSERT_EQUAL(cert->getDataLength(), 1268, num);
...
PTF_TEST_CASE(SSLNewSessionTicketParseTest)
{
	...
	PTF_ASSERT_EQUAL(newSessionTicketMsg->getSessionTicketDataLength(), 214, num);
...
PTF_TEST_CASE(SSLMalformedPacketParsing)
{
	...
	pcpp::SSLClientHelloMessage* clientHelloMessage = handshakeLayer->getHandshakeMessageOfType<pcpp::SSLClientHelloMessage>();
	PTF_ASSERT_NOT_NULL(clientHelloMessage);
	PTF_ASSERT_EQUAL(clientHelloMessage->getExtensionCount(), 1, num);
...
PTF_TEST_CASE(TLS1_3ParsingTest)
{
	...
	PTF_ASSERT_EQUAL(clientHelloMsg->getCipherSuiteCount(), 4, num);
	for (int i = 0; i < 3; i++)
	{
		PTF_ASSERT_EQUAL(clientHelloMsg->getCipherSuite(i)->asString(), cipherSuiteNames[i], string);
		PTF_ASSERT_EQUAL(clientHelloMsg->getCipherSuite(i)->getID(), cipherSuiteIDs[i], num);
	...
	PTF_ASSERT_EQUAL(versionVec.size(), 3, num);
	...
	PTF_ASSERT_NOT_NULL(ecPointFormatExt);
	std::vector<uint8_t> ecPointFormatList = ecPointFormatExt->getECPointFormatList();
	PTF_ASSERT_EQUAL(ecPointFormatList.size(), 3, num);
	PTF_ASSERT_EQUAL(ecPointFormatList.at(0), 0, num);
	PTF_ASSERT_EQUAL(ecPointFormatList.at(1), 1, num);
	PTF_ASSERT_EQUAL(ecPointFormatList.at(2), 2, num);

	pcpp::TLSSupportedGroupsExtension* supportedGroupsExt = clientHelloMsg->getExtensionOfType<pcpp::TLSSupportedGroupsExtension>();
	PTF_ASSERT_NOT_NULL(supportedGroupsExt);
	std::vector<uint16_t> supportedGroups = supportedGroupsExt->getSupportedGroups();
	PTF_ASSERT_EQUAL(supportedGroups.size(), 5, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(0), 29, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(1), 23, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(2), 30, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(3), 25, num);
	PTF_ASSERT_EQUAL(supportedGroups.at(4), 24, num);
	...
	PTF_ASSERT_NOT_NULL(clientHelloMsg);
	PTF_ASSERT_EQUAL(clientHelloMsg->getCipherSuiteCount(), 18, num);
	...
	PTF_ASSERT_EQUAL(versionVec.size(), 2, num);
	...
	versionVec.clear();
	versionVec = supportedVersionsExt->getSupportedVersions();
	PTF_ASSERT_EQUAL(versionVec.size(), 1, num);
	PTF_ASSERT_EQUAL(versionVec[0].asEnum(), pcpp::SSLVersion::TLS1_3_D28, enum);
...
PTF_TEST_CASE(TLSCipherSuiteTest)
{
	...
	while (std::getline(cipherNamesFile, cipherSuiteName))
	{
		...
		PTF_ASSERT_EQUAL(cipherSuiteByID->getID(), cipherSuiteID, num);
