...
PTF_TEST_CASE(GtpLayerParsingTest)
{
	...
	PTF_ASSERT_EQUAL(be16toh(gtpLayer->getHeader()->messageLength), 88, num);
	PTF_ASSERT_EQUAL(be32toh(gtpLayer->getHeader()->teid), 1, num);
	PTF_ASSERT_EQUAL(gtpLayer->getHeader()->protocolType, 1, num);

	uint16_t seqNum;
	PTF_ASSERT_TRUE(gtpLayer->getSequenceNumber(seqNum));
	PTF_ASSERT_EQUAL(seqNum, 10461, num);
	...
	PTF_ASSERT_EQUAL(gtpLayer->getHeaderLen(), 12, num);
	...
	PTF_ASSERT_NOT_NULL(gtpLayer->getHeader());
	PTF_ASSERT_EQUAL(be16toh(gtpLayer->getHeader()->messageLength), 1508, num);
	PTF_ASSERT_EQUAL(be32toh(gtpLayer->getHeader()->teid), 0x00100657, num);
	PTF_ASSERT_EQUAL(gtpLayer->getHeader()->protocolType, 1, num);
	...
	PTF_ASSERT_EQUAL(gtpExt.getTotalLength(), 4, num);
	PTF_ASSERT_EQUAL(gtpExt.getContentLength(), 2, num);
	PTF_ASSERT_EQUAL(gtpExt.getNextExtensionHeaderType(), 0, num);
	PTF_ASSERT_TRUE(gtpExt.getNextExtension().isNull());

	PTF_ASSERT_EQUAL(gtpLayer->getHeaderLen(), 16, num);
	...
	PTF_ASSERT_NOT_NULL(gtpLayer->getHeader());
	PTF_ASSERT_EQUAL(gtpLayer->getHeader()->messageType, 0xff, hex);
	PTF_ASSERT_EQUAL(be16toh(gtpLayer->getHeader()->messageLength), 496, num);
	PTF_ASSERT_EQUAL(be32toh(gtpLayer->getHeader()->teid), 2327461905U, num);
	PTF_ASSERT_EQUAL(gtpLayer->getHeader()->protocolType, 1, num);

	PTF_ASSERT_EQUAL(gtpLayer->getHeaderLen(), 8, num);
	...
	PTF_ASSERT_NOT_NULL(gtpLayer->getHeader());
	PTF_ASSERT_EQUAL(be16toh(gtpLayer->getHeader()->messageLength), 44, num);
	PTF_ASSERT_EQUAL(be32toh(gtpLayer->getHeader()->teid), 0x09fe4b60, num);
	PTF_ASSERT_EQUAL(gtpLayer->getHeader()->protocolType, 1, num);
	...
	PTF_ASSERT_TRUE(gtpLayer->getSequenceNumber(seqNum));
	PTF_ASSERT_EQUAL(seqNum, 34062, num);
	...
	PTF_ASSERT_EQUAL(gtpLayer->getHeaderLen(), 52, num);
...
PTF_TEST_CASE(GtpLayerCreationTest)
{
	...
	PTF_ASSERT_EQUAL(bufferLength1, newGtpPacket.getRawPacket()->getRawDataLen(), num);
	...
	PTF_ASSERT_EQUAL(newExt1.getExtensionType(), 0xc0, num);
	PTF_ASSERT_EQUAL(newExt1.getTotalLength(), 4*sizeof(uint8_t), num);
	PTF_ASSERT_EQUAL(newExt1.getContentLength(), 2*sizeof(uint8_t), num);
	uint16_t* content = (uint16_t*)newExt1.getContent();
	PTF_ASSERT_EQUAL(be16toh(content[0]), 2308, num);
	PTF_ASSERT_TRUE(newExt1.getNextExtension().isNull()); 

	newGtpPacket.computeCalculateFields();

	PTF_ASSERT_EQUAL(bufferLength2, newGtpPacket.getRawPacket()->getRawDataLen(), num);
	...
	PTF_ASSERT_EQUAL(newExt2.getExtensionType(), 0x40, num);
	PTF_ASSERT_EQUAL(newExt2.getTotalLength(), 4*sizeof(uint8_t), num);
	PTF_ASSERT_EQUAL(newExt2.getContentLength(), 2*sizeof(uint8_t), num);
	content = (uint16_t*)newExt2.getContent();
	PTF_ASSERT_EQUAL(be16toh(content[0]), 1308, num);
	...
	PTF_ASSERT_EQUAL(newGtpLayer->getNextExtension().getNextExtensionHeaderType(), 0x40, num);

	PTF_ASSERT_EQUAL(bufferLength3, newGtpPacket.getRawPacket()->getRawDataLen(), num);
...
PTF_TEST_CASE(GtpLayerEditTest)
{
	...
	PTF_ASSERT_EQUAL(be16toh(extContent[0]), 1000, num);

	gtpHeader = gtpLayer->getHeader();
	PTF_ASSERT_EQUAL(be32toh(gtpHeader->teid), 10000, num);

	gtpPacket1.computeCalculateFields();

	PTF_ASSERT_EQUAL(bufferLength2, gtpPacket1.getRawPacket()->getRawDataLen(), num);
