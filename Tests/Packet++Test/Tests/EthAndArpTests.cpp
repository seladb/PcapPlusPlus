...
PTF_TEST_CASE(EthPacketCreation)
{
	...
	PTF_ASSERT_EQUAL(ethPacket.getLayerOfType<pcpp::EthLayer>()->getEthHeader()->etherType, be16toh(PCPP_ETHERTYPE_IP), num);

	pcpp::RawPacket* rawPacket = ethPacket.getRawPacket();
	PTF_ASSERT_NOT_NULL(rawPacket);
	PTF_ASSERT_EQUAL(rawPacket->getRawDataLen(), 18, num);
...
PTF_TEST_CASE(EthAndArpPacketParsing)
{
	...
	PTF_ASSERT_EQUAL(arpLayer->getArpHeader()->hardwareType, htobe16(1), num);
	PTF_ASSERT_EQUAL(arpLayer->getArpHeader()->protocolType, htobe16(PCPP_ETHERTYPE_IP), hex);
	PTF_ASSERT_EQUAL(arpLayer->getArpHeader()->hardwareSize, 6, num);
	PTF_ASSERT_EQUAL(arpLayer->getArpHeader()->protocolSize, 4, num);
	PTF_ASSERT_EQUAL(arpLayer->getArpHeader()->opcode, htobe16(pcpp::ARP_REPLY), num);
...
PTF_TEST_CASE(ArpPacketCreation)
{
	...
	PTF_ASSERT_EQUAL(arpRequestPacket.getRawPacket()->getRawDataLen(), 42, num);
	...
	PTF_ASSERT_EQUAL(arpHeader->hardwareSize, 6, num);
	PTF_ASSERT_EQUAL(arpHeader->protocolType, htobe16(PCPP_ETHERTYPE_IP), num);

	READ_FILE_INTO_BUFFER(1, "PacketExamples/ArpRequestPacket.dat");

	PTF_ASSERT_EQUAL(bufferLength1, arpRequestPacket.getRawPacket()->getRawDataLen(), num);
...
PTF_TEST_CASE(EthDot3LayerParsingTest)
{
	...
	PTF_ASSERT_EQUAL(ethDot3Layer->getHeaderLen(), 14, num);
	...
	PTF_ASSERT_EQUAL(be16toh(ethDot3Layer->getEthHeader()->length), 38, num);
	...
	PTF_ASSERT_EQUAL(payloadLayer->getDataLen(), 46, num);
...
PTF_TEST_CASE(EthDot3LayerCreateEditTest)
{
	...
	PTF_ASSERT_EQUAL(newPayloadLayer.getDataLen(), 46, num);
