...
PTF_TEST_CASE(WGHandshakeInitParsingTest)
{
	...
	size_t headerLen = wgLayer->getHeaderLen();
	PTF_ASSERT_TRUE(headerLen == sizeof(pcpp::wg_handshake_initiation));
...
PTF_TEST_CASE(WGHandshakeRespParsingTest)
{
	...
	size_t headerLen = wgLayer->getHeaderLen();
	PTF_ASSERT_TRUE(headerLen == sizeof(pcpp::wg_handshake_response));
...
PTF_TEST_CASE(WGTransportDataParsingTest)
{
	...
	size_t headerLen = wgLayer->getHeaderLen();
	PTF_ASSERT_TRUE(headerLen >= sizeof(pcpp::wg_transport_data));
