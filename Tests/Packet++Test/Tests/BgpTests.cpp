...
PTF_TEST_CASE(BgpLayerParsingTest)
{
	...
	PTF_ASSERT_EQUAL(bgpLayer->getHeaderLen(), 19, num);
	PTF_ASSERT_EQUAL(bgpKALayer->getHeaderLen(), 19, num);
	for (int ind = 0; ind < 16; ind++)
	{
		PTF_ASSERT_EQUAL(bgpKALayer->getKeepaliveHeader()->marker[ind], 0xff, num);
	}
	PTF_ASSERT_EQUAL(be16toh(bgpKALayer->getKeepaliveHeader()->length), 19, num);
	...
	PTF_ASSERT_EQUAL(bgpLayer->getHeaderLen(), 57, num);
	PTF_ASSERT_EQUAL(bgpOpenLayer->getHeaderLen(), 57, num);
	for (int ind = 0; ind < 16; ind++)
	{
		PTF_ASSERT_EQUAL(bgpOpenLayer->getOpenMsgHeader()->marker[ind], 0xff, num);
	}
	PTF_ASSERT_EQUAL(be16toh(bgpOpenLayer->getOpenMsgHeader()->myAutonomousSystem), 1, num);
	PTF_ASSERT_EQUAL(be16toh(bgpOpenLayer->getOpenMsgHeader()->holdTime), 180, num);
	PTF_ASSERT_EQUAL(bgpOpenLayer->getOpenMsgHeader()->optionalParameterLength, 28, num);
	PTF_ASSERT_EQUAL(bgpOpenLayer->getBgpId(), pcpp::IPv4Address("1.1.1.1"), object);
	PTF_ASSERT_EQUAL(bgpOpenLayer->getOptionalParametersLength(), 28, num);
	std::vector<pcpp::BgpOpenMessageLayer::optional_parameter> optionalParams;
	bgpOpenLayer->getOptionalParameters(optionalParams);
	PTF_ASSERT_EQUAL(optionalParams.size(), 5, num);
	...
	for (int i = 0; i < 5; i++)
	{
		...
		PTF_ASSERT_EQUAL(optParam.type, 2, num);
		PTF_ASSERT_EQUAL(optParam.length, optParamsLength[i], num);
		PTF_ASSERT_EQUAL(optParam.value[optParamsDataInedx[i]], optParamsData[i], num);
	}
	...
	PTF_ASSERT_EQUAL(bgpNotificationLayer->getHeaderLen(), 146, num);
	for (int ind = 0; ind < 16; ind++)
	{
		PTF_ASSERT_EQUAL(bgpNotificationLayer->getNotificationMsgHeader()->marker[ind], 0xff, num);
	}
	PTF_ASSERT_EQUAL(bgpNotificationLayer->getNotificationMsgHeader()->errorCode, 6, num);
	PTF_ASSERT_EQUAL(bgpNotificationLayer->getNotificationMsgHeader()->errorSubCode, 2, num);
	PTF_ASSERT_EQUAL(bgpNotificationLayer->getNotificationDataLen(), 125, num);
	...
	PTF_ASSERT_NOT_NULL(bgpNotificationLayer);
	PTF_ASSERT_EQUAL(bgpNotificationLayer->getNotificationDataLen(), 0, num);
	...
	PTF_ASSERT_EQUAL(bgpRRLayer->getHeaderLen(), 23, num);
	PTF_ASSERT_EQUAL(be16toh(bgpRRLayer->getRouteRefreshHeader()->afi), 1, num);
	PTF_ASSERT_EQUAL(bgpRRLayer->getRouteRefreshHeader()->safi, 1, num);
	PTF_ASSERT_EQUAL(bgpRRLayer->getRouteRefreshHeader()->reserved, 1, num);
	...
	PTF_ASSERT_EQUAL(bgpUpdateLayer->getHeaderLen(), 38, num);
	PTF_ASSERT_EQUAL(bgpUpdateLayer->getWithdrawnRoutesLength(), 15, num);
	std::vector<pcpp::BgpUpdateMessageLayer::prefix_and_ip> withdrawnRoutes;
	bgpUpdateLayer->getWithdrawnRoutes(withdrawnRoutes);
	PTF_ASSERT_EQUAL(withdrawnRoutes.size(), 4, num);
	pcpp::BgpUpdateMessageLayer::prefix_and_ip wr = withdrawnRoutes[0];
	PTF_ASSERT_EQUAL(wr.prefix, 24, num);
	...
	PTF_ASSERT_EQUAL(wr.prefix, 16, num);
	...
	PTF_ASSERT_NOT_NULL(bgpUpdateLayer);
	PTF_ASSERT_EQUAL(bgpUpdateLayer->getBgpMessageType(), pcpp::BgpLayer::Update, enum);
	PTF_ASSERT_EQUAL(bgpUpdateLayer->getHeaderLen(), 55, num);
	PTF_ASSERT_EQUAL(bgpUpdateLayer->getWithdrawnRoutesLength(), 0, num);
	withdrawnRoutes.clear();
	bgpUpdateLayer->getWithdrawnRoutes(withdrawnRoutes);
	PTF_ASSERT_EQUAL(withdrawnRoutes.size(), 0, num);
	pathAttributes.clear();
	PTF_ASSERT_EQUAL(bgpUpdateLayer->getPathAttributesLength(), 28, num);
	bgpUpdateLayer->getPathAttributes(pathAttributes);
	PTF_ASSERT_EQUAL(pathAttributes.size(), 3, num);
	pcpp::BgpUpdateMessageLayer::path_attribute pathAttr;
	pathAttr = pathAttributes[0];
	PTF_ASSERT_EQUAL(pathAttr.flags, 0x40, num);
	PTF_ASSERT_EQUAL(pathAttr.type, 1, num);
	PTF_ASSERT_EQUAL(pathAttr.length, 1, num);
	PTF_ASSERT_EQUAL(pathAttr.data[0], 2, num);
	pathAttr = pathAttributes[1];
	PTF_ASSERT_EQUAL(pathAttr.flags, 0x40, num);
	PTF_ASSERT_EQUAL(pathAttr.type, 2, num);
	PTF_ASSERT_EQUAL(pathAttr.length, 14, num);
	PTF_ASSERT_EQUAL(pathAttr.data[5], 0x0a, num);
	pathAttr = pathAttributes[2];
	PTF_ASSERT_EQUAL(pathAttr.flags, 0x40, num);
	PTF_ASSERT_EQUAL(pathAttr.type, 3, num);
	PTF_ASSERT_EQUAL(pathAttr.length, 4, num);
	PTF_ASSERT_EQUAL(pathAttr.data[2], 0x1e, num);
	PTF_ASSERT_EQUAL(bgpUpdateLayer->getNetworkLayerReachabilityInfoLength(), 4, num);
	std::vector<pcpp::BgpUpdateMessageLayer::prefix_and_ip> nlriVec;
	bgpUpdateLayer->getNetworkLayerReachabilityInfo(nlriVec);
	PTF_ASSERT_EQUAL(nlriVec.size(), 1, num);
	pcpp::BgpUpdateMessageLayer::prefix_and_ip nlri = nlriVec[0];
	PTF_ASSERT_EQUAL(nlri.prefix, 24, num);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		PTF_ASSERT_NOT_NULL(bgpUpdateLayer);
		PTF_ASSERT_EQUAL(bgpUpdateLayer->getPathAttributesLength(), pathAttrSize[i], num);
	}
} // BgpLayerParsingTest
...
PTF_TEST_CASE(BgpLayerCreationTest)
{
	...
	PTF_ASSERT_EQUAL(newKAMessage.getDataLen(), origKAMessage->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpKAPacket.getRawPacket()->getRawDataLen(), bufferLength1, num);
	...
	PTF_ASSERT_EQUAL(newRouteRefreshMessage.getDataLen(), origRouteRefreshMessage->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpRouteRefreshPacket.getRawPacket()->getRawDataLen(), bufferLength2, num);
	...
	PTF_ASSERT_EQUAL(newNotificationMessage.getDataLen(), origNotificationMessage->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpNotificationPacket.getRawPacket()->getRawDataLen(), bufferLength3, num);
	...
	PTF_ASSERT_EQUAL(newNotificationMessage2.getDataLen(), origNotificationMessage->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpNotificationPacket2.getRawPacket()->getRawDataLen(), bufferLength4, num);
	...
	PTF_ASSERT_EQUAL(newUpdateMessage.getDataLen(), origUpdateMessage->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpUpdatePacket1.getRawPacket()->getRawDataLen(), bufferLength5, num);
	...
	PTF_ASSERT_EQUAL(newUpdateMessage2.getDataLen(), origUpdateMessage->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpUpdatePacket2.getRawPacket()->getRawDataLen(), bufferLength6, num);
	...
	PTF_ASSERT_EQUAL(newOpenMessage.getDataLen(), origOpenMessage->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpOpenPacket.getRawPacket()->getRawDataLen(), bufferLength7, num);
	...
	PTF_ASSERT_EQUAL(newBgpMultiLayerPacket.getRawPacket()->getRawDataLen(), bgpUpdatePacket2.getRawPacket()->getRawDataLen(), num);
...
PTF_TEST_CASE(BgpLayerEditTest)
{
	...
	PTF_ASSERT_EQUAL(bgpNotificationMessage1->getDataLen(), bgpNotificationMessage2->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpOpenMessage1->getDataLen(), bgpOpenMessage2->getDataLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpOpenMessage2->getHeaderLen(), 29, num);
	PTF_ASSERT_EQUAL(bgpOpenMessage2->getOptionalParametersLength(), 0, num);
	...
	PTF_ASSERT_EQUAL(bgpUpdateMessage1Packet1->getHeaderLen(), bgpUpdateMessage2Packet1->getHeaderLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpUpdateMessage1Packet1->getHeaderLen(), bgpUpdateMessage1Packet2->getHeaderLen(), num);
	...
	nlri.push_back(pcpp::BgpUpdateMessageLayer::prefix_and_ip(24, "103.103.40.0"));
	PTF_ASSERT_TRUE(bgpUpdateMessage1Packet1->setNetworkLayerReachabilityInfo(nlri));
	PTF_ASSERT_EQUAL(bgpUpdateMessage1Packet1->getHeaderLen(), bgpUpdateMessage2Packet2->getHeaderLen(), num);
	...
	nlri.push_back(pcpp::BgpUpdateMessageLayer::prefix_and_ip(32, "20.100.100.20"));
	PTF_ASSERT_TRUE(bgpUpdateMessage1Packet1->setNetworkLayerReachabilityInfo(nlri));
	PTF_ASSERT_EQUAL(bgpUpdateMessage1Packet1->getHeaderLen(), bgpUpdateMessage3Packet2->getHeaderLen(), num);
	...
	PTF_ASSERT_EQUAL(bgpUpdateMessage1Packet1->getHeaderLen(), bgpUpdateMessage4Packet2->getHeaderLen(), num);
