...
PTF_TEST_CASE(SllPacketParsingTest)
{
	...
	PTF_ASSERT_EQUAL(sllLayer->getSllHeader()->packet_type, 0, num);
	PTF_ASSERT_EQUAL(sllLayer->getSllHeader()->ARPHRD_type, htobe16(1), num);
	PTF_ASSERT_EQUAL(sllLayer->getSllHeader()->link_layer_addr_len, htobe16(6), num);
	...
	PTF_ASSERT_EQUAL(sllLayer->getSllHeader()->protocol_type, htobe16(PCPP_ETHERTYPE_IPV6), num);
...
PTF_TEST_CASE(SllPacketCreationTest)
{
	...
	PTF_ASSERT_EQUAL(sllPacket.getRawPacket()->getRawDataLen(), bufferLength1, num);
...
PTF_TEST_CASE(NullLoopbackTest)
{
	...
	PTF_ASSERT_EQUAL(nullLoopbackLayer->getFamily(), PCPP_BSD_AF_INET6_DARWIN, num);
	...
	PTF_ASSERT_EQUAL(nullLoopbackLayer->getFamily(), PCPP_BSD_AF_INET, num);
	...
	PTF_ASSERT_NOT_NULL(nextLayer);
	PTF_ASSERT_EQUAL(nextLayer->getProtocol(), pcpp::IPv4, enum);
	PTF_ASSERT_GREATER_THAN(nullLoopbackLayer->getFamily(), 1500, num);
	...
	PTF_ASSERT_EQUAL(newNullPacket.getRawPacket()->getRawDataLen(), bufferLength2, num);
