...
static void getpmaps(pid_t pid) {
	...
	while (!feof(f)) {
		...
		/*parse the line */
		sscanf(buf, "%lx-%lx %4s %lx %02x:%02x %d %s", &line->vm_start,
				&line->vm_end, line->perm, &offset, &major, &minor, &inode,
				line->path);
		line->next = NULL;

		if (line->perm[2] != 'x' || strstr(buf, "/usr/lib")
				|| strstr(buf, "/lib/") || 0 == inode || 0 != offset) {
			free(line);
			continue;
		}
	...
	line = pmap_line_head;
	while (line) {
		printf("%08lx-%08lx %s %s\n", line->vm_start, line->vm_end, line->perm,
				line->path);
		line = line->next;
	}
...
static bool get_position_from_addr(char* programe, const long addr) {
	...
	if (programe) {
		...
		cmd =
				(char *) alloca(
						sizeof(addr2line_cmd) - sizeof(CMD_OPT) + strlen(programe) - 1 + 8 + sizeof(ignore_err));
...
bool locate_addr(char* file, int* line) {
	...
	if (program_path) {
		result = get_position_from_addr(program_path,
				((long *) file)[0] - start_addr);
		if (result) {
		...
		while (ptr) {
			...
			if (ret) {
				printf("Leaked object at %p (size %d, %s:%d)\n",
						(char*) ptr + sizeof(new_ptr_list_t), (int) ptr->size,
						ptr->file, ptr->line);
				ptr = ptr->next;
			} else {
				printf("Leaked object at %p (size %d, %s:%d)\n",
...
void* operator new(size_t size, const char* file, int line) {
	...
	if (ptr == NULL) {
		...
		fprintf(stderr,
				"new:  out of memory when allocating %d bytes at %s:%d\n",
				(int) size, file_name, line);
		abort();
	}
		...
		if ((char*) ptr + sizeof(new_ptr_list_t) == pointer) {
			...
			if (ptr->is_static != 0) {
				...
				if (static_alloc_counter == 0 && new_verbose_flag)
					printf(
							"Memory leaks check: all STATIC variables were freed\n");
			...
			free(ptr);
			return;
		} else if ((char*) ptr + sizeof(new_ptr_list_t)
				== (char *) pointer - 8) {
			char file_name[PATH_SIZE];
			int line = 0;
...
#pragma GCC diagnostic pop
			locate_addr(file_name, &line);
			printf(
					"ERR: Maybe delete a array missing [] before the pointer at %s:%d\n",
					file_name, line);
		}
		ptr_last = ptr;
		ptr = ptr->next;
