...
#error "Endian is not LE nor BE..."
   ...
   * units (by default, bytes) */
    uint16_t windowSize;
    /** The 16-bit checksum field is used for error-checking of the header and
   * data */
    uint16_t headerChecksum;
    /** If the URG flag (@ref tcphdr#urgFlag) is set, then this 16-bit field is an
...
class TcpOption : public TLVRecord<uint8_t, uint8_t>
{
   ...
   */
    static bool canAssign(const uint8_t* recordRawData, size_t tlvDataLen)
    {
        ...
        if (data->recordType == (uint8_t)PCPP_TCPOPT_NOP ||
            data->recordType == (uint8_t)PCPP_TCPOPT_EOL)
            return true;
    ...
    size_t getTotalSize() const
    {
        ...
        if (m_Data->recordType == (uint8_t)PCPP_TCPOPT_NOP ||
            m_Data->recordType == (uint8_t)PCPP_TCPOPT_EOL)
...
class TcpOptionBuilder : public TLVRecordBuilder
{
   ...
   */
    TcpOptionBuilder(TcpOptionType optionType, const uint8_t* optionValue,
   ...
   * special in that they contain only 1 byte which is the TCP option type (NOP
   * or EOL). The TcpOption object can be later retrieved by calling build()
...
class TcpLayer : public Layer
{
   ...
   * then copy the entire header from the other TcpLayer (including TCP options)
   */
   ...
   */
    ...
    /**
   * Get the TCP option that comes after a given option. If the given option was
   ...
   * next, or logical NULL if the given TCP option: (1) was the last one; or (2)
   ...
   * @return A TcpOption object that contains the newly added TCP option data or
   * logical NULL (TcpOption#isNull() == true) if addition failed. In case of a
   ...
   * requested TCP option data to be added
   * @param[in] prevOptionType The TCP option which the newly added option
   * should come after. This is an optional parameter which gets a default value
   * of ::TCPOPT_Unknown if omitted, which means the new option will be added as
   * the first option in the layer
   * @return A TcpOption object containing the newly added TCP option data or
   * logical NULL (TcpOption#isNull() == true) if addition failed. In case of a
   * failure a corresponding error message will be printed to log
   */
   ...
   * @return True if TCP option was removed or false if type wasn't found or if
   ...
   * @param[in] writeResultToPacket If set to true then checksum result will be
   * written to @ref tcphdr#headerChecksum
