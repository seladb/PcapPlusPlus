...
class TelnetLayer : public Layer {
   ...
   */
    enum class TelnetCommand : int {
        ...
        /// End of Record
        EndOfRecordCommand,
        /// Marks the end of a Telnet option subnegotiation, used with the SB code
        /// to specify more specific option parameters.
        SubnegotiationEnd,
        /// Null command; does nothing.
        NoOperation,
        /// Used to mark the end of a sequence of data that the recipient should
        /// scan for urgent Telnet commands.
        DataMark,
        /// Represents the pressing of the “break” or “attention” key on the
        /// terminal.
        Break,
        /// Tells the recipient to interrupt, abort, suspend or terminate the
        /// process currently in use.
        InterruptProcess,
        /// Instructs the remote host to continue running the current process, but
        /// discard all remaining output from it. This may be needed if a program
        /// starts to send unexpectedly large amounts of data to the user.
        AbortOutput,
        /// May be used to check that the remote host is still “alive”. When this
        /// character is sent the remote host returns some type of output to
        /// indicate that it is still functioning.
        AreYouThere,
        /// Instructs the recipient to delete the last undeleted character from the
        /// data stream. Used to “undo” the sending of a character.
        EraseCharacter,
        /// Tells the recipient to delete all characters from the data stream back
        /// to (but not including) the last end of line (CR+LF) sequence.
        EraseLine,
        /// Used in Telnet half-duplex mode to signal the other device that it may
        /// transmit.
        GoAhead,
        /// Marks the beginning of a Telnet option subnegotiation, used when an
        /// option requires the client and server to exchange parameters.
        Subnegotiation,
        /// Indicates that the device sending this code is willing to perform or
        /// continue performing a particular option.
        WillPerform,
        /// Indicates that the device sending this code is either not willing to
        /// perform a particular option, or is now refusing to continue to perform
        /// it.
        WontPerform,
        /// Requests that the other device perform a particular option or confirms
        /// the expectation that the other device will perform that option.
        DoPerform,
        /// Specifies that the other party not perform an option, or confirms a
        /// device’s expectation that the other party not perform an option.
        DontPerform,
        /// Precedes command values 240 through 254 as described above. A pair of
        /// IAC bytes in a row represents the data value 255.
        InterpretAsCommand
    };
   ...
   */
    enum class TelnetOption : int {
        ...
        TimingMark,
        /// RCTE, Remote Controlled Transmission and Echo RFC726
        /// https://www.iana.org/go/rfc726
        RemoteControlledTransAndEcho,
        /// Output Line Width
        OutputLineWidth,
        /// Output Page Size
        OutputPageSize,
        /// NAOCRD, Negotiate About Output Carriage-Return Disposition RFC652
        /// https://www.iana.org/go/rfc652
        OutputCarriageReturnDisposition,
        /// NAOHTS, Negotiate About Output Horizontal Tabstops RFC653
        /// https://www.iana.org/go/rfc653
        OutputHorizontalTabStops,
        /// NAOHTD, Negotiate About Output Horizontal Tab Disposition RFC654
        /// https://www.iana.org/go/rfc654
        OutputHorizontalTabDisposition,
        /// NAOFFD, Negotiate About Output Formfeed Disposition RFC655
        /// https://www.iana.org/go/rfc655
        OutputFormfeedDisposition,
        /// NAOVTS, Negotiate About Vertical Tabstops RFC656
        /// https://www.iana.org/go/rfc656
        OutputVerticalTabStops,
        /// NAOVTD, Negotiate About Output Vertcial Tab Disposition RFC657
        /// https://www.iana.org/go/rfc657
        ...
        ByteMacro,
        /// Data Entry Terminal RFC1043 - RFC732 https://www.iana.org/go/rfc1043
        /// https://www.iana.org/go/rfc732
        DataEntryTerminal,
        /// SUPDUP RFC736 - RFC734 https://www.iana.org/go/rfc736
        /// https://www.iana.org/go/rfc734
        SUPDUP,
   ...
   * @param[in] removeEscapeCharacters Whether non-alphanumerical characters
   * should be removed or not
   * @return Full payload as readable string, empty if Telnet packet contains
   ...
   * @return TelnetCommand First detected command value,
   * TelnetCommandEndOfPacket if there is no command field
   */
    TelnetCommand getFirstCommand();

    /**
   * Returns the next command of packet. Uses an internal iterator. The iterator
   * resets when reached end of packet.
   * @return TelnetCommand Detected command value, TelnetCommandEndOfPacket if
   * reached the end of packet.
   */
    TelnetCommand getNextCommand();

    /**
   * Returns the option of current command. Uses an internal iterator. Iterator
   * can be moved with getNextCommand
   * @return TelnetOption Option of current command
   */
    TelnetOption getOption();

    /**
   * Returns the option of provided command. It will return option of first
   * occurrence of the command
   * @param[in] command Telnet command to search
   * @return TelnetOption Option of the command. Returns TelnetOptionNoOption if
   * the provided command not found.
   */
    TelnetOption getOption(TelnetCommand command);

    /**
   * Returns the data of current command. Uses an internal iterator. Iterator
   * can be moved with getNextCommand
   * @param[out] length Length of the data of current command
   * @return uint8_t* Pointer to the data of current command. NULL if there is
   * no data for this command.
   */
    uint8_t* getOptionData(size_t& length);

    /**
   * Returns the data of provided command. It will return data of first
   * occurrence of the command
   * @param[in] command Telnet command to search
   * @param[out] length Length of the data of current command
   * @return uint8_t* Pointer to the data of current command. NULL if there is
   * no data for this command or if can't find the command.
   */
   ...
   */
    ...
    /**
   * A static method that takes a byte array and detects whether it is a Telnet
   * message
   ...
   */
    ...
    /// Parses the next layer. Telnet is the always last so does nothing for this
    /// layer
    void parseNextLayer() {}
