...
#pragma pack(push, 1)
     ...
     * event */
        uint16_t methodID;
        /** Length. Also covers payload. Excludes serviceID, methodID and length
     * field itself */
        uint32_t length;
        /** Client ID */
        uint16_t clientID;
...
#pragma pack(pop)
   ...
   * @param[in] dataLen Size of the data in bytes
   * holds the reference to a data buffer. This option can be used to reduce the
   * number of copies to generate packets.
   */
    SomeIpLayer(uint16_t serviceID, uint16_t methodID, uint16_t clientID,
                uint16_t sessionID, uint8_t interfaceVersion, MsgType type,
                uint8_t returnCode, const uint8_t* const data = nullptr,
                size_t dataLen = 0);
   ...
   */
    ...
    /**
   * A static method that creates a SOME/IP or SOME/IP-TP layer from packet raw
   * data. Returns PayloadLayer if data is not valid.
   * @param[in] data A pointer to the raw data
   * @param[in] dataLen Size of the data in bytes
   * @param[in] prevLayer A pointer to the previous layer
   * @param[in] packet A pointer to the Packet instance where layer will be
   ...
   */
    static Layer* parseSomeIpLayer(uint8_t* data, size_t dataLen,
                                   Layer* prevLayer, Packet* packet);

    /**
   * Get a pointer to the basic SOME/IP header. Notice this points directly to
   ...
   */
    ...
    /**
   * Checks if given port is a SOME/IP protocol port (only Service Discovery
   * ports are checked for now)
   ...
   */
    ...
    /**
   * Removes port from a list of ports where pcap checks for SOME/IP
   * communication.
   ...
   */
    ...
    /**
   * Removes all ports from a list of ports where pcap checks for SOME/IP
   * communication.
   */
   ...
   */
    ...
    /**
   * Identifies the following next layers: SomeIpLayer, SomeIpTpLayer,
   * SomeIpSdLayer. Otherwise sets PayloadLayer
   */
    void parseNextLayer();
  ...
  private:
    ...
    /* Using unordered_set since insertion and search should be almost constant
   * time */
...
#pragma pack(pop)
   ...
   * @param[in] type Type of the message
   * @param[in] returnCode Return Code
   * @param[in] offset Offset indicating the data offset in increments of 16
   * bytes
   * @param[in] moreSegmentsFlag Flag indicating whether more SOME/IP-TP Packets
   * will follow
   * @param[in] data A pointer to the raw data
   * @param[in] dataLen Size of the data in bytes
   */
    SomeIpTpLayer(uint16_t serviceID, uint16_t methodID, uint16_t clientID,
                  uint16_t sessionID, uint8_t interfaceVersion, MsgType type,
                  uint8_t returnCode, uint32_t offset, bool moreSegmentsFlag,
   ...
   */
    ...
    /**
   * Get a pointer to the basic SOME/IP-TP header. Notice this points directly
   ...
   * @param[in] offset Offset to set. Already has to be in multiples of 16
   * bytes.
   */
   ...
   * @param[in] flag True if the More Segments Flag shall be set, false for
   * resetting
   */
