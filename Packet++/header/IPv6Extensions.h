...
class IPv6Extension {
   ...
   */
    ...
    /**
   * @return A pointer to the next header or NULL if the extension is the last
   * one
   */
  ...
  protected:
    ...
    IPv6Extension(IDataContainer* dataContainer, size_t offset)
        : m_NextHeader(NULL), m_ExtType(IPv6ExtensionUnknown),
          m_DataContainer(dataContainer), m_Offset(offset), m_ShadowData(NULL) {}

    // protected empty c'tor
    IPv6Extension()
        : m_NextHeader(NULL), m_ExtType(IPv6ExtensionUnknown),
          m_DataContainer(NULL), m_Offset(0), m_ShadowData(NULL) {}
...
class IPv6FragmentationHeader : public IPv6Extension {
   ...
   */
     ...
     * of the original packet plus 1-bit indicating if more fragments will
     * follow */
        uint16_t fragOffsetAndFlags;
        /** packet identification value. Needed for reassembly of the original
     * packet */
        uint32_t id;
    };

    /**
   * A c'tor for creating a new IPv6 fragmentation extension object not bounded
   * to a packet. Useful for adding new extensions to an IPv6 layer with
   * IPv6Layer#addExtension()
   ...
   */
    IPv6FragmentationHeader(uint32_t fragId, uint16_t fragOffset,
                            bool lastFragment);

    /**
   * Get a pointer to the fragmentation header. Notice the returned pointer
   * points directly to the data, so every change will modify the actual packet
   * data
   ...
   */
    ...
    /**
   * @return True if this is the first fragment (which usually contains the L4
   ...
   */
    ...
    /**
   * @return True if the "more fragments" bit is set, meaning more fragments are
   * expected to follow this fragment
   */
...
/**
 * An abstract base class for Hop-By-Hop and Destination IPv6 extensions which
 * their structure contains Type-Length-Value (TLV) options. This class provides
...
class IPv6TLVOptionHeader : public IPv6Extension {
  ...
  public:
    /**
   * @class IPv6Option
   * A class representing a Type-Length-Value (TLV) options that are used inside
   * Hop-By-Hop and Destinations IPv6 extensions. This class does not create or
   ...
   * A class for building IPv6 Type-Length-Value (TLV) options. This builder
   ...
   */
     ...
     * The IPv6Option object can later be retrieved by calling build()
     * @param[in] optType IPv6 option type
     * @param[in] optValue A buffer containing the option value. This buffer is
     * read-only and isn't modified in any way
     * @param[in] optValueLen Option value length in bytes
     */
        IPv6TLVOptionBuilder(uint8_t optType, const uint8_t* optValue,
                             uint8_t optValueLen)
            : TLVRecordBuilder(optType, optValue, optValueLen) {}

        /**
     * A c'tor for building IPv6 TLV options which have a 1-byte value. The
     * IPv6Option object can later be retrieved by calling build()
     * @param[in] optType IPv6 option type
     ...
     */
        ...
        /**
     * A c'tor for building IPv6 TLV options which have a 2-byte value. The
     * IPv6Option object can later be retrieved by calling build()
     * @param[in] optType IPv6 option type
     ...
     */
        ...
        /**
     * A copy c'tor that creates an instance of this class out of another
     * instance and copies all the data from it
     ...
     */
        ...
        /**
     * Assignment operator that copies all data from another instance of
   ...
   * @return An IPv6Option object that wraps the option data. If option isn't
   * found a logical NULL is returned (IPv6Option#isNull() == true)
   */
    IPv6Option getOption(uint8_t optionType) const;

    /**
   * @return An IPv6Option that wraps the first option data or logical NULL
   * (IPv6Option#isNull() == true) if no options exist
   */
    IPv6Option getFirstOption() const;

    /**
   * Returns a pointer to the option that comes after the option given as the
   * parameter
   * @param[in] option A pointer to an option instance
   * @return An IPv6Option object that wraps the option data. In the following
   * cases logical NULL (IPv6Option#isNull() == true) is returned: (1) input
   * parameter is out-of-bounds for this extension or (2) the next option
   * doesn't exist or (3) the input option is NULL
   */
...
class IPv6HopByHopHeader : public IPv6TLVOptionHeader {
  ...
  public:
    /**
   * A c'tor for creating a new IPv6 Hop-By-Hop extension object not bounded to
   * a packet. Useful for adding new extensions to an IPv6 layer with
   * IPv6Layer#addExtension()
   * @param[in] options A vector of IPv6TLVOptionHeader#TLVOptionBuilder
   * instances which define the options that will be stored in the extension
...
class IPv6DestinationHeader : public IPv6TLVOptionHeader {
  ...
  public:
    /**
   * A c'tor for creating a new IPv6 destination extension object not bounded to
   * a packet. Useful for adding new extensions to an IPv6 layer with
   * IPv6Layer#addExtension()
   * @param[in] options A vector of IPv6TLVOptionHeader#TLVOptionBuilder
   * instances which define the options that will be stored in the extension
...
class IPv6RoutingHeader : public IPv6Extension {
   ...
   */
     ...
     * first 8 octets */
        ...
        uint8_t routingType;
        /** Number of nodes this packet still has to visit before reaching its final
     * destination */
        uint8_t segmentsLeft;
    };

    /**
   * A c'tor for creating a new IPv6 routing extension object not bounded to a
   * packet. Useful for adding new extensions to an IPv6 layer with
   * IPv6Layer#addExtension()
   * @param[in] routingType Routing type value (will be written to
   * ipv6_routing_header#routingType field)
   * @param[in] segmentsLeft Segments left value (will be written to
   * ipv6_routing_header#segmentsLeft field)
   * @param[in] additionalRoutingData A pointer to a buffer containing the
   * additional routing data for this extension. Notice this buffer is read-only
   * and its content isn't modified
   * @param[in] additionalRoutingDataLen The length of the additional routing
   * data buffer
   */
    ...
    /**
   * Get a pointer to the fixed part of the routing header. Notice the return
   * pointer points directly to the data, so every change will modify the actual
   * packet data
   ...
   */
    ...
    /**
   * @return A pointer to the buffer containing the additional routing data for
   * this extension. Notice that any change in this buffer will lead to a change
   ...
   */
    ...
    /**
   * In many cases the additional routing data is actually IPv6 address(es).
   * This method converts the raw buffer data into an IPv6 address
   * @param[in] offset An offset in the additional routing buffer pointing to
   * where the IPv6 address begins. In some cases there are multiple IPv6
   * addresses in the additional routing data buffer so this offset points to
   * where the request IPv6 address begins. Also, even if there is only one IPv6
   * address in this buffer, sometimes it isn't written in the beginning of the
   * buffer, so the offset points to where the IPv6 address begins. This is an
   * optional parameter and the default offset is 0
   * @return The IPv6 address stored in the additional routing data buffer from
   * the offset defined by the user. If offset is out-of-bounds of the extension
   * of doesn't have 16 bytes (== the length of IPv6 address) until the end of
   * the buffer - IPv6Address#Zero is returned
   */
...
class IPv6AuthenticationHeader : public IPv6Extension {
   ...
   */
     ...
     * example, an AH value of 4 equals: [ 3×(32-bit fixed-length AH fields) +
     * 3×(32-bit ICV fields) − 2 ] and thus an AH value of 4 means 24 octets */
        uint8_t headerLen;
        /** Reserved bytes, all zeros */
        uint16_t reserved;
        /** Arbitrary value which is used (together with the destination IP address)
     * to identify the security association of the receiving party */
        uint32_t securityParametersIndex;
        /** A monotonic strictly increasing sequence number (incremented by 1 for
     * every packet sent) */
        uint32_t sequenceNumber;
    };

    /**
   * A c'tor for creating a new IPv6 authentication header extension object not
   * bounded to a packet. Useful for adding new extensions to an IPv6 layer with
   * IPv6Layer#addExtension()
   * @param[in] securityParametersIndex Security Parameters Index (SPI) value
   * (will be written to ipv6_authentication_header#securityParametersIndex
   * field)
   * @param[in] sequenceNumber Sequence number value (will be written to
   * ipv6_authentication_header#sequenceNumber field)
   * @param[in] integrityCheckValue A pointer to a buffer containing the
   * integrity check value (ICV) data for this extension. Notice this pointer is
   * read-only and its content isn't modified in any way
   * @param[in] integrityCheckValueLen The length of the integrity check value
   * (ICV) buffer
   */
    IPv6AuthenticationHeader(uint32_t securityParametersIndex,
                             uint32_t sequenceNumber,
                             const uint8_t* integrityCheckValue,
                             size_t integrityCheckValueLen);

    /**
   * Get a pointer to the fixed part of the authentication header. Notice the
   * return pointer points directly to the data, so every change will modify the
   * actual packet data
   ...
   */
    ...
    /**
   * @return A pointer to the buffer containing the integrity check value (ICV)
   * for this extension. Notice that any change in this buffer will lead to a
   * change in the extension data
   */
   ...
   */
    ...
    /**
   * In the authentication header the extension length is calculated in a
   * different way than other extensions. The calculation is: [ 4 *
