 ...
 *
 * @brief The NTP packet consists of an integral number of 32-bit (4 octet)
 words in network byte order.
 * The packet format consists of three components: the header itself, one or
 more optional extension fields (for v4),
 * and an optional message authentication code (MAC). Currently the extension
 fields are not supported. The NTP header is:
 *
 * @verbatim
   0                   1                   2                   3
...
#pragma pack(push, 1)
    struct ntp_v3_auth
    {
        ...
        uint32_t keyID;
        /// This is an integer identifying the cryptographic key used to generate
...
#pragma pack(pop)
   ...
   */
    enum class ClockSource : uint32_t
    {
        ...
        /// Meinberg DCF77 with phase modulation)/pseudo random phase modulation
        /// (Ref: https://www.meinbergglobal.com/english/info/ntp-refid.htm)
        ...
        /// Meinberg Undisciplined local clock (Ref:
        /// https://www.meinbergglobal.com/english/info/ntp-refid.htm)
        LCL = ('L') | ('C' << 8) | ('L' << 16),
        /// Meinberg Undisciplined local clock (Ref:
        /// https://www.meinbergglobal.com/english/info/ntp-refid.htm)
   ...
   */
    enum class KissODeath : uint32_t
    {
        ...
        /// Rate exceeded.  The server has temporarily denied access because the
        /// client exceeded the rate threshold
        RATE = ('R') | ('A' << 8) | ('T' << 16) | ('E' << 24),
        /// Somebody is tinkering with the association from a remote host running
   ...
   */
    ...
    /**
   * @return The value of reference identifier as a string. String
   * representation of NTP clock source if stratum is 1, IPv4 address or MD5
   * hash of first four octets of IPv6
   */
   ...
   */
    ...
    /**
   * @return The value of reference timestamp in seconds from Unix Epoch (1 Jan
   * 1970)
   */
   ...
   */
    ...
    /**
   * @return The value of origin timestamp in seconds from Unix Epoch (1 Jan
   * 1970)
   */
   ...
   */
    ...
    /**
   * @return The value of receive timestampin seconds from Unix Epoch (1 Jan
   * 1970)
   */
   ...
   */
    ...
    /**
   * @return The value of transmit timestamp in seconds from Unix Epoch (1 Jan
   * 1970)
   */
   ...
   */
    ...
    /**
   * @return Returns the key identifier if exists, returns 0 on unsupported NTP
   * version or key identifier not found
   */
    uint32_t getKeyID() const;

    /**
   * @return Get the digest value as hexadecimal string, empty string on
   * unsupported version
   */
   ...
   */
    ...
    /**
   * A static method that takes a byte array and detects whether it is a NTP
   * message
   ...
   */
    ...
    /// Parses the next layer. NTP is the always last so does nothing for this
    /// layer
    void parseNextLayer() {}

    /**
   * @return Get the size of the layer (Including the extension and
   * authentication fields if exists)
   */
