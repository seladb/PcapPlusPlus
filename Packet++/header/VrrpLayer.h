...
struct vrrp_header {
    ...
    uint8_t version : 4,
        ...
        type : 4;
#endif
    /** The Virtual Router Identifier (VRID) field identifies the virtual router
   * this packet is reporting status for*/
    uint8_t vrId;
   ...
   */
    ...
    /** This specifies authentication type(v2) or (Max) Advertisement interval (in
   * seconds(v2) or centi-seconds(v3)). */
    uint16_t authTypeAdvInt;

    /** This specifies checksum field that is used to detect data corruption in
   * the VRRP message.
   * VRRPv2 uses normal checksum algorithm, while VRRPv3 uses "pseudo-header"
   * checksum algorithm. */
    uint16_t checksum;

    /** This specifies one or more IPvX addresses that are associated with the
   * virtual router. */
    uint8_t* ipAddresses[];
};
...
/**
 * @class VrrpLayer
 * A base class for all VRRP (Virtual Router Redundancy Protocol) protocol
...
class VrrpLayer : public Layer {
  ...
  protected:
    ...
    explicit VrrpLayer(ProtocolType subProtocol, uint8_t virtualRouterId,
                       uint8_t priority);
   ...
   * @return One of the values ::VRRPv2, ::VRRPv3 according to detected VRRP
   * version or ::UnknownProtocol if couldn't detect VRRP version
   */
   ...
   */
    ...
    /**
   * Fill the checksum from header and data and write the result to @ref
   * vrrp_header#checksum
   */
    void calculateAndSetChecksum();

    /**
   * Calculate the checksum from header and data and write the result to @ref
   * vrrp_header#checksum
   ...
   */
    ...
    /**
   * Add a list of virtual IP addresses at a the end of the virtual IP address
   * list. The vrrp_header#ipAddressCount field will be incremented accordingly
   ...
   */
    ...
    /**
   * Add a virtual IP address at a the end of the virtual IP address list. The
   ...
   */
    ...
    /**
   * Remove a virtual IP address at a certain index. The
   * vrrp_header#ipAddressCount field will be decremented accordingly
   * @param[in] index The index of the virtual IP address to be removed
   * @return True if virtual IP address was removed successfully or false
   ...
   */
    ...
    /**
   * Remove all virtual IP addresses in the message. The
   * vrrp_header#ipAddressCount field will be set to 0
   * @return True if virtual IP addresses were cleared successfully or false
   * otherwise. If false is returned an appropriate error message will be
   * printed to log
   */
   ...
   * header + the size of the IP address(es)
   */
...
class VrrpV2Layer : public VrrpLayer {
   ...
   */
    VrrpV2Layer(uint8_t* data, size_t dataLen, Layer* prevLayer, Packet* packet)
        : VrrpLayer(data, dataLen, prevLayer, packet, VRRPv2,
   ...
   */
    explicit VrrpV2Layer(uint8_t virtualRouterId, uint8_t priority,
                         uint8_t advInt, uint8_t authType = 0);
...
class VrrpV3Layer : public VrrpLayer {
   ...
   */
    explicit VrrpV3Layer(IPAddress::AddressType addressType,
                         uint8_t virtualRouterId, uint8_t priority,
                         uint16_t maxAdvInt);
