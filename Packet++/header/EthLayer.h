...
class EthLayer : public Layer
{
   ...
   * @param[in] etherType The EtherType to be used. It's an optional parameter,
   * a value of 0 will be set if not provided
   */
    EthLayer(const MacAddress& sourceMac, const MacAddress& destMac,
             uint16_t etherType = 0);

    ~EthLayer() {}

    /**
   * Get a pointer to the Ethernet header. Notice this points directly to the
