...
/**
 * @class SdpLayer
 * Represents a SDP (Session Description Protocol) message. SDP is a text-based
 * protocol described by a series of fields, one per line (lines are separated
 * by CRLF). The form of each field is as follows:<BR>
 * @code
 * [character]=[value]
 * @endcode
 * Each character represents a certain type of field. All field type are
 * represented as macros in SdpLayer.h file (for example:
 * PCPP_SDP_ORIGINATOR_FIELD is a macro for the originator field (o=) ).<BR> For
...
class SdpLayer : public TextBasedProtocolMessage
{
   ...
   * @param[in] ipAddress The address of the machine from which the session is
   * created
   ...
   */
    ...
    /**
   * A copy constructor for this layer. Inherits the base copy constructor and
   ...
   */
    ...
    /**
   * An assignment operator overload for this layer. Inherits the base
   ...
   */
    ...
    /**
   * The 'originator' field (o=) contains the IP address of the the machine from
   * which the session is created. This IP address can be used to track the RTP
   * data relevant for the call. This method extracts this IP address from the
   * 'originator' field and returns it. A value of IPv4Address#Zero will be
   * returned in the following cases: (1) if 'originator' field doesn't exist;
   * (2) if it doesn't contain the IP address; (3) if it contains a non-IPv4
   * address
   ...
   */
    ...
    /**
   * The 'media-description' field (m=) contains the transport port to which the
   * media stream is sent. This port can be used to track the RTP data relevant
   * for the call. This method extracts this port from the 'media-description'
   * field and returns it. Since a SDP message can contain several
   * 'media-description' fields, one for each media type (e.g audio, image,
   * etc.), the user is required to provide the media type. A value of 0 will be
   * returned in the following cases: (1) if 'media-description' field doesn't
   * exist; (2) if provided media type was not found; (3) if 'media-description'
   * field didn't contain a port
   ...
   */
    ...
    /**
   * Adds a 'media-description' field (m=) with all necessary data and attribute
   * fields (a=) with data relevant for this media.<BR> After this method is run
   * the following block of fields will be added at the end of the message:
   *
   * @code
   ...
   * @endcode
   *
   * @param[in] mediaType The media type, usually "audio", "video", "text" or
   * "image"
   * @param[in] mediaPort The transport port to which the media stream is sent
   * @param[in] mediaProtocol The transport protocol, usually "udp", "RTP/AVP"
   * or "RTP/SAVP"
   * @param[in] mediaFormat A space-separated list of media format description.
   * For example: "8 96"
   * @param[in] mediaAttributes A vector of media attributes. Each string in
   * this vector will be translated into a 'media-attribute' field (a=)
   * @return True if all fields were added properly or false if at least one
   * field was failed to be added
   */
    bool addMediaDescription(const std::string& mediaType, uint16_t mediaPort,
