...
class HttpMessage : public TextBasedProtocolMessage
{
   ...
   */
    ...
    virtual HeaderField* addField(const std::string& fieldName,
                                  const std::string& fieldValue);
    virtual HeaderField* addField(const HeaderField& newField);
    virtual HeaderField* insertField(HeaderField* prevField,
...
/**
 * @class HttpRequestLayer
 * Represents an HTTP request header and inherits all basic functionality of
 * HttpMessage and TextBasedProtocolMessage. The functionality that is added for
 * this class is the HTTP first line concept. An HTTP request has the following
 * first line: <i>GET /bla/blabla.asp HTTP/1.1</i> Since it's not an "ordinary"
 * HTTP field, it requires a special treatment and gets a class of it's own:
 * HttpRequestFirstLine. Unlike most L2-4 protocols, an HTTP request header can
 * spread over more than 1 packet. PcapPlusPlus currently doesn't support a
 * header that is spread over more than 1 packet so in such cases: 1) only the
 * first packet will be parsed as HttpRequestLayer (the other packets won't be
 * recognized as HttpRequestLayer) and 2) the HTTP header for the first packet
 * won't be complete (as it continues in the following packets), this why
...
class HttpRequestLayer : public HttpMessage
{
   ...
   */
    ...
    /**
   * A constructor that allocates a new HTTP request header with only the first
   * line filled. Object will be created without further fields. The user can
   * then add fields using addField() methods
   ...
   */
    HttpRequestLayer(HttpMethod method, const std::string& uri,
                     HttpVersion version);
   ...
   * constructor HttpMessage#HttpMessage() and add the functionality of copying
   ...
   * assignment operator HttpMessage#operator=() and add the functionality of
   ...
   */
    ...
    /**
   * The URL is hostname+uri. So given the following URL, for example:
   * "www.cnn.com/main.html", the hostname is "www.cnn.com" and the URI is
   * "/.main.html". URI and hostname are split to 2 different places inside the
   * HTTP request packet: URI is in the first line and hostname is in "HOST"
   * field. This methods concatenates the hostname and URI to the full URL
...
class HttpResponseStatusCode
{
   ...
   */
    enum Value : int
    {
        ...
        HttpStatus2xxCodeUnknown =
            900002, // 2xx: Success - The action was successfully received,
                    // understood, and accepted
        HttpStatus3xxCodeUnknown =
            900003, // 3xx: Redirection - Further action must be taken in order to
                    // complete the request
        HttpStatus4xxCodeUnknown =
            900004,                        // 4xx: Client Error - The request contains bad syntax or cannot
                                           // be fulfilled
        HttpStatus5xxCodeUnknown = 900005, // 5xx: Server Error - The server failed
   ...
   */
    ...
    /**
   * @brief Construct HttpResponseStatusCode from the code number and the
   * customized message
   * @param[in] statusCodeNumber the status code in number, e.g. 200, 404
   * @param[in] statusMessage the status message, optional, leave empty to use a
   * default message
   */
    ...
    /**
   * @brief Construct HttpResponseStatusCode from Value enum and the customized
   * message
   * @param[in] statusCode the status code enum
   * @param[in] statusMessage the customized status message, optional
   */
    explicit HttpResponseStatusCode(const Value& statusCode,
                                    const std::string& statusMessage);
   ...
   */
    int toInt() const { return static_cast<int>(m_Value); }
   ...
   */
    bool isUnsupportedCode() const { return m_Value > 599; }
...
/**
 * @class HttpResponseLayer
 * Represents an HTTP response header and inherits all basic functionality of
 * HttpMessage and TextBasedProtocolMessage. The functionality that is added for
 * this class is the HTTP first line concept. An HTTP response has the following
 * first line: <i>200 OK HTTP/1.1</i> Since it's not an "ordinary" HTTP field,
 * it requires a special treatment and gets a class of it's own:
 * HttpResponseFirstLine. Unlike most L2-4 protocols, an HTTP response header
 * can spread over more than 1 packet. PcapPlusPlus currently doesn't support a
 * header that is spread over more than 1 packet so in such cases: 1) only the
 * first packet will be parsed as HttpResponseLayer (the other packets won't be
 * recognized as HttpResponseLayer) and 2) the HTTP header for the first packet
 * won't be complete (as it continues in the following packets), this why
...
class HttpResponseLayer : public HttpMessage
{
   ...
   * stored in
   */
    HttpResponseLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                      Packet* packet);

    /**
   * A constructor that allocates a new HTTP response header with only the first
   * line filled. Object will be created without further fields. The user can
   * then add fields using addField() methods
   ...
   * e.g 200 is usually "OK", 404 is usually "Not Found", etc. But the user can
   * set a non-default status code string and it will be written in the header
   * first line. Empty string ("") means using the default status code string
   * @deprecated Use other constructors instead.
   */
    PCPP_DEPRECATED explicit HttpResponseLayer(
        HttpVersion version, const HttpResponseStatusCode& statusCode,
   ...
   * @param[in] statusCode Status code to be used
   */
    explicit HttpResponseLayer(HttpVersion version,
    ...
    /**
   * A copy constructor for this layer. This copy constructor inherits base copy
   * constructor HttpMessage#HttpMessage() and adds the functionality of copying
   * the first line as well
   * @param[in] other The instance to copy from
   */
    ...
    /**
   * An assignment operator overload for this layer. This method inherits base
   * assignment operator HttpMessage#operator=() and adds the functionality of
   * copying the first line as well
   * @param[in] other The instance to copy from
   * @return A reference to the assignee
   */
   ...
   */
    ...
    /**
   * The length of the body of many HTTP response messages is determined by a
   * HTTP header field called "Content-Length". This method sets The
   * content-length field value. The method supports several cases:
   ...
   * @param[in] prevFieldName Optional field, if specified and "Content-Length"
   * field doesn't exist, it will be created after it
   ...
   */
    ...
    /**
   * The length of the body of many HTTP response messages is determined by a
   * HTTP header field called "Content-Length". This method parses this field,
   * extracts its value and return it. If this field doesn't exist the method
   * will return 0
...
/**
 * @class HttpRequestFirstLine
 * Represents an HTTP request header first line. The first line includes 3
 * parameters: HTTP method (e.g GET, POST, etc.), URI (e.g /main/index.html) and
 * HTTP version (e.g HTTP/1.1). All these parameters are included in this class,
 * and the user can retrieve or set them. This class cannot be instantiated by
 * users, it's created inside HttpRequestLayer and user can get a pointer to an
 * instance of it. All "get" methods of this class will retrieve the actual data
 * of the HTTP request and the "set" methods will change the packet data. Since
 * HTTP is a textual protocol, most fields aren't of fixed size and this also
 * applies to the first line parameters. So most "set" methods of this class
...
class HttpRequestFirstLine
{
   ...
   * @return False if newMethod is HttpRequestLayer#HttpMethodUnknown or if
   * shortening/extending the HttpRequestLayer data failed. True otherwise
   */
   ...
   * @return False if shortening/extending the HttpRequestLayer data failed.
   * True otherwise
   */
   ...
   */
    ...
    /**
   * Set the HTTP version. This method doesn't return a value since all
   * supported HTTP versions are of the same size (HTTP/0.9, HTTP/1.0, HTTP/1.1)
   ...
   */
    static HttpRequestLayer::HttpMethod parseMethod(const char* data,
                                                    size_t dataLen);
   ...
   */
    ...
    /**
   * As explained in HttpRequestLayer, an HTTP header can spread over more than
   * 1 packet, so when looking at a single packet the header can be partial.
   * Same goes for the first line - it can spread over more than 1 packet. This
   ...
   * This exception can be thrown while constructing HttpRequestFirstLine (the
   * constructor is private, so the construction happens only in
   * HttpRequestLayer). This kind of exception will be thrown if trying to
  ...
  private:
    ...
    HttpRequestFirstLine(HttpRequestLayer* httpRequest,
                         HttpRequestLayer::HttpMethod method, HttpVersion version,
...
/**
 * @class HttpResponseFirstLine
 * Represents an HTTP response header first line. The first line includes 2
 * parameters: status code (e.g 200 OK, 404 Not Found, etc.), and HTTP version
 * (e.g HTTP/1.1). These 2 parameters are included in this class, and the user
 * can retrieve or set them. This class cannot be instantiated by users, it's
 * created inside HttpResponseLayer and user can get a pointer to an instance of
 * it. The "get" methods of this class will retrieve the actual data of the HTTP
 * response and the "set" methods will change the packet data. Since HTTP is a
 * textual protocol, most fields aren't of fixed size and this also applies to
 * the first line parameters. So most "set" methods of this class need in most
...
class HttpResponseFirstLine
{
   ...
   * @return True if setting the status code was completed successfully, false
   * otherwise
   * @deprecated Use the other overload instead.
   */
    PCPP_DEPRECATED bool
   ...
   */
    static HttpResponseStatusCode parseStatusCode(const char* data,
                                                  size_t dataLen);

    /**
   * A static method for parsing the HTTP version out of raw first line data
   * (e.g "HTTP/x.y")
   * @param[in] data The raw data
   * @param[in] dataLen The raw data length
   * @return The parsed HTTP status code as enum
   ...
   */
    ...
    /**
   * As explained in HttpResponseLayer, an HTTP header can spread over more than
   * 1 packet, so when looking at a single packet the header can be partial.
   * Same goes for the first line - it can spread over more than 1 packet. This
   * method returns an indication whether the first line is partial
   * @return False if the first line is partial, true if it's complete
   ...
   * This exception can be thrown while constructing HttpResponseFirstLine (the
   * constructor is private, so the construction happens only in
   * HttpResponseLayer). This kind of exception will be thrown if trying to
