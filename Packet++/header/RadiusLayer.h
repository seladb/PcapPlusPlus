...
struct radius_header {
    ...
    uint16_t length;
    /** Used to authenticate the reply from the RADIUS server and to encrypt
   * passwords */
 ...
 * A wrapper class for RADIUS attributes. This class does not create or modify
 ...
 * A class for building RADIUS attributes. This builder receives the attribute
...
class RadiusAttributeBuilder : public TLVRecordBuilder {
   ...
   * The RadiusAttribute object can later be retrieved by calling build()
   * @param[in] attrType RADIUS attribute type
   * @param[in] attrValue A buffer containing the attribute value. This buffer
   * is read-only and isn't modified in any way
   * @param[in] attrValueLen Attribute value length in bytes
   */
    RadiusAttributeBuilder(uint8_t attrType, const uint8_t* attrValue,
                           uint8_t attrValueLen)
        : TLVRecordBuilder(attrType, attrValue, attrValueLen) {}

    /**
   * A c'tor for building RADIUS attributes which have a 1-byte value. The
   * RadiusAttribute object can later be retrieved by calling build()
   * @param[in] attrType RADIUS attribute type
   ...
   */
    ...
    /**
   * A c'tor for building RADIUS attributes which have a 2-byte value. The
   * RadiusAttribute object can later be retrieved by calling build()
   * @param[in] attrType RADIUS attribute type
   ...
   */
    ...
    /**
   * A c'tor for building RADIUS attributes which have a 4-byte value. The
   * RadiusAttribute object can later be retrieved by calling build()
   * @param[in] attrType RADIUS attribute type
   ...
   */
    ...
    /**
   * A c'tor for building RADIUS attributes which have an IPv4Address value. The
   * RadiusAttribute object can later be retrieved by calling build()
   * @param[in] attrType RADIUS attribute type
   ...
   */
    ...
    /**
   * A c'tor for building RADIUS attributes which have a string value. The
   * RadiusAttribute object can later be retrieved by calling build()
   * @param[in] attrType RADIUS attribute type
...
class RadiusLayer : public Layer {
  private:
    ...
    RadiusAttribute addAttrAt(const RadiusAttributeBuilder& attrBuilder,
                              int offset);
   ...
   * @param[in] authenticator A pointer to a byte array containing the
   * authenticator value
   * @param[in] authenticatorArrSize The authenticator byte array size. A valid
   * size of the authenticator field is 16 bytes. If the provided size is less
   * than that then the byte array will be copied to the packet but the missing
   * bytes will stay zero. If the size is more than 16 bytes, only the first 16
   * bytes will be copied to the packet
   */
    RadiusLayer(uint8_t code, uint8_t id, const uint8_t* authenticator,
   ...
   * @param[in] code The RADIUS message code
   * @param[in] id The RADIUS message ID
   * @param[in] authenticator A hex string representing the authenticator value.
   * A valid size of the authenticator field is 16 bytes. If the hex string
   * represents an array that is smaller than this then the missing bytes in the
   * packet's authenticator field will stay zero. If the hex string represents
   * an array that is larger than 16 bytes, only the first 16 bytes will be
   * copied to the packet
   */
   ...
   */
    ...
    /**
   * Get a pointer to the RADIUS header. Notice this points directly to the
   ...
   */
    ...
    /**
   * @return A hex string representation of the radius_header#authenticator byte
   * array value
   */
   ...
   * @param[in] authValue A hex string representing the requested authenticator
   * value
   */
    void setAuthenticatorValue(const std::string& authValue);

    /**
   * A static method that returns the RADIUS message string for a give message
   * code. For example: the string "Access-Request" will be returned for code 1
   ...
   */
    ...
    /**
   * @return The first RADIUS attribute in the packet. If there are no
   * attributes the returned value will contain a logical NULL
   * (RadiusAttribute#isNull() == true)
   */
    RadiusAttribute getFirstAttribute() const;

    /**
   * Get the RADIUS attribute that comes after a given attribute. If the given
   * attribute was the last one, the returned value will contain a logical NULL
   * (RadiusAttribute#isNull() == true)
   * @param[in] attr A given attribute
   * @return A RadiusAttribute object containing the attribute data that comes
   * next, or logical NULL if the given attribute: (1) was the last one; (2)
   * contains a logical NULL or (3) doesn't belong to this packet
   */
   ...
   * @param[in] attrType RADIUS attribute type
   * @return A RadiusAttribute object containing the first attribute data that
   * matches this type, or logical NULL (RadiusAttribute#isNull() == true) if no
   * such attribute found
   */
   ...
   * @param[in] attrBuilder A RadiusAttributeBuilder object that contains the
   * requested attribute data to add
   * @return A RadiusAttribute object containing the newly added RADIUS
   * attribute data or logical NULL (RadiusAttribute#isNull() == true) if
   * addition failed
   */
   ...
   * @param[in] attrBuilder A RadiusAttributeBuilder object that contains the
   * requested attribute data to add
   * @param[in] prevAttrType The RADIUS attribute which the newly added
   * attribute will come after
   * @return A RadiusAttribute object containing the newly added RADIUS
   * attribute data or logical NULL (RadiusAttribute#isNull() == true) if
   * addition failed
   */
    RadiusAttribute addAttributeAfter(const RadiusAttributeBuilder& attrBuilder,
   ...
   * @return True if the RADIUS attribute was successfully removed or false if
   * type wasn't found or if removal failed
   */
    bool removeAttribute(uint8_t attrType);

    /**
   * Remove all RADIUS attributes in this layer
   * @return True if all attributes were successfully removed or false if
   ...
   * @param[in] udpData The pointer to the UDP payload data. It points to the
   * first byte of RADIUS header.
   ...
   */
    ...
    /**
   * Calculate and store the value of radius_header#length according to the
   * layer size
   */
