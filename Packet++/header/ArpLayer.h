...
struct arphdr {
   ...
   * size is 4 */
    ...
    /** Specifies the operation that the sender is performing: 1 (::ARP_REQUEST)
   * for request, 2 (::ARP_REPLY) for reply */
    uint16_t opcode;
...
class ArpLayer : public Layer {
   ...
   * @param[in] senderMacAddr The sender MAC address (will be put in
   * arphdr#senderMacAddr)
   * @param[in] targetMacAddr The target MAC address (will be put in
   * arphdr#targetMacAddr)
   * @param[in] senderIpAddr The sender IP address (will be put in
   * arphdr#senderIpAddr)
   * @param[in] targetIpAddr The target IP address (will be put in
   * arphdr#targetIpAddr)
   */
    ArpLayer(ArpOpcode opCode, const MacAddress& senderMacAddr,
             const MacAddress& targetMacAddr, const IPv4Address& senderIpAddr,
             const IPv4Address& targetIpAddr);

    ~ArpLayer() {}

    /**
   * Get a pointer to the ARP header. Notice this points directly to the data,
   ...
   * - if it's an ARP request: @ref arphdr#targetMacAddr =
