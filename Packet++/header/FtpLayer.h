...
class FtpRequestLayer : public FtpLayer
{
   ...
   * stored in
   */
    FtpRequestLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                    Packet* packet)
   ...
   */
    explicit FtpRequestLayer(const FtpCommand& command,
...
class FtpResponseLayer : public FtpLayer
{
   ...
   * stored in
   */
    FtpResponseLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                     Packet* packet)
        : FtpLayer(data, dataLen, prevLayer, packet){};
   ...
   */
    explicit FtpResponseLayer(const FtpStatusCode& code,
                              const std::string& option = "")
