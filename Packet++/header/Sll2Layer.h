...
struct sll2_header
{
   ...
   * order and contains the 1-based index of the interface on which the packet
   *was observed
   **/
   ...
   *else (value=3); sent by us (value=4)
   **/
    uint8_t packet_type;
    /** Contains the length of the link-layer address of the sender of the packet.
   * That length could be zero */
    uint8_t link_layer_addr_len;
    /** Contains the link-layer address of the sender of the packet; the number of
   *bytes of that field that are meaningful is specified by the link-layer
...
class Sll2Layer : public Layer
{
   ...
   */
    ...
    /**
   * Get a pointer to the Sll header. Notice this points directly to the data,
   ...
   * @param[in] data The pointer to the beginning of a byte stream of an IEEE
   * 802.3 Eth packet
   * @param[in] dataLen The length of the byte stream
   * @return True if the data is valid and can represent an IEEE 802.3 Eth
   * packet
   */
   ...
   * @param[in] addrLength Address length, must be lower or equal to 8 (which is
   * max length for SLL2 address)
   * @return True if address was set successfully, or false of addrLength is out
   * of bounds (0 or larger than 8)
   */
    bool setLinkLayerAddr(const uint8_t* addr, size_t addrLength);

    /**
   * Get a MAC address in the link layer address field
   * @return return macAddress pointer was set successfully, null pointer if d
   * MAC address isn't valid or if set failed
   */
   ...
   * @return True if address was set successfully, false if MAC address isn't
   * valid or if set failed
   */
   ...
   * ArpLayer, VlanLayer, PPPoESessionLayer, PPPoEDiscoveryLayer, MplsLayer.
