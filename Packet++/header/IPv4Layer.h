...
struct iphdr {
    ...
    /** Identification field. Primarily used for uniquely identifying the group of
   ...
   * (e.g. going in circles) on an internet.  In practice, the field has become
   * a hop count */
    uint8_t timeToLive;
    /** Defines the protocol used in the data portion of the IP datagram. Must be
   * one of ::IPProtocolTypes */
    uint8_t protocol;
...
/**
 * @class IPv4Option
 * A wrapper class for IPv4 options. This class does not create or modify IPv4
...
class IPv4Option : public TLVRecord<uint8_t, uint8_t> {
   ...
   */
    ...
    /**
   * A method for parsing the IPv4 option value as a list of IPv4 addresses.
   * This method is relevant only for certain types of IPv4 options which their
   * value is a list of IPv4 addresses such as ::IPV4OPT_RecordRoute,
   * ::IPV4OPT_StrictSourceRoute, ::IPV4OPT_LooseSourceRoute, etc. This method
   * returns a vector of the IPv4 addresses. Blank IP addresses (meaning zeroed
   * addresses - 0.0.0.0) will not be added to the returned list. If some error
   * occurs during the parsing or the value is invalid an empty vector is
   * returned
   ...
   */
    ...
    /**
   * A method for parsing the IPv4 timestamp option value. This method is
   * relevant only for IPv4 timestamp option. For other option types an empty
   * result will be returned. The returned structure contains the timestamp
   * value type (timestamp only, timestamp + IP addresses, etc.) as well as 2
   * vectors containing the list of timestamps and the list of IP addresses (if
   * applicable for the timestamp value type). Blank timestamps or IP addresses
   * (meaning zeroed values - timestamp=0 or IP address=0.0.0.0) will not be
   * added to the lists. If some error occurs during the parsing or the value is
   ...
   */
    static bool canAssign(const uint8_t* recordRawData, size_t tlvDataLen) {
        ...
        if (getIPv4OptionType(data) == (uint8_t)IPV4OPT_EndOfOptionsList ||
            data->recordType == (uint8_t)IPV4OPT_NOP)
            return true;
    ...
    size_t getTotalSize() const {
        ...
        if (getIPv4OptionType() == (uint8_t)IPV4OPT_EndOfOptionsList ||
            m_Data->recordType == (uint8_t)IPV4OPT_NOP)
 ...
 * A class for building IPv4 option records. This builder receives the IPv4
...
class IPv4OptionBuilder : public TLVRecordBuilder {
  ...
  public:
    /**
   * A c'tor for building IPv4 options which their value is a byte array. The
   * IPv4Option object can be later retrieved by calling build()
   * @param[in] optionType IPv4 option type
   * @param[in] optionValue A buffer containing the option value. This buffer is
   * read-only and isn't modified in any way. For option types ::IPV4OPT_NOP and
   * ::IPV4OPT_EndOfOptionsList this parameter is ignored (expected to be NULL)
   * as these option types don't contain any data
   ...
   */
    ...
    /**
   * A c'tor for building IPv4 options which have a 2-byte value. The IPv4Option
   ...
   */
    ...
    /**
   * A c'tor for building IPv4 options which their value is a list of IPv4
   * addresses, for example:
   * ::IPV4OPT_RecordRoute, ::IPV4OPT_StrictSourceRoute,
   * ::IPV4OPT_LooseSourceRoute. The IPv4Option object can be later retrieved by
   * calling build()
   * @param[in] optionType IPv4 option type
   * @param[in] ipList A vector of IPv4 addresses that will be used as the
   * option value
   */
    IPv4OptionBuilder(IPv4OptionTypes optionType,
                      const std::vector<IPv4Address>& ipList);

    /**
   * A c'tor for building IPv4 timestamp option (::IPV4OPT_Timestamp). The
   * IPv4Option object can be later retrieved by calling build()
...
class IPv4Layer : public Layer, public IPLayer {
   ...
   * @param[in] packet A pointer to the Packet instance where layer will be
   * stored in
   * @param[in] setTotalLenAsDataLen When setting this value to "true" or when
   * using the other c'tor, the layer data length is calculated from
   * iphdr#totalLength field. When setting to "false" the data length is set as
   * the value of dataLen parameter. Please notice that if iphdr#totalLength is
   * equal to zero (which can happen in TCP Segmentation Offloading), this flag
   * is ignored and the layer data length is calculated by the actual data
   ...
   */
    ...
    /**
   * A constructor that allocates a new IPv4 header with source and destination
   * IPv4 addresses
   ...
   */
    ...
    /**
   * A copy constructor that copy the entire header from the other IPv4Layer
   * (including IPv4 options)
   */
    IPv4Layer(const IPv4Layer& other);

    /**
   * An assignment operator that first delete all data from current layer and
   * then copy the entire header from the other IPv4Layer (including IPv4
   * options)
   */
    IPv4Layer& operator=(const IPv4Layer& other);

    /**
   * Get a pointer to the IPv4 header. Notice this points directly to the data,
   ...
   */
    ...
    /**
   * Get the source IP address in the form of IPAddress. This method is very
   * similar to getSrcIPv4Address(), but adds a level of abstraction because
   ...
   */
    ...
    /**
   * Get the destination IP address in the form of IPAddress. This method is
   * very similar to getDstIPv4Address(), but adds a level of abstraction
   ...
   */
    ...
    /**
   * @return True if this packet is a fragment (in sense of IP fragmentation),
   * false otherwise
   */
    ...
    /**
   * @return True if this packet is a fragment (in sense of IP fragmentation)
   * and is the first fragment (which usually contains the L4 header). Return
   * false otherwise (not a fragment or not the first fragment)
   */
    bool isFirstFragment() const;

    /**
   * @return True if this packet is a fragment (in sense of IP fragmentation)
   * and is the last fragment. Return false otherwise (not a fragment or not the
   * last fragment)
   */
    bool isLastFragment() const;

    /**
   * @return A bitmask containing the fragmentation flags (e.g IP_DONT_FRAGMENT
   * or IP_MORE_FRAGMENTS)
   */
   ...
   * @return An IPv4Option object that contains the first option that matches
   * this type, or logical NULL (IPv4Option#isNull() == true) if no such option
   * found
   */
    IPv4Option getOption(IPv4OptionTypes option) const;

    /**
   * @return The first IPv4 option in the packet. If the current layer contains
   * no options the returned value will contain a logical NULL
   * (IPv4Option#isNull() == true)
   */
    IPv4Option getFirstOption() const;

    /**
   * Get the IPv4 option that comes after a given option. If the given option
   ...
   * @return A IPv4Option object that contains the IPv4 option data that comes
   * next, or logical NULL if the given IPv4 option: (1) was the last one; or
   ...
   * @param[in] optionBuilder An IPv4OptionBuilder object that contains the IPv4
   * option data to be added
   * @return A IPv4Option object that contains the newly added IPv4 option data
   * or logical NULL (IPv4Option#isNull() == true) if addition failed. In case
   ...
   * @param[in] optionBuilder An IPv4OptionBuilder object that contains the
   * requested IPv4 option data to be added
   * @param[in] prevOptionType The IPv4 option which the newly added option
   * should come after. This is an optional parameter which gets a default value
   * of ::IPV4OPT_Unknown if omitted, which means the new option will be added
   * as the first option in the layer
   * @return A IPv4Option object containing the newly added IPv4 option data or
   * logical NULL (IPv4Option#isNull() == true) if addition failed. In case of a
   * failure a corresponding error message will be printed to log
   */
   ...
   * @return True if option was removed successfully or false if option type
   * wasn't found or failed to shorten the layer. If an option appears twice in
   * the layer, its first instance will be removed
   */
    bool removeOption(IPv4OptionTypes option);

    /**
   * Remove all IPv4 options from the layer
   * @return True if options removed successfully or false if some error
   * occurred (an appropriate error message will be printed to log)
   */
   ...
   * - iphdr#protocol = calculated if next layer is known:
   * ::PACKETPP_IPPROTO_TCP for TCP, ::PACKETPP_IPPROTO_UDP for UDP,
   * ::PACKETPP_IPPROTO_ICMP for ICMP
   */
