...
class SingleCommandTextProtocol : public Layer {
  ...
  protected:
    SingleCommandTextProtocol(uint8_t* data, size_t dataLen, Layer* prevLayer,
                              Packet* packet)
        : Layer(data, dataLen, prevLayer, packet){};
    SingleCommandTextProtocol(const std::string& command,
  ...
  public:
    /**
   * Checks if the current message is a multi-line reply. Multi-line messages
   * are indicated with a Hyphen (-) immediately after reply code.
   ...
   */
    ...
    /**
   * A static method that takes a byte array and detects whether it is a single
   * command text based message. All single command text based message
