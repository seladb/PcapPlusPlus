 ...
 * Represents MSTI configuration messages. Each message contains 16 bytes and
...
class StpLayer : public Layer
{
   ...
   */
    ...
    /**
   * Returns the protocol id. Fixed at 0x0 for STP messages which represents
   * IEEE 802.1d
   ...
   * stored
   * @return A newly allocated STP layer of one of the following types
   * (according to the message type): StpConfigurationBPDULayer,
   * StpTopologyChangeBPDULayer, RapidStpLayer, MultipleStpLayer
   */
    static StpLayer* parseStpLayer(uint8_t* data, size_t dataLen,
...
class StpTopologyChangeBPDULayer : public StpLayer
{
   ...
   * stored in
   */
    StpTopologyChangeBPDULayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                               Packet* packet)
        : StpLayer(data, dataLen, prevLayer, packet) {}

    /**
   * Empty c'tor to create a new network topology change (TCN) BPDU layer.
   * Initializes the protocol identifier, version and STP type fields with
   * correct values
   */
   ...
   * A static method that validates the input data
   * @param[in] data The pointer to the beginning of a byte stream of an
   * Spanning Tree Topology Change BPDU packet
   * @param[in] dataLen The length of the byte stream
   * @return True if the data is valid and can represent an Spanning Tree packet
   */
    static bool isDataValid(const uint8_t* data, size_t dataLen)
...
class StpConfigurationBPDULayer : public StpTopologyChangeBPDULayer
{
   ...
   * stored in
   */
    StpConfigurationBPDULayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                              Packet* packet)
   ...
   * A static method that validates the input data
   * @param[in] data The pointer to the beginning of a byte stream of an
   * Spanning Tree Configuration BPDU packet
   * @param[in] dataLen The length of the byte stream
   * @return True if the data is valid and can represent an Spanning Tree packet
   */
    static bool isDataValid(const uint8_t* data, size_t dataLen)
...
class RapidStpLayer : public StpConfigurationBPDULayer
{
   ...
   */
    ...
    /**
   * A static method that validates the input data
   * @param[in] data The pointer to the beginning of a byte stream of an Rapid
   * STP packet
   * @param[in] dataLen The length of the byte stream
   * @return True if the data is valid and can represent an Spanning Tree packet
   */
...
class MultipleStpLayer : public RapidStpLayer
{
   ...
   * stored in
   */
    MultipleStpLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                     Packet* packet)
   ...
   * @param[in] value Configuration name. Length should be less than 32, if
   * longer value provided first 32 characters are used
   */
   ...
   */
    ...
    /**
   * Returns the pointer to configuration message digest. The field itself
   ...
   */
    ...
    /**
   * Sets the pointer to configuration message digest. The field itself always
   * 16 bytes long.
   * @param[in] value Pointer to digest
   * @param[in] len Length of the digest, should be less than 16. If longer
   ...
   */
    ...
    /**
   * Returns a reference to MSTI configuration messages. An MSTP packet can
   * contain between 0 to 64 MSTI messages. The number of messages can be
   * obtained by using getNumberOfMSTIConfMessages()
   * @return An array pointer to MSTI configuration messages. Returns nullptr if
   * there is no MSTI message.
   */
