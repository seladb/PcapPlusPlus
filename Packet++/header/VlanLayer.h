...
class VlanLayer : public Layer
{
   ...
   * @param[in] etherType Protocol EtherType of the next layer. It's an optional
   * parameter, a value of 0 will be set if not provided
   */
    VlanLayer(const uint16_t vlanID, bool cfi, uint8_t priority,
              uint16_t etherType = 0);

    ~VlanLayer() {}

    /**
   * Get a pointer to the VLAN header. Notice this points directly to the data,
   ...
   */
    ...
    /**
   * Get the VLAN ID value. This method differs from vlan_header#vlanID because
   * vlan_header#vlanID is 12 bits long in a 16 bit field. This methods extracts
   * only the 12 bit relevant for the VLAN ID
   ...
   */
    ...
    /**
   * Set VLAN ID. This method differs from setting vlan_header#vlanID because
   * vlan_header#vlanID is 12 bits long in a 16 bit field. This methods sets
   * only the 12 bit relevant for the VLAN ID
   ...
   * ArpLayer, VlanLayer, MplsLayer. Otherwise sets PayloadLayer
   */
    void parseNextLayer();
