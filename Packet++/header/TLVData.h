 ...
 * create or modify TLV records, but rather serves as a wrapper and provides
 * useful methods for retrieving data from them. This class has several abstract
 * methods that should be implemented in derived classes. These methods are for
 * record length value calculation (the 'L' in TLV) which is implemented
...
class TLVRecord
{
  ...
  public:
    /**
   * A c'tor for this class that gets a pointer to the TLV record raw data (byte
   * array)
   * @param[in] recordRawData A pointer to the TLV record raw data
   */
    TLVRecord(uint8_t* recordRawData) { assign(recordRawData); }

    /**
   * A copy c'tor for this class. This copy c'tor doesn't copy the TLV data, but
   * only the pointer to it, which means that after calling it both the old and
   * the new instance will point to the same TLV raw data
   ...
   * @param[in] recordRawData A pointer to the TLV record raw data
   */
    void assign(uint8_t* recordRawData) { m_Data = (TLVRawData*)recordRawData; }
   ...
   */
    ...
    /**
   * Overload of the assignment operator. This operator doesn't copy the TLV
   * data, but rather copies the pointer to it, which means that after calling
   ...
   */
    ...
    /**
   * Overload of the equality operator. Two record are equal if both of them
   * point to the same data, or if they point to different data but their total
   * size is equal and the raw data they both contain is similar.
   ...
   */
    bool operator!=(const TLVRecord& rhs) const { return !operator==(rhs); }
   ...
   */
    ...
    /**
   * A templated method to retrieve the record data as a certain type T. For
   * example, if record data is 4B long (integer) then this method should be
   * used as getValueAs<int>() and it will return the record data as an
   * integer.<BR> Notice this return value is a copy of the data, not a pointer
   * to the actual data
   * @param[in] offset The offset in the record data to start reading the value
   * from. Useful for cases when you want to read some of the data that doesn't
   * start at offset 0. This is an optional parameter and the default value is
   * 0, meaning start reading the value at the beginning of the record data
   ...
   */
    ...
    /**
   * A templated method to copy data of type T into the TLV record data. For
   * example: if record data is 4[Bytes] long use this method with \<int\> to
   * set an integer value into the record data: setValue<int>(num)
   ...
   * setting the record data (default set to 0). For example: if record data is
   * 20 bytes long and you only need to set the 4 last bytes as integer then use
   * this method like this: setValue<int>(num, 16)
   * @return True if value was set successfully or false if the size of T is
   ...
   */
    ...
    /**
   * @return The size of the record value (meaning the size of the 'V' part in
   * TLV)
   */
 ...
 * A class for reading TLV records data out of a byte stream. This class
 * contains helper methods for retrieving and counting TLV records. This is a
...
class TLVRecordReader
{
   ...
   * @return An instance of type TLVRecordType that contains the first TLV
   * record. If tlvDataBasePtr is NULL or tlvDataLen is zero the returned
   * TLVRecordType instance will be logically NULL, meaning
   ...
   */
                                    ...
                                    size_t tlvDataLen) const
    {
        ...
        if (resRec.getRecordBasePtr() + resRec.getTotalSize() >
            tlvDataBasePtr + tlvDataLen)
            resRec.assign(NULL);
   ...
   * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
   * @param[in] tlvDataLen The TLV data byte stream length
   * @return An instance of type TLVRecordType that wraps the record following
   * the record given as input. If the input record.isNull() is true or if the
   * next record is out of bounds of the byte stream, a logical NULL instance of
   * TLVRecordType will be returned, meaning TLVRecordType.isNull() will return
   * true
   */
                                   ...
                                   size_t tlvDataLen) const
    {
        ...
        if (!TLVRecordType::canAssign(record.getRecordBasePtr() +
                                          record.getTotalSize(),
                                      tlvDataBasePtr - record.getRecordBasePtr() +
                                          tlvDataLen - record.getTotalSize()))
            return resRec;
        ...
        // resRec pointer is out-bounds of the TLV records memory
        if (!resRec.isNull() && resRec.getRecordBasePtr() + resRec.getTotalSize() >
                                    tlvDataBasePtr + tlvDataLen)
            resRec.assign(NULL);
    ...
    /**
   * Search for the first TLV record that corresponds to a given record type
   * (the 'T' in __Type__-Length-Value)
   * @param[in] recordType The record type to search for
   * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
   * @param[in] tlvDataLen The TLV data byte stream length
   * @return An instance of type TLVRecordType that contains the result record.
   * If record was not found a logical NULL instance of TLVRecordType will be
   ...
   */
    ...
    /**
   * Get the TLV record count in a given TLV data byte stream. For efficiency
   * purposes the count is being cached so only the first call to this method
   * will go over all the TLV records, while all consequent calls will return
   * the cached number. This implies that if there is a change in the number of
   * records, it's the user's responsibility to call changeTLVRecordCount() with
   * the record count change
   * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
   * @param[in] tlvDataLen The TLV data byte stream length
   ...
   */
    ...
    /**
   * As described in getTLVRecordCount(), the TLV record count is being cached
   * for efficiency purposes. So if the number of TLV records change, it's the
   * user's responsibility to call this method with the number of TLV records
   * being added or removed. If records were added the change should be a
 ...
 * A base class for building Type-Length-Value (TLV) records. This builder
 * receives the record parameters in its c'tor, builds the record raw buffer and
 * provides a method to build a TLVRecord object out of it. Please notice this
 * is a base class that lacks the capability of actually building TLVRecord
 * objects and also cannot be instantiated. The reason for that is that
 * different protocols build TLV records in different ways, so these missing
 * capabilities will be implemented by the derived classes which are specific to
...
class TLVRecordBuilder
{
  protected:
    ...
    TLVRecordBuilder(uint32_t recType, const uint8_t* recValue,
                     uint8_t recValueLen);
