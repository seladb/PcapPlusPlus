...
class PayloadLayer : public Layer {
   ...
   * @param[in] data A raw buffer that will be used as a payload. This data will
   * be copied to the layer
   * @param[in] dataLen The raw buffer length
   * @param[in] dummy A dummy parameter to separate the constructor signature
   * from the other constructor. Its value isn't used anywhere
   ...
   * @param[in] payloadAsHexStream A string that represents an hex stream of the
   * payload. For example: 0001080006040002842b2b774c56c0a80078000000000000c0a8.
   * In order for the hex stream to be valid it has to contain valid hex chars
   * only (which means, for example, that it can't begin with "0x") and it also
   * has to have an even number of chars (each char represents one nibble). If
   * the string is not a valid hex stream an error will be printed to log and
   * the payload layer will be empty (no data)
   */
   ...
   */
    ...
    /**
   * Sets the payload of the PayloadLayer to the given pointer. This will resize
   * (extend/shorten) the underlying packet respectively if there is one.
