...
#pragma pack(pop)
   ...
   * @param[in] len Length of the password array, length of the password should
   * be less than 6 bytes
   */
    WakeOnLanLayer(const pcpp::MacAddress& targetAddr, uint8_t* password,
                   uint8_t len);
   ...
   */
    ...
    /**
   * Get a pointer to the Wake On LAN header. Notice this points directly to the
   ...
   */
    ...
    /**
   * Set the password of the command
   * @param[in] password Password as string. Length of the password should be
   * less than 6 bytes
   * @return True if operation successful, false otherwise
   */
   ...
   */
    ...
    /**
   * A static method that takes a byte array and detects whether it is a Wake on
   * LAN message
   ...
   */
    ...
    /// Parses the next layer. Wake on LAN is the always last so does nothing for
    /// this layer
    void parseNextLayer() {}
