...
class PPPoELayer : public Layer {
   ...
   */
    ...
    /**
   * Get a pointer to the PPPoE header. Notice this points directly to the data,
  ...
  protected:
    ...
    // protected c'tor as this class shouldn't be instantiated
    PPPoELayer(uint8_t version, uint8_t type, PPPoELayer::PPPoECode code,
               uint16_t sessionId, size_t additionalBytesToAllocate = 0);
};
...
class PPPoESessionLayer : public PPPoELayer {
   ...
   */
    ...
    /**
   * A constructor that allocates a new PPPoE Session header with version, type
   * and session ID
   * @param[in] version PPPoE version
   * @param[in] type PPPoE type
   * @param[in] sessionId PPPoE session ID
   * @param[in] pppNextProtocol The next protocol to come after the PPPoE
   * session header. Should be one of the PPP_* macros listed below
   */
    ...
    /**
   * @return The protocol after the PPPoE session header. The return value is
   * one of the PPP_* macros listed below. This method is also used when parsing
   * a packet (this way we know which layer comes after the PPPoE session)
   */
    uint16_t getPPPNextProtocol() const;

    /**
   * Set the field that describes which header comes after the PPPoE session
   * header
   * @param[in] nextProtocol The protocol value. Should be one of the PPP_*
   * macros listed below
   */
...
class PPPoEDiscoveryLayer : public PPPoELayer {
   ...
   */
     ...
     */
        ...
        /**
     * Retrieve the tag data as string. Relevant only if the tag value is indeed
     * a string
   ...
   * A class for building PPPoE Tags. This builder receives the tag parameters
   ...
   */
     ...
     * The PPPoETag object can later be retrieved by calling build()
     ...
     */
        ...
        /**
     * A c'tor for building a PPPoE Tag which has a 4-byte value. The PPPoETag
     ...
     */
        ...
        /**
     * A c'tor for building a PPPoE Tag which has some arbitrary value. The
     * PPPoETag object can later be retrieved by calling build()
     * @param[in] tagType Tag type
     * @param[in] tagValue A byte array that contains the tag data
     * @param[in] tagValueLen The length of the value byte array
     */
        PPPoETagBuilder(PPPoETagTypes tagType, uint8_t* tagValue,
                        uint8_t tagValueLen)
            : TLVRecordBuilder(static_cast<uint16_t>(tagType), tagValue,
                               tagValueLen) {}
   ...
   */
    ...
    /**
   * A constructor that allocates a new PPPoE Discovery header with version,
   * type, PPPoE code and session ID
   * @param[in] version PPPoE version
   * @param[in] type PPPoE type
   ...
   * @return A PPPoETag object that contains the first tag that matches this
   * type, or logical NULL (PPPoETag#isNull() == true) if no such tag found
   */
    PPPoETag getTag(PPPoEDiscoveryLayer::PPPoETagTypes tagType) const;

    /**
   * @return The first tag in the PPPoE discovery layer. If the current layer
   * contains no tags the returned value will contain a logical NULL
   * (PPPoETag#isNull() == true)
   */
    PPPoETag getFirstTag() const;

    /**
   * Get the tag that comes right after the "tag" parameter. If the given tag is
   * the last one, the returned value will contain a logical NULL
   * (PPPoETag#isNull() == true)
   * @param[in] tag A given tag
   * @return A PPPoETag object containing the tag that comes next, or logical
   * NULL if the given tag: (1) was the last one; (2) contains a logical NULL or
   * (3) doesn't belong to this packet
   */
   ...
   * @param[in] tagBuilder A PPPoETagBuilder object that contains the requested
   * tag data to add
   * @return A PPPoETag object containing the newly added PPPoE Tag data or
   * logical NULL (PPPoETag#isNull() == true) if addition failed
   */
   ...
   * @param[in] tagBuilder A PPPoETagBuilder object that contains the requested
   * tag data to add
   * @param[in] prevTagType The PPPoE Tag which the newly added tag will come
   * after
   * @return A PPPoETag object containing the newly added PPPoE Tag data or
   * logical NULL (PPPoETag#isNull() == true) if addition failed
   */
    PPPoETag addTagAfter(const PPPoETagBuilder& tagBuilder,
                         PPPoETagTypes prevTagType);

    /**
   * Remove an existing tag. Tag will be found by the tag type
   * @param[in] tagType The tag type to remove
   * @return True if tag was removed or false if tag wasn't found or if tag
   * removal failed (in each case a proper error will be written to log)
   */
    bool removeTag(PPPoEDiscoveryLayer::PPPoETagTypes tagType);

    /**
   * Remove all tags in this layer
   * @return True if all tags were successfully or false if removal failed for
   * some reason (a proper error will be written to log)
   */
    bool removeAllTags();
   ...
   */
    ...
    /**
   * @return The header length which is size of strcut pppoe_header plus the
   * total size of tags
   */
    virtual size_t getHeaderLen() const;
