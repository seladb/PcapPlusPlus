...
#pragma pack(pop)
   ...
   * @param[in] bottomOfStack Bottom-of-stack value which indicate whether the
   * next layer will also be a MPLS label or not
   */
    MplsLayer(uint32_t mplsLabel, uint8_t ttl, uint8_t experimentalUseValue,
              bool bottomOfStack);
   ...
   * @param[in] val The exp value to set. val must be a valid number meaning
   * between 0 and 7 (inclusive)
   * @return True if exp value was set successfully or false if val has invalid
   * value
   */
   ...
   * @param[in] label The label to set. label must be a valid number meaning
   * between 0 and 0xFFFFF (inclusive)
   * @return True if label was set successfully or false if label has invalid
   * value
   */
   ...
   * MplsLayer. Otherwise sets PayloadLayer
   */
    void parseNextLayer();
   ...
   */
    ...
    /**
   * Set/unset the bottom-of-stack bit according to next layer: if it's a MPLS
   * layer then bottom-of-stack will be unset. If it's not a MPLS layer this bit
   * will be set
   */
