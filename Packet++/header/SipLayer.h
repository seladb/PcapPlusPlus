...
class SipLayer : public TextBasedProtocolMessage {
   ...
   * header field called "Content-Length". This method parses this field,
   * extracts its value and return it. If this field doesn't exist 0 is returned
   * @return SIP response body length determined by "Content-Length" field
   */
    int getContentLength() const;

    /**
   * The length of the body of many SIP messages is determined by a header field
   * called "Content-Length". This method sets The content-length field value.
   * The method supports several cases:
   * - If the "Content-Length" field exists - the method will only replace the
   * existing value with the new value
   * - If the "Content-Length" field doesn't exist - the method will create this
   * field and put the value in it. Here are also 2 cases:
   *   - If prevFieldName is specified - the new "Content-Length" field will be
   * created after it
   *   - If prevFieldName isn't specified or doesn't exist - the new
   * "Content-Length" field will be created as the last field before
   *     end-of-header field
   *
   * @param[in] contentLength The content length value to set
   * @param[in] prevFieldName Optional parameter, if specified and
   * "Content-Length" field doesn't exist, it will be created after this field
   * @return A pointer to the "Content-Length" field, or NULL if creation failed
   */
    HeaderField* setContentLength(int contentLength,
    ...
    /**
   * Currently identifies only SDP if content-length field exists and set to a
   * value greater than zero. If content-length field doesn't exist or set to
   * zero and still there is data after this layer, a PayloadLayer will be
   * created
   */
    void parseNextLayer();

    /**
   * Set the content-length only if a content-length field already exists and if
   * its current value is different than the total length of the next layer(s)
   */
 ...
 * Represents a SIP request header and inherits all basic functionality of
 * SipLayer and TextBasedProtocolMessage. The functionality that is added for
 * this class is the SIP first line concept. A SIP request has the following
 * first line: <i>INVITE sip:bla@bla.com:12345 SIP/2.0</i> Since it's not an
 * "ordinary" header field, it requires a special treatment and gets a class of
 * it's own: SipRequestFirstLine. In most cases a SIP request will be contained
 * in a single packet but for cases it is not, only the first packet will be
...
class SipRequestLayer : public SipLayer {
   ...
   * stored in
   */
    SipRequestLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                    Packet* packet);

    /**
   * A constructor that allocates a new SIP request with only the first line
   * filled. The request will be created without further fields. The user can
   * then add fields using addField() or insertField() methods
   * @param[in] method The SIP method to be used in this SIP request
   * @param[in] requestUri The URI of the request
   * @param[in] version SIP version to be used in this request. Default is
   * "SIP/2.0"
   */
    SipRequestLayer(SipMethod method, const std::string& requestUri,
                    const std::string& version = "SIP/2.0");

    ~SipRequestLayer();

    /**
   * A copy constructor for this layer. Inherits base copy constructor SipLayer
   * and adds the functionality of copying the first line
   * @param[in] other The instance to copy from
   */
    SipRequestLayer(const SipRequestLayer& other);

    /**
   * An assignment operator overload for this layer. This method inherits base
   * assignment operator SipLayer#operator=() and adds the functionality of
 ...
 * Represents an SIP response message and inherits all basic functionality of
 * SipLayer and TextBasedProtocolMessage. The functionality that is added for
 * this class is the SIP first line concept. A SIP response has the following
 * first line: <i>200 OK SIP/2.0</i> Since it's not an "ordinary" header field,
 * it requires a special treatment and gets a class of it's own:
 * SipResponseFirstLine. In most cases a SIP response will be contained in a
 * single packet but for cases it is not, only the first packet will be
...
class SipResponseLayer : public SipLayer {
   ...
   */
    enum SipResponseStatusCode {
       ...
       proxy must send a 100 Trying response */
        ...
        Sip180Ringing,
        /** Servers can optionally send this response to indicate a call is being
       forwarded */
        Sip181CallisBeingForwarded,
        /** Indicates that the destination was temporarily unavailable, so the
       server has queued the call until the destination is available. A server
       may send multiple 182 responses to update progress of the queue */
        Sip182Queued,
       ...
       (including the User Agent Client (UAC)) that an early dialog has been
       terminated */
       ...
       will not be received */
        ...
        /** The address resolved to one of several options for the user or client to
       choose between, which are listed in the message body or the message's
       Contact fields */
        Sip300MultipleChoices,
        /** The original Request-URI is no longer valid, the new address is given in
       the Contact header field, and the client should update any records of the
       original Request-URI with the new value */
     ...
     */
        ...
        /** The Contact field details a proxy that must be used to access the
       ...
       longer than the server is willing to interpret */
        ...
        /** There was a resource-priority option tag, but no Resource-Priority
       header */
     ...
     */
        ...
        /** The received request contains a Session-Expires header field with a
       duration below the minimum timer */
     ...
     */
        ...
        /** The server rejected a non-interactive emergency call, indicating that
       the request was malformed enough that no reasonable emergency response to
       the alert can be determined */
        Sip425BadAlertMessage,
        /** The server policy requires an Identity header, and one has not been
       provided */
        Sip428UseIdentityHeader,
        /** The server did not receive a valid Referred-By token on the request */
        Sip429ProvideReferrerIdentity,
        /** A specific flow to a user agent has failed, although other flows may
       succeed. This response is intended for use between proxy devices, and
       should not be seen by an endpoint (and if it is seen by one, should be
       treated as a 400 Bad Request response) */
        Sip430FlowFailed,
        /** The request has been rejected because it was anonymous */
        Sip433AnonymityDisallowed,
        /** The request has an Identity-Info header, and the URI scheme in that
       header cannot be dereferenced */
        Sip436BadIdentityInfo,
        /** The server was unable to validate a certificate for the domain that
       signed the request */
        Sip437UnsupportedCertificate,
        /** The server obtained a valid certificate that the request claimed was
       used to sign the request, but was unable to verify that signature */
        Sip438InvalidIdentityHeader,
        /** The first outbound proxy the user is attempting to register through does
       not support the "outbound" feature of RFC 5626, although the registrar
       does */
        Sip439FirstHopLacksOutboundSupport,
        /** If a SIP proxy determines a response context has insufficient Incoming
       Max-Breadth to carry out a desired parallel fork, and the proxy is
       unwilling/unable to compensate by forking serially or sending a redirect,
       ...
       can infer that its request did not reach all possible destinations */
        ...
        /** If a SIP UA receives an INFO request associated with an Info Package
       that the UA has not indicated willingness to receive, the UA MUST send a
     ...
     */
        ...
        /** Some aspect of the session description or the Request-URI is not
       acceptable */
        Sip488NotAcceptableHere,
        /** The server did not understand an event package specified in an Event
       header field */
        Sip489BadEvent,
     ...
     */
        ...
        /** The server has received a request that requires a negotiated security
       mechanism, and the response contains a list of suitable security
       mechanisms for the requester to choose between, or a digest
       authentication challenge */
        Sip494SecurityAgreementRequired,
        /** The server could not fulfill the request due to some unexpected
       condition */
        Sip500ServerInternalError,
        /** The server does not have the ability to fulfill the request, such as
       because it does not recognize the request method. (Compare with 405
       Method Not Allowed, where the server recognizes the method but does not
       allow or support it.) */
        Sip501NotImplemented,
        /** The server is acting as a gateway or proxy, and received an invalid
       response from a downstream server while attempting to fulfill the request
     */
        Sip502BadGateway,
        /** The server is undergoing maintenance or is temporarily overloaded and so
       cannot process the request. A "Retry-After" header field may specify when
       the client may reattempt its request */
        Sip503ServiceUnavailable,
        /** The server attempted to access another server in attempting to process
       the request, and did not receive a prompt response */
        Sip504ServerTimeout,
     ...
     */
       ...
       a 'pn-provider' SIP URI parameter */
        ...
        /** The server is unable or unwilling to meet some constraints specified in
       the offer */
        Sip580PreconditionFailure,
        /** All possible destinations are busy. Unlike the 486 response, this
       response indicates the destination knows there are no alternative
       destinations (such as a voicemail server) able to accept the call */
        Sip600BusyEverywhere,
        /** The destination does not wish to participate in the call, or cannot do
       so, and additionally the destination knows there are no alternative
       destinations (such as a voicemail server) willing to accept the call */
        Sip603Decline,
        /** The server has authoritative information that the requested user does
       not exist anywhere */
       ...
       style were not acceptable */
        Sip606NotAcceptable,
        /** The called party did not want this call from the calling party. Future
       attempts from the calling party are likely to be similarly rejected */
        Sip607Unwanted,
   ...
   * stored in
   */
    SipResponseLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                     Packet* packet);

    /**
   * A constructor that allocates a new SIP response with only the first line
   * filled. The request will be created without further fields. The user can
   * then add fields using addField() or insertField() methods
   * @param[in] statusCode SIP status code to set
   * @param[in] statusCodeString Most status codes have their default string,
   * e.g 200 is usually "OK" etc. But the user can set a non-default status code
   * string and it will be written in the header first line. Empty string ("")
   * means using the default status code string. Also, the default is using the
   * default status code string
   * @param[in] sipVersion SIP version to set, default is SIP/2.0
   *
   */
    explicit SipResponseLayer(SipResponseLayer::SipResponseStatusCode statusCode,
    ...
    /**
   * A copy constructor for this layer. This copy constructor inherits base copy
   * constructor SipLayer and adds the functionality of copying the first line
   * as well
   * @param[in] other The instance to copy from
   */
   ...
   * An assignment operator overload for this layer. This method inherits base
   * assignment operator SipLayer#operator=() and adds the functionality of
   * copying the first line as well
   * @param[in] other The instance to copy from
   */
...
/**
 * @class SipRequestFirstLine
 * Represents an SIP request first line. The first line includes 3 parameters:
 * SIP method (e.g INVITE, ACK, BYE, etc.), URI (e.g sip:bla@bla.com:12345) and
 * SIP version (usually SIP/2.0). All these parameters are included in this
 * class, and the user can retrieve or set them. This class cannot be
 * instantiated by users, it's created inside SipRequestLayer and user can get a
 * pointer to an instance of it. All "getters" of this class retrieve the actual
 * data of the SIP request and the "setters" actually change the packet data.
 * Since SIP is a textual protocol, most fields aren't of fixed size and this
 * also applies to the first line parameters. So many "setter" methods of this
...
class SipRequestFirstLine {
   ...
   * @return False if newMethod is SipRequestLayer#SipMethodUnknown or if
   * shortening/extending the SipRequestLayer data failed. True otherwise
   */
    bool setMethod(SipRequestLayer::SipMethod newMethod);

    /**
   * @return A copied version of the URI (notice changing the return value won't
   * change the actual data of the packet)
   */
   ...
   * @return False if shortening/extending the SipRequestLayer data failed. True
   * otherwise
   */
   ...
   */
    static SipRequestLayer::SipMethod parseMethod(const char* data,
                                                  size_t dataLen);
   ...
   */
    ...
    /**
   * As explained in SipRequestLayer, a SIP message can sometimes spread over
   * more than 1 packet, so when looking at a single packet the header can be
   * partial. Same goes for the first line - it can spread over more than 1
   ...
   * This exception can be thrown while constructing SipRequestFirstLine (the
   * constructor is private, so the construction happens only in
   * SipRequestLayer). This kind of exception is thrown if trying to construct
  ...
  private:
    ...
    SipRequestFirstLine(SipRequestLayer* sipRequest,
                        SipRequestLayer::SipMethod method,
...
/**
 * @class SipResponseFirstLine
 * Represents an SIP response message first line. The first line includes 2
 * parameters: status code (e.g 100 Trying ,200 OK, etc.), and SIP version
 * (usually SIP/2.0). These 2 parameters are included in this class, and the
 * user can retrieve or set them. This class cannot be instantiated by users,
 * it's created inside SipResponseLayer and user can get a pointer to an
 * instance of it. The "getter" methods of this class will retrieve the actual
 * data of the SIP response and the "setter" methods will change the packet
 * data. Since SIP is a textual protocol, most fields aren't of fixed size and
 * this also applies to the first line parameters. So most "setter" methods of
...
class SipResponseFirstLine {
   ...
   * @param[in] statusCodeString An optional parameter: set a non-default status
   * code message (e.g "Bla Bla" instead of "Not Found"). If this parameter
   * isn't supplied or supplied as empty string (""), the default message for
   * the status code will be set
   */
    bool setStatusCode(SipResponseLayer::SipResponseStatusCode newStatusCode,
                       std::string statusCodeString = "");
   ...
   */
    ...
    /**
   * Set the SIP version. The version to set is expected to be in the format of
   * SIP/x.y otherwise an error will be written to log
   ...
   */
    static SipResponseLayer::SipResponseStatusCode
    parseStatusCode(const char* data, size_t dataLen);

    /**
   * A static method for parsing the SIP version out of raw data
   * @param[in] data The raw data
   * @param[in] dataLen The raw data length
   * @return The parsed SIP version string or an empty string if version cannot
   * be extracted
   */
   ...
   */
    ...
    /**
   * As explained in SipResponseLayer, A SIP message can sometimes spread over
   * more than 1 packet, so when looking at a single packet the header can be
   * partial. Same goes for the first line - it can spread over more than 1
   * packet. This method returns an indication whether the first line is partial
   * @return False if the first line is partial, true if it's complete
   ...
   * This exception can be thrown while constructing SipResponseFirstLine (the
   * constructor is private, so the construction happens only in
   * SipResponseLayer). This kind of exception will be thrown if trying to
