...
enum DhcpV6OptionType
{
    ...
    /** One or more IPv6 addresses associated with PANA (Protocol for carrying
    ...
    /** List of IPv6 addresses for servers providing particular types of IEEE
     802.21 Mobility Service (MoS) */
    DHCPV6_OPT_IPH6_ADDRESS_MOS = 54,
    /** List of FQDNs for servers providing particular types of IEEE 802.21
     Mobility Service (MoS) */
...
/**
 * @class DhcpV6Option
 * A wrapper class for DHCPv6 options. This class does not create or modify DHCP
 ...
 * A class for building DHCPv6 options. This builder receives the option
...
class DhcpV6OptionBuilder : public TLVRecordBuilder
{
   ...
   * stream of the raw byte value. The DhcpV6Option object can later be
   * retrieved by calling build()
   * @param[in] optionType DHCPv6 option type
   * @param[in] optionValueAsHexStream The value as a hex stream string
   */
    DhcpV6OptionBuilder(DhcpV6OptionType optionType,
                        const std::string& optionValueAsHexStream)
        : TLVRecordBuilder(static_cast<uint16_t>(optionType),
                           optionValueAsHexStream, true) {}

    /**
   * A c'tor for building DHCPv6 options from a byte array representing their
   * value. The DhcpV6Option object can be later retrieved by calling build()
   * @param[in] optionType DHCPv6 option type
   ...
   */
    DhcpV6OptionBuilder(DhcpV6OptionType optionType, const uint8_t* optionValue,
                        uint8_t optionValueLen)
        : TLVRecordBuilder(static_cast<uint16_t>(optionType), optionValue,
                           optionValueLen) {}
...
class DhcpV6Layer : public Layer
{
   ...
   * @param[in] transactionId The transaction ID to be set. Notice the
   * transaction ID is 3-byte long so the value shouldn't exceed 0xFFFFFF
   */
   ...
   */
    ...
    /**
   * @return The first DHCPv6 option in the packet. If there are no DHCPv6
   * options the returned value will contain a logical NULL
   * (DhcpV6Option#isNull() == true)
   */
    DhcpV6Option getFirstOptionData() const;

    /**
   * Get the DHCPv6 option that comes after a given option. If the given option
   * was the last one, the returned value will contain a logical NULL
   * (DhcpV6Option#isNull() == true)
   * @param[in] dhcpv6Option A given DHCPv6 option
   * @return A DhcpV6Option object containing the option data that comes next,
   * or logical NULL if the given DHCPv6 option: (1) was the last one; (2)
   ...
   * @return A DhcpV6OptionType object containing the first DHCP option data
   * that matches this type, or logical NULL (DhcpV6Option#isNull() == true) if
   ...
   * @param[in] optionBuilder A DhcpV6OptionBuilder object that contains the
   * requested DHCPv6 option data to add
   * @return A DhcpV6Option object containing the newly added DHCP option data
   * or logical NULL (DhcpV6Option#isNull() == true) if addition failed
   */
   ...
   * @param[in] optionBuilder A DhcpV6OptionBuilder object that contains the
   * requested DHCPv6 option data to add
   * @param[in] optionType The DHCPv6 option type which the newly added option
   * will come after
   * @return A DhcpV6Option object containing the newly added DHCPv6 option data
   * or logical NULL (DhcpV6Option#isNull() == true) if addition failed
   */
    DhcpV6Option addOptionAfter(const DhcpV6OptionBuilder& optionBuilder,
   ...
   * or logical NULL (DhcpV6Option#isNull() == true) if addition failed
   */
    DhcpV6Option addOptionBefore(const DhcpV6OptionBuilder& optionBuilder,
                                 DhcpV6OptionType optionType);

    /**
   * Remove an existing DHCPv6 option from the layer
   * @param[in] optionType The DHCPv6 option type to remove
   * @return True if DHCPv6 option was successfully removed or false if type
   ...
   * @return True if all DHCPv6 options were successfully removed or false if
   ...
   * @param[in] data The pointer to the beginning of a byte stream of an DHCPv6
   * layer
  ...
  private:
    ...
    DhcpV6Option addOptionAt(const DhcpV6OptionBuilder& optionBuilder,
                             int offset);
