...
struct gtpv1_header {
   ...
   */
    ...
    /** A 16-bit field that indicates the length of the payload in bytes (rest of
   * the packet following the mandatory 8-byte GTP header). Includes the
   * optional fields */
...
class GtpV1Layer : public Layer {
  private:
    ...
    void init(GtpV1MessageType messageType, uint32_t teid, bool setSeqNum,
   ...
   */
    class GtpExtension {
      ...
      private:
        ...
        static GtpExtension createGtpExtension(uint8_t* data, size_t dataLen,
                                               uint8_t extType, uint16_t content);

      public:
        /**
     * An empty c'tor that creates an empty object, meaning one that isNull()
     * returns "true")
     */
        GtpExtension();
     ...
     */
        ...
        /**
     * @return Instances of this class may be initialized as empty, meaning they
     * don't contain any data. In these cases this method returns true
     */
        bool isNull() const;

        /**
     * @return The extension type. If the object is empty a value of zero is
     * returned
     */
        uint8_t getExtensionType() const;

        /**
     * @return The total length of the extension including the length and next
     * extension type fields. If the object is empty a value of zero is returned
     */
        size_t getTotalLength() const;

        /**
     * @return The length of the extension's content, excluding the extension
     * length and next extension type fields. If the object is empty a value of
     * zero is returned
     */
        size_t getContentLength() const;

        /**
     * @return A byte array that includes the extension's content. The length of
     * this array can be determined by getContentLength(). If the object is
     * empty a null value is returned
     */
        uint8_t* getContent() const;

        /**
     * @return The extension type of the next header. If there are no more
     * header extensions or if this object is empty a value of zero is returned
     */
        uint8_t getNextExtensionHeaderType() const;

        /**
     * @return An instance of this class representing the next extension header,
     * if exists in the message. If there are no more header extensions or if
     * this object is empty an empty instance of GtpExtension is returned,
   ...
   */
    ...
    /**
   * A constructor that creates a new GTPv1 layer and sets the message type and
   * the TEID value
   * @param[in] messageType The GTPv1 message type to be set in the newly
   * created layer
   ...
   * created layer
   * @param[in] teid The TEID value to be set in the newly created layer
   * @param[in] setSeqNum A flag indicating whether to set a sequence number. If
   * set to "false" then the parameter "seqNum" will be ignored
   * @param[in] seqNum The sequence number to be set in the newly created later.
   * If "setSeqNum" is set to false this parameter will be ignored
   * @param[in] setNpduNum A flag indicating whether to set the N-PDU number. If
   * set to "false" then the parameter "npduNum" will be ignored
   * @param[in] npduNum The N-PDU number to be set in the newly created later.
   * If "setNpduNum" is set to false this parameter will be ignored
   */
    GtpV1Layer(GtpV1MessageType messageType, uint32_t teid, bool setSeqNum,
               uint16_t seqNum, bool setNpduNum, uint8_t npduNum);

    /**
   * A static method that takes a byte array and detects whether it is a GTP v1
   * message
   ...
   */
    ...
    /**
   * @return The GTP v1 common header structure. Notice this points directly to
   * the data, so every change will change the actual packet data
   */
    gtpv1_header* getHeader() const { return (gtpv1_header*)m_Data; }

    /**
   * Get the sequence number if exists on the message (sequence number is an
   * optional field in GTP messages)
   * @param[out] seqNumber Set with the sequence number value if exists in the
   * layer. Otherwise remains unchanged
   * @return True if the sequence number field exists in layer, in which case
   * seqNumber is set with the value. Or false otherwise
   */
   ...
   * @return True if the value was set successfully, false otherwise. In case of
   * failure a corresponding error message will be written to log
   */
    bool setSequenceNumber(const uint16_t seqNumber);

    /**
   * Get the N-PDU number if exists on the message (N-PDU number is an optional
   * field in GTP messages)
   * @param[out] npduNum Set with the N-PDU number value if exists in the layer.
   * Otherwise remains unchanged
   * @return True if the N-PDU number field exists in layer, in which case
   * npduNum is set with the value. Or false otherwise
   */
   ...
   */
    ...
    /**
   * Get the type of the next header extension if exists on the message
   * (extensions are optional in GTP messages)
   * @param[out] nextExtType Set with the next header extension type if exists
   * in layer. Otherwise remains unchanged
   * @return True if the message contains header extensions, in which case
   * nextExtType is set to the next header extension type. If there are no
   * header extensions false is returned and nextExtType remains unchanged
   */
    bool getNextExtensionHeaderType(uint8_t& nextExtType) const;

    /**
   * @return An object that represents the next extension header, if exists in
   * the message. If there are no extensions an empty object is returned,
   * meaning an object which GtpExtension#isNull() returns "true"
   */
    GtpExtension getNextExtension() const;

    /**
   * Add a GTPv1 header extension. It is assumed that the extension is 4 bytes
   * in length and its content is 2 bytes in length. If you need a different
   * content size please reach out to me. This method takes care of extending
   * the layer to make room for the new extension and also sets the relevant
   * flags and fields
   * @param[in] extensionType The type of the new extension
   * @param[in] extensionContent A 2-byte long content
   * @return An object representing the newly added extension. If there was an
   * error adding the extension a null object will be returned (meaning
   * GtpExtension#isNull() will return "true") and a corresponding error message
   ...
   */
    ...
    /**
   * Identifies the following next layers for GTP-U packets: IPv4Layer,
   * IPv6Layer. Otherwise sets PayloadLayer
   */
    void parseNextLayer();

    /**
   * @return The size of the GTP header. For GTP-C packets the size is
   * determined by the value of gtpv1_header#messageLength and for GTP-U the
   * size only includes the GTP header itself (meaning the size of gtpv1_header
   * plus the size of the optional fields such as sequence number, N-PDU or
   * extensions if exist)
   */
