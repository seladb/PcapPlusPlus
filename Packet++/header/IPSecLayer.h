...
class AuthenticationHeaderLayer : public Layer {
   ...
   */
    ...
    /**
   * Get a pointer to the raw AH header. Notice this points directly to the
   ...
   * @param[in] data The pointer to the beginning of a byte stream of a
   * AuthenticationHeader layer
   * @param[in] dataLen The length of byte stream
   * @return True if the data is valid and can represent an AuthenticationHeader
   * layer
   */
   ...
   */
    ...
    /**
   * Currently identifies the following next layers: UdpLayer, TcpLayer,
   * IPv4Layer, IPv6Layer and ESPLayer. Otherwise sets PayloadLayer
   */
    void parseNextLayer();
...
class ESPLayer : public Layer {
   ...
   */
    ...
    /**
   * A static method that validates the input data
   * @param[in] data The pointer to the beginning of a byte stream of a ESP
   * layer
   * @param[in] dataLen The length of byte stream
   ...
   */
    ...
    /**
   * The payload of an ESP layer is encrypted, hence the next layer is always a
   * generic payload (PayloadLayer)
   */
    void parseNextLayer();
