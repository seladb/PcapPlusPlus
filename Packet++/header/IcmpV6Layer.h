...
struct icmpv6hdr
{
  ...
  (high-order bit is 1) indicate an information message. */
    uint8_t type;
    /** The code field value depends on the message type and provides an
   * additional level of message granularity */
    uint8_t code;
    /** The checksum field provides a minimal level of integrity verification for
   * the ICMP message */
    uint16_t checksum;
};
#pragma pack(pop)
...
class IcmpV6Layer : public Layer
{
   ...
   * @param packet A pointer to the Packet instance where layer will be stored
   * in
   */
   ...
   */
    IcmpV6Layer(ICMPv6MessageType msgType, uint8_t code, const uint8_t* data,
                size_t dataLen);
   ...
   */
    static Layer* parseIcmpV6Layer(uint8_t* data, size_t dataLen,
...
class ICMPv6EchoLayer : public IcmpV6Layer
{
   ...
   * stored in
   */
    ICMPv6EchoLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                    Packet* packet)
   ...
   */
    ICMPv6EchoLayer(ICMPv6EchoType echoType, uint16_t id, uint16_t sequence,
