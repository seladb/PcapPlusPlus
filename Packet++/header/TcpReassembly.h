...
/// segments (or packets).<BR> This logic can be useful in analyzing messages for a large number of protocols
/// implemented on top of TCP including HTTP, SSL/TLS, FTP and many many more.
///
...
/// - The pcpp#TcpReassembly instance manages all TCP connections from the packets it's being fed. For each connection
///   it manages its 2 sides (A->B and B->A).
...
/// - If the packet data matches these criteria a callback is being invoked. This callback is supplied by the user in
///   the creation of the pcpp#TcpReassembly instance. This callback contains the new data (of course), but also
///   information about the connection (5-tuple, 4-byte hash key describing the connection, etc.) and also a pointer to
///   a "user cookie", meaning a pointer to a structure provided by the user during the creation of the
...
/// - pcpp#TcpReassembly supports 2 more callbacks - one is invoked when a new TCP connection is first seen and the
///   other when it's ended (either by a FIN/RST packet or manually by the user). Both of these callbacks contain data
...
///   instance. The end connection callback also provides the reason for closing it ("naturally" or manually).
///
...
/// - pcpp#TcpReassembly#OnTcpMessageReady callback - Invoked when new data arrives on a certain connection. Contains
/// the new data as well as connection data (5-tuple, flow key).
...
/// - pcpp#TcpReassembly#reassemblePacket() should detect the packets that arrive after the FIN packet has been
/// received.
...
/// - pcpp#TcpReassemblyConfiguration#maxOutOfOrderFragments - the maximum number of unmatched fragments to keep per
///   flow before missed fragments are considered lost. A value of 0 means unlimited.
...
namespace pcpp
{
	...
	struct TcpReassemblyConfiguration
	{
		...
		/// @param[in] maxNumToClean The maximum number of items to be cleaned up per one call of
		/// purgeClosedConnections. If it's set to 0 the default value will be used. The default is 30.
	...
	public:
		...
		{
			/// The processed packet contains valid TCP payload, and its payload is processed by
			/// `OnMessageReadyCallback` callback function. The packet may be:
		...
		/// packet opens a new connection, ends a connection or contains new data on an existing connection, the
		/// relevant callback will be invoked (TcpReassembly#OnTcpMessageReady, TcpReassembly#OnTcpConnectionStart,
		/// TcpReassembly#OnTcpConnectionEnd)
		...
		/// Get a map of all connections managed by this TcpReassembly instance (both connections that are open and
		/// those that are already closed)
