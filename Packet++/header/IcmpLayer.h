...
enum IcmpDestUnreachableCodes {
    ...
    /** Protocol unreachable error (the designated transport protocol is not
     supported) */
    IcmpProtocolUnreachable = 2,
    /** Port unreachable error (the designated protocol is unable to inform the
     host of the incoming message) */
    IcmpPortUnreachable = 3,
    /** The datagram is too big. Packet fragmentation is required but the 'don't
     fragment' (DF) flag is on */
    ...
    /** Communication administratively prohibited (administrative filtering
   * prevents packet from being forwarded)
   */
    IcmpCommunicationProhibited = 13,
    /** Host precedence violation (indicates the requested precedence is not
   * permitted for the combination of host or network and port)
   */
    IcmpHostPrecedenceViolation = 14,
    /** Precedence cutoff in effect (precedence of datagram is below the level set
...
typedef struct icmp_echo_request {
   ...
   * length */
    size_t dataLength;
    /** most echo requests/replies contain some payload data. This is a pointer to
   * this data */
    uint8_t* data;
} icmp_echo_request;
...
typedef struct icmp_timestamp_request : icmphdr {
    ...
    uint16_t sequence;
    /** the time (in milliseconds since midnight) the sender last touched the
   * packet */
    uint32_t originateTimestamp;
    /** relevant for timestamp reply only - the time the echoer first touched it
   * on receipt */
    uint32_t receiveTimestamp;
    /** relevant for timestamp reply only - the time the echoer last touched the
 ...
 * Router address structure, relevant for ICMP router advertisement message type
...
struct icmp_router_address_structure {
   ...
   * relative to other router addresses on the same subnet. This is a
   * twos-complement value where higher values indicate that the route is more
   * preferable */
   ...
   */
    ...
    /**
   * @return The IPv4 address extracted from
   * icmp_router_address_structure#routerAddress field
   */
...
typedef struct icmp_router_advertisement_hdr : icmphdr {
   ...
   * contains one router address/preference level pair */
    ...
    /** the number of 32-bit words of information for each router address entry in
   * the list. The value is normally set to 2 (router address + preference
   * level) */
    uint8_t addressEntrySize;
    /** the maximum number of seconds that the router addresses in this list may
   * be considered valid */
    uint16_t lifetime;
...
struct icmp_router_advertisement {
   ...
   * @return A pointer to the router advertisement on the packet or null if
   * index is out of range (less than zero or greater than the number of router
   * advertisement records on this message, determined by advertisementCount
   * field)
   */
...
class IcmpLayer : public Layer {
  private:
    ...
    bool setEchoData(IcmpMessageType echoType, uint16_t id, uint16_t sequence,
                     uint64_t timestamp, const uint8_t* data, size_t dataLen);
   ...
   */
    ...
    /**
   * An empty constructor that creates a new layer with an empty ICMP header
   * without setting the ICMP type or ICMP data. Call the set*Data() methods to
   * set ICMP type and data
   */
    IcmpLayer();
    ...
    /**
   * Get a pointer to the basic ICMP header. Notice this points directly to the
   ...
   */
    ...
    /**
   * @return ICMP echo (ping) request data. If the layer isn't of type ICMP echo
   ...
   * @return A pointer to the echo (ping) request data that have been set or
   * NULL if something went wrong (an appropriate error log is printed in such
   * cases)
   */
    icmp_echo_request* setEchoRequestData(uint16_t id, uint16_t sequence,
                                          uint64_t timestamp, const uint8_t* data,
                                          size_t dataLen);

    /**
   * @return ICMP echo reply data. If the layer isn't of type ICMP echo reply
   * NULL is returned
   */
   ...
   * @return A pointer to the echo (ping) reply data that have been set or NULL
   * if something went wrong (an appropriate error log is printed in such cases)
   */
    icmp_echo_reply* setEchoReplyData(uint16_t id, uint16_t sequence,
                                      uint64_t timestamp, const uint8_t* data,
                                      size_t dataLen);

    /**
   * @return ICMP timestamp request data. If the layer isn't of type ICMP
   * timestamp request NULL is returned
   */
   ...
   * @param[in] originateTimestamp Time (in milliseconds since midnight) the
   * sender last touched the packet
   * @return A pointer to the timestamp request data that have been set or NULL
   * if something went wrong (an appropriate error log is printed in such cases)
   */
    icmp_timestamp_request* setTimestampRequestData(uint16_t id,
                                                    uint16_t sequence,
                                                    timeval originateTimestamp);

    /**
   * @return ICMP timestamp reply data. If the layer isn't of type ICMP
   * timestamp reply NULL is returned
   */
   ...
   * @param[in] transmitTimestamp The time the echoer last touched the message
   * on sending it
   * @return A pointer to the timestamp reply data that have been set or NULL if
   * something went wrong (an appropriate error log is printed in such cases)
   */
    ...
    /**
   * @return ICMP destination unreachable data. If the layer isn't of type ICMP
   * destination unreachable NULL is returned. The IP and L4 (ICMP/TCP/UDP)
   * headers of the destination unreachable data are parsed as separate layers
   * and can be retrieved via this->getNextLayer()
   */
    icmp_destination_unreachable* getDestUnreachableData();

    /**
   * Set destination unreachable message data. This method only works if
   * IcmpLayer is already part of a packet (not a standalone layer). The reason
   * is the Internet and L4 headers given as parameters are added as separate
   * layers and need a packet to be added to
   * @param[in] code Destination unreachable code
   * @param[in] nextHopMTU The MTU of the next-hop network if a code 4 error
   * occurs
   * @param[in] ipHeader The Internet header of the original data. This layer is
   * added as a separate layer on the packet
   * @param[in] l4Header The L4 header of the original data. This layer is added
   * as a separate layer on the packet
   * @return A pointer to the destination unreachable data that have been set or
   * NULL if something went wrong (an appropriate error log is printed in such
   * cases)
   */
    icmp_destination_unreachable*
    setDestUnreachableData(IcmpDestUnreachableCodes code, uint16_t nextHopMTU,
                           IPv4Layer* ipHeader, Layer* l4Header);

    /**
   * @return ICMP source quench data. If the layer isn't of type ICMP source
   * quench NULL is returned. The IP and L4 (ICMP/TCP/UDP) headers of the source
   * quench data are parsed as separate layers and can be retrieved via
   * this->getNextLayer()
   */
    icmp_source_quench* getSourceQuenchdata();

    /**
   * Set source quench message data. This method only works if IcmpLayer is
   * already part of a packet (not a standalone layer). The reason is the
   * Internet and L4 headers given as parameters are added as separate layers
   * and need a packet to be added to
   * @param[in] ipHeader The Internet header of the original data. This layer is
   * added as a separate layer on the packet
   * @param[in] l4Header The L4 header of the original data. This layer is added
   * as a separate layer on the packet
   * @return A pointer to the source quench data that have been set or NULL if
   * something went wrong (an appropriate error log is printed in such cases)
   */
    icmp_source_quench* setSourceQuenchdata(IPv4Layer* ipHeader, Layer* l4Header);

    /**
   * @return ICMP redirect data. If the layer isn't of type ICMP redirect NULL
   * is returned. The IP and L4 (ICMP/TCP/UDP) headers of the redirect data are
   * parsed as separate layers and can be retrieved via this->getNextLayer()
   */
    icmp_redirect* getRedirectData();

    /**
   * Set redirect message data. This method only works if IcmpLayer is already
   * part of a packet (not a standalone layer). The reason is the Internet and
   * L4 headers given as parameters are added as separate layers and need a
   * packet to be added to
   * @param[in] code The redirect message code. Only values between 0 and 3 are
   * legal, the rest will cause the method to fail
   * @param[in] gatewayAddress An IPv4 address of the gateway to which the
   * redirection should be sent
   * @param[in] ipHeader The Internet header of the original data. This layer is
   * added as a separate layer on the packet
   * @param[in] l4Header The L4 header of the original data. This layer is added
   * as a separate layer on the packet
   * @return A pointer to the redirect data that have been set or NULL if
   * something went wrong (an appropriate error log is printed in such cases)
   */
    icmp_redirect* setRedirectData(uint8_t code, IPv4Address gatewayAddress,
                                   IPv4Layer* ipHeader, Layer* l4Header);

    /**
   * @return ICMP router advertisement data. If the layer isn't of type ICMP
   * router advertisement NULL is returned
   */
    icmp_router_advertisement* getRouterAdvertisementData() const;

    /**
   * Set router advertisement message data
   * @param[in] code The router advertisement message code. Only codes 0 or 16
   * are legal, the rest will fail the method
   * @param[in] lifetimeInSeconds The maximum number of seconds that the router
   * addresses in this list may be considered valid
   * @param[in] routerAddresses A vector of router advertisements to set
   * @return A pointer to the router advertisement data that have been set or
   * NULL if something went wrong (an appropriate error log is printed in such
   * cases)
   */
    ...
    /**
   * @return ICMP router solicitation data. If the layer isn't of type ICMP
   * router solicitation NULL is returned
   */
    icmp_router_solicitation* getRouterSolicitationData();

    /**
   * Set router solicitation message data. This message accepts no parameters as
   * there are no parameters to this type of message (code is always zero)
   * @return A pointer to the router solicitation data that have been set or
   * NULL if something went wrong (an appropriate error log is printed in such
   * cases)
   */
    ...
    /**
   * @return ICMP time-to-live exceeded data. If the layer isn't of type ICMP
   * time-to-live exceeded NULL is returned. The IP and L4 (ICMP/TCP/UDP)
   * headers of the time exceeded data are parsed as separate layers and can be
   * retrieved via this->getNextLayer()
   */
    icmp_time_exceeded* getTimeExceededData();

    /**
   * Set time-to-live exceeded message data. This method only works if IcmpLayer
   * is already part of a packet (not a standalone layer). The reason is the
   * Internet and L4 headers given as parameters are added as separate layers
   * and need a packet to be added to
   * @param[in] code Time-to-live exceeded message code. Only codes 0 or 1 are
   * legal, the rest will fail the method
   * @param[in] ipHeader The Internet header of the original data. This layer is
   * added as a separate layer on the packet
   * @param[in] l4Header The L4 header of the original data. This layer is added
   * as a separate layer on the packet
   * @return A pointer to the time-to-live exceeded data that have been set or
   * NULL if something went wrong (an appropriate error log is printed in such
   * cases)
   */
    icmp_time_exceeded* setTimeExceededData(uint8_t code, IPv4Layer* ipHeader,
                                            Layer* l4Header);

    /**
   * @return ICMP parameter problem data. If the layer isn't of type ICMP
   * parameter problem NULL is returned
   */
    icmp_param_problem* getParamProblemData();

    /**
   * Set parameter problem message data. This method only works if IcmpLayer is
   * already part of a packet (not a standalone layer). The reason is the
   * Internet and L4 headers given as parameters are added as separate layers
   * and need a packet to be added to
   * @param[in] code Parameter problem message code. Only code between 0 and 2
   * are legal, the rest will fail the method
   * @param[in] errorOctetPointer In the case of an invalid IP header (Code 0),
   * indicate the byte offset of the error in the header
   * @param[in] ipHeader The Internet header of the original data. This layer is
   * added as a separate layer on the packet
   * @param[in] l4Header The L4 header of the original data. This layer is added
   * as a separate layer on the packet
   * @return A pointer to the parameter problem data that have been set or NULL
   * if something went wrong (an appropriate error log is printed in such cases)
   */
    icmp_param_problem* setParamProblemData(uint8_t code,
                                            uint8_t errorOctetPointer,
                                            IPv4Layer* ipHeader, Layer* l4Header);

    /**
   * @return ICMP address mask request data. If the layer isn't of type ICMP
   * address mask request NULL is returned
   */
   ...
   * @return A pointer to the address mask request data that have been set or
   * NULL if something went wrong (an appropriate error log is printed in such
   * cases)
   */
    icmp_address_mask_request*
    setAddressMaskRequestData(uint16_t id, uint16_t sequence, IPv4Address mask);

    /**
   * @return ICMP address mask reply data. If the layer isn't of type ICMP
   * address mask reply NULL is returned
   */
   ...
   * @param[in] mask The subnet mask of the requesting host
   * @return A pointer to the address mask reply data that have been set or NULL
   * if something went wrong (an appropriate error log is printed in such cases)
   */
    icmp_address_mask_reply*
    setAddressMaskReplyData(uint16_t id, uint16_t sequence, IPv4Address mask);

    /**
   * @return ICMP address information request data. If the layer isn't of type
   * ICMP information request NULL is returned
   */
   ...
   * @return A pointer to the information request data that have been set or
   * NULL if something went wrong (an appropriate error log is printed in such
   * cases)
   */
    ...
    /**
   * @return ICMP address information reply data. If the layer isn't of type
   * ICMP information reply NULL is returned
   */
   ...
   * @return A pointer to the information reply data that have been set or NULL
   * if something went wrong (an appropriate error log is printed in such cases)
   */
   ...
   */
    ...
    /**
   * ICMP messages of types: ICMP_DEST_UNREACHABLE, ICMP_SOURCE_QUENCH,
   * ICMP_TIME_EXCEEDED, ICMP_REDIRECT, ICMP_PARAM_PROBLEM have data that
   * contains IPv4 header and some L4 header (TCP/UDP/ICMP). This method parses
   * these headers as separate layers on top of the ICMP layer
   */
    void parseNextLayer();

    /**
   * @return The ICMP header length. This length varies according to the ICMP
   * message type. This length doesn't include IPv4 and L4 headers in case ICMP
   * message type are: ICMP_DEST_UNREACHABLE, ICMP_SOURCE_QUENCH,
...
bool IcmpLayer::isDataValid(const uint8_t* data, size_t dataLen) {
    ...
    // ICMP_ECHO_REQUEST, ICMP_ECHO_REPLY, ICMP_ROUTER_SOL, ICMP_INFO_REQUEST,
    // ICMP_INFO_REPLY
