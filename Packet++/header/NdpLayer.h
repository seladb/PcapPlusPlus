...
/**
 * @class NdpOption
 * A wrapper class for NDP options. This class does not create or modify NDP
 ...
 * A class for building NDP option records. This builder receives the NDP option
...
class NdpOptionBuilder : public TLVRecordBuilder {
   ...
   * NdpOption object can be later retrieved by calling build(). Each option is
   * padded to have a 64-bit boundary.
   ...
   */
    NdpOptionBuilder(NDPNeighborOptionTypes optionType,
                     const uint8_t* optionValue, uint8_t optionValueLen)
   ...
   * Padding bytes are added to the option for option length with 64-bit
   * boundaries.
...
class NDPLayerBase : public IcmpV6Layer {
   ...
   * @return An NdpOption object that contains the first option that matches
   * this type, or logical NULL (NdpOption#isNull() == true) if no such option
   * found
   */
    NdpOption getNdpOption(NDPNeighborOptionTypes option) const;

    /**
   * @return The first NDP option in the packet. If the current layer contains
   * no options the returned value will contain a logical NULL
   * (NdpOption#isNull() == true)
   */
    NdpOption getFirstNdpOption() const;

    /**
   * Get the NDP option that comes after a given option. If the given option was
   * the last one, the returned value will contain a logical NULL
   * (IdpOption#isNull() == true)
   * @param[in] option An NDP option object that exists in the current layer
   * @return A NdpOption object that contains the NDP option data that comes
   * next, or logical NULL if the given NDP option: (1) was the last one; or (2)
   ...
   * @param[in] optionBuilder An NdpOptionBuilder object that contains the NDP
   * option data to be added
   * @return A NdpOption object that contains the newly added NDP option data or
   * logical NULL (NdpOption#isNull() == true) if addition failed. In case of a
...
#pragma pack(pop)
   ...
   * stored in
   */
    NDPNeighborSolicitationLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                                 Packet* packet)
   ...
   * @param[in] targetIP Target IP address for which the solicitation shall be
   * created
   */
...
#pragma pack(push, 1)
    struct ndpneighboradvertisementhdr : icmpv6hdr {
        ...
        uint32_t
            ...
            override : 1,
            /** Flag indicating that the advertisement was sent in response to a
        ...
        uint32_t
           ...
           Neighbor Solicitation from the Destination address */
            ...
            reserved : 29;
#endif
        /** Target address - Either source address of advertisement or address for
     * requested MAC */
        uint8_t targetIP[16];
    };
#pragma pack(pop)
   ...
   * stored in
   */
    NDPNeighborAdvertisementLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                                  Packet* packet)
        : NDPLayerBase(data, dataLen, prevLayer, packet) {}

    /**
   * A constructor that allocates a new NDP Advertisement Layer with target
   * link-layer address option
   * @param[in] code Code field
   * @param[in] targetIP The target IP address from the Neighbor Solicitation
   * message (solicited advertisements) or the address whose link-layer address
   * has changed (unsolicited advertisement)
   * @param[in] targetMac Adds the target link-layer address into the option
   * field of the layer
   * @param[in] routerFlag The router flag
   * @param[in] unicastFlag The solicited flag
   * @param[in] overrideFlag The override flag
   */
    NDPNeighborAdvertisementLayer(uint8_t code, const IPv6Address& targetIP,
   ...
   * @param targetIP The target IP address from the Neighbor Solicitation
   * message (solicited advertisements) or the address whose link-layer address
   * has changed (unsolicited advertisement)
   ...
   */
    ...
    /**
   * @return Get information if the target link-layer address was added in the
   * option field of the header
   */
