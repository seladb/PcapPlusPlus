 ...
 * to supplemental data placed at the end of a block of data being stored or
 * transmitted, which may contain information for the handling of the data
 * block, or just mark its end (taken from Wikipedia:
 * https://en.wikipedia.org/wiki/Trailer_(computing) )
 *
 * There are various reasons for adding a packet trailer, one of the most famous
 * is FCS (Frame check sequence) which refers to the extra error-detecting code
 * added to a frame. Another usage is padding which means adding data to reach a
 * minimum required packet length.
 *
 * Although this layer inherits from the Layer class, it is not a standard layer
 * in the sense that it can't be constructed by the user. This layer may be only
 * be constructed in the Packet class, in the process of parsing the packet and
 * creating the layers; if at the end of the parsing process there is data left
 * that is not allocated to any layer, it's assumed to be the packet trailer and
 * an instance of this class is created. This means this layer can only exist as
 * the last layer in a packet, if a packet trailer indeed exists.
 *
 * No layer can be added by the user after this layer (trying to do that will
