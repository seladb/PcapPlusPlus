...
// specific hack to the command line during build Its easier/more consistent to
// test _MSC_VER in VS
 ...
 * A wrapper class for storing DNS RR (resource record) data. This is the base
 * class which introduces several abstract methods for derived classes to
 * implement for setting and retrieving the stored data. Each derived class will
...
class IDnsResourceData {
  protected:
    ...
    size_t decodeName(const char* encodedName, char* result,
                      IDnsResource* dnsResource) const;
    void encodeName(const std::string& decodedName, char* result,
                    size_t& resultLen, IDnsResource* dnsResource) const;
   ...
   */
    ...
    /**
   * A templated method which takes a class that derives from IDnsResourceData
   * as the template argument and checks whether this instance is of this type
   ...
   */
    ...
    /**
   * A templated method which take a class that derives from IDnsResourceData as
   * the template argument and tries to cast the current instance as that type
   * @return A pointer to the current instance casted as the requested type or
   ...
   * @param[out] arr A pointer to a pre-allocated byte array where the result
   * will be written to
   * @param[out] arrLength A reference to a 2-byte number where the result array
   * length will be written to
   * @param[in] dnsResource A pointer to a DNS resource object where this DNS RR
   * data will be stored
   * @return True if the DNS RR data was successfully converted into a byte
   * array and written to the given array or false if stored DNS RR data is
   * invalid or if it could not be written to the given array
   */
    virtual bool toByteArr(uint8_t* arr, size_t& arrLength,
                           IDnsResource* dnsResource) const = 0;
};
...
class DnsResourceDataPtr : public PCPP_SMART_PTR(IDnsResourceData) {
   ...
   */
    explicit DnsResourceDataPtr(IDnsResourceData* ptr)
        : PCPP_SMART_PTR(IDnsResourceData)(ptr) {}
    ...
    /**
   * A templated method which takes a class that derives from IDnsResourceData
   * as the template argument and checks whether the pointer stored in this
   * object is of this type
   * @return True if the stored pointer is of the requested type, false
   * otherwise
   */
    template <class IDnsResourceDataType>
    bool isTypeOf() const {
    ...
    /**
   * A templated method which take a class that derives from IDnsResourceData as
   * the template argument and tries to cast the pointer stored in this object
   * as that type
   * @return A pointer to the stored pointer casted as the requested type or
...
class StringDnsResourceData : public IDnsResourceData {
   ...
   * @param[in] data The string data to store in this object. If this string
   * represents a hostname it's possible to include a pointer to another string
   * in the DNS layer (as explained here:
   * http://www.zytrax.com/books/dns/ch15/#name). These pointers are often used
   * to reduce the DNS packet size and avoid unnecessary duplications. The way
   * to include pointers in a hostname string is to use the following format:
   * 'some.domain.#{offset}' where '#{offset}' is the offset from the start of
   * the DNS layer. For example: if the string 'yahoo.com' already appears in
   * offset 12 in the packet and you want to set the DNS RR data as
   * 'my.subdomain.yahoo.com' you may use the following string:
   * 'my.subdomain.#12'. This will result in writing 'my.subdomain' and a
   * pointer to offset 12
   */
    explicit StringDnsResourceData(const std::string& data) : m_Data(data) {}

    StringDnsResourceData(const uint8_t* dataPtr, size_t dataLen,
                          IDnsResource* dnsResource);

    ~StringDnsResourceData() {}

    /**
   * Equality operator overload for this class that compares the strings stored
   * in each object
   ...
   */
    ...
    bool toByteArr(uint8_t* arr, size_t& arrLength,
                   IDnsResource* dnsResource) const;
};
...
class IPv4DnsResourceData : public IDnsResourceData {
  ...
  public:
    /**
   * A c'tor for this class
   * @param[in] dataPtr A byte array of size 4 that contains an IPv4 address
   * (each byte represents 1 octet)
   ...
   */
    ...
    /**
   * A c'tor for this class
   * @param[in] addrAsString A string representation of an IPv4 address to store
   * in this object
   */
    explicit IPv4DnsResourceData(const std::string& addrAsString)
        : m_Data(addrAsString) {}

    /**
   * Equality operator overload for this class that compares the IPv4 addresses
...
class IPv6DnsResourceData : public IDnsResourceData {
  ...
  public:
    /**
   * A c'tor for this class
   * @param[in] dataPtr A byte array of size 16 that contains an IPv6 address
   * (each byte represents 1 octet)
   ...
   */
    ...
    /**
   * A c'tor for this class
   * @param[in] addrAsString A string representation of an IPv6 address to store
   * in this object
   */
    explicit IPv6DnsResourceData(const std::string& addrAsString)
        : m_Data(addrAsString) {}

    /**
   * Equality operator overload for this class that compares the IPv6 addresses
   * stored in each object
   * @param[in] other The object to compare with
...
class MxDnsResourceData : public IDnsResourceData {
   ...
   */
    ...
    /**
   * A c'tor for this class
   * @param[in] dataPtr A byte array that contains the raw MX data (as written
   * in the DNS packet)
   * @param[in] dataLen The byte array size
   * @param[in] dnsResource A pointer to a DNS resource object where this DNS RR
   * data will be stored
   */
    MxDnsResourceData(uint8_t* dataPtr, size_t dataLen,
                      IDnsResource* dnsResource);

    /**
   * A c'tor for this class
   * @param[in] preference The MX preference value to store in this object
   * @param[in] mailExchange The MX hostname value to store in this object. It's
   * possible to include a pointer to another string in the DNS layer (as
   * explained here: http://www.zytrax.com/books/dns/ch15/#name). These pointers
   * are often used to reduce the DNS packet size and avoid unnecessary
   * duplications. The way to include pointers in the hostname string is to use
   * the following format: 'some.domain.#{offset}' where '#{offset}' is the
   * offset from the start of the DNS layer. For example: if the string
   * 'yahoo.com' already appears in offset 12 in the packet and you want to set
   * the DNS RR data as 'my.subdomain.yahoo.com' you may use the following
   * string: 'my.subdomain.#12'. This will result in writing 'my.subdomain' and
   * a pointer to offset 12
   */
    MxDnsResourceData(const uint16_t& preference,
                      const std::string& mailExchange);

    ~MxDnsResourceData() {}

    /**
   * Equality operator overload for this class that compares the MX data stored
   * in each object
   * @param[in] other The object to compare with
   * @return True if MX data is the same in both objects, meaning both
   * preference and MX hostname are the same, false otherwise
   */
   ...
   */
    ...
    /**
   * A string representation of the MX data stored in this object. The string
   * format is as follows: 'pref: {preference_value}; mx:
   * {mail_exchange_hostname_value}'
   */
...
class GenericDnsResourceData : public IDnsResourceData {
   ...
   * in the DNS packet). The data will be copied from this byte array to the
   * object
   * @param[in] dataLen The byte array size
   */
   ...
   */
    ...
    /**
   * Equality operator overload for this class that compares the raw data stored
   * in each object
   * @param[in] other The object to compare with
   * @return True if data is the same in both objects, meaning byte streams are
