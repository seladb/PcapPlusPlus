...
#pragma pack(pop)
    ...
    /**
   * Destroy the SOME/IP-SD Option object and delete allocated data if it has
   * been allocated by a constructor
   */
  ...
  protected:
    ...
    SomeIpSdOption()
        : m_DataContainer(nullptr), m_Offset(0), m_ShadowData(nullptr),
          m_DataLen(0) {}

    SomeIpSdOption(const IDataContainer* dataContainer, size_t offset)
        : m_DataContainer(dataContainer), m_Offset(offset), m_ShadowData(nullptr),
          m_DataLen(0) {}
...
class SomeIpSdIPv4Option : public SomeIpSdOption {
   ...
   */
    SomeIpSdIPv4Option(IPv4OptionType type, IPv4Address ipAddress, uint16_t port,
...
class SomeIpSdIPv6Option : public SomeIpSdOption {
   ...
   * @param[in] l4Protocol Protocol to use
   */
    SomeIpSdIPv6Option(IPv6OptionType type, IPv6Address ipAddress, uint16_t port,
                       SomeIpSdProtocolType l4Protocol);
...
class SomeIpSdConfigurationOption : public SomeIpSdOption {
   ...
   * @param[in] offset Offset for dataContainer
   */
    SomeIpSdConfigurationOption(const IDataContainer* dataContainer,
                                size_t offset);
...
class SomeIpSdLoadBalancingOption : public SomeIpSdOption {
   ...
   * @param[in] offset Offset for dataContainer
   */
    SomeIpSdLoadBalancingOption(const IDataContainer* dataContainer,
                                size_t offset);
...
#pragma pack(push, 1)
    struct someipsdhdrentry {
        ...
        /** Minor Version (Service Entry Type) or Counter + Eventgroup ID
...
#pragma pack(pop)
   ...
   */
    SomeIpSdEntry(EntryType type, uint16_t serviceID, uint16_t instanceID,
                  uint8_t majorVersion, uint32_t TTL, uint32_t minorVersion);
   ...
   */
    SomeIpSdEntry(EntryType type, uint16_t serviceID, uint16_t instanceID,
                  uint8_t majorVersion, uint32_t TTL, uint8_t counter,
   ...
   */
    ...
    /**
   * Destroy the SomeIpSd Entry object and delete allocated data if it has been
   * allocated by a constructor
   */
    ~SomeIpSdEntry();
   ...
   */
    ...
    void initStdFields(EntryType type, uint16_t serviceID, uint16_t instanceID,
                       uint8_t majorVersion, uint32_t TTL);
...
class SomeIpSdLayer : public SomeIpLayer {
   ...
   * stored in
   */
    SomeIpSdLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                  Packet* packet);
   ...
   */
    SomeIpSdLayer(uint16_t serviceID, uint16_t methodID, uint16_t clientID,
   ...
   */
    ...
    /**
   * Adds an option to an entry that has already been added to the layer by
   * using addEntry(). The option is also added to the layer itself. If the
   * option cannot by assigned to the entry, the option is not copied into the
   * layer.
   * @param[in] indexEntry Index of the entry where the option shall be added.
   * First Entry has index 0
   * @param[in] option Pointer to the option that shall be added
   * @return True if the option could be assigned to the entry and was copied
   * into the layer, false otherwise
   */
