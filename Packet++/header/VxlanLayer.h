...
class VxlanLayer : public Layer
{
   ...
   */
    ...
    /**
   * A constructor that creates a new VXLAN header and allocates the data. Note:
   * the VNI present flag is set automatically
   * @param[in] vni VNI (VXLAN Network ID) to set. Optional parameter (default
   * is 0)
   * @param[in] groupPolicyID Group Policy ID to set. Optional parameter
   * (default is 0)
   * @param[in] setGbpFlag Set GBP flag. Optional parameter (default is false)
   * @param[in] setPolicyAppliedFlag Set Policy Applied flag. Optional parameter
   * (default is false)
   * @param[in] setDontLearnFlag Set Don't Learn flag. Optional parameter
   * (default is false)
   */
    explicit VxlanLayer(uint32_t vni = 0, uint16_t groupPolicyID = 0,
    ...
    /**
   * Get a pointer to the VXLAN header. Notice this points directly to the data,
