...
struct ip6_hdr
{
    ...
    /** Specifies the type of the next header (protocol). Must be one of
   * ::IPProtocolTypes */
    uint8_t nextHeader;
...
class IPv6Layer : public Layer, public IPLayer
{
   ...
   */
    ...
    /**
   * A constructor that allocates a new IPv6 header with source and destination
   * IPv6 addresses
   ...
   * then copy the entire header from the other IPv6Layer (including IPv6
   * extensions)
   */
    IPv6Layer& operator=(const IPv6Layer& other);

    /**
   * Get a pointer to the IPv6 header. Notice this points directly to the data,
   ...
   * similar to getSrcIPv6Address(), but adds a level of abstraction because
   ...
   * very similar to getDstIPv6Address(), but adds a level of abstraction
   ...
   */
    ...
    /**
   * A templated getter for an IPv6 extension of a type TIPv6Extension.
   * TIPv6Extension has to be one of the supported IPv6 extensions, meaning a
   * class that inherits IPv6Extension. If the requested extension type isn't
   * found NULL is returned
   * @return A pointer to the extension instance or NULL if the requested
   * extension type isn't found
   */
    template <class TIPv6Extension>
    TIPv6Extension* getExtensionOfType() const;

    /**
   * Add a new extension of type TIPv6Extension to the layer. This is a
   * templated method and TIPv6Extension has to be one of the supported IPv6
   * extensions, meaning a class that inherits IPv6Extension. If the extension
   * is added successfully a pointer to the newly added extension object is
   * returned, otherwise NULL is returned
   * @param[in] extensionHeader The extension object to add. Notice the object
   * passed here is read-only, meaning its data is copied but the object itself
   * isn't modified
   * @return If the extension is added successfully a pointer to the newly added
   * extension object is returned. Otherwise NULL is returned
   */
    template <class TIPv6Extension>
   ...
   */
    ...
    /**
   * @return True if this packet is an IPv6 fragment, meaning if it has an
   * IPv6FragmentationHeader extension
   */
   ...
   * - ip6_hdr#ipVersion = 6
   * - ip6_hdr#nextHeader = calculated if next layer is known:
