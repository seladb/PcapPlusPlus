...
enum LinkLayerType {
    ...
    /** ARCNET Data Packets, as described by the ARCNET Trade Association standard
     ATA 878.1-1999 */
    ...
    /** PPP in HDLC-like encapsulation, like LINKTYPE_PPP_HDLC, but with the 0xff
     address byte replaced by a direction indication - 0x00 for incoming and
     0x01 for outgoing */
    ...
    /** Various link-layer types, with a pseudo-header, for Endace DAG cards;
     encapsulates Endace ERF record */
    LINKTYPE_ERF = 197,
    /** Bluetooth HCI UART transport layer */
    ...
    LINKTYPE_IPNET = 226,
    /** CAN (Controller Area Network) frames, with a pseudo-header as supplied by
     Linux SocketCAN */
    ...
    LINKTYPE_IPV6 = 229,
    /** IEEE 802.15.4 wireless Personal Area Network, without the FCS at the end
     of the frame */
    ...
    /** D_PDUs as described by NATO standard STANAG 5066, starting with the
     synchronization sequence, and including both header and data CRCs */
    LINKTYPE_STANAG_5066_D_PDU = 237,
    /** Linux netlink NETLINK NFLOG socket log messages */
    LINKTYPE_NFLOG = 239,
    /** Pseudo-header for Hilscher Gesellschaft für Systemautomation mbH
     netANALYZER devices, followed by an Ethernet frame, beginning with the MAC
     header and ending with the FCS */
    ...
    /** MPEG-2 Transport Stream transport packets, as specified by ISO
     13818-1/ITU-T Recommendation H.222.0 */
    LINKTYPE_MPEG_2_TS = 243,
    /** Pseudo-header for ng4T GmbH's UMTS Iub/Iur-over-ATM and Iub/Iur-over-IP
     format as used by their ng40 protocol tester */
    LINKTYPE_NG40 = 244,
    /** Pseudo-header for NFC LLCP packet captures, followed by frame data for the
     LLCP Protocol as specified by NFCForum-TS-LLCP_1.1 */
   ...
   */
    ...
    /** Per Joshua Wright <jwright@hasborg.com>, formats for Z-Wave RF profiles R1
     and R2 captures */
    LINKTYPE_ZWAVE_R1_R2 = 261,
    /** Per Joshua Wright <jwright@hasborg.com>, formats for Z-Wave RF profile R3
     captures */
    ...
    /** Messages between ISO 14443 contactless smartcards (Proximity Integrated
     Circuit Card, PICC) and card readers (Proximity Coupling Device, PCD), with
     the message format specified by the PCAP format for ISO14443 specification
   */
...
/**
 * @class RawPacket
 * This class holds the packet as raw (not parsed) data. The data is held as
 * byte array. In addition to the data itself every instance also holds a
 * timestamp representing the time the packet was received by the NIC. RawPacket
...
class RawPacket {
  ...
  public:
    /**
   * A constructor that receives a pointer to the raw data (allocated
   * elsewhere). This constructor is usually used when packet is captured using
   * a packet capturing engine (like libPcap. WinPcap, Npcap, PF_RING, etc.).
   * The capturing engine allocates the raw data memory and give the user a
   * pointer to it + a timestamp it has arrived to the device
   * @param[in] pRawData A pointer to the raw data
   * @param[in] rawDataLen The raw data length in bytes
   * @param[in] timestamp The timestamp packet was received by the NIC (in usec
   * precision)
   * @param[in] deleteRawDataAtDestructor An indicator whether raw data pointer
   * should be freed when the instance is freed or not. If set to 'true' than
   * pRawData will be freed when instanced is being freed
   * @param[in] layerType The link layer type of this raw packet. The default is
   * Ethernet
   */
    RawPacket(const uint8_t* pRawData, int rawDataLen, timeval timestamp,
   ...
   * @param[in] pRawData A pointer to the raw data
   * @param[in] rawDataLen The raw data length in bytes
   * @param[in] timestamp The timestamp packet was received by the NIC (in nsec
   * precision)
   * @param[in] deleteRawDataAtDestructor An indicator whether raw data pointer
   * should be freed when the instance is freed or not. If set to 'true' than
   * pRawData will be freed when instanced is being freed
   ...
   * Ethernet
   */
    RawPacket(const uint8_t* pRawData, int rawDataLen, timespec timestamp,
              bool deleteRawDataAtDestructor,
              LinkLayerType layerType = LINKTYPE_ETHERNET);

    /**
   * A default constructor that initializes class'es attributes to default
   * value:
   ...
   */
    ...
    /**
   * A destructor for this class. Frees the raw data if
   * deleteRawDataAtDestructor was set to 'true'
   */
    virtual ~RawPacket();

    /**
   * A copy constructor that copies all data from another instance. Notice all
   * raw data is copied (using memcpy), so when the original or the other
   * instance are freed, the other won't be affected
   * @param[in] other The instance to copy from
   */
    RawPacket(const RawPacket& other);

    /**
   * Assignment operator overload for this class. When using this operator on an
   * already initialized RawPacket instance, the original raw data is freed
   * first. Then the other instance is copied to this instance, the same way the
   ...
   */
    ...
    /**
   * @return RawPacket object type. Each derived class should return a different
   * value
   */
    virtual uint8_t getObjectType() const { return 0; }

    /**
   * Set a raw data. If data was already set and deleteRawDataAtDestructor was
   * set to 'true' the old data will be freed first
   ...
   */
    virtual bool setRawData(const uint8_t* pRawData, int rawDataLen,
                            timeval timestamp,
                            LinkLayerType layerType = LINKTYPE_ETHERNET,
                            int frameLength = -1);
   ...
   */
    ...
    /**
   * Initialize a raw packet with data. The main difference between this method
   * and setRawData() is that setRawData() is meant for replacing the data in an
   * existing raw packet, whereas this method is meant to be used right after
   ...
   * @param timestamp The timestamp packet was received by the NIC (in nsec
   * precision)
   * @param layerType The link layer type for this raw data
   * @return True if raw data was set successfully, false otherwise
   */
    bool initWithRawData(const uint8_t* pRawData, int rawDataLen,
                         timespec timestamp,
                         LinkLayerType layerType = LINKTYPE_ETHERNET);
   ...
   * @return True if the link type value is valid and can be casted into
   * LinkLayerType enum, false otherwise
   */
   ...
   * @return True if raw data was set for this instance. Raw data can be set
   * using the non-default constructor, using setRawData(), using the copy
   * constructor or using the assignment operator. Returns false otherwise, for
   * example: if the instance was created using the default constructor or
   * clear() was called
   */
    bool isPacketSet() const { return m_RawPacketSet; }

    /**
   * Clears all members of this instance, meaning setting raw data to NULL, raw
   * data length to 0, etc. Currently raw data is always freed, even if
   * deleteRawDataAtDestructor was set to 'false'
   ...
   */
    ...
    /**
   * Append data to the end of current data. This method works without
   * allocating more memory, it just uses memcpy() to copy dataToAppend at the
   * end of the current data. This means that the method assumes this memory was
   * already allocated by the user. If it isn't the case then this method will
   ...
   */
    ...
    /**
   * Insert new data at some index of the current data and shift the remaining
   * old data to the end. This method works without allocating more memory, it
   * just copies dataToAppend at the relevant index and shifts the remaining
   * data to the end. This means that the method assumes this memory was already
   * allocated by the user. If it isn't the case then this method will cause
   * memory corruption
   ...
   */
    virtual void insertData(int atIndex, const uint8_t* dataToInsert,
                            size_t dataToInsertLen);

    /**
   * Remove certain number of bytes from current raw data buffer. All data after
   * the removed bytes will be shifted back
   ...
   * atIndex+numOfBytesToRemove is out-of-bounds of the raw data buffer
   */
    virtual bool removeData(int atIndex, size_t numOfBytesToRemove);

    /**
   * Re-allocate raw packet buffer meaning add size to it without losing the
   * current packet data. This method allocates the required buffer size as
   * instructed by the use and then copies the raw data from the current
   * allocated buffer to the new one. This method can become useful if the user
   * wants to insert or append data to the raw data, and the previous allocated
   * buffer is too small, so the user wants to allocate a larger buffer and get
   * RawPacket instance to point to it
   * @param[in] newBufferLength The new buffer length as required by the user.
   * The method is responsible to allocate the memory
