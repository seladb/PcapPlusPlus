...
/**
 * @class NflogTlv
 * A wrapper class for NFLOG TLV fields. This class does not create or modify
...
class NflogTlv {
   ...
   */
    explicit NflogTlv(uint8_t* recordRawData) { assign(recordRawData); }
...
class NflogLayer : public Layer {
   ...
   * On one netlink socket it's possible to listen to several nflog groups; the
   * resource ID is the nflog group for the packet
   */
    uint16_t getResourceId();

    /**
   * Get a TLV object found with the input type. if no tlv is found, the
   * internal value of the object will set to nullptr
   ...
   */
    ...
    /**
   * Currently identifies the following next layers: IPv4Layer, IPv6Layer using
   * address family Otherwise sets PayloadLayer
   */
    void parseNextLayer();
   ...
   * @param[in] data The pointer to the beginning of a byte stream of an NFLOG
   * packet
