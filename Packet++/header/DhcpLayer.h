...
struct dhcp_header
{
    ...
    /** The elapsed time, in seconds since the client sent its first BOOTREQUEST
   * message */
    ...
    /** Client hardware address, by default contains the MAC address (only 6 first
   * bytes are used) */
...
enum DhcpOptionTypes
{
    ...
    /** Includes one or multiple lists of PCP server IP addresses; each list is
     treated as a separate PCP server */
    DHCPOPT_OPTION_V4_PCP_SERVER = 158,
    /** This option is used to configure a set of ports bound to a shared IPv4
     address */
...
/**
 * @class DhcpOption
 * A wrapper class for DHCP options. This class does not create or modify DHCP
...
class DhcpOption : public TLVRecord<uint8_t, uint8_t>
{
   ...
   */
    ...
    /**
   * Retrieve DHCP option data as IPv4 address. Relevant only if option value is
   * indeed an IPv4 address
   * @return DHCP option data as IPv4 address
   */
    IPv4Address getValueAsIpAddr() const { return getValueAs<uint32_t>(); }

    /**
   * Set DHCP option data as IPv4 address. This method copies the 4 bytes of the
   * IP address to the option value
   * @param[in] addr The IPv4 address to set
   * @param[in] valueOffset An optional parameter that specifies where to start
   * set the option data (default set to 0). For example: if option data is 20
   ...
   */
    ...
    /**
   * Retrieve DHCP option data as string. Relevant only if option value is
   * indeed a string
   * @param[in] valueOffset An optional parameter that specifies where to start
   * copy the DHCP option data. For example: when retrieving Client FQDN option,
   * you may ignore the flags and RCODE fields using this method like this:
   * getValueAsString(3). The default is 0 - start copying from the beginning of
   * option data
   ...
   */
    std::string getValueAsString(int valueOffset = 0) const
    {
        ...
        return std::string((const char*)m_Data->recordValue + valueOffset,
                           (int)m_Data->recordLen - valueOffset);
    }

    /**
   * Set DHCP option data as string. This method copies the string to the option
   * value. If the string is longer than option length the string is trimmed so
   * it will fit the option length
   ...
   */
    void setValueString(const std::string& stringValue, int valueOffset = 0)
    {
        ...
        // use the length of input string if a buffer is large enough for whole
        // string
   ...
   */
    static bool canAssign(const uint8_t* recordRawData, size_t tlvDataLen)
    {
        ...
        if (data->recordType == (uint8_t)DHCPOPT_END ||
            data->recordType == (uint8_t)DHCPOPT_PAD)
            return true;
    ...
    size_t getTotalSize() const
    {
        ...
        if (m_Data->recordType == (uint8_t)DHCPOPT_END ||
            m_Data->recordType == (uint8_t)DHCPOPT_PAD)
 ...
 * A class for building DHCP options. This builder receives the option
...
class DhcpOptionBuilder : public TLVRecordBuilder
{
   ...
   * DhcpOption object can later be retrieved by calling build()
   ...
   */
    DhcpOptionBuilder(DhcpOptionTypes optionType, const uint8_t* optionValue,
                      uint8_t optionValueLen)
        : TLVRecordBuilder((uint8_t)optionType, optionValue, optionValueLen) {}

    /**
   * A c'tor for building DHCP options which have a 1-byte value. The DhcpOption
   * object can later be retrieved by calling build()
   * @param[in] optionType DHCP option type
   ...
   */
    ...
    /**
   * A c'tor for building DHCP options which have a 2-byte value. The DhcpOption
   * object can later be retrieved by calling build()
   * @param[in] optionType DHCP option type
   ...
   */
    ...
    /**
   * A c'tor for building DHCP options which have a 4-byte value. The DhcpOption
   * object can later be retrieved by calling build()
   * @param[in] optionType DHCP option type
   ...
   */
    ...
    /**
   * A c'tor for building DHCP options which have an IPv4Address value. The
   * DhcpOption object can later be retrieved by calling build()
   * @param[in] optionType DHCP option type
   ...
   */
    ...
    /**
   * A c'tor for building DHCP options which have a string value. The DhcpOption
   * object can later be retrieved by calling build()
   * @param[in] optionType DHCP option type
   ...
   */
    ...
    /**
   * A copy c'tor which copies all the data from another instance of
   * DhcpOptionBuilder
...
class DhcpLayer : public Layer
{
   ...
   */
    ...
    /**
   * A constructor that creates the layer from scratch. Adds a
   * ::DHCPOPT_DHCP_MESSAGE_TYPE and a ::DHCPOPT_END options
   * @param[in] msgType A DHCP message type to be set
   * @param[in] clientMacAddr A client MAC address to set in
   * dhcp_header#clientHardwareAddress field
   */
   ...
   */
    ...
    /**
   * Get a pointer to the DHCP header. Notice this points directly to the data,
   ...
   */
    ...
    /**
   * @return The client MAC address as extracted from
   * dhcp_header#clientHardwareAddress, assuming dhcp_header#hardwareType is 1
   * (Ethernet) and dhcp_header#hardwareAddressLength is 6 (MAC address length).
   * Otherwise returns MacAddress#Zero
   */
    MacAddress getClientHardwareAddress() const;

    /**
   * Set a MAC address into the first 6 bytes of
   * dhcp_header#clientHardwareAddress. This method also sets
   * dhcp_header#hardwareType to 1 (Ethernet) and
   * dhcp_header#hardwareAddressLength to 6 (MAC address length)
   * @param[in] addr The MAC address to set
   */
    void setClientHardwareAddress(const MacAddress& addr);

    /**
   * @return DHCP message type as extracted from ::DHCPOPT_DHCP_MESSAGE_TYPE
   * option. If this option doesn't exist the value of
   * ::DHCP_UNKNOWN_MSG_TYPE is returned
   */
    DhcpMessageType getMessageType() const;

    /**
   * Set DHCP message type. This method searches for existing
   * ::DHCPOPT_DHCP_MESSAGE_TYPE option. If found, it sets the requested message
   * type as its value. If not, it creates a ::DHCPOPT_DHCP_MESSAGE_TYPE option
   * and sets the requested message type as its value
   * @param[in] msgType Message type to set
   * @return True if message type was set successfully or false if msgType is
   * ::DHCP_UNKNOWN_MSG_TYPE or if failed to add
   * ::DHCPOPT_DHCP_MESSAGE_TYPE option
   */
    bool setMessageType(DhcpMessageType msgType);

    /**
   * @return The first DHCP option in the packet. If there are no DHCP options
   * the returned value will contain a logical NULL (DhcpOption#isNull() ==
   * true)
   */
    DhcpOption getFirstOptionData() const;

    /**
   * Get the DHCP option that comes after a given option. If the given option
   * was the last one, the returned value will contain a logical NULL
   * (DhcpOption#isNull() == true)
   * @param[in] dhcpOption A given DHCP option
   * @return A DhcpOption object containing the option data that comes next, or
   * logical NULL if the given DHCP option: (1) was the last one; (2) contains a
   * logical NULL or (3) doesn't belong to this packet
   */
   ...
   * @return A DhcpOption object containing the first DHCP option data that
   * matches this type, or logical NULL (DhcpOption#isNull() == true) if no such
   * option found
   */
   ...
   * @param[in] optionBuilder A DhcpOptionBuilder object that contains the
   * requested DHCP option data to add
   * @return A DhcpOption object containing the newly added DHCP option data or
   * logical NULL (DhcpOption#isNull() == true) if addition failed
   */
   ...
   * @param[in] optionBuilder A DhcpOptionBuilder object that contains the
   * requested DHCP option data to add
   * @param[in] prevOption The DHCP option type which the newly added option
   * will come after
   * @return A DhcpOption object containing the newly added DHCP option data or
   * logical NULL (DhcpOption#isNull() == true) if addition failed
   */
    DhcpOption addOptionAfter(const DhcpOptionBuilder& optionBuilder,
   ...
   * @return True if DHCP option was successfully removed or false if type
   * wasn't found or if removal failed
   */
    bool removeOption(DhcpOptionTypes optionType);

    /**
   * Remove all DHCP options in this layer
   * @return True if all DHCP options were successfully removed or false if
   ...
   * - @ref dhcp_header#opCode = ::DHCP_BOOTREQUEST for message types:
   * ::DHCP_DISCOVER, ::DHCP_REQUEST, ::DHCP_DECLINE, ::DHCP_RELEASE,
   *                            ::DHCP_INFORM, ::DHCP_UNKNOWN_MSG_TYPE
   *                            ::DHCP_BOOTREPLY for message types:
   * ::DHCP_OFFER, ::DHCP_ACK, ::DHCP_NAK
