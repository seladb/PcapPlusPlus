...
struct igmp_header {
   ...
   * resolution of 100 milliseconds */
    ...
    /** This is the 16-bit one's complement of the one's complement sum of the
   * entire IGMP message */
    uint16_t checksum;
    /** This is the multicast address being queried when sending a Group-Specific
...
struct igmpv3_query_header {
   ...
   * (::IgmpType_MembershipQuery)  */
    uint8_t type;
    /** This field specifies the maximum time (in 1/10 second) allowed before
 ...
 * A block of fields containing information pertaining to the sender's
...
struct igmpv3_group_record {
   ...
   * value other than 0 isn't supported */
    uint8_t auxDataLen;
    ...
    /** A vector of n IP unicast addresses, where n is the value in this record's
   * Number of Sources field */
    uint8_t sourceAddresses[];

    /**
   * @return The multicast address in igmpv3_group_record#multicastAddress as
   ...
   * @return The source address in the requested index. If index is negative or
   * higher than the number of source addresses in this group record the value
   * if IPv4Address#Zero is returned
   */
...
/**
 * @class IgmpLayer
 * A base class for all IGMP (Internet Group Management Protocol) protocol
 * classes. This is an abstract class and cannot be instantiated, only its child
...
class IgmpLayer : public Layer {
  protected:
    ...
    IgmpLayer(IgmpType type, const IPv4Address& groupAddr,
              uint8_t maxResponseTime, ProtocolType igmpVer);

    uint16_t calculateChecksum();

    size_t getHeaderSizeByVerAndType(ProtocolType igmpVer,
  ...
  public:
    ...
    /**
   * Get a pointer to the raw IGMPv1/IGMPv2 header. Notice this points directly
   ...
   */
    ...
    /**
   * @return IGMP type set in igmp_header#type as ::IgmpType enum. Notice that
   * if igmp_header#type contains a value that doesn't appear in the ::IgmpType
   * enum, ::IgmpType_Unknown will be returned
   */
   ...
   */
    ...
    /**
   * A static method that gets raw IGMP data (byte stream) and returns the IGMP
   * version of this IGMP message
   * @param[in] data The IGMP raw data (byte stream)
   * @param[in] dataLen Raw data length
   * @param[out] isQuery Return true if IGMP message type is
   * ::IgmpType_MembershipQuery and false otherwise
   * @return One of the values ::IGMPv1, ::IGMPv2, ::IGMPv3 according to
   * detected IGMP version or ::UnknownProtocol if couldn't detect IGMP version
   */
...
class IgmpV1Layer : public IgmpLayer {
   ...
   * @param[in] groupAddr The multicast address to set. This is an optional
   * parameter and has a default value of IPv4Address#Zero if not provided
   */
    explicit IgmpV1Layer(IgmpType type,
                         const IPv4Address& groupAddr = IPv4Address())
   ...
   */
    ...
    /**
   * Calculate the IGMP checksum and set igmp_header#maxResponseTime to 0 (this
   * field is unused in IGMPv1)
   */
...
class IgmpV2Layer : public IgmpLayer {
   ...
   * @param[in] type The message type to set
   * @param[in] groupAddr The multicast address to set. This is an optional
   * parameter and has a default value of unspecified/zero IPv4 address
   * @param[in] maxResponseTime The max response time to set. This is an
   * optional parameter and has a default value of 0 if not provided
   */
    explicit IgmpV2Layer(IgmpType type,
                         const IPv4Address& groupAddr = IPv4Address(),
...
class IgmpV3QueryLayer : public IgmpLayer {
   ...
   * stored in
   */
    IgmpV3QueryLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                     Packet* packet);

    /**
   * A constructor that allocates a new IGMPv3 membership query
   * @param[in] multicastAddr The multicast address to set. This is an optional
   * parameter and has a default value of unspecified/zero IPv4 address if not
   * provided
   * @param[in] maxResponseTime The max response time to set. This is an
   * optional parameter and has a default value of 0 if not provided
   * @param[in] s_qrv A 1-byte value representing the value in Suppress
   * Router-side Processing Flag + Querier's Robustness Variable
   * (igmpv3_query_header#s_qrv field). This is an optional parameter and has a
   * default value of 0 if not provided
   */
    explicit IgmpV3QueryLayer(const IPv4Address& multicastAddr = IPv4Address(),
                              uint8_t maxResponseTime = 0, uint8_t s_qrv = 0);

    /**
   * Get a pointer to the raw IGMPv3 membership query header. Notice this points
   ...
   */
    ...
    /**
   * @return The number of source addresses in this message (as extracted from
   * the igmpv3_query_header#numOfSources field)
   */
    uint16_t getSourceAddressCount() const;
   ...
   * @return The IPv4 source address, or IPv4Address#Zero if index is out of
   * bounds (of the message or of the layer)
   */
    IPv4Address getSourceAddressAtIndex(int index) const;

    /**
   * Add a new source address at the end of the source address list. The
   * igmpv3_query_header#numOfSources field will be incremented accordingly
   * @param[in] addr The IPv4 source address to add
   * @return True if source address was added successfully or false otherwise.
   * If false is returned an appropriate error message will be printed to log
   */
    bool addSourceAddress(const IPv4Address& addr);

    /**
   * Add a new source address at a certain index of the source address list. The
   * igmpv3_query_header#numOfSources field will be incremented accordingly
   * @param[in] addr The IPv4 source address to add
   ...
   */
    ...
    /**
   * Remove a source address at a certain index. The
   * igmpv3_query_header#numOfSources field will be decremented accordingly
   * @param[in] index The index of the source address to be removed
   * @return True if source address was removed successfully or false otherwise.
   * If false is returned an appropriate error message will be printed to log
   */
    bool removeSourceAddressAtIndex(int index);

    /**
   * Remove all source addresses in the message. The
   * igmpv3_query_header#numOfSources field will be set to 0
   * @return True if all source addresses were cleared successfully or false
   * otherwise. If false is returned an appropriate error message will be
   * printed to log
   */
   ...
   */
    ...
    /**
   * @return The message size in bytes which include the size of the basic
   * header + the size of the source address list
   */
...
class IgmpV3ReportLayer : public IgmpLayer {
   ...
   * stored in
   */
    IgmpV3ReportLayer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                      Packet* packet)
        : IgmpLayer(data, dataLen, prevLayer, packet, IGMPv3) {}

    /**
   * A constructor that allocates a new IGMPv3 membership report with 0 group
   * addresses
   */
    IgmpV3ReportLayer()
        : IgmpLayer(IgmpType_MembershipReportV3, IPv4Address(), 0, IGMPv3) {}

    /**
   * Get a pointer to the raw IGMPv3 membership report header. Notice this
   * points directly to the data, so every change will change the actual packet
   * data
   ...
   */
    ...
    /**
   * @return A pointer to the first group record or NULL if no group records
   * exist. Notice the return value is a pointer to the real data, so changes in
   * the return value will affect the packet data
   */
    igmpv3_group_record* getFirstGroupRecord() const;

    /**
   * Get the group record that comes next to a given group record. If
   * "groupRecord" is NULL then NULL will be returned. If "groupRecord" is the
   * last group record or if it is out of layer bounds NULL will be returned
   * also. Notice the return value is a pointer to the real data casted to
   * igmpv3_group_record type (as opposed to a copy of the option data). So
   * changes in the return value will affect the packet data
   * @param[in] groupRecord The group record to start searching from
   * @return The next group record or NULL if "groupRecord" is NULL, last or out
   * of layer bounds
   */
    igmpv3_group_record*
    getNextGroupRecord(igmpv3_group_record* groupRecord) const;

    /**
   * Add a new group record at a the end of the group record list. The
   * igmpv3_report_header#numOfGroupRecords field will be incremented
   * accordingly
   ...
   * @param[in] sourceAddresses A vector containing all the source addresses of
   * the new group record
   * @return The method constructs a new group record, adds it to the end of the
   * group record list of IGMPv3 report message and returns a pointer to the new
   * message. If something went wrong in creating or adding the new group record
   * a NULL value is returned and an appropriate error message is printed to log
   */
    igmpv3_group_record*
    addGroupRecord(uint8_t recordType, const IPv4Address& multicastAddress,
   ...
   * @return The method constructs a new group record, adds it to the IGMPv3
   * report message and returns a pointer to the new message. If something went
   * wrong in creating or adding the new group record a NULL value is returned
   * and an appropriate error message is printed to log
   */
    igmpv3_group_record*
    addGroupRecordAtIndex(uint8_t recordType, const IPv4Address& multicastAddress,
                          const std::vector<IPv4Address>& sourceAddresses,
                          int index);

    /**
   * Remove a group record at a certain index. The
   * igmpv3_report_header#numOfGroupRecords field will be decremented
   * accordingly
   * @param[in] index The index of the group record to be removed
   * @return True if group record was removed successfully or false otherwise.
   * If false is returned an appropriate error message will be printed to log
   */
    bool removeGroupRecordAtIndex(int index);

    /**
   * Remove all group records in the message. The
   * igmpv3_report_header#numOfGroupRecords field will be set to 0
   * @return True if all group records were cleared successfully or false
   * otherwise. If false is returned an appropriate error message will be
   * printed to log
   */
   ...
   */
    ...
    /**
   * @return The message size in bytes which include the size of the basic
   * header + the size of the group record list
   */
