...
IDnsResource::IDnsResource(uint8_t* emptyRawData)
    : m_DnsLayer(nullptr), m_OffsetInLayer(0), m_NextResource(nullptr),
      m_DecodedName(""), m_NameLength(0), m_ExternalRawData(emptyRawData) {}
...
size_t IDnsResource::decodeName(const char* encodedName, char* result,
                                int iteration)
{
    ...
    while (wordLength != 0)
    {
        ...
        if ((wordLength & 0xc0) == 0xc0)
        {
            if (curOffsetInLayer + 2 > m_DnsLayer->m_DataLen ||
                encodedNameLength > 255)
                return cleanup(resultPtr, result, encodedNameLength);

            uint16_t offsetInLayer = (wordLength & 0x3f) * 256 +
                                     (0xFF & encodedName[1]) +
                                     m_DnsLayer->m_OffsetAdjustment;
            if (offsetInLayer < sizeof(dnshdr) ||
                offsetInLayer >= m_DnsLayer->m_DataLen)
            {
            ...
            // in this case the length of the pointer is: 1 byte for 0xc0 + 1 byte for
            // the offset itself
            return encodedNameLength + sizeof(uint16_t);
        }
        else
        {
            // return if next word would be outside of the DNS layer or overflow the
            // buffer behind resultPtr
            if (curOffsetInLayer + wordLength + 1 > m_DnsLayer->m_DataLen ||
                encodedNameLength + wordLength > 255)
            {
...
void IDnsResource::encodeName(const std::string& decodedName, char* result,
                              size_t& resultLen)
{
    resultLen = 0;
...
{
    uint16_t dnsClass =
        *(uint16_t*)(getRawData() + m_NameLength + sizeof(uint16_t));
...
void IDnsResource::setDnsClass(DnsClass newClass)
{
    ...
    memcpy(getRawData() + m_NameLength + sizeof(uint16_t), &newClassAsInt,
           sizeof(uint16_t));
}
...
bool IDnsResource::setName(const std::string& newName)
{
    ...
    if (m_DnsLayer != nullptr)
    {
        ...
        {
            if (!m_DnsLayer->extendLayer(m_OffsetInLayer,
                                         encodedNameLen - m_NameLength, this))
            {
                PCPP_LOG_ERROR(
                    "Couldn't set name for DNS query, unable to extend layer");
                return false;
            }
        }
        else if (encodedNameLen < m_NameLength)
        {
            if (!m_DnsLayer->shortenLayer(m_OffsetInLayer,
                                          m_NameLength - encodedNameLen, this))
            {
                PCPP_LOG_ERROR(
                    "Couldn't set name for DNS query, unable to shorten layer");
                return false;
            }
        }
    }
...
{
    uint32_t ttl =
        *(uint32_t*)(getRawData() + m_NameLength + 2 * sizeof(uint16_t));
    return be32toh(ttl);
}
...
void DnsResource::setTTL(uint32_t newTTL)
{
    ...
    memcpy(getRawData() + m_NameLength + 2 * sizeof(uint16_t), &newTTL,
           sizeof(uint32_t));
}
...
size_t DnsResource::getDataLength() const
{
    ...
    // first Due to dataLength which is uint16_t, here m_DnsLayer->m_Data must
    // have at least 2 bytes to read
...
{
    uint8_t* resourceRawData =
        getRawData() + m_NameLength + 3 * sizeof(uint16_t) + sizeof(uint32_t);
    ...
    case DNS_TYPE_A:
    {
        return DnsResourceDataPtr(
            new IPv4DnsResourceData(resourceRawData, dataLength));
    }

    case DNS_TYPE_AAAA:
    {
        return DnsResourceDataPtr(
            new IPv6DnsResourceData(resourceRawData, dataLength));
    }
    ...
    case DNS_TYPE_PTR:
    {
        return DnsResourceDataPtr(new StringDnsResourceData(
            resourceRawData, dataLength,
            const_cast<IDnsResource*>(static_cast<const IDnsResource*>(this))));
    }
    ...
    {
        return DnsResourceDataPtr(
            new GenericDnsResourceData(resourceRawData, dataLength));
    }
    }
}
...
{
    return (size_t)(m_OffsetInLayer + m_NameLength + 3 * sizeof(uint16_t) +
                    sizeof(uint32_t));
}
...
bool DnsResource::setData(IDnsResourceData* data)
{
    ...
    case DNS_TYPE_A:
    {
        ...
        {
            PCPP_LOG_ERROR("DNS record is of type A but given data isn't of type "
                           "IPv4DnsResourceData");
            return false;
        }
        break;
    ...
    case DNS_TYPE_AAAA:
    {
        ...
        {
            PCPP_LOG_ERROR("DNS record is of type AAAA but given data isn't of type "
                           "IPv6DnsResourceData");
            return false;
        }
        break;
    ...
    case DNS_TYPE_PTR:
    {
        if (!data->isTypeOf<StringDnsResourceData>())
        {
            PCPP_LOG_ERROR("DNS record is of type NS, CNAME, DNAM or PTR but given "
                           "data isn't of type StringDnsResourceData");
            return false;
        }
        break;
    }
    ...
    case DNS_TYPE_MX:
    {
        ...
        {
            PCPP_LOG_ERROR("DNS record is of type MX but given data isn't of type "
                           "MxDnsResourceData");
            return false;
        }
        break;
    ...
    {
        PCPP_LOG_ERROR("Cannot convert DNS resource data to byte array, data is "
                       "probably invalid");
        return false;
    }

    size_t dataLengthOffset =
        m_NameLength + (2 * sizeof(uint16_t)) + sizeof(uint32_t);
    ...
    if (m_DnsLayer != nullptr)
    {
        ...
        {
            if (!m_DnsLayer->extendLayer(m_OffsetInLayer + dataOffset,
                                         dataLength - curLength, this))
            {
                PCPP_LOG_ERROR(
                    "Couldn't set data for DNS query, unable to extend layer");
                return false;
            }
        }
        else if (dataLength < curLength)
        {
            if (!m_DnsLayer->shortenLayer(m_OffsetInLayer + dataOffset,
                                          curLength - dataLength, this))
            {
                PCPP_LOG_ERROR(
                    "Couldn't set data for DNS query, unable to shorten layer");
                return false;
            }
        }
    }
...
{
    memcpy(getRawData() + m_NameLength + sizeof(uint16_t), &customValue,
           sizeof(uint16_t));
}
