...
void splitUint32Id(uint32_t uint32Id, uint16_t& uint16IdUpper,
                   uint16_t& uint16IdLower)
{
...
SomeIpLayer::SomeIpLayer(uint16_t serviceID, uint16_t methodID,
                         uint16_t clientID, uint16_t sessionID,
                         uint8_t interfaceVersion, MsgType type,
                         uint8_t returnCode, const uint8_t* const data,
                         size_t dataLen)
{
...
Layer* SomeIpLayer::parseSomeIpLayer(uint8_t* data, size_t dataLen,
                                     Layer* prevLayer, Packet* packet)
{
    ...
    uint32_t lengthBE = 0;
    memcpy(&lengthBE, data + sizeof(uint32_t),
           sizeof(uint32_t)); // length field in SOME/IP header
...
bool SomeIpLayer::isSomeIpPort(uint16_t port)
{
    return SomeIpSdLayer::isSomeIpSdPort(port) ||
           std::any_of(
               m_SomeIpPorts.begin(), m_SomeIpPorts.end(),
               [&](const uint16_t& someIpPort) { return someIpPort == port; });
}

void SomeIpLayer::addSomeIpPort(uint16_t port) { m_SomeIpPorts.insert(port); }

void SomeIpLayer::removeSomeIpPort(uint16_t port) { m_SomeIpPorts.erase(port); }

void SomeIpLayer::removeAllSomeIpPorts() { m_SomeIpPorts.clear(); }
...
void SomeIpLayer::setPayloadLength(uint32_t payloadLength)
{
    someiphdr* hdr = getSomeIpHeader();
    hdr->length =
        htobe32(sizeof(someiphdr) - sizeof(hdr->serviceID) -
                sizeof(hdr->methodID) - sizeof(hdr->length) + payloadLength);
}
...
std::string SomeIpLayer::toString() const
{
    ...
    dataStream << "SOME/IP Layer" << std::hex << ", Service ID: 0x"
               << getServiceID() << ", Method ID: 0x" << getMethodID() << std::dec
...
SomeIpTpLayer::SomeIpTpLayer(uint16_t serviceID, uint16_t methodID,
                             uint16_t clientID, uint16_t sessionID,
                             uint8_t interfaceVersion, MsgType type,
                             uint8_t returnCode, uint32_t offset,
                             bool moreSegmentsFlag, const uint8_t* const data,
                             size_t dataLen)
{
...
{
    return (be32toh(getSomeIpTpHeader()->offsetAndFlag) &
...
{
    uint32_t val = (offset << 4) | (be32toh(getSomeIpTpHeader()->offsetAndFlag) &
                                    ~SOMEIP_TP_OFFSET_MASK);
...
std::string SomeIpTpLayer::toString() const
{
    ...
    dataStream << "SOME/IP-TP Layer" << std::hex << ", Service ID: 0x"
               << getServiceID() << ", Method ID: 0x" << getMethodID() << std::dec
               << ", Length: " << getLengthField();
