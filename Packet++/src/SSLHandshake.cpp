	...
	: SSLHandshakeMessage(data, dataLen, container)
{
	...
	while (true)
	{
		...
		LOG_DBG("Parsing certificate: pos=" << (int)(curPos-data) << "; len=" << certificateLength);
...
SSLx509Certificate* SSLCertificateMessage::getCertificate(int index) const
{
	...
	{
		LOG_DBG("certificate index out of range: asked for index " << index << ", total size is " << m_CertificateList.size());
		return NULL;
	}
