...
namespace pcpp
{
	...
	uint8_t S7CommLayer::getProtocolId() const { return getS7commHeader()->protocolId; }

	uint8_t S7CommLayer::getMsgType() const { return getS7commHeader()->msgType; }

	uint16_t S7CommLayer::getParamLength() const { return be16toh(getS7commHeader()->paramLength); }

	uint16_t S7CommLayer::getPduRef() const { return be16toh(getS7commHeader()->pduRef); }

	uint16_t S7CommLayer::getDataLength() const { return be16toh(getS7commHeader()->dataLength); }

	void S7CommLayer::setMsgType(uint8_t msgType) const { getS7commHeader()->msgType = msgType; }
