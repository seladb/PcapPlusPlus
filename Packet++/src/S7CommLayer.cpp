...
S7CommLayer::S7CommLayer(uint8_t msgType, uint16_t pduRef, uint16_t paramLength,
                         uint16_t dataLength, uint8_t errorClass,
                         uint8_t errorCode)
{
    size_t basicHeaderLen =
        msgType == 0x03 ? sizeof(s7comm_ack_data_hdr) : sizeof(s7commhdr);
