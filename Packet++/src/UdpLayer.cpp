...
namespace pcpp
{
	...
	uint16_t UdpLayer::calculateChecksum(bool writeResultToPacket)
	{
		...
		uint16_t const currChecksumValue = udpHdr->headerChecksum;
		...
		if (m_PrevLayer != nullptr)
		{
			...
			{
				IPv4Address const srcIP = (dynamic_cast<IPv4Layer*>(m_PrevLayer))->getSrcIPv4Address();
				IPv4Address const dstIP = (dynamic_cast<IPv4Layer*>(m_PrevLayer))->getDstIPv4Address();
			...
			{
				IPv6Address const srcIP = (dynamic_cast<IPv6Layer*>(m_PrevLayer))->getSrcIPv6Address();
				IPv6Address const dstIP = (dynamic_cast<IPv6Layer*>(m_PrevLayer))->getDstIPv6Address();
	...
	void UdpLayer::parseNextLayer()
	{
		...
		uint16_t const portDst = getDstPort();
		uint16_t const portSrc = getSrcPort();

		uint8_t* udpData = m_Data + sizeof(udphdr);
		size_t const udpDataLen = m_DataLen - sizeof(udphdr);
		...
		else if (SipLayer::isSipPort(portDst) || SipLayer::isSipPort(portSrc))
		{
			if (SipRequestFirstLine::parseMethod((char*)udpData, udpDataLen) != SipRequestLayer::SipMethodUnknown)
			{
				m_NextLayer = new SipRequestLayer(udpData, udpDataLen, this, m_Packet);
			         ...
			         !SipResponseFirstLine::parseVersion((char*)udpData, udpDataLen).empty())
			{
