    ...
    if (m_PrevLayer != nullptr) {
        ...
        if (m_PrevLayer->getProtocol() == IPv4) {
            ...
            checksumRes = pcpp::computePseudoHdrChecksum(
                (uint8_t*)udpHdr, getDataLen(), IPAddress::IPv4AddressType,
            ...
            checksumRes = computePseudoHdrChecksum(
                (uint8_t*)udpHdr, getDataLen(), IPAddress::IPv6AddressType,
                PACKETPP_IPPROTO_UDP, srcIP, dstIP);

            PCPP_LOG_DEBUG("calculated IPv6 UDP checksum = 0xX"
                           << std::uppercase << std::hex << checksumRes);
        }
    }
...
void UdpLayer::parseNextLayer() {
    ...
    if ((portSrc == 68 && portDst == 67) || (portSrc == 67 && portDst == 68) ||
        ...
        else if (SipResponseFirstLine::parseStatusCode((char*)udpData,
                                                       udpDataLen) !=
    ...
    else if ((DhcpV6Layer::isDhcpV6Port(portSrc) ||
              DhcpV6Layer::isDhcpV6Port(portDst)) &&
        ...
        m_NextLayer =
            SomeIpLayer::parseSomeIpLayer(udpData, udpDataLen, this, m_Packet);
    else if ((WakeOnLanLayer::isWakeOnLanPort(portDst) &&
              WakeOnLanLayer::isDataValid(udpData, udpDataLen)))
...
std::string UdpLayer::toString() const {
    ...
    return "UDP Layer, Src port: " + srcPortStream.str() +
