...
const std::unordered_map<std::string, SipRequestLayer::SipMethod>
    SipMethodStringToEnum{
...
int SipLayer::getContentLength() const {
    ...
    std::transform(contentLengthFieldName.begin(), contentLengthFieldName.end(),
                   contentLengthFieldName.begin(), ::tolower);
...
HeaderField* SipLayer::setContentLength(int contentLength,
    ...
    if (contentLengthField == nullptr) {
        ...
        contentLengthField = insertField(prevField, PCPP_SIP_CONTENT_LENGTH_FIELD,
                                         contentLengthAsString.str());
    } else
...
void SipLayer::parseNextLayer() {
    ...
    if (getContentLength() > 0) {
        m_NextLayer =
            new SdpLayer(m_Data + headerLen, m_DataLen - headerLen, this, m_Packet);
    } else {
        m_NextLayer = new PayloadLayer(m_Data + headerLen, m_DataLen - headerLen,
                                       this, m_Packet);
    }
}
...
void SipLayer::computeCalculateFields() {
    HeaderField* contentLengthField =
        getFieldByName(PCPP_SIP_CONTENT_LENGTH_FIELD);
    if (contentLengthField == nullptr)
        return;
...
SipRequestFirstLine::SipRequestFirstLine(SipRequestLayer* sipRequest)
    : m_SipRequest(sipRequest) {
    m_Method =
        parseMethod((char*)m_SipRequest->m_Data, m_SipRequest->getDataLen());
    ...
    char* endOfFirstLine;
    if ((endOfFirstLine = (char*)memchr(
             (char*)(m_SipRequest->m_Data + m_VersionOffset), '\n',
    ...
    if (Logger::getInstance().isDebugEnabled(PacketLogModuleSipLayer)) {
        std::string method = (m_Method == SipRequestLayer::SipMethodUnknown
                                  ? "Unknown"
                                  : SipMethodEnumToString[m_Method]);
        PCPP_LOG_DEBUG("Method='" << method << "'; SIP version='" << m_Version
                                  << "'; URI='" << getUri() << "'");
    }
}

SipRequestFirstLine::SipRequestFirstLine(
    SipRequestLayer* sipRequest, SipRequestLayer::SipMethod method,
    const std::string& version,
    const std::string& uri) try // throw(SipRequestFirstLineException)
{
    ...
    std::string firstLine =
        SipMethodEnumToString[m_Method] + " " + uri + " " + version + "\r\n";
...
SipRequestLayer::SipMethod SipRequestFirstLine::parseMethod(const char* data,
                                                            size_t dataLen) {
    if (!data || dataLen < 4) {
    ...
    auto methodAdEnum =
        SipMethodStringToEnum.find(std::string(data, data + spaceIndex));
...
void SipRequestFirstLine::parseVersion() {
    ...
    char* verPos = (char*)cross_platform_memmem(
        data, m_SipRequest->getDataLen() - m_UriOffset, " SIP/", 5);
    if (verPos == nullptr) {
        m_Version = "";
        m_VersionOffset = -1;
        return;
    ...
    // verify packet doesn't end before the version, meaning still left place for
    // " SIP/x.y" (7 chars)
    if ((uint16_t)(verPos + 7 - (char*)m_SipRequest->m_Data) >
    ...
    int endOfVerPos = 0;
    while (((verPos + endOfVerPos) <
            (char*)(m_SipRequest->m_Data + m_SipRequest->m_DataLen)) &&
           ((verPos + endOfVerPos)[0] != '\r') &&
           ((verPos + endOfVerPos)[0] != '\n'))
        endOfVerPos++;
...
bool SipRequestFirstLine::setMethod(SipRequestLayer::SipMethod newMethod) {
    ...
    int lengthDifference = SipMethodEnumToString[newMethod].length() -
                           SipMethodEnumToString[m_Method].length();
    ...
    if (lengthDifference != 0) {
        m_SipRequest->shiftFieldsOffset(m_SipRequest->getFirstField(),
                                        lengthDifference);
...
std::string SipRequestFirstLine::getUri() const {
    ...
    if (m_UriOffset != -1 && m_VersionOffset != -1)
        result.assign((char*)(m_SipRequest->m_Data + m_UriOffset),
...
SipRequestLayer::SipRequestLayer(uint8_t* data, size_t dataLen,
...
SipRequestLayer::SipRequestLayer(SipMethod method,
                                 const std::string& requestUri,
                                 const std::string& version) {
    m_Protocol = SIPRequest;
    m_FirstLine = new SipRequestFirstLine(this, method, std::move(version),
                                          std::move(requestUri));
    m_FieldsOffset = m_FirstLine->getSize();
}

SipRequestLayer::SipRequestLayer(const SipRequestLayer& other)
    : SipLayer(other) {
    m_FirstLine = new SipRequestFirstLine(this);
}
...
SipRequestLayer::~SipRequestLayer() { delete m_FirstLine; }
...
const int StatusCodeEnumToInt[77] = {
    100, 180, 181, 182, 183, 199, 200, 202, 204, 300, 301, 302, 305,
...
const std::unordered_map<std::string, SipResponseLayer::SipResponseStatusCode>
    StatusCodeStringToEnumMap{
        {"100", SipResponseLayer::SipResponseStatusCode::Sip100Trying},
        {"180", SipResponseLayer::SipResponseStatusCode::Sip180Ringing},
        {"181",
         SipResponseLayer::SipResponseStatusCode::Sip181CallisBeingForwarded},
        {"182", SipResponseLayer::SipResponseStatusCode::Sip182Queued},
        {"183",
         SipResponseLayer::SipResponseStatusCode::Sip183SessioninProgress},
        {"199",
         SipResponseLayer::SipResponseStatusCode::Sip199EarlyDialogTerminated},
        ...
        {"301",
         SipResponseLayer::SipResponseStatusCode::Sip301MovedPermanently},
        {"302",
         SipResponseLayer::SipResponseStatusCode::Sip302MovedTemporarily},
        {"305", SipResponseLayer::SipResponseStatusCode::Sip305UseProxy},
        {"380",
         SipResponseLayer::SipResponseStatusCode::Sip380AlternativeService},
        ...
        {"405",
         SipResponseLayer::SipResponseStatusCode::Sip405MethodNotAllowed},
        {"406", SipResponseLayer::SipResponseStatusCode::Sip406NotAcceptable},
        {"407", SipResponseLayer::SipResponseStatusCode::
        ...
        {"412", SipResponseLayer::SipResponseStatusCode::
                    Sip412ConditionalRequestFailed},
        {"413",
         SipResponseLayer::SipResponseStatusCode::Sip413RequestEntityTooLarge},
        {"414",
         SipResponseLayer::SipResponseStatusCode::Sip414RequestURITooLong},
        {"415",
         SipResponseLayer::SipResponseStatusCode::Sip415UnsupportedMediaType},
        {"416",
         SipResponseLayer::SipResponseStatusCode::Sip416UnsupportedURIScheme},
        {"417", SipResponseLayer::SipResponseStatusCode::
                    Sip417UnknownResourcePriority},
        {"420", SipResponseLayer::SipResponseStatusCode::Sip420BadExtension},
        {"421",
         SipResponseLayer::SipResponseStatusCode::Sip421ExtensionRequired},
        {"422", SipResponseLayer::SipResponseStatusCode::
                    Sip422SessionIntervalTooSmall},
        {"423",
         SipResponseLayer::SipResponseStatusCode::Sip423IntervalTooBrief},
        {"424",
         SipResponseLayer::SipResponseStatusCode::Sip424BadLocationInformation},
        {"425", SipResponseLayer::SipResponseStatusCode::Sip425BadAlertMessage},
        {"428",
         SipResponseLayer::SipResponseStatusCode::Sip428UseIdentityHeader},
        {"429", SipResponseLayer::SipResponseStatusCode::
                    Sip429ProvideReferrerIdentity},
        {"430", SipResponseLayer::SipResponseStatusCode::Sip430FlowFailed},
        {"433",
         SipResponseLayer::SipResponseStatusCode::Sip433AnonymityDisallowed},
        {"436", SipResponseLayer::SipResponseStatusCode::Sip436BadIdentityInfo},
        {"437",
         SipResponseLayer::SipResponseStatusCode::Sip437UnsupportedCertificate},
        {"438",
         SipResponseLayer::SipResponseStatusCode::Sip438InvalidIdentityHeader},
        {"439", SipResponseLayer::SipResponseStatusCode::
                    Sip439FirstHopLacksOutboundSupport},
        {"440",
         SipResponseLayer::SipResponseStatusCode::Sip440MaxBreadthExceeded},
        ...
        {"480",
         SipResponseLayer::SipResponseStatusCode::Sip480TemporarilyUnavailable},
        {"481", SipResponseLayer::SipResponseStatusCode::
        ...
        {"484",
         SipResponseLayer::SipResponseStatusCode::Sip484AddressIncomplete},
        {"485", SipResponseLayer::SipResponseStatusCode::Sip485Ambiguous},
        {"486", SipResponseLayer::SipResponseStatusCode::Sip486BusyHere},
        {"487",
         SipResponseLayer::SipResponseStatusCode::Sip487RequestTerminated},
        {"488",
         SipResponseLayer::SipResponseStatusCode::Sip488NotAcceptableHere},
        ...
        {"494", SipResponseLayer::SipResponseStatusCode::
                    Sip494SecurityAgreementRequired},
        {"500",
         SipResponseLayer::SipResponseStatusCode::Sip500ServerInternalError},
        ...
        {"503",
         SipResponseLayer::SipResponseStatusCode::Sip503ServiceUnavailable},
        {"504", SipResponseLayer::SipResponseStatusCode::Sip504ServerTimeout},
        {"505",
         SipResponseLayer::SipResponseStatusCode::Sip505VersionNotSupported},
        {"513", SipResponseLayer::SipResponseStatusCode::Sip513MessageTooLarge},
        {"555", SipResponseLayer::SipResponseStatusCode::
                    Sip555PushNotificationServiceNotSupported},
        {"580",
         SipResponseLayer::SipResponseStatusCode::Sip580PreconditionFailure},
        {"600", SipResponseLayer::SipResponseStatusCode::Sip600BusyEverywhere},
        {"603", SipResponseLayer::SipResponseStatusCode::Sip603Decline},
        {"604",
         SipResponseLayer::SipResponseStatusCode::Sip604DoesNotExistAnywhere},
...
SipResponseLayer::SipResponseLayer(uint8_t* data, size_t dataLen,
                                   Layer* prevLayer, Packet* packet)
    : SipLayer(data, dataLen, prevLayer, packet) {
...
SipResponseLayer::SipResponseLayer(
    ...
    std::string statusCodeString, const std::string& sipVersion) {
    m_Protocol = SIPResponse;
    m_FirstLine = new SipResponseFirstLine(
        this, std::move(sipVersion), statusCode, std::move(statusCodeString));
    m_FieldsOffset = m_FirstLine->getSize();
}

SipResponseLayer::~SipResponseLayer() { delete m_FirstLine; }

SipResponseLayer::SipResponseLayer(const SipResponseLayer& other)
    : SipLayer(other) {
    m_FirstLine = new SipResponseFirstLine(this);
}
...
bool SipResponseFirstLine::setStatusCode(
    SipResponseLayer::SipResponseStatusCode newStatusCode,
    ...
    int lengthDifference =
        statusCodeString.length() - getStatusCodeString().length();
    ...
    if (lengthDifference > 0) {
        ...
        if (!m_SipResponse->shortenLayer(statusStringOffset,
                                         0 - lengthDifference)) {
            PCPP_LOG_ERROR("Cannot change layer size");
            return false;
        }
    ...
    if (lengthDifference != 0) {
        m_SipResponse->shiftFieldsOffset(m_SipResponse->getFirstField(),
                                         lengthDifference);
        m_SipResponse->m_FieldsOffset += lengthDifference;
    }

    // copy status string
    memcpy(m_SipResponse->m_Data + statusStringOffset, statusCodeString.c_str(),
...
void SipResponseFirstLine::setVersion(const std::string& newVersion) {
    ...
    if (newVersion.length() != m_Version.length()) {
        PCPP_LOG_ERROR("Expected version length is "
                       << m_Version.length()
...
SipResponseLayer::SipResponseStatusCode
SipResponseFirstLine::parseStatusCode(const char* data, size_t dataLen) {
    ...
    auto codeAsEnum =
        StatusCodeStringToEnumMap.find(std::string(statusCodeData, 3));
...
SipResponseFirstLine::SipResponseFirstLine(SipResponseLayer* sipResponse)
    : m_SipResponse(sipResponse) {
    m_Version =
        parseVersion((char*)m_SipResponse->m_Data, m_SipResponse->getDataLen());
    if (m_Version == "") {
        m_StatusCode = SipResponseLayer::SipStatusCodeUnknown;
    } else {
        m_StatusCode = parseStatusCode((char*)m_SipResponse->m_Data,
                                       m_SipResponse->getDataLen());
    }
    ...
    if (Logger::getInstance().isDebugEnabled(PacketLogModuleSipLayer)) {
        int statusCode = (m_StatusCode == SipResponseLayer::SipStatusCodeUnknown
                              ? 0
                              : StatusCodeEnumToInt[m_StatusCode]);
        PCPP_LOG_DEBUG("Version='" << m_Version << "'; Status code=" << statusCode
                                   << " '" << getStatusCodeString() << "'");
    }
}

SipResponseFirstLine::SipResponseFirstLine(
    SipResponseLayer* sipResponse, const std::string& version,
    SipResponseLayer::SipResponseStatusCode statusCode,
    std::string statusCodeString) {
    ...
    std::string firstLine = m_Version + " " + statusCodeAsString.str() + " " +
                            statusCodeString + "\r\n";
...
std::string SipResponseFirstLine::parseVersion(const char* data,
                                               size_t dataLen) {
