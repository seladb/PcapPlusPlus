...
{
    return MacAddress((id >> 40) & 0xFF, (id >> 32) & 0xFF, (id >> 24) & 0xFF,
...
uint64_t StpLayer::macAddressToID(const pcpp::MacAddress& addr)
{
    uint8_t value[6];
    addr.copyTo(value);
    return ((uint64_t(value[0]) << 40) | (uint64_t(value[1]) << 32) |
            (uint64_t(value[2]) << 24) | (uint64_t(value[3]) << 16) |
            (uint64_t(value[4]) << 8) | (uint64_t(value[5])));
}
...
StpLayer* StpLayer::parseStpLayer(uint8_t* data, size_t dataLen,
    ...
    if (dataLen >= sizeof(stp_tcn_bpdu))
    {
        ...
        case 0x00:
            return StpConfigurationBPDULayer::isDataValid(data, dataLen)
                       ? new StpConfigurationBPDULayer(data, dataLen, prevLayer,
                                                       packet)
                       : nullptr;
        case 0x02:
            if (ptr->version == 0x2)
        ...
        case 0x80:
            return StpTopologyChangeBPDULayer::isDataValid(data, dataLen)
                       ? new StpTopologyChangeBPDULayer(data, dataLen, prevLayer,
                                                        packet)
                       : nullptr;
...
void StpTopologyChangeBPDULayer::parseNextLayer()
{
    if (m_DataLen > sizeof(stp_tcn_bpdu))
        m_NextLayer = new PayloadLayer(m_Data, m_DataLen - sizeof(stp_tcn_bpdu),
                                       this, m_Packet);
}

// ---------------------- Class StpConfigurationBPDULayer ----------------------
StpConfigurationBPDULayer::StpConfigurationBPDULayer()
    : StpTopologyChangeBPDULayer(sizeof(stp_conf_bpdu))
{
...
{
    getStpConfHeader()->rootId =
        (getStpConfHeader()->rootId & ~htobe16(0xf000)) | htobe16(value & 0xf000);
}
...
{
    getStpConfHeader()->rootId =
        (getStpConfHeader()->rootId & ~htobe16(0x0fff)) | htobe16(value & 0x0fff);
}
...
{
    getStpConfHeader()->bridgeId =
        (getStpConfHeader()->bridgeId & ~htobe16(0xf000)) |
        htobe16(value & 0xf000);
}
...
{
    getStpConfHeader()->bridgeId =
        (getStpConfHeader()->bridgeId & ~htobe16(0x0fff)) |
        htobe16(value & 0x0fff);
}

void StpConfigurationBPDULayer::setBridgeSystemID(
    const pcpp::MacAddress& value)
{
    setBridgeId((getBridgeId() & (uint64_t(0xffff) << 48)) |
                macAddressToID(value));
}
...
void StpConfigurationBPDULayer::parseNextLayer()
{
    if (m_DataLen > sizeof(stp_conf_bpdu))
        m_NextLayer = new PayloadLayer(m_Data, m_DataLen - sizeof(stp_conf_bpdu),
                                       this, m_Packet);
}

// ---------------------- Class RapidStpLayer ----------------------
RapidStpLayer::RapidStpLayer()
    : StpConfigurationBPDULayer(sizeof(rstp_conf_bpdu))
{
...
void RapidStpLayer::parseNextLayer()
{
    if (m_DataLen > sizeof(rstp_conf_bpdu))
        m_NextLayer = new PayloadLayer(m_Data, m_DataLen - sizeof(rstp_conf_bpdu),
                                       this, m_Packet);
}
...
void MultipleStpLayer::setCISTBridgePriority(uint16_t value)
{
    getMstpHeader()->cistBridgeId =
        (getMstpHeader()->cistBridgeId & ~htobe16(0xf000)) |
        htobe16(value & 0xf000);
}
...
{
    getMstpHeader()->cistBridgeId =
        (getMstpHeader()->cistBridgeId & ~htobe16(0x0fff)) |
        htobe16(value & 0x0fff);
}

void MultipleStpLayer::setCISTBridgeSystemID(const pcpp::MacAddress& value)
{
    setCISTBridgeId((getCISTBridgeId() & (uint64_t(0xffff) << 48)) |
                    macAddressToID(value));
}
...
void MultipleStpLayer::setMstConfigurationName(const std::string& value)
{
    ...
    memcpy(getMstpHeader()->mstConfigName, value.c_str(),
           std::min<size_t>(value.size(), 32));
}
