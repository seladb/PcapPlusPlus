...
DhcpV6OptionType DhcpV6Option::getType() const {
    ...
    if (optionType <= 62 && optionType != 10 && optionType != 35 &&
        optionType != 57 && optionType != 58) {
        return static_cast<DhcpV6OptionType>(optionType);
    }
    if (optionType == 65 || optionType == 66 || optionType == 68 ||
        optionType == 79 || optionType == 112) {
        return static_cast<DhcpV6OptionType>(optionType);
    }
...
DhcpV6Layer::DhcpV6Layer(uint8_t* data, size_t dataLen, Layer* prevLayer,
                         Packet* packet)
    : Layer(data, dataLen, prevLayer, packet) {
    m_Protocol = DHCPv6;
}

DhcpV6Layer::DhcpV6Layer(DhcpV6MessageType messageType,
...
uint32_t DhcpV6Layer::getTransactionID() const {
    ...
    uint32_t result = hdr->transactionId1 << 16 | hdr->transactionId2 << 8 |
                      hdr->transactionId3;
    return result;
}
...
DhcpV6Option DhcpV6Layer::getFirstOptionData() const {
    return m_OptionReader.getFirstTLVRecord(
        getOptionsBasePtr(), getHeaderLen() - sizeof(dhcpv6_header));
}
...
DhcpV6Option DhcpV6Layer::getNextOptionData(DhcpV6Option dhcpv6Option) const {
    return m_OptionReader.getNextTLVRecord(dhcpv6Option, getOptionsBasePtr(),
                                           getHeaderLen() -
                                               sizeof(dhcpv6_header));
}

DhcpV6Option DhcpV6Layer::getOptionData(DhcpV6OptionType option) const {
    return m_OptionReader.getTLVRecord(static_cast<uint32_t>(option),
                                       getOptionsBasePtr(),
...
DhcpV6Option DhcpV6Layer::addOptionAt(const DhcpV6OptionBuilder& optionBuilder,
                                      int offset) {
    ...
    if (!extendLayer(offset, sizeToExtend)) {
        PCPP_LOG_ERROR("Could not extend DhcpLayer in [" << newOpt.getTotalSize()
                                                         << "] bytes");
...
DhcpV6Option
DhcpV6Layer::addOptionAfter(const DhcpV6OptionBuilder& optionBuilder,
...
DhcpV6Option
DhcpV6Layer::addOptionBefore(const DhcpV6OptionBuilder& optionBuilder,
                             DhcpV6OptionType optionType) {
    int offset = 0;
