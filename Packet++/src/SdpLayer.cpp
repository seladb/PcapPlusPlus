...
SdpLayer::SdpLayer(const std::string& username, long sessionID,
    ...
    // must initialize m_Data otherwise addField() will fail while trying to
    // extend the layer initializing in length of 1 but keeping m_DataLen with
    // value of 0. when extending the field m_Data is purged so there isn't a
    // memory leak here
    ...
    std::string originatorFieldValue = username + " " + sessionIDStream.str() +
                                       " " + sessionVersionStream.str() + " " +
                                       networkInfo;
    ...
    addField(PCPP_SDP_TIME_FIELD,
             startTimeStream.str() + " " + stopTimeStream.str());
}
...
IPv4Address SdpLayer::getOwnerIPv4Address() const {
    ...
    std::vector<std::string> tokens =
        splitByWhiteSpaces(originator->getFieldValue());
    if (tokens.size() < 6)
        return IPv4Address::Zero;
...
bool SdpLayer::addMediaDescription(const std::string& mediaType,
                                   uint16_t mediaPort,
