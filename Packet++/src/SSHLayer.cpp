...
SSHHandshakeMessage* SSHHandshakeMessage::tryParse(uint8_t* data, size_t dataLen, Layer* prevLayer, Packet* packet)
{
	...
	{
		LOG_DBG("Data length is smaller than the minimum size of an SSH handshake message. It's probably not an SSH hanshake message");
		return NULL;
	}
	...
	{
		LOG_DBG("Message size is larger than layer size. It's probably not an SSH hanshake message");
		return NULL;
	}

	if (msgBase->paddingLength > msgLength)
	{
		LOG_DBG("Message padding is larger than message size. It's probably not an SSH hanshake message");
		return NULL;
	}
	...
	if (msgBase->messageCode != 20 &&
		...
		{
			LOG_DBG("Unknown message type " << (int)msgBase->messageCode << ". It's probably not an SSH hanshake message");
			return NULL;
		}
...
void SSHKeyExchangeInitMessage::parseMessageAndInitOffsets()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		LOG_DBG("Field offset [" << i << "] = " << offset << ", length = " << fieldLength);
