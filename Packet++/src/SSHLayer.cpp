                                     ...
                                     Layer* prevLayer, Packet* packet)
{
    ...
    SSHHandshakeMessage* sshHandshakeMessage =
        SSHHandshakeMessage::tryParse(data, dataLen, prevLayer, packet);
...
void SSHLayer::parseNextLayer()
{
    ...
    if (m_DataLen <= headerLen)
        return;
    m_NextLayer = SSHLayer::createSSHMessage(
                                                             ...
                                                             Packet* packet)
{
    ...
    if (data[0] == 0x53 && data[1] == 0x53 && data[2] == 0x48 &&
        data[3] == 0x2d && data[dataLen - 1] == 0x0a)
...
SSHHandshakeMessage::getMessageType() const
{
    ...
    if (messageCode == 20 || messageCode == 21 ||
        ...
        return static_cast<SSHHandshakeMessage::SSHHandshakeMessageType>(
            messageCode);
                                                   ...
                                                   Packet* packet)
{
    ...
    SSHHandshakeMessage::ssh_message_base* msgBase =
...
SSHKeyExchangeInitMessage::SSHKeyExchangeInitMessage(uint8_t* data,
                                                     size_t dataLen,
                                                     Layer* prevLayer,
                                                     Packet* packet)
...
void SSHKeyExchangeInitMessage::parseMessageAndInitOffsets()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        size_t fieldLength =
            static_cast<size_t>(be32toh(*(uint32_t*)(m_Data + offset)));
        if (offset + sizeof(uint32_t) + fieldLength > m_DataLen)
            return;

        PCPP_LOG_DEBUG("Field offset [" << i << "] = " << offset
                                        << ", length = " << fieldLength);
...
std::string SSHKeyExchangeInitMessage::getFieldValue(int fieldOffsetIndex)
{
    ...
    return std::string(
        reinterpret_cast<const char*>(m_Data + fieldOffset + sizeof(uint32_t)),
        (size_t)fieldLength);
}
