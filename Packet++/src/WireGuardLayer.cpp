...
namespace pcpp
{
	WireGuardLayer* WireGuardLayer::parseWireGuardLayer(uint8_t* data, size_t dataLen, Layer* prevLayer, Packet* packet)
	{
		...
		auto* wgHeader = reinterpret_cast<wg_common_header*>(data);
	...
	{
		uint32_t const messageType = getMessageType();
		switch (messageType)
		{
		case static_cast<uint8_t>(WireGuardMessageType::HandshakeInitiation):
	...
	{
		auto* msg = reinterpret_cast<wg_common_header*>(m_Data);
	...
	bool WireGuardLayer::isDataValid(const uint8_t* data, size_t dataLen)
	{
		...
		uint8_t const messageType = data[0];
	                                                                     ...
	                                                                     const uint8_t mac1[16], const uint8_t mac2[16])
	{
		...
		auto* msg = reinterpret_cast<wg_handshake_initiation*>(m_Data);
	                                                                 ...
	                                                                 const uint8_t mac1[16], const uint8_t mac2[16])
	{
		...
		m_DataLen = messageLen;
		m_Data = new uint8_t[messageLen];
		auto* msg = reinterpret_cast<wg_handshake_response*>(m_Data);
	                                                     ...
	                                                     const uint8_t encryptedCookie[32])
	{
		...
		auto* msg = reinterpret_cast<wg_cookie_reply*>(m_Data);
	                                                         ...
	                                                         const uint8_t* encryptedData, size_t encryptedDataLen)
	{
		...
		auto* msg = reinterpret_cast<wg_transport_data*>(m_Data);
