...
namespace pcpp
{
	...
	std::vector<sctp_gap_ack_block> SctpChunk::getSackGapBlocks() const
	{
		...
		{
			const auto* block = reinterpret_cast<const sctp_gap_ack_block*>(gapBlocksPtr + i * sizeof(sctp_gap_ack_block));
	...
	std::vector<sctp_forward_tsn_stream> SctpChunk::getForwardTsnStreams() const
	{
		...
		{
			const auto* entry = reinterpret_cast<const sctp_forward_tsn_stream*>(streamsPtr + i * sizeof(sctp_forward_tsn_stream));
	...
	std::vector<sctp_iforward_tsn_stream> SctpChunk::getIForwardTsnStreams() const
	{
		...
		for (size_t i = 0; i < numEntries; ++i)
		{
			const auto* entry = reinterpret_cast<const sctp_iforward_tsn_stream*>(streamsPtr + i * sizeof(sctp_iforward_tsn_stream));
