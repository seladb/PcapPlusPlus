...
void SomeIpSdOption::initStdFields(OptionType type)
{
    ...
    optionHdr->length = htobe16((uint16_t)(m_DataLen - sizeof(optionHdr->length) -
                                           sizeof(optionHdr->type)));
}
    ...
    const std::string& configurationString)
{
    ...
    std::memcpy(getDataPtr() + sizeof(someipsdhdroptionsbase),
                configurationString.c_str(), configurationString.length());
}
                             ...
                             uint8_t returnCode, uint8_t flags)
{
    ...
    setPayloadLength(sizeof(uint32_t) *
                     3); // Flags+Reserved, Length Entries, Length Options
...
const SomeIpSdLayer::OptionsVec SomeIpSdLayer::getOptions() const
{
    ...
    while (remainingLen > 0)
    {
        ...
        SomeIpSdOption::OptionType optionType =
            static_cast<SomeIpSdOption::OptionType>(hdr->type);
...
SomeIpSdLayer::getOptionsFromEntry(uint32_t index) const
{
    ...
    size_t offsetToEntry = sizeof(someipsdhdr) + sizeof(uint32_t) +
                           index * sizeof(SomeIpSdEntry::someipsdhdrentry);
    SomeIpSdEntry::someipsdhdrentry* hdrEntry =
        (SomeIpSdEntry::someipsdhdrentry*)(m_Data + offsetToEntry);
...
bool SomeIpSdLayer::countOptions(uint32_t& count, const uint8_t* data)
{
    ...
    while (len < lenOptions)
    {
        ...
        uint32_t lenOption = be16toh(*((uint16_t*)(data + offsetOption + len))) +
                             3 * sizeof(uint8_t);
        len += lenOption;
...
bool SomeIpSdLayer::addOptionIndex(uint32_t indexEntry, uint32_t indexOffset)
{
  ...
  */
    ...
    size_t offsetToAddAt = sizeof(someipsdhdr) + sizeof(uint32_t) +
                           indexEntry * sizeof(SomeIpSdEntry::someipsdhdrentry);
