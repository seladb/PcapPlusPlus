...
public:
	...
	{
		return new IPReassembly::IPv4PacketKey(be16toh(m_IPLayer->getIPv4Header()->ipId), m_IPLayer->getSrcIpAddress().toUInt(), m_IPLayer->getDstIpAddress().toUInt()); // TODO: delete toUInt() when migration has completed
	}
...
public:
	...
	IPReassembly::PacketKey* createPacketKey()
	{
		return new IPReassembly::IPv6PacketKey(be32toh(m_FragHeader->getFragHeader()->id), m_IPLayer->getSrcIpAddress(), m_IPLayer->getDstIpAddress());
	}
