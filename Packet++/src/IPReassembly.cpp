...
Packet* IPReassembly::processPacket(Packet* fragment, ReassemblyStatus& status, ProtocolType parseUntil, OsiModelLayer parseUntilLayer)
{
	...
	{
		LOG_DBG("Got a non-IP packet, returning packet to user");
		status = NON_IP_PACKET;
		return fragment;
	}
	...
	{
		LOG_DBG("Got a non fragment packet with FragID=0x" << std::hex << fragWrapper->getFragmentId() << ", returning packet to user");
		status = NON_FRAGMENT;
		return fragment;
	}
	...
	{
		LOG_DBG("Got new packet with FragID=0x" << std::hex << fragWrapper->getFragmentId() << ", allocating place in map");
	...
	if (fragWrapper->isFirstFragment())
	{
		...
		{
			LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Got first fragment, allocating RawPacket");
		...
		{
			LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Got duplicated first fragment");
			status = FRAGMENT;
			return NULL;
		}
	}
	...
	{
		LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Got fragment");
		...
		if (fragData->currentOffset == fragOffset)
		{
			...
			{
				LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Fragment is malformed");
				status = MALFORMED_FRAGMENT;
				return NULL;
			}

			LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Found next matching fragment with offset " << fragOffset << ", adding fragment data to reassembled packet");
		...
		{
			LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Got out-of-ordered fragment with offset " << fragOffset << " (expected: " << fragData->currentOffset << "). Adding it to out-of-order list");
		...
		{
			LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Got a fragment with an offset that was already seen: " << fragOffset << " (current offset is: " << fragData->currentOffset << "), probably duplicated fragment");
		}
	...
	if (gotLastFragment)
	{
		LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Reassembly process completed, allocating a packet and returning it");
		...
		LOG_DBG("[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Deleting fragment data from map");
...
void IPReassembly::addNewFragment(uint32_t hash, IPFragmentData* fragData)
{
	...
	if (m_PacketLRU.put(hash, &packetRemoved) == 1) // this means LRU list was full and the least recently used item was removed
	{
		...
		LOG_DBG("Reached maximum packet capacity, removing data for FragID=0x" << std::hex << dataRemoved->fragmentID);
		delete dataRemoved;
		m_FragmentMap.erase(iter);
...
{
	LOG_DBG("[FragID=0x" << std::hex << fragData->fragmentID << "] Searching out-of-order fragment list for the next fragment");
	...
	while (!foundLastSegment)
	{
		...
		while (index < (int)fragData->outOfOrderFragments.size())
		{
			...
			if (fragData->currentOffset == frag->fragmentOffset)
			{
				...
				LOG_DBG("[FragID=0x" << std::hex << fragData->fragmentID << "] Found the next matching fragment in out-of-order list with offset " << frag->fragmentOffset << ", adding its data to reassembled packet");
				...
				{
					LOG_DBG("[FragID=0x" << std::hex << fragData->fragmentID << "] Found last fragment inside out-of-order list");
		...
		if (!foundOutOfOrderFrag)
		{
			...
			LOG_DBG("[FragID=0x" << std::hex << fragData->fragmentID << "] Didn't find the next fragment in out-of-order list");
			break;
		}
	}
