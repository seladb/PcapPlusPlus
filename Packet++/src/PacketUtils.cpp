...
namespace experimental
{
	...
	uint32_t hash5TupleWithoutChecks(Packet *packet, const Layer *networkLayer, const Layer *transportLayer)
	{
		...
		int srcPosition = (portDst < portSrc); // 0 if portDst >= portSrc, 1 otherwise
	...
	uint32_t hash5Tuple(Packet *packet)
	{
		if(packet->isPacketOfType(ICMP)) // TODO: Does this check matter? The ICMP is encapsulated into IP layer.
			return 0;
		...
		return hash5TupleWithoutChecks(packet, networkLayer, transportLayer);
	}
