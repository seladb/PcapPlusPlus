...
uint16_t computeChecksum(ScalarBuffer<uint16_t> vec[], size_t vecSize)
{
    ...
    for (size_t i = 0; i < vecSize; i++)
    {
        ...
        if (vec[i].len % 2)
        {
            ...
            PCPP_LOG_DEBUG("1 byte left, adding value: 0x" << std::uppercase
                                                           << std::hex << lastByte);
            // We have read the latest byte manually but this byte should be properly
            // interpreted as a 0xFF on LE and a 0xFF00 on BE to have a proper
    ...
    PCPP_LOG_DEBUG("Calculated checksum = " << sum << ", 0x" << std::uppercase
                                            << std::hex << result);
                                  ...
                                  IPAddress dstIPAddress)
{
    PCPP_LOG_DEBUG("Compute pseudo header checksum.\n DataLen = "
                   ...
                   << "SrcIP = " << srcIPAddress << "DstIP = " << dstIPAddress);
    ...
    PCPP_LOG_DEBUG("Pseudo header checksum = 0xX" << std::uppercase << std::hex
                                                  << checksumRes);
...
uint32_t hash2Tuple(Packet* packet)
{
    ...
    else
    {
        ...
        if ((uint64_t)ipv6Layer->getIPv6Header()->ipDst <
                (uint64_t)ipv6Layer->getIPv6Header()->ipSrc &&
            (uint64_t)(ipv6Layer->getIPv6Header()->ipDst + 8) <
                (uint64_t)(ipv6Layer->getIPv6Header()->ipSrc + 8))
            srcPosition = 1;
