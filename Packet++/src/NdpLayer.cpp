...
NdpOption NdpOptionBuilder::build() const
{
    ...
    recordBuffer[1] = static_cast<uint8_t>(
        optionSizeWithPadding / 8); // length value is stored in units of 8 octets
                                             ...
                                             const IPv6Address& targetIP)
{
    ...
    pHdr->type =
        static_cast<uint8_t>(ICMPv6MessageType::ICMPv6_NEIGHBOR_SOLICITATION);
    pHdr->code = code;
...
std::string NDPNeighborSolicitationLayer::toString() const
{
    ...
    hasLinkLayerAddress()
        ? typeStream << ", SourceMAC: " + getLinkLayerAddress().toString()
                                              ...
                                              bool overrideFlag)
{
    ...
    pHdr->type =
        static_cast<uint8_t>(ICMPv6MessageType::ICMPv6_NEIGHBOR_ADVERTISEMENT);
    pHdr->code = code;
...
std::string NDPNeighborAdvertisementLayer::toString() const
{
    ...
    typeStream << "ICMPv6 Layer, Neighbor Advertisement Message, TargetIP: "
               << getTargetIP().toString();
    hasTargetMacInfo() ? typeStream << ", TargetMAC: " + getTargetMac().toString()
                       : typeStream << ", no Option";
