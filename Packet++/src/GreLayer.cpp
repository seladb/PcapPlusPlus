...
uint8_t* GreLayer::getFieldValue(GreField field,
                                 bool returnOffsetEvenIfFieldMissing) const
{
    ...
    for (int curFieldAsInt = static_cast<int>(GreChecksumOrRouting);
         curFieldAsInt <
         4 /* this value is out of scope of GreField enum values */;
         ++curFieldAsInt)
    {
...
bool GREv0Layer::unsetChecksum()
{
    ...
    if (!needToShortenLayer) // meaning routing bit is set - only zero the
                             // checksum field
    {
        uint16_t* checksumPtr = (uint16_t*)(m_Data + offset);
        *checksumPtr = 0;
...
std::string GREv0Layer::toString() const { return "GRE Layer, version 0"; }
...
std::string GREv1Layer::toString() const { return "GRE Layer, version 1"; }
