...
IPv4OptionBuilder::IPv4OptionBuilder(IPv4OptionTypes optionType,
                                     const std::vector<IPv4Address>& ipList)
{
    ...
    for (std::vector<IPv4Address>::const_iterator iter = ipList.begin();
    ...
    const IPv4TimestampOptionValue& timestampValue)
{
    ...
    {
        PCPP_LOG_ERROR("Cannot build timestamp option of type "
    ...
    {
        PCPP_LOG_ERROR("Cannot build timestamp option of type "
                       "IPv4TimestampOptionValue::TimestampAndIP because number of "
                       "timestamps and IP addresses is not equal");
        m_BuilderParamsValid = false;
        return;
    }

    m_RecValueLen =
        timestampValue.timestamps.size() * sizeof(uint32_t) + 2 * sizeof(uint8_t);
...
IPv4Option IPv4OptionBuilder::build() const
{
         ...
         recType == (uint8_t)IPV4OPT_EndOfOptionsList))
    {
        if (m_RecValueLen != 0)
        {
            PCPP_LOG_ERROR("Can't set IPv4 NOP option or IPv4 End-of-options option "
                           "with size different than 0, tried to set size "
                           << (int)m_RecValueLen);
            return IPv4Option(nullptr);
        }
...
void IPv4Layer::initLayerInPacket(bool setTotalLenAsDataLen)
{
    ...
    if (setTotalLenAsDataLen)
    {
        ...
        // if totalLen == 0 this usually means TCP Segmentation Offload (TSO). In
        // this case we should ignore the value of totalLen and look at the data
...
void IPv4Layer::parseNextLayer()
{
    ...
    // fragment
    // TODO: assuming first fragment contains at least L4 header, what if it's not
    // true?
    if (isFragment())
    {
    ...
    case PACKETPP_IPPROTO_ICMP:
        m_NextLayer = IcmpLayer::isDataValid(payload, payloadLen)
                          ? static_cast<Layer*>(
                                new IcmpLayer(payload, payloadLen, this, m_Packet))
                          : static_cast<Layer*>(new PayloadLayer(
                                payload, payloadLen, this, m_Packet));
        break;
    ...
    case PACKETPP_IPPROTO_IGMP:
        igmpVer = IgmpLayer::getIGMPVerFromData(
            payload,
            std::min<size_t>(payloadLen,
                             be16toh(getIPv4Header()->totalLength) - hdrLen),
            igmpQuery);
        if (igmpVer == IGMPv1)
        ...
        else if (igmpVer == IGMPv3)
        {
            if (igmpQuery)
                ...
                m_NextLayer =
                    new IgmpV3ReportLayer(payload, payloadLen, this, m_Packet);
        }
        else
            m_NextLayer = new PayloadLayer(payload, payloadLen, this, m_Packet);
        break;
    case PACKETPP_IPPROTO_AH:
        m_NextLayer = AuthenticationHeaderLayer::isDataValid(payload, payloadLen)
                          ? static_cast<Layer*>(new AuthenticationHeaderLayer(
                                payload, payloadLen, this, m_Packet))
                          : static_cast<Layer*>(new PayloadLayer(
                                payload, payloadLen, this, m_Packet));
        break;
    case PACKETPP_IPPROTO_ESP:
        m_NextLayer = ESPLayer::isDataValid(payload, payloadLen)
                          ? static_cast<Layer*>(
                                new ESPLayer(payload, payloadLen, this, m_Packet))
                          : static_cast<Layer*>(new PayloadLayer(
                                payload, payloadLen, this, m_Packet));
        break;
...
{
    return m_OptionReader.getNextTLVRecord(option, getOptionsBasePtr(),
                                           getHeaderLen() - sizeof(iphdr));
}
...
void IPv4Layer::adjustOptionsTrailer(size_t totalOptSize)
{
    ...
    m_TempHeaderExtension = 0;
    getIPv4Header()->internetHeaderLength =
                                  ...
                                  int offset)
{
    ...
    {
        PCPP_LOG_ERROR("Cannot add option - adding this option will exceed IPv4 "
                       "total option size which is "
                       << IPV4_MAX_OPT_SIZE);
    ...
    {
        PCPP_LOG_ERROR("Could not extend IPv4Layer in [" << sizeToExtend
                                                         << "] bytes");
        newOption.purgeRecordData();
        return IPv4Option(nullptr);
    }
    ...
    // extend or shorten the layer and the extend or shorten methods need to know
    // the accurate current size of the header. m_TempHeaderExtension will be
    // added to the length extracted from getIPv4Header()->internetHeaderLength as
    // the temp new size
...
IPv4Option IPv4Layer::addOptionAfter(const IPv4OptionBuilder& optionBuilder,
                                     IPv4OptionTypes prevOptionType)
{
    int offset = 0;
