...
uint8_t NtpLayer::getVersion() const { return getNtpHeader()->version; }

void NtpLayer::setVersion(uint8_t val) { getNtpHeader()->version = val; }
...
void NtpLayer::setMode(Mode val) { getNtpHeader()->mode = val; }

uint8_t NtpLayer::getStratum() const { return getNtpHeader()->stratum; }

void NtpLayer::setStratum(uint8_t val) { getNtpHeader()->stratum = val; }
...
int8_t NtpLayer::getPrecision() const { return getNtpHeader()->precision; }

void NtpLayer::setPrecision(int8_t val) { getNtpHeader()->precision = val; }

double NtpLayer::getPrecisionInSecs() const { return pow(2, getPrecision()); }

uint32_t NtpLayer::getRootDelay() const { return getNtpHeader()->rootDelay; }

void NtpLayer::setRootDelay(uint32_t val) { getNtpHeader()->rootDelay = val; }
    ...
    if (stratum == 0) {
        ...
        case 4: {
            ...
            case KissODeath::NKEY:
                return "No key found.  Either the key was never installed or is not "
                       "trusted";
            case KissODeath::RATE:
                return "Rate exceeded.  The server has temporarily denied access "
                       "because the client exceeded the rate "
                       "threshold";
            case KissODeath::RMOT:
                return "Somebody is tinkering with the association from a remote host "
                       "running ntpdc.  Not to worry "
                       "unless some rascal has stolen your keys";
            case KissODeath::STEP:
                return "A step change in system time has occurred, but the association "
        ...
        case 4: {
            ...
            case ClockSource::DCFp:
                return "Meinberg DCF77 with phase modulation)/pseudo random phase "
                       "modulation";
        ...
        // TODO: Support IPv6 cases for NTPv4, it equals to MD5 hash of first four
        // octets of IPv6 address
...
uint32_t NtpLayer::getKeyID() const {
    ...
    case 4: {
        ...
        if (m_DataLen == (sizeof(ntp_header) + sizeof(ntp_v4_auth_md5))) {
            ntp_v4_auth_md5* header =
                (ntp_v4_auth_md5*)(m_Data + m_DataLen - sizeof(ntp_v4_auth_md5));
            return header->keyID;
        }
        if (m_DataLen == (sizeof(ntp_header) + sizeof(ntp_v4_auth_sha1))) {
            ntp_v4_auth_sha1* header =
                (ntp_v4_auth_sha1*)(m_Data + m_DataLen - sizeof(ntp_v4_auth_sha1));
            return header->keyID;
        }

        PCPP_LOG_ERROR(
            "NTP authentication parsing with extension fields are not supported");
        return 0;
    }
    default: {
...
double NtpLayer::convertFromTimestampFormat(const uint64_t val) {
    ...
    double fractionPart =
        netToHost32(((val & 0xFFFFFFFF00000000) >> 32)) / NTP_FRAC;
...
std::string NtpLayer::convertToIsoFormat(const double timestamp) {
    ...
    strftime(buffer, sizeof(buffer) - sizeof(bufferFraction), "%Y-%m-%dT%H:%M:%S",
             timer);

    snprintf(bufferFraction, sizeof(bufferFraction), "%.04lfZ",
             fabs(fractionPart));
...
std::string NtpLayer::toString() const {
    return std::string("NTP Layer v") + std::to_string(getVersion()) +
           ", Mode: " + getModeString();
}
} // namespace pcpp
