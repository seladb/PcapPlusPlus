...
IgmpType IgmpLayer::getType() const {
    ...
    if (type < (uint8_t)IgmpType_MembershipQuery ||
        (type > (uint8_t)IgmpType_LeaveGroup &&
         type < (uint8_t)IgmpType_MulticastTracerouteResponse) ||
        (type > (uint8_t)IgmpType_MulticastTraceroute &&
         type < (uint8_t)IgmpType_MembershipReportV3) ||
        (type > (uint8_t)IgmpType_MembershipReportV3 &&
         type < (uint8_t)IgmpType_MulticastRouterAdvertisement) ||
...
IgmpV3QueryLayer::IgmpV3QueryLayer(uint8_t* data, size_t dataLen,
...
IPv4Address IgmpV3QueryLayer::getSourceAddressAtIndex(int index) const {
    ...
    // verify numOfRecords is a reasonable number that points to data within the
    // packet
...
IgmpV3ReportLayer::getNextGroupRecord(igmpv3_group_record* groupRecord) const {
    ...
    if ((uint8_t*)groupRecord + groupRecord->getRecordLen() - m_Data >=
        (int)getHeaderLen())
        return nullptr;

    igmpv3_group_record* nextGroup =
        (igmpv3_group_record*)((uint8_t*)groupRecord +
                               groupRecord->getRecordLen());
...
igmpv3_group_record* IgmpV3ReportLayer::addGroupRecordAt(
    ...
    igmpv3_group_record* newGroupRecord =
        (igmpv3_group_record*)groupRecordBuffer;
