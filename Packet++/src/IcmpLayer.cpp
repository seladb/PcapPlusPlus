...
bool IcmpLayer::setEchoData(IcmpMessageType echoType, uint16_t id,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_echo_hdr) - sizeof(icmphdr) + dataLen))
        return false;
...
IcmpLayer::setTimestampRequestData(uint16_t id, uint16_t sequence,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_timestamp_request) - sizeof(icmphdr)))
        return nullptr;
    ...
    header->originateTimestamp = htobe32(originateTimestamp.tv_sec * 1000 +
                                         originateTimestamp.tv_usec / 1000);
...
icmp_timestamp_reply* IcmpLayer::setTimestampReplyData(
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_timestamp_reply) - sizeof(icmphdr)))
        return nullptr;
    ...
    header->receiveTimestamp =
        htobe32(receiveTimestamp.tv_sec * 1000 + receiveTimestamp.tv_usec / 1000);
    header->transmitTimestamp = htobe32(transmitTimestamp.tv_sec * 1000 +
                                        transmitTimestamp.tv_usec / 1000);
...
IcmpLayer::setDestUnreachableData(IcmpDestUnreachableCodes code,
    ...
    if (!this->extendLayer(m_DataLen, sizeof(icmp_destination_unreachable) -
                                          sizeof(icmphdr)))
        return nullptr;
...
icmp_source_quench* IcmpLayer::setSourceQuenchdata(IPv4Layer* ipHeader,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_source_quench) - sizeof(icmphdr)))
        return nullptr;
...
icmp_router_advertisement* IcmpLayer::setRouterAdvertisementData(
    ...
    icmp_router_address_structure* curPos =
        (icmp_router_address_structure*)((uint8_t*)header->header +
                                         sizeof(icmp_router_advertisement_hdr));
...
icmp_time_exceeded* IcmpLayer::setTimeExceededData(uint8_t code,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_time_exceeded) - sizeof(icmphdr)))
        return nullptr;
...
icmp_param_problem* IcmpLayer::setParamProblemData(uint8_t code,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_param_problem) - sizeof(icmphdr)))
        return nullptr;
...
IcmpLayer::setAddressMaskRequestData(uint16_t id, uint16_t sequence,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_address_mask_request) - sizeof(icmphdr)))
        return nullptr;
...
icmp_address_mask_reply* IcmpLayer::setAddressMaskReplyData(uint16_t id,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_address_mask_reply) - sizeof(icmphdr)))
        return nullptr;
...
icmp_info_request* IcmpLayer::setInfoRequestData(uint16_t id,
    ...
    if (!this->extendLayer(m_DataLen,
                           sizeof(icmp_info_request) - sizeof(icmphdr)))
        return nullptr;
        ...
        m_NextLayer =
            IPv4Layer::isDataValid(m_Data + headerLen, m_DataLen - headerLen)
                ? static_cast<Layer*>(new IPv4Layer(
                      m_Data + headerLen, m_DataLen - headerLen, this, m_Packet))
...
size_t IcmpLayer::getHeaderLen() const {
    ...
    case ICMP_ROUTER_ADV:
        routerAdvSize = sizeof(icmp_router_advertisement_hdr) +
                        (getRouterAdvertisementData()->header->advertisementCount *
                         sizeof(icmp_router_address_structure));
