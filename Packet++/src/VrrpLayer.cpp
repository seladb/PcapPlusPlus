...
VrrpLayer::VrrpLayer(ProtocolType subProtocol, uint8_t virtualRouterId,
                     uint8_t priority) {
...
uint8_t VrrpLayer::getVersion() const { return getVrrpHeader()->version; }
...
uint8_t VrrpLayer::getVirtualRouterID() const { return getVrrpHeader()->vrId; }

void VrrpLayer::setVirtualRouterID(uint8_t virtualRouterID) {
    getVrrpHeader()->vrId = virtualRouterID;
}

uint8_t VrrpLayer::getPriority() const { return getVrrpHeader()->priority; }
...
bool VrrpLayer::addIPAddressesAt(const std::vector<IPAddress>& ipAddresses,
                                 int offset) {
    if (offset > (int)getHeaderLen()) {
        PCPP_LOG_ERROR("Cannot add virtual IP address offset("
                       << offset << ") is out of layer bounds");
        return false;
    }
    ...
    for (auto ipAddress : ipAddresses) {
        if (!isIPAddressValid(ipAddress)) {
            PCPP_LOG_ERROR(
                "Cannot add virtual IP address, for IP address is invalid.");
            return false;
        }
    }

    if (getIPAddressesCount() + ipAddresses.size() >
        VRRP_PACKET_MAX_IP_ADDRESS_NUM) {
        PCPP_LOG_ERROR("Cannot add virtual IP address, for virtual IP address has "
                       "already exceed maximum.");
        return false;
    }
...
bool VrrpLayer::removeIPAddressAtIndex(int index) {
    ...
    if (index < 0 || index >= ipAddressCount) {
        PCPP_LOG_ERROR("Cannot remove virtual IP address, index "
    ...
    for (int i = 0; i < index; i++) {
        if (curIpAddressPtr == nullptr) {
            PCPP_LOG_ERROR("Cannot remove virtual IP address at index "
                           << index << ", cannot find virtual IP address at index "
                           << i);
            return false;
        }
...
bool VrrpLayer::removeAllIPAddresses() {
    ...
    if (!shortenLayer((int)offset, packetLen - offset)) {
        PCPP_LOG_ERROR(
            "Cannot remove all virtual IP address(es), cannot shorted layer");
        return false;
    }
...
void VrrpLayer::copyIPAddressToData(uint8_t* data,
...
VrrpV2Layer::VrrpV2Layer(uint8_t virtualRouterId, uint8_t priority,
                         uint8_t advInt, uint8_t authType)
...
VrrpV3Layer::VrrpV3Layer(IPAddress::AddressType addressType,
...
uint16_t VrrpV3Layer::calculateChecksum() const {
    ...
    uint16_t checksum;
    if (getAddressType() == IPAddress::IPv4AddressType) {
        checksum = computePseudoHdrChecksum(
            (uint8_t*)vrrpHeader, getDataLen(), IPAddress::IPv4AddressType,
            PACKETPP_IPPROTO_VRRP, srcIPAddr, dstIPAddr);
    } else {
        checksum = computePseudoHdrChecksum(
            (uint8_t*)vrrpHeader, getDataLen(), IPAddress::IPv6AddressType,
            PACKETPP_IPPROTO_VRRP, srcIPAddr, dstIPAddr);
    }
