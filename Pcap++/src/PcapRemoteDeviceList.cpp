...
PcapRemoteDeviceList::getRemoteDeviceList(const IPAddress& ipAddress,
                                          uint16_t port)
{
    return PcapRemoteDeviceList::getRemoteDeviceList(ipAddress, port, NULL);
}

PcapRemoteDeviceList* PcapRemoteDeviceList::getRemoteDeviceList(
    ...
    PcapRemoteAuthentication* remoteAuth)
{
    ...
    PCPP_LOG_DEBUG("Searching remote devices on IP: " << ipAddress
                                                      << " and port: " << port);
    char remoteCaptureString[PCAP_BUF_SIZE];
    char errbuf[PCAP_ERRBUF_SIZE];
    std::ostringstream portAsString;
    portAsString << port;
    if (pcap_createsrcstr(remoteCaptureString, PCAP_SRC_IFREMOTE,
                          ipAddress.toString().c_str(),
                          portAsString.str().c_str(), NULL, errbuf) != 0)
    {
        PCPP_LOG_ERROR("Error in creating the remote connection string. Error was: "
                       << errbuf);
        return NULL;
    }
    ...
    if (remoteAuth != NULL)
    {
        PCPP_LOG_DEBUG("Authentication requested. Username: "
                       << remoteAuth->userName
    ...
    if (pcap_findalldevs_ex(remoteCaptureString, pRmAuth, &interfaceList,
                            errorBuf) < 0)
    {
        PCPP_LOG_ERROR(
            "Error retrieving device on remote machine. Error string is: "
            << errorBuf);
        return NULL;
    }
...
PcapRemoteDevice* PcapRemoteDeviceList::getRemoteDeviceByIP(
...
PcapRemoteDevice*
PcapRemoteDeviceList::getRemoteDeviceByIP(const IPAddress& ipAddr) const
{
    if (!ipAddr.isValid())
    {
...
PcapRemoteDevice*
PcapRemoteDeviceList::getRemoteDeviceByIP(const IPv4Address& ip4Addr) const
{
    PCPP_LOG_DEBUG("Searching all remote devices in list...");
    for (ConstRemoteDeviceListIterator devIter = m_RemoteDeviceList.begin();
         devIter != m_RemoteDeviceList.end(); devIter++)
    {
        ...
        {
            if (Logger::getInstance().isDebugEnabled(PcapLogModuleRemoteDevice) &&
                addrIter->addr != NULL)
            {
...
PcapRemoteDevice*
PcapRemoteDeviceList::getRemoteDeviceByIP(const IPv6Address& ip6Addr) const
{
    PCPP_LOG_DEBUG("Searching all remote devices in list...");
    for (ConstRemoteDeviceListIterator devIter = m_RemoteDeviceList.begin();
         devIter != m_RemoteDeviceList.end(); devIter++)
...
void PcapRemoteDeviceList::setRemoteMachineIpAddress(
    const IPAddress& ipAddress)
{
    if (!ipAddress.isValid())
    {
        PCPP_LOG_ERROR(
            "Trying to set an invalid IP address to PcapRemoteDeviceList");
        return;
    }
...
void PcapRemoteDeviceList::setRemoteAuthentication(
