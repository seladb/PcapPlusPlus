                              ...
                              void* userCookie)
{
    ...
    // verify that it's an ARP packet (although it must be because I set an ARP
    // reply filter on the interface)
    if (!packet.isPacketOfType(ARP))
        return;

    // extract the ARP layer from the packet
    ArpLayer* arpReplyLayer =
        packet.getLayerOfType<ArpLayer>(true); // lookup in reverse order
    ...
    // verify the ARP response is the response for out request (and not some
    // arbitrary ARP response)
                            ...
                            IPv4Address sourceIP, int arpTimeout) const
{
    ...
    // since packet capture is done on another thread, I use a conditional mutex
    // with timeout to synchronize between the capture thread and the main thread.
    // When the capture thread starts running the main thread is blocking on the
    // conditional mutex. When the ARP response is captured the capture thread
    // signals the main thread and the main thread stops capturing and continues
    // to the next iteration. If a timeout passes and no ARP response is captured,
    // the main thread stops capturing

    std::mutex mutex;
    std::condition_variable cond;

    // this is the token that passes between the 2 threads. It contains pointers
    // to the conditional mutex, the target IP for identifying the ARP response,
    // the iteration index and a timestamp to calculate the response time
    ...
    // "arpPacketReceived" is running on that thread
    device->startCapture(arpPacketReceived, &data);

    // send the ARP request
    device->sendPacket(&arpRequest);

    // block on the conditional mutex until capture thread signals or until
    // timeout expires cppcheck-suppress localMutex
                                ...
                                void* userCookie)
{
    ...
    // verify that it's an DNS packet (although it must be because DNS port filter
    // was set on the interface)
    if (!packet.isPacketOfType(DNS))
        return;

    // extract the DNS layer from the packet
    DnsLayer* dnsResponseLayer =
        packet.getLayerOfType<DnsLayer>(true); // lookup in reverse order
    ...
    // DNS resolving can be recursive as many DNS responses contain multiple
    // answers with recursive canonical names (CNAME) for the hostname. For
    // example: a DNS response for www.a.com can have multiple answers:
    //- First with CNAME: www.a.com -> www.b.com
    //- Second with CNAME: www.b.com -> www.c.com
    //- Third with resolving: www.c.com -> 1.1.1.1
    // So the search must be recursive until an IPv4 resolving is found or until
    // no hostname or canonical name are found (and then return)
    ...
    data->result =
        dnsAnswer->getData()->castAs<IPv4DnsResourceData>()->getIpAddress();
...
IPv4Address NetworkUtils::getIPv4Address(const std::string& hostname,
                                         ...
                                         IPv4Address gatewayIP) const
{
    ...
    // with timeout to synchronize between the capture thread and the main thread.
    // When the capture thread starts running the main thread is blocking on the
    // conditional mutex. When the DNS response are captured the capture thread
    // signals the main thread and the main thread stops capturing and continues
    // to the next iteration. if a timeout passes and no DNS response is captured,
    // the main thread stops capturing
    ...
    DNSReceivedData data = {
        mutex, cond, hostname, transactionID, clock(), IPv4Address::Zero, 0, 0};

    struct timeval now;
    gettimeofday(&now, nullptr);

    // start capturing. The capture is done on another thread, hence
    // "dnsResponseReceived" is running on that thread
