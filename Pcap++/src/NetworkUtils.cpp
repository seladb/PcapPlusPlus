...
namespace pcpp
{
	                                       ...
	                                       MacAddress sourceMac, IPv4Address sourceIP, int arpTimeout) const
	{
		...
		// since packet capture is done on another thread, I use a conditional mutex with timeout to synchronize between
		// the capture thread and the main thread. When the capture thread starts running the main thread is blocking on
		// the conditional mutex. When the ARP response is captured the capture thread signals the main thread and the
		// main thread stops capturing and continues to the next iteration. If a timeout passes and no ARP response is
		// captured, the main thread stops capturing
	...
	static void dnsResponseReceived(RawPacket* rawPacket, PcapLiveDevice*, void* userCookie)
	{
		...
		// DNS resolving can be recursive as many DNS responses contain multiple answers with recursive canonical names
		// (CNAME) for the hostname. For example: a DNS response for www.a.com can have multiple answers:
		//- First with CNAME: www.a.com -> www.b.com
		//- Second with CNAME: www.b.com -> www.c.com
		//- Third with resolving: www.c.com -> 1.1.1.1
		// So the search must be recursive until an IPv4 resolving is found or until no hostname or canonical name are
		// found (and then return)
		...
		while (true)
		{
			...
			// if answer contains a cname - continue to search this cname in the packet - hopefully find the IP
			// resolving
			else if (dnsType == DNS_TYPE_CNAME)
			{
				PCPP_LOG_DEBUG("Got a DNS response for hostname '" << hostToFind << "' with CNAME '"
	...
	IPv4Address NetworkUtils::getIPv4Address(const std::string& hostname, PcapLiveDevice* device,
	                                         double& dnsResponseTimeMS, uint32_t& dnsTTL, int dnsTimeout,
	                                         IPv4Address dnsServerIP, IPv4Address gatewayIP) const
	{
		...
		if (!dnsRequest.addLayer(&ethLayer) || !dnsRequest.addLayer(&ipLayer) || !dnsRequest.addLayer(&udpLayer) ||
		    !dnsRequest.addLayer(&dnsLayer))
		{
		...
		// since packet capture is done on another thread, I use a conditional mutex with timeout to synchronize between
		// the capture thread and the main thread. When the capture thread starts running the main thread is blocking on
		// the conditional mutex. When the DNS response are captured the capture thread signals the main thread and the
		// main thread stops capturing and continues to the next iteration. if a timeout passes and no DNS response is
		// captured, the main thread stops capturing
