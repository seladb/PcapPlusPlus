...
void KniDevice::KniThread::cancel() { m_StopThread = true; }
    ...
    : m_Device(NULL), m_MBufMempool(NULL)
{
    ...
    if ((m_MBufMempool = createMempool(mempoolSize, unique, conf.name.c_str())) ==
        NULL)
        return;
...
KniDevice::KniPromiscuousMode KniDevice::getPromiscuous(KniInfoState state)
{
                                      ...
                                      &req))
    {
        ...
        PCPP_LOG_DEBUG("Last known Promiscuous mode for device '"
                       << m_DeviceInfo.name << "' is returned");
        return m_DeviceInfo.promisc;
    }
    return m_DeviceInfo.promisc = (req.ifr_flags & IFF_PROMISC)
                                      ? KniDevice::PROMISC_ENABLE
    ...
    if (timeout <= 0)
    {
        for (;;)
        {
            ...
            if (likely(numOfPktsReceived != 0))
            {
                ...
                if (!m_Capturing.callback(rawPackets, numOfPktsReceived, this,
                                          m_Capturing.userCookie))
                    return 1;
            }
        }
    }
