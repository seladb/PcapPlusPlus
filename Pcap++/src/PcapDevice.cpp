...
bool IPcapDevice::setFilter(string filterAsString)
{
	...
	if (pcap_compile(m_pPcapDescriptor, &prog, filterAsString.c_str(), 1, 0) < 0)
	{
		/*
		* Print out appropriate text, followed by the error message
		* generated by the packet capture library.
		*/
		LOG_ERROR("Error compiling filter. Error message is: %s", pcap_geterr(m_pPcapDescriptor));
		return false;
	}

	LOG_DEBUG("Setting the compiled filter");
	if (pcap_setfilter(m_pPcapDescriptor, &prog) < 0)
	{
		/*
		 ...
		 */
		LOG_ERROR("Error setting a compiled filter. Error message is: %s", pcap_geterr(m_pPcapDescriptor));
		return false;
	}
