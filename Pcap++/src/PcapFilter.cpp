...
void IPFilter::convertToIPAddressWithMask(std::string& ipAddrmodified,
    ...
    // The following code lines verify both ipAddress and ipv4Mask are valid IPv4
    // addresses The IPv4 limitation comes from the fact libPcap/WinPcap/Npcap
    ...
    // If all addresses are IPv4 valid addresses, make sure ipAddress matches the
    // mask. If it's not, mask the address with the mask The reason for doing that
    // is libPcap/WinPcap/Npcap doesn't allow filtering an IP address that doesn't
    // match the mask
