...
PfRingDevice::PfRingDevice(const char* deviceName)
    : m_MacAddress(MacAddress::Zero)
{
...
int PfRingDevice::openSingleRxChannel(const char* deviceName, pfring** ring)
{
    ...
    uint32_t flags =
        PF_RING_PROMISC | PF_RING_HW_TIMESTAMP | PF_RING_DNA_SYMMETRIC_RSS;
...
bool PfRingDevice::setPfRingDeviceClock(pfring* ring)
{
    ...
    {
        PCPP_LOG_DEBUG(
            "Could not set pfring devices clock, pfring_set_device_clock failed");
        return false;
    }
...
bool PfRingDevice::openMultiRxChannels(const uint8_t* channelIds,
                                       int numOfChannelIds)
{
    if (m_DeviceOpened)
    {
    ...
    // It allows opening the device on a channel that doesn't exist, but of course
    ...
    for (int i = 0; i < numOfChannelIds; i++)
    {
        ...
        {
            PCPP_LOG_ERROR("Trying to open the device with a RX channel that doesn't "
                           "exist. Total RX channels are ["
                           << (int)totalChannels << "], tried to open channel ["
                           << (int)channelId << "]");
            return false;
        }
    }
    ...
    for (int i = 0; i < numOfChannelIds; i++)
    {
        ...
        PCPP_LOG_DEBUG("Trying to open device [" << m_DeviceName << "] on channel ["
                                                 << channelId << "]. Channel name ["
                                                 << ringName << "]");
        int res = openSingleRxChannel(ringName.c_str(), &m_PfRingDescriptors[i]);
        if (res == 0)
        {
            PCPP_LOG_DEBUG("Succeeded opening device ["
                           << m_DeviceName << "] on channel [" << channelId
                           << "]. Channel name [" << ringName << "]");
            m_NumOfOpenedRxChannels++;
            continue;
        }
        else if (res == 1)
            PCPP_LOG_ERROR("Couldn't open a ring on channel ["
                           << (int)channelId << "] for device [" << m_DeviceName
                           << "]");
        else if (res == 2)
            PCPP_LOG_ERROR("Unable to enable ring on channel ["
                           << (int)channelId << "] for device [" << m_DeviceName
                           << "]");
    ...
    {
        // if an error occurred, close all rings from index=0 to
        // index=m_NumOfOpenedRxChannels-1 there's no need to close
        // m_PfRingDescriptors[m_NumOfOpenedRxChannels] because it has already been
...
bool PfRingDevice::openMultiRxChannels(uint8_t numOfRxChannelsToOpen,
                                       ChannelDistribution dist)
{
    ...
    {
        PCPP_LOG_ERROR("Cannot open more than [" << MAX_NUM_RX_CHANNELS
                                                 << "] channels");
        return false;
    }

    uint32_t flags = PF_RING_PROMISC | PF_RING_REENTRANT | PF_RING_HW_TIMESTAMP |
    ...
    for (uint8_t channelId = 0; channelId < numOfRxChannelsOnNIC; channelId++)
    {
        ...
        {
            m_PfRingDescriptors[ringsOpen] =
                pfring_open(ringName.str().c_str(), DEFAULT_PF_RING_SNAPLEN, flags);
            ...
            // setting a cluster for all rings in the same channel to enable hashing
            // between them
            if (pfring_set_cluster(m_PfRingDescriptors[ringsOpen], channelId + 1,
                                   clusterType) < 0)
            {
                PCPP_LOG_ERROR("Couldn't set ring ["
                               << (int)ringId << "] in channel [" << (int)channelId
        ...
        if (remainderRings > 0)
        {
            ...
            {
                PCPP_LOG_ERROR("Couldn't set ring ["
                               << (int)(numOfRingsPerRxChannel + 1) << "] in channel ["
            ...
            ringsOpen++;
            remainderRings--;
            PCPP_LOG_DEBUG("Opened " << (int)(numOfRingsPerRxChannel + 1)
                                     << " rings on channel [" << (int)channelId
                                     << "]");
        }
        else
            PCPP_LOG_DEBUG("Opened " << (int)numOfRingsPerRxChannel
                                     << " rings on channel [" << (int)channelId
                                     << "]");
    }
    ...
    for (int i = 0; i < ringsOpen; i++)
    {
        if (pfring_enable_rss_rehash(m_PfRingDescriptors[i]) < 0 ||
            pfring_enable_ring(m_PfRingDescriptors[i]) < 0)
        {
            PCPP_LOG_ERROR("Unable to enable ring [" << i << "] for device ["
                                                     << m_DeviceName << "]");
...
uint8_t PfRingDevice::getTotalNumOfRxChannels() const
{
    ...
    {
        uint32_t flags = PF_RING_PROMISC | PF_RING_REENTRANT |
                         PF_RING_HW_TIMESTAMP | PF_RING_DNA_SYMMETRIC_RSS;
        pfring* ring =
            pfring_open(m_DeviceName.c_str(), DEFAULT_PF_RING_SNAPLEN, flags);
...
bool PfRingDevice::setFilter(std::string filterAsString)
{
    ...
    {
        int res = pfring_set_bpf_filter(m_PfRingDescriptors[i],
                                        (char*)filterAsString.c_str());
        if (res < 0)
        {
            if (res == PF_RING_ERROR_NOT_SUPPORTED)
                PCPP_LOG_ERROR(
                    "BPF filtering isn't supported on current PF_RING version. Please "
                    "re-compile PF_RING with the --enable-bpf flag");
            else
...
bool PfRingDevice::initCoreConfigurationByCoreMask(CoreMask coreMask)
{
    ...
    {
        PCPP_LOG_ERROR("Trying to use a core ["
                       << i << "] that doesn't exist while machine has "
                       << numOfCores << " cores");
        clearCoreConfiguration();
        return false;
    }
...
bool PfRingDevice::startCaptureMultiThread(
    OnPfRingPacketsArriveCallback onPacketsArrive,
    void* onPacketsArriveUserCookie, CoreMask coreMask)
{
    ...
    {
        PCPP_LOG_ERROR(
            "Cannot use a different number of channels and cores. Opened "
            << m_NumOfOpenedRxChannels << " channels but set "
    ...
    for (int coreId = 0; coreId < MAX_NUM_OF_CORES; coreId++)
    {
        ...
        m_CoreConfiguration[coreId].RxThread =
            std::thread(&pcpp::PfRingDevice::captureThreadMain, this, &cond, &mutex,
                        &startThread);
        ...
        int err = pthread_setaffinity_np(
            m_CoreConfiguration[coreId].RxThread.native_handle(), sizeof(cpu_set_t),
            &cpuset);
        if (err != 0)
        {
            PCPP_LOG_ERROR("Error while binding thread to core "
                           << coreId << ": errno=" << err);
            startThread = 1;
            clearCoreConfiguration();
            return false;
...
bool PfRingDevice::startCaptureSingleThread(
    OnPfRingPacketsArriveCallback onPacketsArrive,
    void* onPacketsArriveUserCookie)
{
    ...
    if (m_NumOfOpenedRxChannels != 1)
    {
        PCPP_LOG_ERROR("Cannot start capturing on a single thread when more than 1 "
                       "RX channel is opened");
        return false;
    }

    PCPP_LOG_DEBUG("Trying to start capturing on a single thread for device ["
                   << m_DeviceName << "]");
    ...
    int err =
        pthread_setaffinity_np(m_CoreConfiguration[0].RxThread.native_handle(),
...
void PfRingDevice::captureThreadMain(std::condition_variable* startCond,
                                     ...
                                     const int* startState)
{
    ...
    if (ring == NULL)
    {
        PCPP_LOG_ERROR("Couldn't find ring for core "
                       << coreId << ". Exiting capture thread");
        return;
    }
    ...
    while (!this->m_StopThread)
    {
        ...
        // in multi-threaded mode flag PF_RING_REENTRANT is set, and this flag
        // doesn't work with zero copy so I need to allocate a buffer and set buffer
        // to point to it
        ...
        if (recvRes > 0)
        {
            // if caplen < len it means we don't have the whole packet. Treat this
            // case as packet drop
            // TODO: add this packet to dropped packet stats
            //			if (pktHdr.caplen != pktHdr.len)
            //			{
            //				PCPP_LOG_ERROR("Packet dropped due to
            //len != caplen"); 				continue;
            //			}

            RawPacket rawPacket(buffer, pktHdr.caplen, pktHdr.ts, false);
            this->m_OnPacketsArriveCallback(&rawPacket, 1, coreId, this,
                                            this->m_OnPacketsArriveUserCookie);
        }
        else if (recvRes < 0)
        {
...
void PfRingDevice::getThreadStatistics(SystemCore core,
                                       PfRingStats& stats) const
{
    ...
    if (ring != NULL)
    {
        ...
        {
            PCPP_LOG_ERROR("Can't retrieve statistics for core ["
                           << (int)coreId << "], pfring_stats failed");
            return;
        }
    ...
    {
        PCPP_LOG_ERROR("Core [" << (int)coreId
                                << "] is not in use, can't retrieve statistics");
    }
}
...
void PfRingDevice::setPfRingDeviceAttributes()
{
    ...
    if (ring == NULL)
    {
        PCPP_LOG_ERROR("Could not open a pfring for setting device attributes: MAC "
                       "address, interface index and HW clock");
        return;
    }
    ...
    if (pfring_get_bound_device_address(ring, macAddress) < 0)
        PCPP_LOG_ERROR("Unable to read the device MAC address for interface '"
                       << m_DeviceName << "'");
    else
    ...
    if (mtu < 0)
        // cppcheck-suppress shiftNegative
        PCPP_LOG_ERROR(
            "Could not get MTU. pfring_get_mtu_size returned an error: " << mtu);
    else
...
bool PfRingDevice::sendData(const uint8_t* packetData, int packetDataLength,
                            bool flushTxQueues)
{
    if (!m_DeviceOpened)
    {
...
#define MAX_TRIES 5
    ...
    {
        // don't allow sending of data larger than the MTU, otherwise pfring_send
        // will fail
        if (packetDataLength > m_DeviceMTU)
            packetDataLength = m_DeviceMTU;

        // if the device is opened, m_PfRingDescriptors[0] will always be set and
        // enables
        res = pfring_send(m_PfRingDescriptors[0], (char*)packetData,
                          packetDataLength, flushTxAsUint);

        // res == -1 means it's an error coming from "sendto" which is the Linux API
        // PF_RING is using to send packets errno == ENOBUFS means write buffer is
        // full. PF_RING driver expects the userspace to handle this case My
        // implementation is to sleep for 10 usec and try again
        if (res == -1 && errno == ENOBUFS)
        {
            tries++;
            PCPP_LOG_DEBUG("Try #"
                           << tries
                           << ": Got ENOBUFS (write buffer full) error while sending "
                              "packet. Sleeping 20 usec and trying again");
            usleep(2000);
        }
        else
            break;
    ...
    if (tries >= MAX_TRIES)
    {
        PCPP_LOG_ERROR("Tried to send data " << MAX_TRIES
                                             << " times but write buffer is full");
        return false;
    }
    ...
    {
        // res == -1 means it's an error coming from "sendto" which is the Linux API
        // PF_RING is using to send packets
        if (res == -1)
            PCPP_LOG_ERROR("Error sending packet: Linux errno: "
    ...
    {
        PCPP_LOG_ERROR("Couldn't send all bytes, only " << res << " bytes out of "
                                                        << packetDataLength
                                                        << " bytes were sent");
        return false;
    }
...
{
    return sendData(packet.getRawPacketReadOnly()->getRawData(),
                    packet.getRawPacketReadOnly()->getRawDataLen(), true);
}
...
int PfRingDevice::sendPackets(const RawPacket* rawPacketsArr, int arrLength)
{
    ...
    {
        if (!sendData(rawPacketsArr[i].getRawData(),
                      rawPacketsArr[i].getRawDataLen(), false))
            break;
        else
            packetsSent++;
    }

    // The following method isn't supported in PF_RING aware drivers, probably
    // only in DNA and ZC
    pfring_flush_tx_packets(m_PfRingDescriptors[0]);

    PCPP_LOG_DEBUG(packetsSent << " out of " << arrLength
                               << " raw packets were sent successfully");
...
int PfRingDevice::sendPackets(const Packet** packetsArr, int arrLength)
{
    ...
    for (int i = 0; i < arrLength; i++)
    {
        if (!sendData(packetsArr[i]->getRawPacketReadOnly()->getRawData(),
                      packetsArr[i]->getRawPacketReadOnly()->getRawDataLen(),
                      false))
            break;
        else
            packetsSent++;
...
{
    int packetsSent = 0;
    for (RawPacketVector::ConstVectorIterator iter = rawPackets.begin();
         iter != rawPackets.end(); iter++)
    {
    ...
    PCPP_LOG_DEBUG(packetsSent << " out of " << rawPackets.size()
                               << " raw packets were sent successfully");
