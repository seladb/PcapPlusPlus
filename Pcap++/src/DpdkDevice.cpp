                                 ...
                                 const DpdkDeviceConfiguration& config)
{
    ...
    // There is a VMXNET3 limitation that when opening a device with a certain
    // number of RX+TX queues it's impossible to close it and open it again with a
...
bool DpdkDevice::configurePort(uint8_t numOfRxQueues, uint8_t numOfTxQueues)
{
    ...
    if (!isDeviceSupportRssHashFunction(getConfiguredRssHashFunction()))
    {
        PCPP_LOG_ERROR(
            "PMD '" << m_PMDName
                    ...
                    << std::hex << getConfiguredRssHashFunction());
        return false;
    }
    ...
    portConf.rx_adv_conf.rss_conf.rss_hf =
        convertRssHfToDpdkRssHf(getConfiguredRssHashFunction());
    ...
    PCPP_LOG_DEBUG("Successfully called rte_eth_dev_configure for device ["
                   << m_DeviceName << "] with " << numOfRxQueues
                   << " RX queues and " << numOfTxQueues << " TX queues");
                            ...
                            uint8_t numOfTxQueuesToInit)
{
    ...
    PCPP_LOG_DEBUG("Successfully initialized " << numOfRxQueuesToInit
    ...
    m_TxBufferDrainTsc = (rte_get_tsc_hz() + US_PER_S - 1) / US_PER_S *
                         m_Config.flushTxBufferTimeout;

    memset(m_TxBufferLastDrainTsc, 0, sizeof(uint64_t) * numOfTxQueuesToInit);

    PCPP_LOG_DEBUG("Successfully initialized " << numOfTxQueuesToInit
                                               << " TX queues for device ["
                                               << m_DeviceName << "]");
...
void DpdkDevice::setDeviceInfo()
{
...
#if (RTE_VER_YEAR < 18) || \
    (RTE_VER_YEAR == 18 && RTE_VER_MONTH < 5) // before 18.05
    char pciName[30];
#if (RTE_VER_YEAR > 17) || \
    (RTE_VER_YEAR == 17 && RTE_VER_MONTH >= 11) // 17.11 - 18.02
    ...
    (RTE_VER_YEAR == 22 && RTE_VER_MONTH < 11) // before 22.11
    m_PciAddress = std::string(portInfo.device->name);
#else                                          // 22.11 forward
    m_PciAddress = std::string(rte_dev_name(portInfo.device));
#endif

    PCPP_LOG_DEBUG("Device [" << m_DeviceName << "] has "
                              << portInfo.max_rx_queues << " RX queues");
    PCPP_LOG_DEBUG("Device [" << m_DeviceName << "] has "
                              << portInfo.max_tx_queues << " TX queues");
...
void DpdkDevice::getLinkStatus(LinkStatus& linkStatus) const
{
    ...
    linkStatus.linkDuplex = (link.link_duplex == DPDK_CONFIG_ETH_LINK_FULL_DUPLEX)
                                ? LinkStatus::FULL_DUPLEX
        ...
        if (coreMask & 1)
        {
            if (i == DpdkDeviceList::getInstance().getDpdkMasterCore().Id)
            {
                PCPP_LOG_ERROR(
                    "Core "
                    << i
                    << " is the master core, you can't use it for capturing threads");
                clearCoreConfiguration();
                return false;
            }
    ...
    void* onPacketsArriveUserCookie)
{
    ...
    PCPP_LOG_ERROR("Could not find initialized core so capturing thread cannot "
                   "be initialized");
    return false;
}
...
int DpdkDevice::dpdkCaptureThreadMain(void* ptr)
{
    ...
    while (likely(!pThis->m_StopThread))
    {
        ...
        if (likely(pThis->m_OnPacketsArriveCallback != NULL))
        {
            ...
            pThis->m_OnPacketsArriveCallback(rawPackets, numOfPktsReceived, coreId,
                                             pThis,
...
void DpdkDevice::getStatistics(DpdkDeviceStats& stats) const
{
    ...
    stats.aggregatedRxStats.bytesPerSec =
        (stats.aggregatedRxStats.bytes - m_PrevStats.aggregatedRxStats.bytes) /
        secsElapsed;
    ...
    stats.aggregatedTxStats.packetsPerSec =
        (stats.aggregatedTxStats.packets -
         m_PrevStats.aggregatedTxStats.packets) /
        secsElapsed;
    stats.aggregatedTxStats.bytesPerSec =
        (stats.aggregatedTxStats.bytes - m_PrevStats.aggregatedTxStats.bytes) /
        secsElapsed;
    ...
    for (int i = 0; i < numRxQs; i++)
    {
        ...
        stats.rxStats[i].packetsPerSec =
            (stats.rxStats[i].packets - m_PrevStats.rxStats[i].packets) /
            secsElapsed;
        stats.rxStats[i].bytesPerSec =
    ...
    for (int i = 0; i < numTxQs; i++)
    {
        ...
        stats.txStats[i].packetsPerSec =
            (stats.txStats[i].packets - m_PrevStats.txStats[i].packets) /
            secsElapsed;
        stats.txStats[i].bytesPerSec =
            (stats.txStats[i].bytes - m_PrevStats.txStats[i].bytes) / secsElapsed;
    }
                                    ...
                                    uint16_t rxQueueId) const
{
    ...
    struct rte_mbuf* mBufArray[MAX_BURST_SIZE];
    uint32_t numOfPktsReceived =
        rte_eth_rx_burst(m_Id, rxQueueId, mBufArray, MAX_BURST_SIZE);
                                 ...
                                 uint16_t txQueueId, bool useTxBuffer)
{
    ...
    uint16_t packetsSent =
        sendPacketsInner(txQueueId, (void*)mBufArr, getNextPacketFromMBufArray,
                         arrLength, useTxBuffer);
                                 ...
                                 uint16_t txQueueId, bool useTxBuffer)
{
    ...
    uint16_t packetsSent =
        sendPacketsInner(txQueueId, (void*)(&rawPacketsVec),
                         getNextPacketFromMBufRawPacketVec, vecSize, useTxBuffer);
                            ...
                            bool useTxBuffer)
{
    ...
    bool packetSent =
        (sendPacketsInner(txQueueId, &mbufRawPacket,
                          getNextPacketFromMBufRawPacket, 1, useTxBuffer) == 1);
    bool needToFreeMbuf = (!useTxBuffer && !packetSent);
