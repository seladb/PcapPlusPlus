 ...
 * A singleton class that creates, stores and provides access to all
 * PcapLiveDevice (on Linux) or WinPcapLiveDevice (on Windows) instances. All
 * live devices are initialized on startup and wrap the network interfaces
 * installed on the machine. This class enables access to them through their IP
...
class PcapLiveDeviceList {
   ...
   */
    ...
    /**
   * Get a pointer to the live device by its IP address. IP address can be both
   * IPv4 or IPv6
   * @param[in] ipAddr The IP address defined for the device
   * @return A pointer to the live device if this IP address exists. NULL
   * otherwise
   */
   ...
   * @return A pointer to the live device if this IPv4 address exists. NULL
   * otherwise
   */
   ...
   * @return A pointer to the live device if this IPv6 address exists. NULL
   * otherwise
   */
    PcapLiveDevice* getPcapLiveDeviceByIp(const IPv6Address& ip6Addr) const;

    /**
   * Get a pointer to the live device by its IP address represented as string.
   * IP address can be both IPv4 or IPv6
   * @param[in] ipAddrAsString The IP address defined for the device as string
   * @return A pointer to the live device if this IP address is valid and
   * exists. NULL otherwise
   */
    PcapLiveDevice*
    getPcapLiveDeviceByIp(const std::string& ipAddrAsString) const;
   ...
   */
    PcapLiveDevice*
    getPcapLiveDeviceByIpOrName(const std::string& ipOrName) const;
   ...
   */
    ...
    /**
   * Reset the live device list and DNS server list, meaning clear and refetch
   * them
   */
    void reset();
