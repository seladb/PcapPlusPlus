...
/**
 * @file
 * Most packet capture engines contain packet filtering capabilities. In order
 * to set the filters there should be a known syntax user can use. The most
 * popular syntax is Berkeley Packet Filter (BPF) - see more in here:
 * http://en.wikipedia.org/wiki/Berkeley_Packet_Filter. Detailed explanation of
 * the syntax can be found here:
 * http://www.tcpdump.org/manpages/pcap-filter.7.html.<BR> The problem with BPF
 * is that, for my opinion, the syntax is too complicated and too poorly
 * documented. In addition the BPF filter compilers may output syntax errors
 * that are hard to understand. My experience with BPF was not good, so I
 * decided to make the filters mechanism more structured, easier to understand
 * and less error-prone by creating classes that represent filters. Each
 * possible filter phrase is represented by a class. The filter, at the end, is
 * that class.<BR> For example: the filter "src host 1.1.1.1" will be
 * represented by IPFilter instance; "dst port 80" will be represented by
 * PortFilter, and so on.<BR> So what about complex filters that involve "and",
 * "or"? There are also 2 classes: AndFilter and OrFilter that can store more
 * filters (in a composite idea) and connect them by "and" or "or". For example:
 * "src host 1.1.1.1 and dst port 80" will be represented by an AndFilter that
...
class BpfFilterWrapper
{
   ...
   */
    ...
    /**
   * A d'tor for this class. Makes sure to clear the bpf_program object if was
   * previously set.
   */
    ~BpfFilterWrapper();

    /**
   * Set a filter. This method receives a filter in BPF syntax
   * (https://biot.com/capstats/bpf.html) and an optional link type, compiles
   * them, and if compilation is successful it stores the filter.
   * @param[in] filter A filter in BPF syntax
   * @param[in] linkType An optional parameter to set the filter's link type.
   * The default is LINKTYPE_ETHERNET
   * @return True if compilation is successful and filter is stored in side this
   ...
   */
    ...
    /**
   * Match a packet with the filter stored in this object. If the filter is
   * empty the method returns "true". If the link type of the raw packet is
   * different than the one set in setFilter(), the filter will be re-compiled
   * and stored in the object.
   * @param[in] rawPacket A pointer to a raw packet which the filter will be
   * matched against
   * @return True if the filter matches (or if it's empty). False if the packet
   * doesn't match or if the filter could not be compiled
   */
    bool matchPacketWithFilter(const RawPacket* rawPacket);

    /**
   * Match a packet data with the filter stored in this object. If the filter is
   * empty the method returns "true". If the link type provided is different
   * than the one set in setFilter(), the filter will be re-compiled and stored
   * in the object.
   ...
   * doesn't match or if the filter could not be compiled
   */
    bool matchPacketWithFilter(const uint8_t* packetData,
...
/**
 * @class GeneralFilter
 * The base class for all filter classes. This class is virtual and abstract,
...
class GeneralFilter
{
   ...
   * @param[out] result An empty string that the parsing will be written into.
   * If the string isn't empty, its content will be overridden
   */
    virtual void parseToString(std::string& result) = 0;

    /**
   * Match a raw packet with a given BPF filter.
   * @param[in] rawPacket A pointer to the raw packet to match the BPF filter
   * with
...
class BPFStringFilter : public GeneralFilter
{
  ...
  public:
    explicit BPFStringFilter(const std::string& filterStr)
   ...
   * A method that parses the class instance into BPF string format
   * @param[out] result An empty string that the parsing will be written into.
   * If the string isn't empty, its content will be overridden If the filter is
   * not valid the result will be an empty string
   */
 ...
 * An abstract class that is the base class for all filters which contain a
 ...
 * An abstract class that is the base class for all filters which contain an
 * operator (e.g X equals Y; A is greater than B; Z1 not equals Z2, etc.). This
...
/**
 * @class IPFilter
 * A class for representing IPv4 address filter, equivalent to "net src x.x.x.x"
 * or "net dst x.x.x.x"<BR> For deeper understanding of the filter concept
...
class IPFilter : public IFilterWithDirection
{
  private:
    ...
    int m_Len;
    void convertToIPAddressWithMask(std::string& ipAddrmodified,
                                    std::string& mask) const;
    void convertToIPAddressWithLen(std::string& ipAddrmodified) const;

  public:
    /**
   * The basic constructor that creates the filter from an IPv4 address and
   * direction (source or destination)
   * @param[in] ipAddress The IPv4 address to build the filter with. If this
   * address is not a valid IPv4 address an error will be written to log and
   ...
   */
    IPFilter(const std::string& ipAddress, Direction dir)
        : IFilterWithDirection(dir), m_Address(ipAddress), m_IPv4Mask(""),
          m_Len(0) {}

    /**
   * A constructor that enable to filter only part of the address by using a
   * mask (aka subnet). For example: "filter only IP addresses that matches the
   * subnet 10.0.0.x"
   * @param[in] ipAddress The IPv4 address to use. Only the part of the address
   * that is not masked will be matched. For example: if the address is
   * "1.2.3.4" and the mask is "255.255.255.0" than the part of the address that
   * will be matched is "1.2.3.X". If this address is not a valid IPv4 address
   * an error will be written to log and parsing this filter will fail
   * @param[in] dir The address direction to filter (source or destination)
   ...
   */
    IPFilter(const std::string& ipAddress, Direction dir,
        ...
        : IFilterWithDirection(dir), m_Address(ipAddress), m_IPv4Mask(ipv4Mask),
          m_Len(0) {}

    /**
   * A constructor that enables to filter by a subnet. For example: "filter only
   * IP addresses that matches the subnet 10.0.0.3/24" which means the part of
   * the address that will be matched is "10.0.0.X"
   * @param[in] ipAddress The IPv4 address to use. Only the part of the address
   * that is not masked will be matched. For example: if the address is
   * "1.2.3.4" and the subnet is "/24" than the part of the address that will be
   * matched is "1.2.3.X". If this address is not a valid IPv4 address an error
   * will be written to log and parsing this filter will fail
   * @param[in] dir The address direction to filter (source or destination)
...
/**
 * @class IPv4IDFilter
 * A class for filtering IPv4 traffic by IP ID field of the IPv4 protocol, For
...
class IPv4IDFilter : public IFilterWithOperator
{
  ...
  public:
    /**
   * A constructor that gets the IP ID to filter and the operator and creates
   ...
   */
    IPv4IDFilter(uint16_t ipID, FilterOperator op)
        : IFilterWithOperator(op), m_IpID(ipID) {}
 ...
 * A class for filtering IPv4 traffic by "total length" field of the IPv4
 * protocol, For example: "filter only IPv4 traffic which "total length" value
...
class IPv4TotalLengthFilter : public IFilterWithOperator
{
  ...
  public:
    /**
   * A constructor that gets the total length to filter and the operator and
   * creates the filter out of them
...
/**
 * @class PortFilter
 * A class for filtering TCP or UDP traffic by port, for example: "dst port 80"
 ...
 * A class for filtering TCP or UDP port ranges, meaning match only packets
 * which port is within this range, for example: "src portrange 1000-2000" will
 * match only TCP or UDP traffic which source port is in the range of 1000 -
...
class PortRangeFilter : public IFilterWithDirection
{
  ...
  public:
    /**
   * A constructor that gets the port range the the direction and creates the
   * filter with them
 ...
 * A class for filtering Ethernet traffic by MAC addresses, for example: "ether
...
class MacAddressFilter : public IFilterWithDirection
{
  ...
  public:
    /**
   * A constructor that gets the MAC address and the direction and creates the
   * filter with them
   ...
   */
    MacAddressFilter(MacAddress address, Direction dir)
        : IFilterWithDirection(dir), m_MacAddress(address) {}
 ...
 * A class for filtering by EtherType field of the Ethernet protocol. This
 * enables to filter packets from certain protocols, such as ARP, IPv4, IPv6,
...
/**
 * @class AndFilter
 * A class for connecting several filters into one filter with logical "and"
 * between them. For example: if the 2 filters are: "IPv4 address = x.x.x.x" +
 * "TCP port dst = 80", then the new filter will be: "IPv4 address = x.x.x.x
 * _AND_ TCP port dst = 80"<BR> This class follows the composite design
 * pattern<BR> For deeper understanding of the filter concept please refer to
 * PcapFilter.h
...
class AndFilter : public GeneralFilter
{
  ...
  public:
    /**
   * An empty constructor for this class. Use addFilter() to add filters to the
   * and condition
   */
    AndFilter() {}

    /**
   * A constructor that gets a list of pointers to filters and creates one
   * filter from all filters with logical "and" between them
   ...
   * @param[in] filters The new filters to set. The previous ones will be
   * removed
   */
...
/**
 * @class OrFilter
 * A class for connecting several filters into one filter with logical "or"
 * between them. For example: if the 2 filters are: "IPv4 address = x.x.x.x" +
 * "TCP port dst = 80", then the new filter will be: "IPv4 address = x.x.x.x
 * _OR_ TCP port dst = 80"<BR> This class follows the composite design
 * pattern<BR> For deeper understanding of the filter concept please refer to
 * PcapFilter.h
 * @todo add some methods: "addFilter", "removeFilter", "clearAllFilter"
 */
class OrFilter : public GeneralFilter
{
   ...
   */
    ...
    /**
   * A constructor that gets a list of pointers to filters and creates one
   * filter from all filters with logical "or" between them
   * @param[in] filters The list of pointers to filters
   */
...
class NotFilter : public GeneralFilter
{
  ...
  public:
    /**
   * A constructor that gets a pointer to a filter and create the inverse
   * version of it
...
/**
 * @class ProtoFilter
 * A class for filtering traffic by protocol. Notice not all protocols are
 * supported, only the following are supported:
 * ::TCP, ::UDP, ::ICMP, ::VLAN, ::IPv4, ::IPv6, ::ARP, ::Ethernet, ::GRE
 * (distinguish between ::GREv0 and ::GREv1 is not supported),
 * ::IGMP (distinguish between ::IGMPv1, ::IGMPv2 and ::IGMPv3 is not
...
class ProtoFilter : public GeneralFilter
{
   ...
   * @param[in] proto The protocol to filter, only packets matching this
   * protocol will be received. Please note not all protocols are supported.
   * List of supported protocols is found in the class description
   */
...
/**
 * @class ArpFilter
 * A class for filtering ARP packets according the ARP opcode. When using this
...
/**
 * @class VlanFilter
 * A class for filtering VLAN tagged packets by VLAN ID. When using this filter
 * only packets tagged with VLAN which has the specific VLAN ID will be received
 ...
 * A class for filtering only TCP packets which certain TCP flags are set in
...
class TcpFlagsFilter : public GeneralFilter
{
   ...
   */
    ...
    /**
   * An enum for representing 2 type of matches: match only packets that contain
  ...
  public:
    /**
   * A constructor that gets a 1-byte bitmask containing all TCP flags
   * participating in the filter and the match option, and creates the filter
   * @param[in] tcpFlagBitMask A 1-byte bitmask containing all TCP flags
   * participating in the filter. This parameter can contain the following value
   * for example: TcpFlagsFilter::tcpSyn | TcpFlagsFilter::tcpAck |
...
class TcpWindowSizeFilter : public IFilterWithOperator
{
  ...
  public:
    /**
   * A constructor that get the window-size and operator and creates the filter.
   * For example: "filter all TCP packets with window-size less than 1000"
   ...
   */
    TcpWindowSizeFilter(uint16_t windowSize, FilterOperator op)
        : IFilterWithOperator(op), m_WindowSize(windowSize) {}
...
class UdpLengthFilter : public IFilterWithOperator
{
  ...
  public:
    /**
   * A constructor that get the UDP length and operator and creates the filter.
   * For example: "filter all UDP packets with length greater or equal to 500"
   * @param[in] length The length value that will be used in the filter
   * @param[in] op The operator to use (e.g "equal", "greater than", etc.)
   */
    UdpLengthFilter(uint16_t length, FilterOperator op)
        : IFilterWithOperator(op), m_Length(length) {}
