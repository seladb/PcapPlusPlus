...
class KniDeviceList {
   ...
   * @warning NOT MT SAFE
   * @warning This method is forbidden as the result of discussion about packet
   * memory pool lifetime made <a
   * href="https://github.com/seladb/PcapPlusPlus/pull/196#discussion_r286649704">here</a>.
   ...
   */
    enum KniCallbackVersion {
       ...
       structure */
        CALLBACKS_NEW = 0,
        /** Reports that DPDK supports only KniDevice#KniOldIoctlCallbacks callback
       structure */
        CALLBACKS_OLD = 1
    };
    /**
   ...
   */
    enum KniCallbackType {
       ...
       KniDevice#KniOldIoctlCallbacks#change_mtu callback */
        CALLBACK_MTU,
        /** KniDevice#KniIoctlCallbacks#config_network_if and
       KniDevice#KniOldIoctlCallbacks#config_network_if callback */
        CALLBACK_LINK,
   ...
   * @warning Initialization of Kni module depends on initialization of DPDK
   * made by DpdkDeviceList
   ...
   * Newly created device is remembered under portId and name provided in config
   * and can be found later by them. If KNI device is not destroyed explicitly
   * thru KniDeviceList#destroyDevice then it will be destroyed implicitly by
   * the time application exits.
   * @warning NOT MT SAFE
   ...
   */
    KniDevice* createDevice(const KniDevice::KniDeviceConfiguration& config,
