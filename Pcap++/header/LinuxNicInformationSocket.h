 ...
 * <a
 * href="http://man7.org/linux/man-pages/man7/netdevice.7.html">netdevice(7)</a>.
...
class LinuxNicInformationSocket {
   ...
   * Then makes an ioctl(2) request to handled socket with provided request
   * structure. See: <a
   * href="http://man7.org/linux/man-pages/man7/netdevice.7.html">netdevice(7)</a>
   * for description of possible values of ioctlType and content of request.
   * @note User have no need to fill ifr_name field of request. It will be
   * filled automatically from provided nicName argument.
   * @param[in] nicName Name of internet controller as displayed by Linux
   * @param[in] ioctlType Value of ioctl to make
   * @param[in,out] request Pointer to ifreq structure that contains some
   * information or will be used for obtaining the information (depends on
   * ioctlType)
   * @return false if request was not made or socket can't be opened otherwise
   * true
   * @warning For some types of requests to succeed You need to be a root
   * or have the CAP_NET_ADMIN capability.
   */
    bool makeRequest(const char* nicName, const IoctlType ioctlType,
                     ifreq* request);
