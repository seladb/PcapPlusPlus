 ...
 * A class that wraps a Windows network interface (each of the interfaces listed
 * in ipconfig). This class is almost similar in its capabilities to
 * PcapLiveDevice (its parent class) with some small changes that mainly result
...
class WinPcapLiveDevice : public PcapLiveDevice
{
  ...
  public:
    ...
    bool startCapture(OnPacketArrivesCallback onPacketArrives,
    ...
    /**
   * WinPcap/Npcap have a feature (that doesn't exist in libpcap) to change the
   * minimum amount of data in the kernel buffer that causes a read from the
   * application to return (unless the timeout expires). Please see
   * documentation for pcap_setmintocopy for more info. This method enables the
   * user to change this size. Note the device must be open for this method to
   * work
   * @param[in] size The size to set in bytes
   * @return True if set succeeded, false if the device is closed or if
   * pcap_setmintocopy failed
   */
    bool setMinAmountOfDataToCopyFromKernelToApplication(int size);

    /**
   * @return The current amount of data in the kernel buffer that causes a read
