...
typedef void (*OnPfRingPacketsArriveCallback)(RawPacket* packets,
...
class PfRingDevice : public IDevice, public IFilterableDevice {
  ...
  private:
    ...
    void captureThreadMain(std::condition_variable* startCond,
                           std::mutex* startMutex, const int* startState);
    ...
    bool sendData(const uint8_t* packetData, int packetDataLength,
                  bool flushTxQueues);
   ...
   */
     ...
     */
        RoundRobin,
        /**
     * Packets are distributed between channels per flow (each flow goes for
     * different channel)
     */
        PerFlow
    };
   ...
   */
    ...
    /**
   * Return true if device supports hardware timestamping. If it does, this
   * feature will be automatically set for this device. You can read more about
   ...
   */
    ...
    /**
   * Start single-threaded capturing with callback. Works with open() or
   * openSingleRxChannel().
   * @param[in] onPacketsArrive A callback to call whenever a packet arrives
   * @param[in] onPacketsArriveUserCookie A cookie that will be delivered to
   * onPacketsArrive callback on every packet
   * @return True if this action succeeds, false otherwise
   */
    bool startCaptureSingleThread(OnPfRingPacketsArriveCallback onPacketsArrive,
                                  void* onPacketsArriveUserCookie);

    /**
   * Start multi-threaded (multi-core) capturing with callback. Works with
   * openMultiRxChannels(). This method will return an error if the number of
   * opened channels is different than the number of threads/cores requested
   * @param[in] onPacketsArrive A callback to call whenever a packet arrives
   * @param[in] onPacketsArriveUserCookie A cookie that will be delivered to
   * onPacketsArrive callback on every packet
   ...
   * @return True if this action succeeds, false otherwise
   */
    bool startCaptureMultiThread(OnPfRingPacketsArriveCallback onPacketsArrive,
                                 void* onPacketsArriveUserCookie,
                                 CoreMask coreMask);
   ...
   */
    ...
    /**
   * Opens a single RX channel (=RX queue) on this interface. All packets will
   * be received on a single thread without core affinity. If the channel ID
   * requested doesn't exist on this interface, the method will fail (return
   * false)
   ...
   */
    ...
    /**
   * Opens a set of RX channels (=RX queues) on this interface, identified by
   * their IDs. All packets will be received on a single thread without core
   * affinity. If one of the channel IDs requested doesn't exist on this
   ...
   */
    ...
    /**
   * Opens numOfRxChannelsToOpen RX channels. If numOfRxChannelsToOpen is larger
   * than available RX queues for this interface than a number of RX channels
   * will be opened on each RX queue. For example: if the user asks for 10 RX
   * channels but the interface has only 4 RX queues, then 3 RX channels will be
   * opened for RX-queue0 and RX-queue2, and 2 RX channels will be opened for
   * RX-queue2 and RX-queue3. Packets will be distributed between different RX
   * queues on per-flow manner, but within multiple RX channels in the same RX
   * queue packet will be distributed according to distribution requested by
   * "dist"
   * @param[in] numOfRxChannelsToOpen Number of RX channels to open
   * @param[in] dist Distribution method
   * @return True if this action succeeds, false otherwise
   */
    bool openMultiRxChannels(uint8_t numOfRxChannelsToOpen,
                             ChannelDistribution dist);

    /**
   * Gets the number of RX channels currently open. RX channels aren't necessary
   * interface's RX queues because in some cases the user asks to open several
   * channels on the same queue. For example: if the user uses
   * openMultiRxChannels() and asks to open 8 channels but interface has only 4
   * RX queues, 2 channels will be opened for each RX queue
   ...
   * @param[out] stats A reference for the stats object where the stats are
   * written. Current values will be overridden
   */
   ...
   */
    ...
    /**
   * Get the statistics for the entire device. If more than 1 RX channel is
   * opened, this method aggregates the stats of all channels
   * @param[out] stats A reference for the stats object where the stats are
   * written. Current values will be overridden
   */
   ...
   * This method doesn't change or manipulate the data in any way (hence the
   * "const" declaration). Note this method flushes the TX queues after the data
   * is sent. So if you want to send several packets In the burst please use
   * sendPackets()
   ...
   */
    ...
    /**
   * Send raw data. This data must be a valid and fully specified packet (the
   * MAC address up); it will be transmitted as-is without any further
   * manipulation. This method doesn't change or manipulate the data in any way
   * (hence the "const" declaration). Note this method flushes the TX queues
   * after the data is sent. So if you want to send several packets in the burst
   * please use sendPackets()
   ...
   */
    ...
    /**
   * Send raw packets. All raw packets must be fully specified (the MAC address
   * up) and it will be transmitted as-is without any further manipulation. This
   * method doesn't change or manipulate the raw packets data in any way (hence
   * the "const" declaration). This method flushes the TX queues only when the
   * last packet is sent
   ...
   * and it will be transmitted as-is without any further manipulation.
   * This method doesn't change or manipulate the packets data in any way (hence
   * the "const" declaration). This method flushes the TX queues only when the
   * last packet is sent
   ...
   */
    ...
    /**
   * Send all raw packets pointed by the RawPacketVector. All packets must be
   * fully specified (the MAC address up) and it will be transmitted as-is
   * without any further manipulation. This method doesn't change or manipulate
   * the packets data in any way (hence the "const" declaration). This method
   * flushes the TX queues only when the last packet is sent
   ...
   */
    ...
    /**
   * Opens the entire device (including all RX channels/queues on this
   * interface). All packets will be received on a single thread without core
   * affinity
   * @return True if this action succeeds, false otherwise
   */
    bool open();
   ...
   * @return True if filter was removed successfully or if no filter was set,
   * false otherwise
   */
    bool clearFilter();
};
