 ...
 * Useful links:
 *  - <a
 * href="https://doc.dpdk.org/guides/prog_guide/kernel_nic_interface.html">KNI
 * interface concept DPDK documentation</a>
 *  - <a href="https://doc.dpdk.org/guides/nics/kni.html">KNI PMD</a>
 *  - <a
 * href="https://doc.dpdk.org/guides/sample_app_ug/kernel_nic_interface.html">KNI
 * DPDK sample application</a>
 *  - <a href="https://doc.dpdk.org/dts/test_plans/kni_test_plan.html">KNI DPDK
 * test plan</a>
 */
 ...
 */
typedef bool (*OnKniPacketArriveCallback)(MBufRawPacket* packets,
                                          uint32_t numOfPackets,
                                          KniDevice* device, void* userCookie);

/**
 * @class KniDevice
 * This class represents special kind of DPDK devices called KNI - Kernel
 * Network Interface that are used to exchange DPDK mbuf packets with Linux
 * kernel network stack. This devices have only one RX and one TX queue so MT
 * receiving or MT sending is not safe but simultaneous receiving and sending
 * packets is MT safe. The RX queue of KNI device is pointed from kernel to
 * application and TX queue is pointed in opposite direction - from application
 * to kernel. So receive* methods will obtain packets from kernel and send*
 * methods will send them to kernel.<BR> The lifecycle of the KNI device is as
 * follows:
 *  - KniDeviceConfiguration structure is created by user and filled with device
 * settings;
 *  - The KniDeviceList#getInstance method is called to initialize the KNI
 * module and obtain the KniDeviceList singleton;
 *  - KniDeviceList#createDevice method of the singleton is called to allocate
 * new KNI device;
 *  - Device then must be opened by calling KniDevice#open;
 *  - During lifetime the application must handle the requests from kernel
 * either by calling KniDevice#handleRequests to handle them synchronously or by
 * starting separate thread to do this by calling
 * KniDevice#startRequestHandlerThread;
 ...
 *  - KNI device will be destroyed or implicitly on application exit. User must
...
class KniDevice : public IDevice {
   ...
   */
     ...
     * DPDK port port_id via
     * rte_eth_promiscuous_enable/rte_eth_promiscuous_disable. Must return 0 in
     * case of success or negative error code
     */
   ...
   */
     ...
     * If provided (not MacAddress::Zero) will be cached by new KNI device info
     * structure.
     */
        MacAddress mac;
        /**
     ...
     * (see KniIoctlCallbacks#config_mac_address and
     * KniIoctlCallbacks#config_promiscusity). Can be set to UINT16_MAX.
     */
        uint16_t portId;
     ...
     */
        uint16_t mtu;
        /**
     * If set forces to bind KNI Linux kernel thread (NOT userspace thread) to
     * specific core. If rte_kni kernel module is loaded with
     * "kthread_mode=single" then - rebinds kernel thread used for all KNI
     * devices to specified core. If rte_kni kernel module is loaded with
     * "kthread_mode=multiple" then - binds new kernel thread for this device to
     * specified core.
     */
        bool bindKthread;
   ...
   * If called with INFO_CACHED - returns cached data about link state (SUPER
   * FAST may be INACCURATE). If called with INFO_RENEW - makes system call to
   * Linux to obtain link information and caches it (VERY SLOW but ACCURATE).
   * @param[in] state Defines information relevance level
   * @return LINK_UP, LINK_DOWN, LINK_NOT_SUPPORTED if device is not
   * initialized, some times LINK_ERROR
   */
    KniLinkState getLinkState(KniInfoState state = INFO_CACHED);
    /**
   * @brief Obtains MAC address of KNI device.
   * If called with INFO_CACHED - returns cached data about MAC address (SUPER
   * FAST may be INACCURATE). If called with INFO_RENEW - makes system call to
   * Linux to obtain MAC address and caches it (VERY SLOW but ACCURATE).
   * @param[in] state Defines information relevance level
   ...
   * If called with INFO_CACHED - returns cached data about MTU (SUPER FAST may
   * be INACCURATE). If called with INFO_RENEW - makes system call to Linux to
   * obtain MTU and caches it (VERY SLOW but ACCURATE).
   * @param[in] state Defines information relevance level
   ...
   * If called with INFO_CACHED - returns cached data about promiscuous mode
   * (SUPER FAST may be INACCURATE). If called with INFO_RENEW - makes system
   * call to Linux to obtain promiscuous mode and caches it (VERY SLOW but
   * ACCURATE).
   * @param[in] state Defines information relevance level
   ...
   * Firstly the link information is updated as by call to
   * getLinkState(INFO_RENEW). Then link state is set only if obtained state
   * differs from provided. New link state of KNI device is cached.
   * @note You must be using sudo or be root or have CAP_NET_ADMIN capability to
   * use this function
   * @note Generates change link state request
   * @param[in] state Must be LINK_UP or LINK_DOWN
   * @return true if desired link state of KNI device is set (was as provided or
   * set successfully), false if some error occurred (debug info is printed)
   */
   ...
   * @return true if desired MAC address is set, false if not and some error
   * occurred (debug info is printed)
   */
   ...
   * @warning Low MTU values may crush Linux kernel. Follow Your kernel version
   * documentation for details
   * @param[in] mtu New MTU address of KNI device
   * @return true if desired MTU is set, false if not and some error occurred
   * (debug info is printed)
   */
    bool setMtu(uint16_t mtu);
    /**
   * @brief Sets promiscuous mode of KNI device.
   * Firstly the promiscuous mode information is updated as by call to
   * getPromiscuous(INFO_RENEW). Then promiscuous mode is set only if obtained
   * mode differs from provided. New promiscuous mode of KNI device is cached.
   * @note You must be using sudo or be root or have CAP_NET_ADMIN capability to
   * use this function
   * @note Generates promiscuous mode request
   * @param[in] mode Must be PROMISC_DISABLE or PROMISC_ENABLE
   * @return true if desired promiscuous mode of KNI device is set (was as
   * provided or set successfully), false if some error occurred (debug info is
   * printed)
   */
    bool setPromiscuous(KniPromiscuousMode mode);
    /**
   * @brief Updates link state of KNI device.
   * Unconditionally updates link state of KNI device via call to DPDK
   * librte_kni API. FASTER THAN setLinkState(state) but may not be supported or
   * may fail. If link state is updated successfully then it is cached.
   * @param[in] state New link state of KNI device
   * @return LINK_NOT_SUPPORTED if this capability is not supported by DPDK
   * version used (DPDK ver < 18.11), LINK_ERROR if attempt to set link state
   * failed (may always fail on some systems see class description) LINK_DOWN -
   * previous link state was DOWN, state is successfully updated to provided one
   *    LINK_UP - previous link state was UP, state is successfully updated to
   * provided one
   */
   ...
   * When one of events which is needed application attention occurres it must
   * be handled by calling this function (or by running RequestHandlerThread for
   * this device). Until the request is handled the Linux kernel thread that
   * manages this KNI is blocked. If it is not handled by application in 3
   * seconds the request is reported to kernel as failed one. Current known
   * requests are:
   ...
   * @warning Functions setLinkState, setMacAddress, setMtu and setPromiscuous
   * will generate this requests.
   * @note Callbacks provided for this KNI device will be called synchronously
   * in calling thread during execution of this function
   * @return true if no error happened during request handling false otherwise
   */
    bool handleRequests();
    /**
   * @brief Starts new thread (using pthread) to asynchronously handle KNI
   * device requests. See description of handleRequests() about requests. New
   * thread is detached using pthread_detach. This thread can be stopped
   * explicitly by calling stopRequestHandlerThread() or implicitly on KNI
   * device destruction. Linux <a
   * href="http://man7.org/linux/man-pages/man2/nanosleep.2.html">nanosleep()</a>
   * function is used for sleeping.
   * @note Callbacks provided for this KNI device will be called asynchronously
   * in new thread
   ...
   * @warning There may be a rare error when request thread handles requests on
   * already destroyed device. It occurres only because request thread is
   * detached one but it is really really rare. In case of this error occurring
   * (must be SIGSEGV) change type of created thread in
   * startRequestHandlerThread function from DETACHED to JOINABLE.
   */
   ...
   * for receiving packets because out of all receivePackets overloads this
   * method requires the least overhead and is almost as efficient as receiving
   * packets directly through DPDK. So if performance is a critical factor in
   ...
   * Please notice that if the raw packet isn't of type MBufRawPacket, a new
   * temp MBufRawPacket will be created and the data will be copied to it. This
   * is necessary to allocate an mbuf which will store the data to be sent. If
   * performance is a critical factor please make sure you send a raw packet of
   * type MBufRawPacket. Please also notice that the mbuf used or allocated in
   * this method isn't freed by this method, it will be transparently freed by
   * DPDK
   * @param[in] rawPacket The raw packet to send
   * @return True if packet was sent successfully or false if the packet wasn't
   * sent for any other reason
   */
    bool sendPacket(RawPacket& rawPacket);
    /**
   * @brief Send a MBufRawPacket to kernel.
   * Please notice that the mbuf used in this method isn't freed by this method,
   ...
   * opened or if the packet wasn't sent for any other reason
   */
    bool sendPacket(MBufRawPacket& rawPacket);
    /**
   * @brief Send a parsed packet to kernel.
   * Please notice that the mbuf used or allocated in this method isn't freed by
   * this method, it will be transparently freed by DPDK
   ...
   * Each time a burst of packets is captured the onPacketArrives callback is
   * called. The capture is done on a new thread created by this method, meaning
   * all callback calls are done in a thread other than the caller thread.
   * Capture process will stop and this capture thread will be terminated when
   ...
   * returned. Capturing thread will be terminated automatically on KNI device
   * destruction or when close() is called.
   * @param[in] onPacketArrives A callback that is called each time a burst of
   * packets is captured
   ...
   */
    bool startCapture(OnKniPacketArriveCallback onPacketArrives,
                      void* onPacketArrivesUserCookie);
    /**
   * @brief Start capturing packets synchronously on this KNI interface in
   * blocking mode. Blocking mode means that this method block and won't return
   * until the user frees the blocking (via onPacketArrives callback) or until a
   * user defined timeout expires. Whenever a burst of packets is captured the
   * onPacketArrives callback is called and lets the user handle the packet. In
   * each callback call the user should return true if he wants to release the
   * block or false if it wants it to keep blocking. Regardless of this callback
   * a timeout is defined when stop capturing. When this timeout expires the
   * method will return.<BR> Please notice that stopCapture() isn't needed here
   * because when the method returns (after timeout or per user decision)
   * capturing on the device is stopped.
   * @param[in] onPacketArrives A callback given by the user for handling
   * incoming packets. After handling each burst of packets the user needs to
   * return a boolean value. True value indicates stop capturing and stop
   ...
   */
    int startCaptureBlockingMode(OnKniPacketArriveCallback onPacketArrives,
                                 void* onPacketArrivesUserCookie, int timeout);
    /**
   ...
   * already opened, or KNI device configuration and startup failed
   */
    bool open();
    /**
