 ...
 *
 * It provides methods for configuring and initializing an AF_XDP socket, and
...
class XdpDevice : public IDevice
{
   ...
   * @param[in] device The XdpDevice packets are received from (represents the
   * AF_XDP socket)
   * @param[in] userCookie A pointer to an object set by the user when
   * receivePackets() started
   */
    typedef void (*OnPacketsArrive)(RawPacket packets[], uint32_t packetCount,
                                    XdpDevice* device, void* userCookie);
   ...
   */
     ...
     */
        enum AttachMode
        {
         ...
         network driver doesn't have support for XDP */
            ...
            DriverMode = 2,
            /** Automatically detect whether driver mode is supported, otherwise
     ...
     *frames. This parameter determines the number of frames that will be
     *allocated as pert of the UMEM.
     **/
        uint16_t umemNumFrames;

        /**
     * UMEM is a region of virtual contiguous memory, divided into equal-sized
     *frames. This parameter determines the frame size that will be allocated.
     * NOTE: the frame size should be equal to the memory page size (use
     *getpagesize() to determine this size)
     **/
        uint16_t umemFrameSize;

        /**
     * The size of the fill ring used by the AF_XDP socket. This size should be
     * a power of two and less or equal to the total number of UMEM frames
     */
        uint32_t fillRingSize;

        /**
     * The size of the completion ring used by the AF_XDP socket. This size
     * should be a power of two and less or equal to the total number of UMEM
     * frames
     */
        uint32_t completionRingSize;

        /**
     * The size of the RX ring used by the AF_XDP socket. This size should be a
     * power of two and less or equal to the total number of UMEM frames
     */
        uint32_t rxSize;

        /**
     * The size of the TX ring used by the AF_XDP socket. This size should be a
     * power of two and less or equal to the total number of UMEM frames
     */
        uint32_t txSize;
     ...
     */
        ...
        /**
     * A c'tor for this struct. Each parameter has a default value described
     * below.
     * @param[in] attachMode AF_XDP operation mode. The default value is auto
     * mode
     * @param[in] umemNumFrames Number of UMEM frames to allocate. The default
     * value is 4096
     * @param[in] umemFrameSize The size of each UMEM frame. The default value
     * is equal to getpagesize()
     * @param[in] fillRingSize The size of the fill ring used by the AF_XDP
     * socket. The default value is 4096
     * @param[in] completionRingSize The size of the completion ring used by the
     * AF_XDP socket. The default value is 2048
     * @param[in] rxSize The size of the RX ring used by the AF_XDP socket. The
     * default value is 2048
     * @param[in] txSize The size of the TX ring used by the AF_XDP socket. The
     * default value is 2048
     * @param[in] rxTxBatchSize The max number of packets to be received or sent
     * in one batch. The default value is 64
     */
   ...
   */
    struct XdpDeviceStats
    {
        ...
        uint64_t rxPackets;
        /** Packets received per second. Measured from to the previous time stats
     * were collected */
        uint64_t rxPacketsPerSec;
        /** Number of bytes received */
        uint64_t rxBytes;
        /** Bytes per second received. Measured from to the previous time stats were
     * collected */
        ...
        /** Packets sent from the app per second. Measured from to the previous time
     * stats were collected */
        uint64_t txSentPacketsPerSec;
        /** Number of bytes sent from the application */
        uint64_t txSentBytes;
        /** Bytes per second sent from the app. Measured from to the previous time
     * stats were collected */
        uint64_t txSentBytesPerSec;
        /** Number of completed sent packets, meaning packets that were confirmed as
     * sent by the kernel */
        uint64_t txCompletedPackets;
        /** Completed sent packets per second. Measured from to the previous time
     * stats were collected */
    ...
    /**
   * A c'tor for this class. Please note that calling this c'tor doesn't
   * initialize the AF_XDP socket. In order to set up the socket call open().
   ...
   */
    ...
    /**
   * Open the device with default configuration. Call getConfig() after opening
   * the device to get the current configuration. This method initializes the
   * UMEM, and then creates and configures the AF_XDP socket. If it succeeds the
   ...
   * This method initializes the UMEM, and then creates and configures the
   * AF_XDP socket. If it succeeds the socket is ready to receive and send
   * packets.
   ...
   */
    ...
    /**
   * Close the device. This method closes the AF_XDP socket and frees the UMEM
   * that was allocated for it.
   */
    void close() override;

    /**
   * Start receiving packets. In order to use this method the device should be
   * open. Note that this method is blocking and will return if:
   ...
   * @param[in] onPacketsArrive A callback to be called when packets are
   * received
   * @param[in] onPacketsArriveUserCookie The callback is invoked with this
   * cookie as a parameter. It can be used to pass information from the user
   * application to the callback
   * @param[in] timeoutMS Timeout in milliseconds to stop if no packets are
   * received. The default value is 5000 ms
   * @return True if stopped receiving packets because stopReceivePackets() was
   * called or because timeoutMS passed, or false if an error occurred.
   */
    bool receivePackets(OnPacketsArrive onPacketsArrive,
                        void* onPacketsArriveUserCookie, int timeoutMS = 5000);

    /**
   * Stop receiving packets. Call this method from within the callback passed to
   * receivePackets() whenever you want to stop receiving packets.
   */
   ...
   * @param[in] waitForTxCompletion Wait for confirmation from the kernel that
   * packets were sent. If set to true this method will wait until the number of
   * packets in the completion ring is equal or greater to the number of packets
   * that were sent. The default value is false
   * @param[in] waitForTxCompletionTimeoutMS If waitForTxCompletion is set to
   * true, poll the completion ring with this timeout. The default value is 5000
   * ms
   * @return True if all packets were sent, or if waitForTxCompletion is true -
   * all sent packets were confirmed. Returns false if an error occurred or if
   * poll timed out.
   */
    bool sendPackets(const RawPacketVector& packets,
   ...
   * poll timed out.
   */
    bool sendPackets(RawPacket packets[], size_t packetCount,
                     bool waitForTxCompletion = false,
                     int waitForTxCompletionTimeoutMS = 5000);

    /**
   * @return A pointer to the current device configuration. If the device is not
   * open this method returns nullptr
   */
  ...
  private:
    ...
    bool sendPackets(const std::function<RawPacket(uint32_t)>& getPacketAt,
                     const std::function<uint32_t()>& getPacketCount,
                     bool waitForTxCompletion = false,
                     int waitForTxCompletionTimeoutMS = 5000);
