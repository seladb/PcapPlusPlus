...
/**
 * @class IPcapDevice
 * An abstract class representing all libpcap-based packet capturing devices:
...
class IPcapDevice : public IDevice, public IFilterableDevice {
   ...
   */
    ...
    /**
   * A static method for retrieving pcap lib (libpcap/WinPcap/etc.) version
   * information. This method is actually a wrapper for
   * [pcap_lib_version()](https://www.tcpdump.org/manpages/pcap_lib_version.3pcap.html)
   * @return A string containing the pcap lib version information
   */
    static std::string getPcapLibVersionInfo();

    /**
   * Match a raw packet with a given BPF filter. Notice this method is static
   * which means you don't need any device instance in order to perform this
   * match
   ...
   */
    static bool matchPacketWithFilter(GeneralFilter& filter,
                                      RawPacket* rawPacket);
    ...
    /**
   * Set a filter for the device. When implemented by the device, only packets
   * that match the filter will be received. Please note that when the device is
   * closed the filter is reset so when reopening the device you need to call
   * this method again in order to reactivate the filter
