...
/**
 * @class LRUList
 * A template class that implements a LRU cache with limited size. Each time the
 * user puts an element it goes to head of the list as the most recently used
 * element (if the element was already in the list it advances to the head of
 * the list). The last element in the list is the one least recently used and
 * will be pulled out of the list if it reaches its max size and a new element
...
class LRUList
{
   ...
   */
    ...
    /**
   * Puts an element in the list. This element will be inserted (or advanced if
   * it already exists) to the head of the list as the most recently used
   * element. If the list already reached its max size and the element is new
   * this method will remove the least recently used element and return a value
   * in deletedValue. Method complexity is O(log(getSize())). This is a
   * optimized version of the method T* put(const T&).
   * @param[in] element The element to insert or to advance to the head of the
   * list (if already exists)
   * @param[out] deletedValue The value of deleted element if a pointer is not
   * NULL. This parameter is optional.
   * @return 0 if the list didn't reach its max size, 1 otherwise. In case the
   ...
   */
    int put(const T& element, T* deletedValue = NULL)
    {
        ...
        // exists the method returns an iterator to the element that prevented the
        // insertion
        std::pair<MapIterator, bool> pair = m_CacheItemsMap.insert(
            std::make_pair(element, m_CacheItemsList.begin()));
   ...
   */
    ...
    /**
   * Erase an element from the list. If element isn't found in the list nothing
   * happens
