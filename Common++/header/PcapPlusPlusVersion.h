...
#define PCAPPLUSPLUS_VERSION_FULL \
    "v" PCAPPLUSPLUS_VERSION " (" PCAPPLUSPLUS_VERSION_OFFICIAL ")"

/**
 * @return PcapPlusPlus current version, e.g: 23.09. Notice that for
 * non-official releases (which were pulled from GitHub) the version will end
 * with a '+'. For example: '23.09+' means non-official release but '23.09'
 * means official release
 */
inline std::string getPcapPlusPlusVersion() { return PCAPPLUSPLUS_VERSION; }

/**
 * @return PcapPlusPlus long version string which includes the version and info
