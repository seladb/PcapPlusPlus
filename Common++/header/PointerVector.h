...
/**
 * @class PointerVector
 * A template class for representing a std::vector of pointers. Once (a pointer
 * to) an element is added to this vector, the element responsibility moves to
 * the vector, meaning the PointerVector will free the object once it's removed
 * from the vector This class wraps std::vector and adds the capability of
...
class PointerVector {
   ...
   */
    ...
    /**
   * Const iterator object that is used for iterating all elements in a constant
   * vector
   */
   ...
   */
    ...
    /**
   * Copy constructor. Once a vector is copied from another vector, all elements
   ...
   */
    ...
    /**
   * Removes from the vector a single element (position). Once the element is
   * erased, it's also freed
