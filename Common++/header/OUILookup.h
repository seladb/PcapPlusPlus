 ...
 * internal database to define name of the vendor. The class itself should be
 * initialized by using initOUIDatabaseFromJson() otherwise all requests will
 * return "Unknown" as vendor. The class itself currently does not support
 * on-fly modifying the database but anyone who wants to add/modify/remove
...
class OUILookup {
   ...
   * integer equivalent of "XX:XX:XX" formatted MAC address
   */
   ...
   */
    ...
    /**
   * Returns the vendor of the MAC address. OUI database should be initialized
   * with initOUIDatabaseFromJson()
