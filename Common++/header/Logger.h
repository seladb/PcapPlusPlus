...
#define PCPP_LOG(level, message)                                      \
    ...
    {                                                                 \
        ...
        (*sstream) << message;                                        \
        pcpp::Logger::getInstance().internalPrintLogMessage(          \
...
enum LogModule
{
    ...
    PacketLogModuleSingleCommandTextProtocolLayer, ///< SingleCommandTextProtocol
                                                   ///< module (Packet++)
 ...
 *
 * PcapPlusPlus is divided into modules (described in #LogModule enum). The user
 * can set the log level got each module or to all modules at once. The default
 * is Logger#Info which outputs only error messages. Changing log level for
 ...
 *
 * Logs are printed to console by default in a certain format. The user can set
 * a different print function to change the format or to print to other media
 * (such as files, etc.).
 *
 * PcapPlusPlus logger is a singleton which can be reached from anywhere in the
 * code.
 *
...
class Logger
{
   ...
   * @param[in] file The source file in PcapPlusPlus code the log message is
   * coming from
   * @param[in] method The method in PcapPlusPlus code the log message is coming
   * from
   * @param[in] line The line in PcapPlusPlus code the log message is coming
   * from
   */
    typedef void (*LogPrinter)(LogLevel logLevel, const std::string& logMessage,
                               const std::string& file, const std::string& method,
                               const int line);
   ...
   * @param[in] printer A log printer function that will be called for every log
   * message
   */
   ...
   */
    void internalPrintLogMessage(std::ostringstream* logStream,
                                 Logger::LogLevel logLevel, const char* file,
  ...
  private:
    ...
    static void defaultLogPrinter(LogLevel logLevel,
