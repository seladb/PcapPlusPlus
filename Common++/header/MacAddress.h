...
class MacAddress
{
   ...
   * The byte array length must be equal or greater to 6 (as MAC address is
   * 6-byte long)
   * @todo there is no verification array length >= 6. If this is not the case,
   * address will read uninitialized memory
   ...
   */
    ...
    /**
   * A constructor that creates an instance of the class out of a (char*)
   * string. If the string doesn't represent a valid MAC address, instance will
   * be invalid, meaning isValid() will return false
   ...
   * If the string doesn't represent a valid MAC address, instance will be
   * invalid, meaning isValid() will return false
   * @param[in] addr A pointer to the string
   */
    MacAddress(const std::string& addr) { init(addr.c_str()); }

    /**
   * A constructor that creates an instance of 6 bytes representing the MAC
   * address
   ...
   */
    inline MacAddress(uint8_t firstOctest, uint8_t secondOctet,
                      uint8_t thirdOctet, uint8_t fourthOctet, uint8_t fifthOctet,
                      uint8_t sixthOctet);

#if __cplusplus > 199711L || _MSC_VER >= 1800
    /**
   * A constructor that creates an instance out of the initializer list. The
   * length of the list must be equal to 6 (as MAC address is 6-byte long)
   * @param[in] addr An initializer list containing the values of type uint8_t
   ...
   */
    ...
    /**
   * Get an indication whether the MAC address is valid. An address can be
   * invalid if it was constructed from illegal input, for example: invalid
   * string
   ...
   */
    ...
    /**
   * Allocates a byte array of length 6 and copies address value into it. Array
   ...
   * value to it. This method assumes array allocated size is at least 6 (the
   ...
   */
    ...
    /**
   * A static value representing a zero value of MAC address, meaning address of
