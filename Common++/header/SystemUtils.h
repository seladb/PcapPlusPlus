...
struct SystemCores
{
   ...
   */
    ...
    /**
   * A static array for mapping core ID (integer) to the corresponding static
   * SystemCore representation
   */
...
/**
 * Retrieve a system-wide real-time accurate clock. It's actually a
 * multi-platform version of clock_gettime() which is fully supported only on
 * Linux
...
/**
 * A multi-platform version of the popular sleep method. This method simply runs
 * the right sleep method, according to the platform it is running on.
 * @param[in] seconds Number of seconds to sleep
 */
void multiPlatformSleep(uint32_t seconds);

/**
 * A multi-platform version of sleep in milliseconds resolution. This method
 * simply runs the right sleep method, according to the platform it is running
 * on.
...
/**
 * @class AppName
 * This class extracts the application name from the current running executable
 * and stores it for usage of the application throughout its runtime. This class
 * should be initialized once in the beginning of the main() method using
...
class AppName
{
  ...
  public:
    /**
   * Static init method which should be called once at the beginning of the main
   * method.
 ...
 * A singleton class that provides callbacks for events that occur during
...
class ApplicationEventHandler
{
   ...
   * @param[in] cookie A pointer the the cookie provided by the user in
   * ApplicationEventHandler c'tor
   */
    typedef void (*EventHandlerCallback)(void* cookie);

    /**
   * As ApplicationEventHandler is a singleton, this is the static getter to
   ...
   * @param[in] cookie A pointer to a user provided object. This object will be
   * transferred to the EventHandlerCallback callback. This cookie is very
   * useful for transferring objects that give context to the event callback
   */
