...
namespace pcpp
{
	namespace internal
	{
		in_addr* sockaddr2in_addr(struct sockaddr* sa)
		{
			...
			LOG_DBG("sockaddr family is not AF_INET. Returning NULL");
			return NULL;
		}
		...
		in6_addr* sockaddr2in6_addr(struct sockaddr* sa)
		{
			...
			LOG_DBG("sockaddr family is not AF_INET6. Returning NULL");
			return NULL;
		}
		...
		void sockaddr2string(struct sockaddr* sa, char* resultString)
		{
			...
			if (ipv4Addr != NULL)
			{
				LOG_DBG("IPv4 packet address");
				inet_ntop(AF_INET, &(((sockaddr_in*)sa)->sin_addr), resultString, INET_ADDRSTRLEN);
			}
			else
			{
				LOG_DBG("Not IPv4 packet address. Assuming IPv6 packet");
