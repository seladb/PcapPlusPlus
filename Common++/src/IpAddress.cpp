...
const IPv6Address IPv6Address::MulticastRangeLowerBound(
...
bool IPv4Address::isMulticast() const
{
    return !operator<(MulticastRangeLowerBound) &&
           (operator<(MulticastRangeUpperBound) || operator==(
                                                       MulticastRangeUpperBound));
}
...
bool IPv4Address::matchSubnet(const IPv4Address& subnet,
...
bool IPv6Address::matchSubnet(const IPv6Address& subnet,
...
IPAddress::IPAddress(const std::string& addrAsString)
    : m_Type(IPv6AddressType), m_IPv6(addrAsString)
{
...
void IPv4Network::initFromAddressAndPrefixLength(const IPv4Address& address,
                                                 uint8_t prefixLen)
{
...
void IPv4Network::initFromAddressAndNetmask(const IPv4Address& address,
...
IPv4Network::IPv4Network(const std::string& addressAndNetmask)
{
    ...
    {
        throw std::invalid_argument(
            "The input should be in the format of <address>/<netmask> or "
    ...
    {
        throw std::invalid_argument(
            "The input doesn't contain a valid IPv4 network prefix");
    }
...
bool IPv4Network::includes(const IPv4Network& network) const
{
    ...
    return ((lowestAddress & m_Mask) == m_NetworkPrefix &&
            (highestAddress & m_Mask) == m_NetworkPrefix);
}
...
{
    bool isAllZeros =
        std::all_of(netmask.begin(), netmask.end(),
...
void IPv6Network::initFromAddressAndPrefixLength(const IPv6Address& address,
                                                 uint8_t prefixLen)
{
...
void IPv6Network::initFromAddressAndNetmask(const IPv6Address& address,
                                            const std::string& netmask)
{
...
IPv6Network::IPv6Network(const IPv6Address& address,
                         const std::string& netmask)
{
    if (!address.isValid())
    {
...
IPv6Network::IPv6Network(const std::string& addressAndNetmask)
{
    ...
    {
        throw std::invalid_argument(
            "The input doesn't contain a valid IPv6 network prefix");
    }
...
{
    return includes(network.getLowestAddress()) &&
           includes(network.getHighestAddress());
}
