...
std::string MacAddress::toString() const {
    char str[19];
    snprintf(str, sizeof str, "%02x:%02x:%02x:%02x:%02x:%02x", m_Address[0],
             m_Address[1], m_Address[2], m_Address[3], m_Address[4],
             m_Address[5]);
...
void MacAddress::init(const char* addr) {
    ...
    for (; *addr != 0 && i < addrLen; ++i) {
        ...
        // The strtol function returns zero value in two cases: when an error occurs
        // or the string '00' is converted. This code verifies that it's the second
        // case.
